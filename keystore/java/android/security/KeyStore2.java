...
public class KeyStore2 {
     ...
     * As a result, good operations may be terminated prematurely.
     ...
     *
     * As a side effect of this new pruning strategy operation creation can now fail if if the
     ...
     * or single child aging (5s <= age < 35s) operations are off limits. An aging
     * sibling of two, however, would have a malus of 3 and would be fair game.
     *
     * ## Consequence for API users.
     * Starting with Android S the initialization of cipher and signature objects may throw
     * a {@link KeyStoreBackendBusyException} with an error code of
     * {@link ResponseCode#BACKEND_BUSY} to allow API users to implement their own back-off
     * strategy.
     *
     * For older SDK version, {@link KeyStore} will poll the Keystore daemon for a free operation
            ...
            try {
                ...
                if (firstTry) {
                    Log.w(TAG, "Looks like we may have lost connection to the Keystore "
                            + "daemon.");
                    Log.w(TAG, "Retrying after giving Keystore 20ms to recover.");
                    interruptedPreservingSleep(20);
    ...
    /**
     * Update the subcomponets of a key entry designated by the key descriptor.
