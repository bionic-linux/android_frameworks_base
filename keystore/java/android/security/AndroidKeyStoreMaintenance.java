...
public class AndroidKeyStoreMaintenance {
     ...
     * @param isMigration - true if the keys already existing should not be considered an error
     * @return 0 if successful or a {@code ResponseCode}
     * @hide
     */
    public static int initUserSuperKeys(@NonNull int userId, byte[] password, boolean isMigration) {
        StrictMode.noteDiskWrite();
        try {
            getService().initUserSuperKeys(userId, password, isMigration);
            return 0;
        } catch (ServiceSpecificException e) {
