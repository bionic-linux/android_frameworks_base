...
public class KeyStoreCryptoOperationChunkedStreamer {
    ...
    public KeyStoreCryptoOperationChunkedStreamer(KeyStoreOperation consumer) {
        this(consumer, DEFAULT_MAX_CHUNK_SIZE);
    }

    public KeyStoreCryptoOperationChunkedStreamer(KeyStoreOperation consumer, int maxChunkSize) {
        mKeyStoreOperation = consumer;
        mMaxChunkSize = maxChunkSize;
    }

    public byte[] update(byte[] input, int inputOffset, int inputLength) throws KeymasterException {
        if (inputLength == 0) {
            // No input provided -- the contract is to return an empty array.
        ...
        while (inputLength > 0) {
            ...
            if (opResult.inputConsumed == chunk.length) {
                ...
                int inputConsumed = opResult.inputConsumed - mBuffered.length;
                if (inputConsumed >= 0) {
