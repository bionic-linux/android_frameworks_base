...
public class LegacyVpnProfileStore {
     ...
     * @throws LegacyVpnStoreException on error. There are no protocol defined error conditions.
     *                    An exception is thrown either because of an IPC/Binder error or due
     *                    to an unexpected situation in the backend, e.g., file system error.
     * @hide
     */
    ...
    public static @NonNull String[] list(@NonNull String prefix) {
        try {
            if (AndroidKeyStoreProvider.isKeystore2Enabled()) {
                ...
                return Arrays.stream(aliases).map(a -> a.substring(prefix.length()))
                        .toArray(String[]::new);
            } else {
