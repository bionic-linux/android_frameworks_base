...
jint DoLoadWithRelroFile(JNIEnv* env, const char* lib, const char* relro, jobject clazzLoader) {
    ...
    android_namespace_t* ns = android::FindNamespaceByClassLoader(env, clazzLoader);
    if (ns == NULL) {
    ...
    android_dlextinfo extinfo;
    extinfo.flags =
            ANDROID_DLEXT_RESERVED_ADDRESS | ANDROID_DLEXT_USE_RELRO | ANDROID_DLEXT_USE_NAMESPACE;
    extinfo.reserved_addr = gReservedAddress;
    extinfo.reserved_size = gReservedSize;
...
jint LoadWithRelroFile(JNIEnv* env, jclass, jstring lib, jstring relro, jobject clazzLoader) {
...
const char kWebViewLibraryLoaderClassName[] = "android/webkit/WebViewLibraryLoader";
const JNINativeMethod kJniMethods[] = {
        {"nativeReserveAddressSpace", "(J)Z", reinterpret_cast<void*>(ReserveAddressSpace)},
        {"nativeCreateRelroFile", "(Ljava/lang/String;Ljava/lang/String;)Z",
...
void RegisterWebViewFactory(JNIEnv* env) {
    ...
    if (clazz) {
        LIBLOAD_SUCCESS =
                env->GetStaticIntField(clazz, env->GetStaticFieldID(clazz, "LIBLOAD_SUCCESS", "I"));

        LIBLOAD_FAILED_TO_OPEN_RELRO_FILE = env->GetStaticIntField(
                clazz, env->GetStaticFieldID(clazz, "LIBLOAD_FAILED_TO_OPEN_RELRO_FILE", "I"));

        LIBLOAD_FAILED_TO_LOAD_LIBRARY = env->GetStaticIntField(
                clazz, env->GetStaticFieldID(clazz, "LIBLOAD_FAILED_TO_LOAD_LIBRARY", "I"));

        LIBLOAD_FAILED_JNI_CALL = env->GetStaticIntField(
                clazz, env->GetStaticFieldID(clazz, "LIBLOAD_FAILED_JNI_CALL", "I"));

        LIBLOAD_FAILED_TO_FIND_NAMESPACE = env->GetStaticIntField(
                clazz, env->GetStaticFieldID(clazz, "LIBLOAD_FAILED_TO_FIND_NAMESPACE", "I"));
    }
}
