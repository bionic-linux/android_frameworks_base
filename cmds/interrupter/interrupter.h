...
#define DEFINE_INTERCEPT_N(N, sym, ret, ...)                                               \
    ...
    CONCATENATE(CALL_FUNCTION_, N)(sym, ret, __VA_ARGS__) static void __init_##sym(void) { \
...
#define DEFINE_INTERCEPT(sym, ret, ...) \
    DEFINE_INTERCEPT_N(INTERCEPT_NARG(__VA_ARGS__), sym, ret, __VA_ARGS__)
