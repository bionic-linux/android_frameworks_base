...
int main(const int argc, const char *argv[]) {
    ...
    if (selinux_enabled && selinux_status_open(1) < 0) {
        ALOGE("Could not open selinux status page or netlink socket; exiting.\n");
        exit(1);
    }
    ...
    for (;;) {
        ...
        for (;;) {
            ...
            if (selinux_enabled && selinux_status_updated() > 0) {
                if (selinux_android_seapp_context_reload() < 0) {
                    ALOGE("failed to reload seapp_contexts\n");
                    break;
                }
            }
