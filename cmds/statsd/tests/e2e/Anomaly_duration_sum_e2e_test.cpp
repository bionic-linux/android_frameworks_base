...
StatsdConfig CreateStatsdConfig(int num_buckets, uint64_t threshold_ns,
                                DurationMetric::AggregationType aggregationType, bool nesting) {
    StatsdConfig config;
...
TEST(AnomalyDetectionE2eTest, TestDurationMetric_SUM_single_bucket) {
    ...
    release_event =
            CreateReleaseWakelockEvent(attributions2, "wl1", bucketStartTimeNs + NS_PER_SEC + 109);
    processor->OnLogEvent(release_event.get());
    EXPECT_EQ(0u, anomalyTracker->getAlarmTimestampSec(dimensionKey));
    ...
    // Acquire wakelock wl1.
    acquire_event =
            CreateAcquireWakelockEvent(attributions1, "wl1", bucketStartTimeNs + NS_PER_SEC + 112);
    processor->OnLogEvent(acquire_event.get());
    ...
    acquire_event =
            CreateAcquireWakelockEvent(attributions1, "wl1", bucketStartTimeNs + bucketSizeNs - 2);
    processor->OnLogEvent(acquire_event.get());
    EXPECT_EQ((bucketStartTimeNs + bucketSizeNs) / NS_PER_SEC,
              anomalyTracker->getAlarmTimestampSec(dimensionKey));

    // Release the event at early bucket #1.
    release_event = CreateReleaseWakelockEvent(attributions1, "wl1",
                                               bucketStartTimeNs + bucketSizeNs + NS_PER_SEC - 1);
    processor->OnLogEvent(release_event.get());
    EXPECT_EQ(0u, anomalyTracker->getAlarmTimestampSec(dimensionKey));
    ...
    acquire_event = CreateAcquireWakelockEvent(attributions2, "wl1",
                                               bucketStartTimeNs + 2 * bucketSizeNs + 30);
    processor->OnLogEvent(acquire_event.get());
...
TEST(AnomalyDetectionE2eTest, TestDurationMetric_SUM_multiple_buckets) {
    ...
    acquire_event =
            CreateAcquireWakelockEvent(attributions2, "wl1", bucketStartTimeNs + bucketSizeNs + 1);
    processor->OnLogEvent(acquire_event.get());
    EXPECT_EQ((bucketStartTimeNs + bucketSizeNs) / NS_PER_SEC + 1,
              anomalyTracker->getAlarmTimestampSec(dimensionKey));
    EXPECT_EQ(0u, anomalyTracker->getRefractoryPeriodEndsSec(dimensionKey));

    release_event = CreateReleaseWakelockEvent(attributions2, "wl1",
    ...
    acquire_event = CreateAcquireWakelockEvent(attributions3, "wl2",
    ...
    release_event = CreateReleaseWakelockEvent(attributions3, "wl2",
                                               bucketStartTimeNs + 6 * bucketSizeNs + 2);
    processor->OnLogEvent(release_event.get());
    release_event = CreateReleaseWakelockEvent(attributions1, "wl1",
                                               bucketStartTimeNs + 6 * bucketSizeNs + 6);
    processor->OnLogEvent(release_event.get());
    EXPECT_EQ(0u, anomalyTracker->getAlarmTimestampSec(dimensionKey));
    EXPECT_EQ(0u, anomalyTracker->getAlarmTimestampSec(dimensionKey2));
...
TEST(AnomalyDetectionE2eTest, TestDurationMetric_SUM_long_refractory_period) {
    ...
    // Acquire wakelock "wc1" in bucket #0.
    auto acquire_event = CreateAcquireWakelockEvent(attributions1, "wl1",
                                                    bucketStartTimeNs + bucketSizeNs - 100);
    processor->OnLogEvent(acquire_event.get());
    ...
    acquire_event = CreateAcquireWakelockEvent(attributions1, "wl1",
    ...
    auto release_event = CreateReleaseWakelockEvent(attributions1, "wl1",
                                                    bucketStartTimeNs + 2 * bucketSizeNs + 1);
    processor->OnLogEvent(release_event.get());
    EXPECT_EQ(0u, anomalyTracker->getAlarmTimestampSec(dimensionKey));
    ...
    release_event = CreateReleaseWakelockEvent(attributions1, "wl1",
                                               bucketStartTimeNs + 3 * bucketSizeNs - NS_PER_SEC);
