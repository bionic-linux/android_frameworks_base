...
void extractIntoVector(vector<shared_ptr<LogEvent>> events, vector<vector<int>>& ret) {
    ret.clear();
    status_t err;
...
TEST(puller_util, NoNeedToMerge) {
    vector<shared_ptr<LogEvent>> inputData;
    shared_ptr<LogEvent> event = make_shared<LogEvent>(nonUidAtomTagId, timestamp);
    // 32
    event->write(isolatedNonAdditiveData);
    event->init();
    inputData.push_back(event);
