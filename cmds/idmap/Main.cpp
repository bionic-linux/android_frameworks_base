...
namespace {
    ...
    bool is_idmap_stale_fd(const char* orig_apk_path, const char* skin_apk_path, int idmap_fd)
    {
        ...
        String8 cached_orig_path, cached_skin_path;
        if (!ResTable::getIdmapInfo(buf, N, &cached_orig_crc, &cached_skin_crc,
                    &cached_orig_path, &cached_skin_path)) {
            return true;
        }
        ...
        if (cached_skin_path != skin_apk_path) {
            return true;
        }
    ...
    int write_idmap_path(const char* idmap_path, const uint32_t* data, size_t size)
    {
        ...
        if (TEMP_FAILURE_RETRY(fchmod(idmap_fd, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH)) < 0) {
            goto fail;
        }
...
#if 0
    printf("idmap mode=%s orig_apk_path=%s skin_apk_path=%s argv[4]=%s\n",
            mode, orig_apk_path, skin_apk_path, argv[4]);
#endif
