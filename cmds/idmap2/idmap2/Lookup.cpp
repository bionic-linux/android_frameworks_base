...
namespace {
bool WARN_UNUSED ParseResId(const AssetManager2& am, const std::string& raw, uint32_t& out) {
  // try to parse as a hex number
  char* endptr;
  out = strtol(raw.c_str(), &endptr, 16);
  if (*endptr == '\0') {
    return true;
  }

  // next, try to parse as a package:type/name string
  out = am.GetResourceId(raw);
  if (out != 0) {
    return true;
  }
...
bool WARN_UNUSED GetValue(const AssetManager2& am, uint32_t resid, std::string& out) {
  Res_value value;
  ...
  switch (value.dataType) {
    ...
    case Res_value::TYPE_STRING: {
      ...
      if (str != nullptr) {
        ...
        out.append("FIXME: utf16 encoded string");
      }
    } break;
    default:
      out.append("FIXME: dataType=");
...
int Lookup(const std::vector<std::string>& args, std::ostream& out_error) {
  ...
  uint32_t resid;
  if (!ParseResId(am, resid_str, resid)) {
    out_error << "error: failed to parse resource ID" << std::endl;
    return 1;
  }

  std::string value;
  if (!GetValue(am, resid, value)) {
