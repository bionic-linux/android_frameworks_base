...
TEST(EncodedBufferTest, WriteSimple) {
    ...
    for (size_t i = 0; i < TEST_CHUNK_HALF_SIZE; i++) {
        buffer->writeRawByte(reinterpret_cast<uint8_t>(50 + i));
    }
    ...
    for (size_t i = 0; i < TEST_CHUNK_SIZE; i++) {
        buffer->writeRawByte(reinterpret_cast<uint8_t>(80 + i));
    }
    ...
    for (size_t i = 0; i < TEST_CHUNK_HALF_SIZE; i++) {
        EXPECT_EQ(buffer->readRawByte(), reinterpret_cast<uint8_t>(50 + i));
    }
    for (size_t i = 0; i < TEST_CHUNK_SIZE; i++) {
        EXPECT_EQ(buffer->readRawByte(), reinterpret_cast<uint8_t>(80 + i));
    }
