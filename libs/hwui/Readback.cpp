...
CopyResult Readback::copySurfaceInto(ANativeWindow* window, const Rect& inSrcRect,
    ...
    if (cropRect.left < cropRect.right && cropRect.top < cropRect.bottom) {
        ...
        FloatRect fCrop(cropRect.left, cropRect.top, cropRect.right, cropRect.bottom);
        if (imageWH.width() > 1 && imageWH.width() < (int32_t)description.width) {
            fCrop.left += shrinkAmount;
            fCrop.right -= shrinkAmount;
        }
        if (imageWH.height() > 1 && imageWH.height() < (int32_t)description.height) {
            fCrop.top += shrinkAmount;
            fCrop.bottom -= shrinkAmount;
        }

        imageSrcRect = SkRect::MakeLTRB(fCrop.left, fCrop.top, fCrop.right, fCrop.bottom);
    }
