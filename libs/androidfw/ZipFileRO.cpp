...
{
    return getEntryInfo(entry, pMethod, pUncompLen, pCompLen, pOffset, pModWhen, pCrc32, nullptr, nullptr);
}

bool ZipFileRO::getEntryInfo(ZipEntryRO entry, uint16_t* pMethod,
    uint32_t* pUncompLen, uint32_t* pCompLen, off64_t* pOffset,
    uint32_t* pModWhen, uint32_t* pCrc32, off64_t* pExtraFieldOffset, uint16_t* pExtraFieldLength) const
{
    ...
    if(pExtraFieldOffset != nullptr) {
        *pExtraFieldOffset = ze.extra_field_offset;
    }
    if(pExtraFieldLength != nullptr) {
        *pExtraFieldLength = ze.extra_field_length;
    }
