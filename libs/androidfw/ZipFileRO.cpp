 ...
 * appear to be bogus.
 */
bool ZipFileRO::getEntryInfo(ZipEntryRO entry, uint16_t* pMethod, uint32_t* pUncompLen,
...
bool ZipFileRO::startIteration(void** cookie, const char* prefix, const char* suffix) {
  ...
  int32_t error = StartIteration(mHandle, &(ze->cookie), prefix ? &pe : NULL, suffix ? &se : NULL);
  if (error) {
 ...
 */
int ZipFileRO::getEntryFileName(ZipEntryRO entry, char* buffer, size_t bufLen) const {
  const _ZipEntryRO* zipEntry = reinterpret_cast<_ZipEntryRO*>(entry);
...
bool ZipFileRO::uncompressEntry(ZipEntryRO entry, void* buffer, size_t size) const {
  ...
  const int32_t error = ExtractToMemory(mHandle, &(zipEntry->entry), (uint8_t*)buffer, size);
  if (error) {
