 ...
 public:
  void SetUp() override {
    ASSERT_TRUE(ReadFileFromZipToString(GetTestDataPath() + "/basic/basic.apk", "resources.arsc",
                                        &basic_contents_));
    ASSERT_TRUE(ReadFileFromZipToString(GetTestDataPath() + "/basic/basic_de_fr.apk",
                                        "resources.arsc", &basic_de_fr_contents_));
    ASSERT_TRUE(ReadFileFromZipToString(GetTestDataPath() + "/basic/basic_hdpi-v4.apk",
                                        "resources.arsc", &basic_hdpi_contents_));
    ASSERT_TRUE(ReadFileFromZipToString(GetTestDataPath() + "/basic/basic_xhdpi-v4.apk",
                                        "resources.arsc", &basic_xhdpi_contents_));
    ASSERT_TRUE(ReadFileFromZipToString(GetTestDataPath() + "/basic/basic_xxhdpi-v4.apk",
                                        "resources.arsc", &basic_xxhdpi_contents_));
    ASSERT_TRUE(ReadFileFromZipToString(GetTestDataPath() + "/feature/feature.apk",
...
TEST_F(SplitTest, TestLoadBase) {
  ResTable table;
  ASSERT_EQ(NO_ERROR, table.add(basic_contents_.data(), basic_contents_.size()));
}
...
TEST_F(SplitTest, TestGetResourceFromSplit) {
  ...
  ASSERT_EQ(NO_ERROR, table.add(basic_contents_.data(), basic_contents_.size()));
  ASSERT_EQ(NO_ERROR, table.add(basic_de_fr_contents_.data(), basic_de_fr_contents_.size()));
...
TEST_F(SplitTest, ResourcesFromBaseAndSplitHaveSameNames) {
  ...
  EXPECT_EQ(String16(baseName.type, baseName.typeLen), String16(frName.type, frName.typeLen));

  EXPECT_EQ(String16(baseName.name, baseName.nameLen), String16(frName.name, frName.nameLen));
}
...
TEST_F(SplitTest, SelectBestDensity) {
  ...
  ASSERT_EQ(NO_ERROR, table.add(basic_contents_.data(), basic_contents_.size()));
  ASSERT_EQ(NO_ERROR, table.add(basic_hdpi_contents_.data(), basic_hdpi_contents_.size()));

  EXPECT_TRUE(IsStringEqual(table, R::string::density, "hdpi"));

  ASSERT_EQ(NO_ERROR, table.add(basic_xhdpi_contents_.data(), basic_xhdpi_contents_.size()));

  EXPECT_TRUE(IsStringEqual(table, R::string::density, "xhdpi"));

  ASSERT_EQ(NO_ERROR, table.add(basic_xxhdpi_contents_.data(), basic_xxhdpi_contents_.size()));
...
TEST_F(SplitTest, TestNewResourceIsAccessible) {
  ...
  ASSERT_EQ(NO_ERROR, table.add(feature_contents_.data(), feature_contents_.size()));
...
TEST_F(SplitTest, TestNewResourceNameHasCorrectName) {
  ...
  EXPECT_EQ(String16("com.android.basic"), String16(name.package, name.packageLen));
...
TEST_F(SplitTest, TestNewResourceIsAccessibleByName) {
  ...
  ASSERT_EQ(R::string::test3,
            table.identifierForName(name.string(), name.size(), type.string(), type.size(),
