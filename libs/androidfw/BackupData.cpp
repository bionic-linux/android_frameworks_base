...
static inline size_t padding_extra(size_t n) {
  return ROUND_UP[n % 4];
}

BackupDataWriter::BackupDataWriter(int fd) : m_fd(fd), m_status(NO_ERROR), m_entityCount(0) {
...
status_t BackupDataWriter::write_padding_for(int n) {
  ssize_t amt;
  ssize_t paddingSize;
...
status_t BackupDataWriter::WriteEntityHeader(const String8& key, size_t dataSize) {
  ...
  if (kIsDebug) {
    ALOGD("Writing header: prefix='%s' key='%s' dataSize=%zu", m_keyPrefix.string(), key.string(),
          dataSize);
  }
...
status_t BackupDataWriter::WriteEntityData(const void* data, size_t size) {
...
void BackupDataWriter::SetKeyPrefix(const String8& keyPrefix) {
  m_keyPrefix = keyPrefix;
}

BackupDataReader::BackupDataReader(int fd)
    : m_fd(fd), m_done(false), m_status(NO_ERROR), m_entityCount(0) {
...
status_t BackupDataReader::Status() {
  return m_status;
}
  ...
  do {                                                                                 \
    if ((actual) != (expected)) {                                                      \
      if ((actual) == 0) {                                                             \
        ...
        m_status = errno;                                                              \
        ALOGD("CHECK_SIZE(a=%ld e=%ld) failed at line %d m_status='%s'", long(actual), \
              long(expected), __LINE__, strerror(m_status));                           \
      }                                                                                \
      return m_status;                                                                 \
    }                                                                                  \
  } while (0)
...
status_t BackupDataReader::ReadNextHeader(bool* done, int* type) {
  *done = m_done;
  if (m_status != NO_ERROR) {
    return m_status;
  }
  ...
  *done = m_done = (amt == 0);
  ...
  switch (m_header.type) {
    ...
    default:
      ALOGD("Chunk header at %d has invalid type: 0x%08x", (int)(m_pos - sizeof(m_header)),
            (int)m_header.type);
      m_status = EINVAL;
  }
...
bool BackupDataReader::HasEntities() {
  return m_status == NO_ERROR && m_header.type == BACKUP_HEADER_ENTITY_V1;
}

status_t BackupDataReader::ReadEntityHeader(String8* key, size_t* dataSize) {
  if (m_status != NO_ERROR) {
    return m_status;
  }
...
status_t BackupDataReader::SkipEntityData() {
  if (m_status != NO_ERROR) {
    return m_status;
  }
  if (m_header.type != BACKUP_HEADER_ENTITY_V1) {
...
ssize_t BackupDataReader::ReadEntityData(void* data, size_t size) {
  if (m_status != NO_ERROR) {
    return -1;
  }
...
status_t BackupDataReader::skip_padding() {
  ssize_t amt;
  ssize_t paddingSize;
