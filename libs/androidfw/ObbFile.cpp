...
#define kFooterMinSize                      \
  33 /* 32-bit signature version (4 bytes)  \
...
ObbFile::ObbFile() : mPackageName(""), mVersion(-1), mFlags(0) {
...
bool ObbFile::parseObbFile(int fd) {
...

  {
    ...
    if (footerSize > (size_t)fileLength - kFooterTagSize || footerSize > kMaxBufSize) {
