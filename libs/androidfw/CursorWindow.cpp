  ...
  if (ashmemFd < 0) {
    ...
    if (result >= 0) {
      ...
      if (data == MAP_FAILED) {
        ...
        if (result >= 0) {
          CursorWindow* window = new CursorWindow(name, ashmemFd, data, size, false /*readOnly*/);
          result = window->clear();
          if (!result) {
            LOG_WINDOW(
                "Created new CursorWindow: freeOffset=%d, "
                "numRows=%d, numColumns=%d, mSize=%d, mData=%p",
                window->mHeader->freeOffset, window->mHeader->numRows, window->mHeader->numColumns,
    ...
    if (size < 0) {
      ...
      if (dupAshmemFd < 0) {
        ...
        if (data == MAP_FAILED) {
          ...
          CursorWindow* window = new CursorWindow(name, dupAshmemFd, data, size, true /*readOnly*/);
          LOG_WINDOW(
              "Created CursorWindow from parcel: freeOffset=%d, "
              "numRows=%d, numColumns=%d, mSize=%d, mData=%p",
              window->mHeader->freeOffset, window->mHeader->numRows, window->mHeader->numColumns,
              window->mSize, window->mData);
          *outCursorWindow = window;
...
uint32_t CursorWindow::alloc(size_t size, bool aligned) {
  ...
  if (nextFreeOffset > mSize) {
    ALOGW(
        "Window is full: requested allocation %zu bytes, "
...
CursorWindow::RowSlot* CursorWindow::getRowSlot(uint32_t row) {
  ...
  RowSlotChunk* chunk = static_cast<RowSlotChunk*>(offsetToPtr(mHeader->firstChunkOffset));
...
status_t CursorWindow::putBlobOrString(uint32_t row, uint32_t column, const void* value,
