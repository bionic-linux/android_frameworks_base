...
static inline int round_up(int n) {
  return n + ROUND_UP[n % 4];
}

static int read_snapshot_file(int fd, KeyedVector<String8, FileState>* snapshot) {
  int bytesRead = 0;
  int amt;
  ...
  for (int i = 0; i < header.fileCount; i++) {
    ...
    char* filename =
        nameBufSize <= (int)sizeof(filenameBuf) ? filenameBuf : (char*)malloc(nameBufSize);
...
static int write_snapshot_file(int fd, const KeyedVector<String8, FileRec>& snapshot) {
  int fileCount = 0;
  ...
  for (int i = 0; i < N; i++) {
    ...
    if (!r.deleted) {
      ...
      if (paddingLen != 0) {
        ...
        if (amt != paddingLen) {
          ALOGW("write_snapshot_file error writing %d bytes of filename padding %s", paddingLen,
                strerror(errno));
          return 1;
        }
      }
...
static int write_delete_file(BackupDataWriter* dataStream, const String8& key) {
  LOGP("write_delete_file %s\n", key.string());
  return dataStream->WriteEntityHeader(key, -1);
}

static int write_update_file(BackupDataWriter* dataStream, int fd, int mode, const String8& key,
  ...
  if (bytesLeft != 0) {
    ...
    ALOGE(
        "write_update_file size mismatch for %s. expected=%d actual=%d."
        " You aren't doing proper locking!",
        realFilename, fileSize, fileSize - bytesLeft);
  }
...
static int write_update_file(BackupDataWriter* dataStream, const String8& key,
                             char const* realFilename) {
  int err;
  struct stat st;
...
static int compute_crc32(const char* file, FileRec* out) {
...
int back_up_files(int oldSnapshotFD, BackupDataWriter* dataStream, int newSnapshotFD,
  ...
  while (n < N && m < M) {
    ...
    if (cmp < 0) {
      ...
      LOGP("%s", q.string());
      LOGP("  old: modTime=%d,%d mode=%04o size=%-3d crc32=0x%08x", f.modTime_sec, f.modTime_nsec,
           f.mode, f.size, f.crc32);
      LOGP("  new: modTime=%d,%d mode=%04o size=%-3d crc32=0x%08x", g.s.modTime_sec,
           g.s.modTime_nsec, g.s.mode, g.s.size, g.s.crc32);
...
static int write_pax_header_entry(char* buf, size_t buf_size, const char* key, const char* value) {
...
int write_tarfile(const String8& packageName, const String8& domain, const String8& rootpath,
                  const String8& filepath, off_t* outSize, BackupDataWriter* writer) {
  ...
  *outSize = 512 + (needExtended ? 1024 : 0) + 512 * ((s.st_size + 511) / 512);
  ...
  if (needExtended) {
    ...
    paxLen +=
        write_pax_header_entry(paxData + paxLen, PAXDATA_SIZE - paxLen, "path", fullname.string());
  ...
  if (!isdir) {
    ...
    while (toWrite > 0) {
      ...
      if (nRead < 0) {
        err = errno;
        ALOGE("Unable to read file [%s], err=%d (%s)", filepath.string(), err, strerror(err));
        break;
      } else if (nRead == 0) {
        ALOGE("EOF but expect %lld more bytes in [%s]", (long long)toWrite, filepath.string());
        err = EIO;
        break;
      }
...
status_t RestoreHelperBase::WriteFile(const String8& filename, BackupDataReader* in) {
  ssize_t err;
  size_t dataSize;
  String8 key;
  int fd;
  ...
  if (amt != sizeof(metadata)) {
    ALOGW("Could not read metadata for %s -- %ld / %s", filename.string(), (long)amt,
          strerror(errno));
    return EIO;
  }
  ...
  if (metadata.version > CURRENT_METADATA_VERSION) {
    if (!m_loggedUnknownMetadata) {
      ...
      ALOGW("Restoring file with unsupported metadata version %d (currently %d)", metadata.version,
...
status_t RestoreHelperBase::WriteSnapshot(int fd) {
...
static int compare_file(const char* path, const unsigned char* data, int len) {
  int fd;
  int amt;
...
int backup_helper_test_empty() {
  ...
  static const unsigned char correct_data[] = {0x53, 0x6e, 0x61, 0x70, 0x00, 0x00, 0x00, 0x00,
...
int backup_helper_test_four() {
  ...
  static const unsigned char correct_data[] = {
      // header
      0x53, 0x6e, 0x61, 0x70, 0x04, 0x00, 0x00, 0x00, 0x46, 0x69, 0x6c, 0x65, 0xbc, 0x00, 0x00,
      0x00,

      // bytes_of_padding
      0x98, 0xba, 0xdc, 0xfe, 0xef, 0xbe, 0xad, 0xde, 0xff, 0x01, 0x00, 0x00, 0xbc, 0xbc, 0xab,
      0xab, 0x78, 0x56, 0x34, 0x12, 0x10, 0x00, 0x00, 0x00, 0x62, 0x79, 0x74, 0x65, 0x73, 0x5f,
      0x6f, 0x66, 0x5f, 0x70, 0x61, 0x64, 0x64, 0x69, 0x6e, 0x67,

      // bytes_of_padding3
      0x31, 0x00, 0x40, 0x93, 0xef, 0xbe, 0xad, 0xde, 0xb6, 0x01, 0x00, 0x00, 0x66, 0x77, 0x55,
      0x88, 0x22, 0x44, 0x33, 0x22, 0x11, 0x00, 0x00, 0x00, 0x62, 0x79, 0x74, 0x65, 0x73, 0x5f,
      0x6f, 0x66, 0x5f, 0x70, 0x61, 0x64, 0x64, 0x69, 0x6e, 0x67, 0x33, 0xab, 0xab, 0xab,

      // bytes of padding2
      0x44, 0x11, 0x22, 0x33, 0xef, 0xbe, 0xad, 0xde, 0xe4, 0x01, 0x00, 0x00, 0x44, 0x33, 0x22,
      0x11, 0x34, 0x23, 0x12, 0x01, 0x12, 0x00, 0x00, 0x00, 0x62, 0x79, 0x74, 0x65, 0x73, 0x5f,
      0x6f, 0x66, 0x5f, 0x70, 0x61, 0x64, 0x64, 0x69, 0x6e, 0x67, 0x5f, 0x32, 0xab, 0xab,

      // bytes of padding3
      0x44, 0x11, 0x22, 0x33, 0xef, 0xbe, 0xad, 0xde, 0xed, 0x01, 0x00, 0x00, 0x44, 0x33, 0x22,
      0x11, 0x34, 0x23, 0x12, 0x01, 0x13, 0x00, 0x00, 0x00, 0x62, 0x79, 0x74, 0x65, 0x73, 0x5f,
      0x6f, 0x66, 0x5f, 0x70, 0x61, 0x64, 0x64, 0x69, 0x6e, 0x67, 0x5f, 0x5f, 0x31, 0xab};
  ...
  for (size_t i = 0; i < readSnapshot.size(); i++) {
    ...
    if (name != filenames[i] || states[i].modTime_sec != state.modTime_sec ||
      ...
      fprintf(stderr,
              "state %zu expected={%d/%d, %04o, 0x%08x, 0x%08x, %3zu} '%s'\n"
              "          actual={%d/%d, %04o, 0x%08x, 0x%08x, %3d} '%s'\n",
              i, states[i].modTime_sec, states[i].modTime_nsec, states[i].mode, states[i].size,
              states[i].crc32, name.length(), filenames[i].string(), state.modTime_sec,
              state.modTime_nsec, state.mode, state.size, state.crc32, state.nameLen,
              name.string());
      matched = false;
    }
  }
...
const unsigned char DATA_GOLDEN_FILE[] = {
    

};
const int DATA_GOLDEN_FILE_SIZE = sizeof(DATA_GOLDEN_FILE);

static int test_write_header_and_entity(BackupDataWriter& writer, const char* str) {
  int err;
  String8 text(str);
...
int test_read_header_and_entity(BackupDataReader& reader, const char* str) {
  int err;
  ...
  if (0 != memcmp(buf, str, bufSize)) {
    fprintf(stderr,
            "ReadEntityData expected '%s' but got something starting with "
            "%02x %02x %02x %02x  '%c%c%c%c'\n",
            str, buf[0], buf[1], buf[2], buf[3], buf[0], buf[1], buf[2], buf[3]);
    err = EINVAL;
    goto finished;
  }
...
static int get_mod_time(const char* filename, struct timeval times[2]) {
  int err;
  struct stat64 st;
...
int backup_helper_test_files() {
  ...
  char const* files_before[] = {SCRATCH_DIR "data/b", SCRATCH_DIR "data/c", SCRATCH_DIR "data/d",
                                SCRATCH_DIR "data/e", SCRATCH_DIR "data/f"};

  char const* keys_before[] = {"data/b", "data/c", "data/d", "data/e", "data/f"};
