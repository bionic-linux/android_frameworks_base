...
Asset::Asset(void) : mAccessMode(ACCESS_UNKNOWN), mNext(NULL), mPrev(NULL) {
}
 ...
 */
/*static*/ Asset* Asset::createFromCompressedFile(const char* fileName, AccessMode mode) {
  ...
  unsigned long crc32;
  scanResult = ZipUtils::examineGzip(fp, &method, &uncompressedLen, &compressedLen, &crc32);
  offset = ftell(fp);
  fclose(fp);
  if (!scanResult) {
  ...
  result = pAsset->openChunk(fd, offset, method, uncompressedLen, compressedLen);
  if (result != NO_ERROR) {
    delete pAsset;
    return NULL;
  }
 ...
 */
/*static*/ Asset* Asset::createFromCompressedMap(FileMap* dataMap, size_t uncompressedLen,
                                                 AccessMode mode) {
  _CompressedAsset* pAsset;
  status_t result;
...
status_t _FileAsset::openChunk(const char* fileName, int fd, off64_t offset, size_t length) {
  ...
  mFp = fdopen(fd, "rb");
  if (mFp == NULL) return UNKNOWN_ERROR;
...
const void* _FileAsset::ensureAlignment(FileMap* map) {
  ...
  if ((((size_t)data) & 0x3) == 0) {
    ...
    // boundary.
    ALOGV("Returning aligned FileAsset %p (%s).", this, getAssetSource());
    return data;
  }
  // If not aligned on a word boundary, then we need to copy it into
  // our own buffer.
  ALOGV("Copying FileAsset %p (%s) to buffer size %d to make it aligned.", this, getAssetSource(),
        (int)mLength);
 ...
 */
status_t _CompressedAsset::openChunk(int fd, off64_t offset, int compressionMethod,
...
const void* _CompressedAsset::getBuffer(bool) {
  ...
  if (mMap != NULL) {
    if (!ZipUtils::inflateToBuffer(mMap->getDataPtr(), buf, mUncompressedLen, mCompressedLen))
      goto bail;
  } else {
    assert(mFd >= 0);
     ...
     */
    if (lseek(mFd, mStart, SEEK_SET) != mStart) goto bail;

    /*
     * Expand the data into it.
     */
    if (!ZipUtils::inflateToBuffer(mFd, buf, mUncompressedLen, mCompressedLen)) goto bail;
  }
