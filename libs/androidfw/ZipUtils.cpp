...
class BufferReader : public zip_archive::Reader {
 public:
  BufferReader(const void* input, size_t inputSize)
      : Reader(), mInput(reinterpret_cast<const uint8_t*>(input)), mInputSize(inputSize) {
  }
...
class BufferWriter : public zip_archive::Writer {
 public:
  BufferWriter(void* output, size_t outputSize)
      : Writer(),
        mOutput(reinterpret_cast<uint8_t*>(output)),
...
/*static*/ bool ZipUtils::inflateToBuffer(FILE* fp, void* buf, long uncompressedLen,
                                          long compressedLen) {
...
/*static*/ bool ZipUtils::inflateToBuffer(int fd, void* buf, long uncompressedLen,
                                          long compressedLen) {
  FdReader reader(fd);
  BufferWriter writer(buf, uncompressedLen);
  return (zip_archive::Inflate(reader, compressedLen, uncompressedLen, &writer, nullptr) == 0);
...
/*static*/ bool ZipUtils::inflateToBuffer(const void* in, void* buf, long uncompressedLen,
                                          long compressedLen) {
 ...
 */
/*static*/ bool ZipUtils::examineGzip(FILE* fp, int* pCompressionMethod, long* pUncompressedLen,
                                      long* pCompressedLen, unsigned long* pCRC32) {
  enum {  // flags
    FTEXT = 0x01,
    FHCRC = 0x02,
    FEXTRA = 0x04,
  ...
  ic = getc(fp);
  if (ic != 0x1f || getc(fp) != 0x8b) return false;  // not gzip
  method = getc(fp);
  flags = getc(fp);

  /* quick sanity checks */
  if (method == EOF || flags == EOF) return false;
  if (method != kCompressDeflated) return false;
