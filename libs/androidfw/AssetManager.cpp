...
bool AssetManager::addAssetPath(const String8& path, int32_t* cookie, bool appAsLib,
  ...
  if (ap.type == kFileTypeRegular) {
    ...
    if (ap.type != kFileTypeDirectory && ap.type != kFileTypeRegular) {
      ALOGW("Asset path %s is neither a directory nor file (type=%d).", path.string(),
            (int)ap.type);
      return false;
    }
  }
  ...
  ALOGV("In %p Asset %s path: %s", this, ap.type == kFileTypeDirectory ? "dir" : "zip",
        ap.path.string());
...
bool AssetManager::addOverlayPath(const String8& packagePath, int32_t* cookie) {
  ...
  String8 targetPath;
  String8 overlayPath;
  if (!ResTable::getIdmapInfo(idmap->getBuffer(false), idmap->getLength(), NULL, NULL, NULL,
...
bool AssetManager::addAssetFd(int fd, const String8& debugPathName, int32_t* cookie, bool appAsLib,
...
bool AssetManager::createIdmap(const char* targetApkPath, const char* overlayApkPath,
  ...
  bool ret = false;
  {
    ...
    for (int i = 0; i < 2; ++i) {
      ...
      ap.type = kFileTypeRegular;
      ap.path = paths[i];
      assets[i] = openNonAssetInPathLocked("resources.arsc", Asset::ACCESS_BUFFER, ap);
      if (assets[i] == NULL) {
    ...
    ret = tables[0].createIdmap(tables[1], targetCrc, overlayCrc, targetApkPath, overlayApkPath,
...
Asset* AssetManager::open(const char* fileName, AccessMode mode) {
  ...
  while (i > 0) {
    ...
    Asset* pAsset = openNonAssetInPathLocked(assetName.string(), mode, mAssetPaths.editItemAt(i));
    if (pAsset != NULL) {
...
Asset* AssetManager::openNonAsset(const char* fileName, AccessMode mode, int32_t* outCookie) {
  ...
  while (i > 0) {
    ...
    Asset* pAsset = openNonAssetInPathLocked(fileName, mode, mAssetPaths.editItemAt(i));
    if (pAsset != NULL) {
...
Asset* AssetManager::openNonAsset(const int32_t cookie, const char* fileName, AccessMode mode) {
  ...
  if (which < mAssetPaths.size()) {
    ...
    Asset* pAsset = openNonAssetInPathLocked(fileName, mode, mAssetPaths.editItemAt(which));
    if (pAsset != NULL) {
      return pAsset != kExcludedAsset ? pAsset : NULL;
    }
  }
  ...
  if (ap.type != kFileTypeDirectory && ap.rawFd < 0) {
    ...
    if (sharedRes == NULL) {
      ...
      if (ass == NULL) {
        ...
        ass = const_cast<AssetManager*>(this)->openNonAssetInPathLocked("resources.arsc",
...
#endif
        sharedRes =
            const_cast<AssetManager*>(this)->mZipSet.setZipResourceTable(ap.path, sharedRes);
      }
    }
  } else {
    ALOGV("loading resource table %s\n", ap.path.string());
...
const ResTable* AssetManager::getResTable(bool required) const {
  ...
  for (size_t i = 0; i < N; i++) {
    bool empty = appendPathToResTable(const_cast<AssetManager*>(this)->mAssetPaths.editItemAt(i));
...
Asset* AssetManager::openIdmapLocked(const struct asset_path& ap) const {
  Asset* ass = NULL;
  if (ap.idmap.size() != 0) {
    ass = const_cast<AssetManager*>(this)->openAssetFromFileLocked(ap.idmap, Asset::ACCESS_BUFFER);
    if (ass) {
...
void AssetManager::addSystemOverlays(const char* pathOverlaysList, const String8& targetPackagePath,
  ...
  while (fgets(buf, sizeof(buf), fin)) {
    ...
    Asset* oass = const_cast<AssetManager*>(this)->openNonAssetInPathLocked(
        "resources.arsc", Asset::ACCESS_BUFFER, oap);
 ...
 */
String8 AssetManager::createZipSourceNameLocked(const String8& zipFileName, const String8& dirName,
 ...
 */
Asset* AssetManager::openAssetFromFileLocked(const String8& pathName, AccessMode mode) {
  Asset* pAsset = NULL;
 ...
 */
Asset* AssetManager::openAssetFromZipLocked(const ZipFileRO* pZipFile, const ZipEntryRO entry,
                                            AccessMode mode, const String8& entryName) {
  Asset* pAsset = NULL;
  ...
  if (method == ZipFileRO::kCompressStored) {
    ...
    pAsset = Asset::createFromCompressedMap(dataMap, static_cast<size_t>(uncompressedLen), mode);
...
bool AssetManager::scanAndMergeDirLocked(SortedVector<AssetDir::FileInfo>* pMergedInfo,
  ...
  if (dirName[0] != '\0') path.appendPath(dirName);
  ...
  for (i = 0; i < count; i++) {
    ...
    nameLen = strlen(name);
    if (nameLen > exclExtLen && strcmp(name + (nameLen - exclExtLen), kExcludeExtension) == 0) {
...
AssetManager::SharedZip::SharedZip(int fd, const String8& path)
    : mPath(path), mZipFile(NULL), mModWhen(0), mResourceTableAsset(NULL), mResourceTable(NULL) {
  if (kIsDebug) {
...
AssetManager::ZipSet::~ZipSet(void) {
  ...
  for (size_t i = 0; i < N; i++) closeZip(i);
}
...
Asset* AssetManager::ZipSet::setZipResourceTableAsset(const String8& path, Asset* asset) {
  int idx = getIndex(path);
  sp<SharedZip> zip = mZipFile[idx];
...
ResTable* AssetManager::ZipSet::setZipResourceTable(const String8& path, ResTable* res) {
  int idx = getIndex(path);
  sp<SharedZip> zip = mZipFile[idx];
  // doesn't make sense to call before previously accessing.
