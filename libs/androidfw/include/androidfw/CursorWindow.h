 ...
 */
class CursorWindow {
  CursorWindow(const String8& name, int ashmemFd, void* data, size_t size, bool readOnly);
 ...
 public:
  ...
  inline const char* getFieldSlotValueString(FieldSlot* fieldSlot, size_t* outSizeIncludingNull) {
    *outSizeIncludingNull = fieldSlot->data.buffer.size;
    return static_cast<char*>(
        offsetToPtr(fieldSlot->data.buffer.offset, fieldSlot->data.buffer.size));
  }
 ...
 private:
  ...
  inline void* offsetToPtr(uint32_t offset, uint32_t bufferSize = 0) {
    ...
    if (offset + bufferSize > mSize) {
      ALOGE("End offset %" PRIu32 " out of bounds, max value %zu", offset + bufferSize, mSize);
      return NULL;
    }
  ...
  status_t putBlobOrString(uint32_t row, uint32_t column, const void* value, size_t size,
                           int32_t type);
};
