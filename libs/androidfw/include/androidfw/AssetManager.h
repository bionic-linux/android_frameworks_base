...
class AssetManager : public AAssetManager {
 public:
   ...
   * newly-added asset source.
   */
  bool addAssetFd(int fd, const String8& debugPathName, int32_t* cookie, bool appAsLib = false,
   ...
   */
  bool createIdmap(const char* targetApkPath, const char* overlayApkPath, uint32_t targetCrc,
                   uint32_t overlayCrc, uint32_t** outData, size_t* outSize);
 ...
 private:
  ...
  Asset* openNonAssetInPathLocked(const char* fileName, AccessMode mode, asset_path& path);
  String8 createPathNameLocked(const asset_path& path, const char* rootDir);
  String8 createZipSourceNameLocked(const String8& zipFileName, const String8& dirName,
  ...
  Asset* openAssetFromZipLocked(const ZipFileRO* pZipFile, const ZipEntryRO entry, AccessMode mode,
                                const String8& entryName);

  bool scanAndMergeDirLocked(SortedVector<AssetDir::FileInfo>* pMergedInfo, const asset_path& path,
                             const char* rootDir, const char* dirName);
  SortedVector<AssetDir::FileInfo>* scanDirLocked(const String8& path);
  bool scanAndMergeZipLocked(SortedVector<AssetDir::FileInfo>* pMergedInfo, const asset_path& path,
                             const char* rootDir, const char* dirName);
