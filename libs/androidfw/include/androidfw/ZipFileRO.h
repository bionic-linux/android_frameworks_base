...
class ZipFileRO {
 public:
  ...
  enum : uint16_t { kCompressStored = 0, kCompressDeflated = 8 };
   ...
   */
  static ZipFileRO* openFd(int fd, const char* debugFileName, bool assume_ownership = true);
   ...
   * appears to be bad.
   */
  bool getEntryInfo(ZipEntryRO entry, uint16_t* pMethod, uint32_t* pUncompLen, uint32_t* pCompLen,
                    off64_t* pOffset, uint32_t* pModWhen, uint32_t* pCrc32) const;
 ...
 private:
  ...
  ZipFileRO(ZipArchiveHandle handle, char* fileName) : mHandle(handle), mFileName(fileName) {
  }
