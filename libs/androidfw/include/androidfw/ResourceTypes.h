...
struct alignas(uintptr_t) Res_png_9patch {
  Res_png_9patch() : wasDeserialized(false), xDivsOffset(0), yDivsOffset(0), colorsOffset(0) {
  }
...
class ResXMLParser {
 public:
  ...
  ssize_t indexOfAttribute(const char16_t* ns, size_t nsLen, const char16_t* attr,
...
struct ResTable_config {
  ...
  inline bool operator<(const ResTable_config& o) const {
    return compare(o) < 0;
  }
...
class ResTable {
 public:
  ResTable();
  ResTable(const void* data, size_t size, const int32_t cookie, bool copyData = false);
  ~ResTable();
  ...
  ssize_t getResource(uint32_t resID, Res_value* outValue, bool mayBeBag = false,
                      uint16_t density = 0, uint32_t* outSpecFlags = NULL,
  ...
  ssize_t resolveReference(Res_value* inOutValue, ssize_t blockIndex, uint32_t* outLastRef = NULL,
                           uint32_t* inoutTypeSpecFlags = NULL,
                           ResTable_config* outConfig = NULL) const;

  enum { TMP_BUFFER_SIZE = 16 };
  ...
  uint32_t identifierForName(const char16_t* name, size_t nameLen, const char16_t* type = 0,
                             size_t typeLen = 0, const char16_t* defPackage = 0,
                             size_t defPackageLen = 0, uint32_t* outTypeSpecFlags = NULL) const;

  static bool expandResourceRef(const char16_t* refStr, size_t refLen, String16* outPackage,
                                String16* outType, String16* outName,
                                const String16* defType = NULL, const String16* defPackage = NULL,
                                const char** outErrorMsg = NULL, bool* outPublicOnly = NULL);
  ...
  bool stringToValue(Res_value* outValue, String16* outString, const char16_t* s, size_t len,
                     bool preserveSpaces, bool coerceType, uint32_t attrID = 0,
                     const String16* defType = NULL, const String16* defPackage = NULL,
                     Accessor* accessor = NULL, void* accessorCookie = NULL,
                     uint32_t attrType = ResTable_map::TYPE_ANY, bool enforcePrivate = true) const;

  // Perform processing of escapes and quotes in a string.
  static bool collectString(String16* outString, const char16_t* s, size_t len, bool preserveSpaces,
                            const char** outErrorMsg = NULL, bool append = false);
  ...
  status_t createIdmap(const ResTable& overlay, uint32_t targetCrc, uint32_t overlayCrc,
                       const char* targetPath, const char* overlayPath, void** outData,
  ...
  static bool getIdmapInfo(const void* idmap, size_t size, uint32_t* pVersion, uint32_t* pTargetCrc,
                           uint32_t* pOverlayCrc, String8* pTargetPath, String8* pOverlayPath);
 ...
 private:
  ...
  status_t getEntry(const PackageGroup* packageGroup, int typeIndex, int entryIndex,
  ...
  void forEachConfiguration(bool ignoreMipmap, bool ignoreAndroidPackage, bool includeSystemConfigs,
                            const Func& f) const;
