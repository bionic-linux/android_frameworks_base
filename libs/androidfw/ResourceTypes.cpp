...
struct OverlayEntry {
    OverlayEntry(uint32_t t, uint32_t o) : targetResid(t), overlayResid(o), leadingPadding(0) {}
    ...
    mutable uint32_t leadingPadding;
};
...
struct MatchingResources {
    void add(uint32_t targetResid, uint32_t overlayResid) {
        ...
        map[targetTypeid].insert(OverlayEntry(targetResid, overlayResid));
    }

    std::map<uint16_t, std::set<OverlayEntry>> map;
    std::map<uint16_t, uint16_t> entryCount;
};
        ...
        void** outData, size_t* outSize) const
{
    ...
    const size_t tmpNameSize = sizeof(targetPackageStruct->name) / sizeof(targetPackageStruct->name[0]);
    ...
    for (size_t typeIndex = 0; typeIndex < packageGroup->types.size(); typeIndex++) {
        ...
        for (size_t entryIndex = 0; entryIndex < typeConfigs->entryCount; entryIndex++) {
            ...
            resource_name name;
            if (!getResourceName(overlay_resid, false, &name)) {
                continue;
            }

            const String16 type_str(name.type, name.typeLen);
            const String16 entry_str(name.name, name.nameLen);
            uint32_t typeSpecFlags = 0u;
            const uint32_t target_resid = targetResTable.identifierForName(entry_str.string(),
                    entry_str.size(),
                    type_str.string(),
                    type_str.size(),
    ...
    // fix padding between entries
    const auto typesEnd = matchingResources.map.cend();
    for (auto ti = matchingResources.map.begin(); ti != typesEnd; ++ti) {
        uint16_t entryCount = 0;
        uint32_t lastSeenTargetEntry = 0xffffffff;
        for (auto ei = ti->second.cbegin(); ei != ti->second.cend(); ++ei) {
            if (lastSeenTargetEntry != 0xffffffff) {
                ei->leadingPadding = Res_GETENTRY(ei->targetResid) - lastSeenTargetEntry - 1;
            }
            lastSeenTargetEntry = Res_GETENTRY(ei->targetResid);
            entryCount += 1 + ei->leadingPadding;
        }
        matchingResources.entryCount[ti->first] = entryCount;
    }
    ...
    *outSize += 2 * sizeof(uint16_t); // target package id, type count
    for (auto ti = matchingResources.map.cbegin(); ti != typesEnd; ++ti) {
        ...
        *outSize += matchingResources.entryCount[ti->first] * sizeof(uint32_t); // entries
    }
    ...
    *data++ = htodl(IDMAP_MAGIC); // magic
    *data++ = htodl(ResTable::IDMAP_CURRENT_VERSION); // version
    *data++ = htodl(targetCrc); // target crc
    *data++ = htodl(overlayCrc); // overlay crc

    char* charData = (char*)data;
    size_t pathLen = strlen(targetPath);
    for (size_t i = 0; i < 256; ++i) {
        *charData++ = i < pathLen ? targetPath[i] : '\0'; // target path
    }
    pathLen = strlen(overlayPath);
    for (size_t i = 0; i < 256; ++i) {
        *charData++ = i < pathLen ? overlayPath[i] : '\0'; // overlay path
    }
    data += (2 * 256) / sizeof(uint32_t);

    // write idmap data header
    uint16_t* typeData = reinterpret_cast<uint16_t*>(data);
    *typeData++ = htods(targetPackageStruct->id); // target package id
    *typeData++ = htods(static_cast<uint16_t>(matchingResources.map.size())); // type count

    // write idmap data
    for (auto ti = matchingResources.map.cbegin(); ti != typesEnd; ++ti) {
        auto ei = ti->second.cbegin();
        *typeData++ = htods(Res_GETTYPE(ei->targetResid) + 1); // target type id
        *typeData++ = htods(Res_GETTYPE(ei->overlayResid) + 1); // overlay type id
        *typeData++ = htods(matchingResources.entryCount[ti->first]); // entry count
        *typeData++ = htods(Res_GETENTRY(ei->targetResid)); // (target) entry offset
        uint32_t *entryData = reinterpret_cast<uint32_t*>(typeData);
        for (; ei != ti->second.cend(); ++ei) {
            for (size_t i = 0; i < ei->leadingPadding; i++) {
                *entryData++ = htodl(0xffffffff); // padding
            }
            *entryData++ = htodl(Res_GETENTRY(ei->overlayResid)); // (overlay) entry
        }
        typeData += matchingResources.entryCount[ti->first] * 2;
    }
