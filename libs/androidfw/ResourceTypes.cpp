...
static status_t validate_chunk(const ResChunk_header* chunk, size_t minSize, const uint8_t* dataEnd,
  ...
  if (headerSize >= minSize) {
    if (headerSize <= size) {
      ...
      ALOGW("%s size 0x%x or headerSize 0x%x is not on an integer boundary.", name, (int)size,
            (int)headerSize);
      return BAD_TYPE;
    }
...
static bool assertIdmapHeader(const void* idmap, size_t size) {
  ...
  if (magic != IDMAP_MAGIC) {
    ALOGW("idmap: no magic found in header (is 0x%08x, expected 0x%08x)", magic, IDMAP_MAGIC);
    return false;
  }
  ...
  if (version != ResTable::IDMAP_CURRENT_VERSION) {
    ...
    ALOGW("idmap: version mismatch in header (is 0x%08x, expected 0x%08x)", version,
          ResTable::IDMAP_CURRENT_VERSION);
    return false;
  }
  return true;
}
 ...
 public:
  ...
  status_t setTo(const void* entryHeader, size_t size) {
    ...
    if (size < sizeof(uint32_t) * (entryCount + 2)) {
      ALOGE("idmap: too small (%u bytes) for the number of entries (%u)", (uint32_t)size,
            (uint32_t)entryCount);
      return UNKNOWN_ERROR;
    }
    mData = header;
...
status_t parseIdmap(const void* idmap, size_t size, uint8_t* outPackageId,
  ...
  const uint16_t* data = reinterpret_cast<const uint16_t*>(reinterpret_cast<const uint8_t*>(idmap) +
  ...
  if (mHeader->styleCount > 0) {
    ...
    if (((const uint8_t*)mEntryStyles - (const uint8_t*)mHeader) > (int)size) {
      ALOGW("Bad string block: entry of %d styles extends past data size %d\n",
            (int)((const uint8_t*)mEntryStyles - (const uint8_t*)mHeader), (int)size);
      return (mError = BAD_TYPE);
    }
    ...
    const ResStringPool_span endSpan = {{htodl(ResStringPool_span::END)},
                                        ...
                                        htodl(ResStringPool_span::END)};
    if (memcmp(&mStyles[mStylePoolSize - (sizeof(endSpan) / sizeof(uint32_t))], &endSpan,
               sizeof(endSpan)) != 0) {
        ...
        if ((uint32_t)(u8str + u8len - strings) < mStringPoolSize) {
          ...
          *u16len = (size_t)actualLen;
          ...
          ALOGW("Bad string block: string #%lld extends to %lld, past end at %lld\n",
                (long long)idx, (long long)(u8str + u8len - strings), (long long)mStringPoolSize);
        }
      }
    } else {
    ...
    if (off < (mStringPoolSize - 1)) {
      ...
      *outLen = encLen;
      ...
      if ((uint32_t)(str + encLen - strings) < mStringPoolSize) {
        ...
        ALOGW("Bad string block: string #%d extends to %d, past end at %d\n", (int)idx,
              (int)(str + encLen - strings), (int)mStringPoolSize);
      }
    } else {
      ALOGW("Bad string block: string #%d entry is at %d, past end at %d\n", (int)idx,
            (int)(off * sizeof(uint16_t)), (int)(mStringPoolSize * sizeof(uint16_t)));
...
const char* ResStringPool::stringDecodeAt(size_t idx, const uint8_t* str, const size_t encLen,
  ...
  while ((uint32_t)(str + end - strings) < mStringPoolSize) {
    if (str[end] == 0x00) {
      if (i != 0) {
        ALOGW("Bad string block: string #%d is truncated (actual length is %d)", (int)idx,
              (int)end);
      }
  ...
  if (mError == NO_ERROR && idx < mHeader->styleCount) {
    ...
    if (off < mStylePoolSize) {
      ...
      ALOGW("Bad string block: style #%d entry is at %d, past end at %d\n", (int)idx,
            (int)(off * sizeof(uint32_t)), (int)(mStylePoolSize * sizeof(uint32_t)));
    }
  }
  return NULL;
}
...
ResXMLParser::ResXMLParser(const ResXMLTree& tree) : mTree(tree), mEventCode(BAD_DOCUMENT) {
}
  ...
  if (mEventCode == START_TAG) {
    ...
    if (idx < dtohs(tag->attributeCount)) {
      const ResXMLTree_attribute* attr =
          (const ResXMLTree_attribute*)(((const uint8_t*)tag) + dtohs(tag->attributeStart) +
...
ssize_t ResXMLParser::indexOfAttribute(const char* ns, const char* attr) const {
  ...
  return indexOfAttribute(ns ? nsStr.string() : NULL, ns ? nsStr.size() : 0, attrStr.string(),
                          attrStr.size());
}

ssize_t ResXMLParser::indexOfAttribute(const char16_t* ns, size_t nsLen, const char16_t* attr,
  ...
  if (mEventCode == START_TAG) {
    ...
    if (mTree.mStrings.isUTF8()) {
      ...
      if (kDebugStringPoolNoisy) {
        ALOGI("indexOfAttribute UTF8 %s (%zu) / %s (%zu)", ns8.string(), nsLen, attr8.string(),
              attrLen);
      }
      ...
      if (kDebugStringPoolNoisy) {
        ALOGI("indexOfAttribute UTF16 %s (%zu) / %s (%zu)", String8(ns, nsLen).string(), nsLen,
      ...
      for (size_t i = 0; i < N; i++) {
        ...
        if (kDebugStringPoolNoisy) {
          ALOGI("  curNs=%s (%zu), curAttr=%s (%zu)", String8(curNs, curNsLen).string(), curNsLen,
...
status_t ResXMLTree::setTo(const void* data, size_t size, bool copyData) {
  ...
  while (((const uint8_t*)chunk) < (mDataEnd - sizeof(ResChunk_header)) &&
    ...
    if (type == RES_STRING_POOL_TYPE) {
      ...
      mResIds = (const uint32_t*)(((const uint8_t*)chunk) + dtohs(chunk->headerSize));
...
/* static */ inline int compareLocales(const ResTable_config& l, const ResTable_config& r) {
  ...
  return memcmp(l.localeNumberingSystem, r.localeNumberingSystem, sizeof(l.localeNumberingSystem));
}
...
inline int ResTable_config::getImportanceScoreOfLocale() const {
  return (localeVariant[0] ? 4 : 0) + (localeScript[0] && !localeScriptWasComputed ? 2 : 0) +
...
bool ResTable_config::isLocaleBetterThan(const ResTable_config& o,
  ...
  const int region_comparison = localeDataCompareRegions(
      country, o.country, requested->language, requested->localeScript, requested->country);
  if (region_comparison != 0) {
    return (region_comparison > 0);
  }

  // The regions are the same. Try the variant.
  const bool localeMatches =
      strncmp(localeVariant, requested->localeVariant, sizeof(localeVariant)) == 0;
  const bool otherMatches =
      strncmp(o.localeVariant, requested->localeVariant, sizeof(localeVariant)) == 0;
  if (localeMatches != otherMatches) {
    return localeMatches;
  }

  // The variants are the same, try numbering system.
  const bool localeNumsysMatches = strncmp(localeNumberingSystem, requested->localeNumberingSystem,
                                           sizeof(localeNumberingSystem)) == 0;
  const bool otherNumsysMatches = strncmp(o.localeNumberingSystem, requested->localeNumberingSystem,
                                          sizeof(localeNumberingSystem)) == 0;
...
bool ResTable_config::isBetterThan(const ResTable_config& o,
  ...
  if (requested) {
    ...
    if (colorMode || o.colorMode) {
      ...
      if (((colorMode ^ o.colorMode) & MASK_HDR) != 0 && (requested->colorMode & MASK_HDR)) {
...
void ResTable_config::getBcp47Locale(char str[RESTABLE_MAX_LOCALE_LEN], bool canonicalize) const {
  ...
  if (language[0] != '\0') {
    if (canonicalize && areIdentical(language, kTagalog)) {
      ...
      str[1] = 'i';
      str[2] = 'l';
      str[3] = '\0';  // 3-letter code for Filipino
      charsWritten += 3;
    } else {
...
struct LocaleParserState {
  enum State : uint8_t { BASE, UNICODE_EXTENSION, IGNORE_THE_REST } parserState;
    ...
    switch (size) {
      ...
      case 2:
        if (state.unicodeState == LocaleParserState::NO_KEY ||
          ...
          if ((start[0] == 'n' || start[0] == 'N') && (start[1] == 'u' || start[1] == 'U')) {
  ...
  if ((screenLayout & MASK_LAYOUTDIR) != 0) {
    ...
    switch (screenLayout & ResTable_config::MASK_LAYOUTDIR) {
      ...
      case ResTable_config::LAYOUTDIR_RTL:
        ...
        break;
      default:
        res.appendFormat("layoutDir=%d", dtohs(screenLayout & ResTable_config::MASK_LAYOUTDIR));
        break;
    }
  }
  ...
  if ((uiMode & MASK_UI_MODE_TYPE) != UI_MODE_TYPE_ANY) {
    ...
    switch (uiMode & ResTable_config::MASK_UI_MODE_TYPE) {
      ...
      case ResTable_config::UI_MODE_TYPE_VR_HEADSET:
        ...
        break;
      default:
        res.appendFormat("uiModeType=%d", dtohs(screenLayout & ResTable_config::MASK_UI_MODE_TYPE));
        break;
    }
  }
  ...
  if ((inputFlags & MASK_NAVHIDDEN) != 0) {
    ...
    switch (inputFlags & MASK_NAVHIDDEN) {
      ...
      case ResTable_config::NAVHIDDEN_YES:
        ...
        break;
      default:
        res.appendFormat("inputFlagsNavHidden=%d", dtohs(inputFlags & MASK_NAVHIDDEN));
        break;
    }
  }
...
struct ResTable::Header {
  explicit Header(ResTable* _owner)
      : owner(_owner), ownedData(NULL), header(NULL), resourceIDMap(NULL), resourceIDMapSize(0) {
  }
...
struct ResTable::Type {
  Type(const Header* _header, const Package* _package, size_t count)
      : header(_header), package(_package), entryCount(count), typeSpec(NULL), typeSpecFlags(NULL) {
  }
...
struct ResTable::PackageGroup {
  PackageGroup(ResTable* _owner, const String16& _name, uint32_t _id, bool appAsLib,
               bool _isSystemAsset, bool _isDynamic)
      : owner(_owner),
        name(_name),
        id(_id),
        largestTypeId(0),
...
ResTable::Theme::Theme(const ResTable& table) : mTable(table), mTypeSpecFlags(0) {
  ...
  while (bag < end) {
    ...
    if (curType != t) {
      ...
      if (curEntries == NULL) {
        ...
        size_t cnt_max = SIZE_MAX / sizeof(theme_entry);
        size_t buff_size = (cnt < cnt_max && cnt < 0xFFFFFFFF - 1) ? cnt * sizeof(theme_entry) : 0;
    ...
    if (kDebugTableNoisy) {
      ALOGV("Attr 0x%08x: type=0x%x, data=0x%08x; curType=0x%x", attrRes, bag->map.value.dataType,
            bag->map.value.data, curEntry->value.dataType);
    }
...
ssize_t ResTable::Theme::resolveAttributeReference(Res_value* inOutValue, ssize_t blockIndex,
  ...
  if (inOutValue->dataType == Res_value::TYPE_ATTRIBUTE) {
    ...
    if (kDebugTableTheme) {
      ALOGI("Resolving attr reference: blockIndex=%d, type=0x%x, data=0x%x\n", (int)blockIndex,
            (int)inOutValue->dataType, inOutValue->data);
    }
  ...
  return mTable.resolveReference(inOutValue, blockIndex, outLastRef, inoutTypeSpecFlags,
                                 inoutConfig);
}
...
status_t ResTable::add(Asset* asset, Asset* idmapAsset, const int32_t cookie, bool copyData,
  ...
  return addInternal(data, static_cast<size_t>(asset->getLength()), idmapData, idmapSize, appAsLib,
                     cookie, copyData, isSystemAsset);
}
...
status_t ResTable::add(ResTable* src, bool isSystemAsset) {
  ...
  for (size_t i = 0; i < src->mPackageGroups.size(); i++) {
    ...
    PackageGroup* pg = new PackageGroup(this, srcPg->name, srcPg->id, false /* appAsLib */,
                                        isSystemAsset || srcPg->isSystemAsset, srcPg->isDynamic);
...
status_t ResTable::addInternal(const void* data, size_t dataSize, const void* idmapData,
                               size_t idmapDataSize, bool appAsLib, const int32_t cookie,
                               bool copyData, bool isSystemAsset) {
  if (!data) {
    return NO_ERROR;
  }

  if (dataSize < sizeof(ResTable_header)) {
    ALOGE("Invalid data. Size(%d) is smaller than a ResTable_header(%d).", (int)dataSize,
          (int)sizeof(ResTable_header));
    return UNKNOWN_ERROR;
  }
  ...
  const ResChunk_header* chunk = (const ResChunk_header*)(((const uint8_t*)header->header) +
  ...
  while (((const uint8_t*)chunk) <= (header->dataEnd - sizeof(ResChunk_header)) &&
    ...
    if (kDebugTableNoisy) {
      ALOGV("Chunk: type=0x%x, headerSize=0x%x, size=0x%x, pos=%p\n", dtohs(chunk->type),
            dtohs(chunk->headerSize), dtohl(chunk->size),
    ...
    if (ctype == RES_STRING_POOL_TYPE) {
      ...
      if (parsePackage((ResTable_package*)chunk, header, appAsLib, isSystemAsset) != NO_ERROR) {
        return mError;
      }
      curPackage++;
    } else {
...
ssize_t ResTable::resolveReference(Res_value* value, ssize_t blockIndex, uint32_t* outLastRef,
                                   uint32_t* inoutTypeSpecFlags, ResTable_config* outConfig) const {
  int count = 0;
  while (blockIndex >= 0 && value->dataType == Res_value::TYPE_REFERENCE && value->data != 0 &&
         count < 20) {
    ...
    if (kDebugTableTheme) {
      ALOGI("Resolving reference 0x%x: newIndex=%d, type=0x%x, data=0x%x\n", value->data,
            (int)newIndex, (int)value->dataType, value->data);
    }
...
const char16_t* ResTable::valueToString(const Res_value* value, size_t stringBlock,
...
ssize_t ResTable::getBagLocked(uint32_t resID, const bag_entry** outBag,
  ...
  if (typeSet) {
    ...
    if (set) {
      ...
      ALOGW("Attempt to retrieve bag 0x%08x which is invalid or in a cycle.", resID);
      return BAD_INDEX;
    }
  }
  ...
  while (pos < count) {
    ...
    if (!Res_INTERNALID(newName)) {
      ...
      if (grp->dynamicRefTable.lookupResourceId(&newName) != NO_ERROR) {
        ALOGE("Failed resolving ResTable_map name at %d with ident 0x%08x", (int)curOff,
              (int)newName);
        free(set);
        return UNKNOWN_ERROR;
      }
    ...
    if ((!isInside) || oldName != newName) {
      ...
      if (set->numAttrs >= set->availAttrs) {
        ...
        void* oldSet = set;
        set = (bag_set*)realloc(set, sizeof(bag_set) + sizeof(bag_entry) * newAvail);
        if (set == NULL) {
          free(oldSet);
          return NO_MEMORY;
        }
    ...
    if (kDebugTableNoisy) {
      ALOGI("Setting entry #%zu %p: block=%zd, name=0x%08d, type=%d, data=0x%08x\n", curEntry, cur,
            cur->stringBlock, cur->map.name.ident, cur->map.value.dataType, cur->map.value.data);
    }
  ...
  for (size_t p = 0; p < mPackageGroups.size(); p++) {
    ...
    for (size_t t = 0; t < packageGroup->types.size(); t++) {
      ...
      for (size_t ts = 0; ts < typeList.size(); ts++) {
        ...
        if (kDebugTableNoisy) {
          ALOGD("Updating pkg=%zu type=%zu with %zu filtered configs", p, t,
                newFilteredConfigs->size());
        }
...
uint32_t ResTable::identifierForName(const char16_t* name, size_t nameLen, const char16_t* type,
                                     size_t typeLen, const char16_t* package, size_t packageLen,
                                     uint32_t* outTypeSpecFlags) const {
  if (kDebugTableSuperNoisy) {
  ...
  if (name[0] == '^') {
    ...
    if (nameLen > 7) {
      if (name[1] == 'i' && name[2] == 'n' && name[3] == 'd' && name[4] == 'e' && name[5] == 'x' &&
          name[6] == '_') {
  ...
  if (kDebugTableNoisy) {
    printf("Looking for identifier: type=%s, name=%s, package=%s\n",
           String8(type, typeLen).string(), String8(name, nameLen).string(),
  ...
  for (size_t ig = 0; ig < NG; ig++) {
    ...
    if (strzcmp16(package, packageLen, group->name.string(), group->name.size())) {
      if (kDebugTableNoisy) {
    ...
    for (size_t pi = 0; pi < packageCount; pi++) {
      ...
      do {
        ssize_t ti = group->packages[pi]->typeStrings.indexOfString(targetType, targetTypeLen);
        if (ti < 0) {
          continue;
        }
        ...
        const uint32_t identifier = findEntry(group, ti, name, nameLen, outTypeSpecFlags);
        if (identifier != 0) {
          ...
          return identifier;
               ...
               (targetType = attrPrivate.string()) && (targetTypeLen = attrPrivate.size()));
    }
  }
  return 0;
}
...
uint32_t ResTable::findEntry(const PackageGroup* group, ssize_t typeIndex, const char16_t* name,
  ...
  for (size_t i = 0; i < typeCount; i++) {
    ...
    for (size_t j = 0; j < configCount; j++) {
      ...
      for (TypeVariant::iterator iter = tv.beginEntries(); iter != tv.endEntries(); iter++) {
...
bool ResTable::expandResourceRef(const char16_t* refStr, size_t refLen, String16* outPackage,
                                 String16* outType, String16* outName, const String16* defType,
...
static const unit_entry unitNames[] = {
    ...
    {"%p", strlen("%p"), Res_value::TYPE_FRACTION, Res_value::COMPLEX_UNIT_FRACTION_PARENT,
     1.0f / 100},
    {NULL, 0, 0, 0, 0}};

static bool parse_unit(const char* str, Res_value* outValue, float* outScale, const char** outEnd) {
  const char* end = str;
...
bool U16StringToInt(const char16_t* s, size_t len, Res_value* outValue) {
  ...
  if (outValue) {
    outValue->dataType = isHex ? outValue->TYPE_INT_HEX : outValue->TYPE_INT_DEC;
...
bool ResTable::stringToFloat(const char16_t* s, size_t len, Res_value* outValue) {
  ...
  if (*end != 0 && !isspace((unsigned char)*end)) {
    ...
    if (parse_unit(end, outValue, &scale, &end)) {
      ...
      int32_t mantissa = (int32_t)((bits >> shift) & Res_value::COMPLEX_MANTISSA_MASK);
      if (neg) {
...
bool ResTable::stringToValue(Res_value* outValue, String16* outString, const char16_t* s,
                             size_t len, bool preserveSpaces, bool coerceType, uint32_t attrID,
                             const String16* defType, const String16* defPackage,
                             Accessor* accessor, void* accessorCookie, uint32_t attrType,
  ...
  if (attrID != 0 && !Res_INTERNALID(attrID)) {
    ...
    if (cnt >= 0) {
      ...
      fromAccessor = true;
      if (attrType == ResTable_map::TYPE_ENUM || attrType == ResTable_map::TYPE_FLAGS ||
  ...
  const bool canStringCoerce = coerceType && (attrType & ResTable_map::TYPE_STRING) != 0;
  ...
  if (*s == '@') {
    ...
    if (len == 5 && s[1] == 'n' && s[2] == 'u' && s[3] == 'l' && s[4] == 'l') {
      ...
      if (!expandResourceRef(resourceRefName, resourceNameLen, &package, &type, &name, defType,
                             defPackage, &errorMsg)) {
        if (accessor != NULL) {
      ...
      uint32_t rid = identifierForName(name.string(), name.size(), type.string(), type.size(),
      ...
      if (rid != 0) {
        ...
        if (accessor) {
          rid = Res_MAKEID(accessor->getRemappedPackage(Res_GETPACKAGE(rid)), Res_GETTYPE(rid),
                           Res_GETENTRY(rid));
          if (kDebugTableNoisy) {
  ...
  if (*s == '?') {
    ...
    String16 package, type, name;
    if (!expandResourceRef(s + 1, len - 1, &package, &type, &name, &attr16, defPackage,
                           &errorMsg)) {
      if (accessor != NULL) {
        accessor->reportError(accessorCookie, errorMsg);
      }
...
bool ResTable::collectString(String16* outString, const char16_t* s, size_t len,
                             bool preserveSpaces, const char** outErrorMsg, bool append) {
  String16 tmp;
...
const String16 ResTable::getBasePackageName(size_t idx) const {
  ...
  LOG_FATAL_IF(idx >= mPackageGroups.size(), "Requested package index %d past package count %d",
  ...
  for (size_t i = 0; i < packageCount; i++) {
    ...
    for (size_t j = 0; j < typeCount; j++) {
      ...
      for (size_t k = 0; k < numTypes; k++) {
        ...
        if (ignoreMipmap && typeStrings.string8ObjectAt(type->typeSpec->id - 1) == "mipmap") {
          continue;
        }
...
status_t ResTable::getEntry(const PackageGroup* packageGroup, int typeIndex, int entryIndex,
    ...
    for (size_t c = 0; c < numConfigs; c++) {
      ...
      if (thisType->flags & ResTable_type::FLAG_SPARSE) {
        ...
        const ResTable_sparseTypeEntry* result = std::lower_bound(
            sparseIndices, sparseIndices + dtohl(thisType->entryCount), realEntryIndex, keyCompare);
  ...
  if (outEntry != NULL) {
    ...
    outEntry->typeStr =
        StringPoolRef(&bestPackage->typeStrings, actualTypeIndex - bestPackage->typeIdOffset);
...
status_t ResTable::parsePackage(const ResTable_package* const pkg, const Header* const header,
  ...
  if (dtohl(pkg->keyStrings) >= pkgSize) {
    ALOGW("ResTable_package key strings at 0x%x are past chunk size 0x%x.", dtohl(pkg->keyStrings),
          pkgSize);
    return (mError = BAD_TYPE);
  }
  ...
  if (header->resourceIDMap != NULL) {
    ...
    status_t err = parseIdmap(header->resourceIDMap, header->resourceIDMapSize, &targetPackageId,
                              &idmapEntries);
    if (err != NO_ERROR) {
  ...
  // Iterate through all chunks.
  const ResChunk_header* chunk =
      (const ResChunk_header*)(((const uint8_t*)pkg) + dtohs(pkg->header.headerSize));
  ...
  while (((const uint8_t*)chunk) <= (endPos - sizeof(ResChunk_header)) &&
    ...
    if (ctype == RES_TABLE_TYPE_SPEC_TYPE) {
      ...
      err = validate_chunk(&typeSpec->header, sizeof(*typeSpec), endPos, "ResTable_typeSpec");
      if (err != NO_ERROR) {
        return (mError = err);
      }
      ...
      if (kDebugLoadTableNoisy) {
        ALOGI("TypeSpec off %p: type=0x%x, headerSize=0x%x, size=%p\n",
              (void*)(base - (const uint8_t*)chunk), dtohs(typeSpec->header.type),
              dtohs(typeSpec->header.headerSize), (void*)typeSpecSize);
      }
        ...
        if (addToType) {
          ...
          if (!typeList.isEmpty()) {
            ...
            if (existingType->entryCount != newEntryCount && idmapIndex < 0) {
              ALOGW(
                  "ResTable_typeSpec entry count inconsistent: "
          ...
          t->typeSpec = typeSpec;
          t->typeSpecFlags =
              (const uint32_t*)(((const uint8_t*)typeSpec) + dtohs(typeSpec->header.headerSize));
          if (idmapIndex >= 0) {
      ...
      err = validate_chunk(&type->header, sizeof(*type) - sizeof(ResTable_config) + 4, endPos,
                           "ResTable_type");
      if (err != NO_ERROR) {
        return (mError = err);
      }
      ...
      if (kDebugLoadTableNoisy) {
        printf("Type off %p: type=0x%x, headerSize=0x%x, size=%u\n",
               (void*)(base - (const uint8_t*)chunk), dtohs(type->header.type),
               dtohs(type->header.headerSize), typeSize);
      }
      ...
      if (newEntryCount != 0 && dtohl(type->entriesStart) > (typeSize - sizeof(ResTable_entry))) {
      ...
      if (group->dynamicRefTable.entries().size() == 0) {
        ...
        status_t err = validate_chunk(&lib->header, sizeof(*lib), endPos, "ResTable_lib_header");
        if (err != NO_ERROR) {
          return (mError = err);
        }
      ...
      status_t err =
          validate_chunk(chunk, sizeof(ResChunk_header), endPos, "ResTable_package:unknown");
      if (err != NO_ERROR) {
        return (mError = err);
      }
    }
...
std::unique_ptr<DynamicRefTable> DynamicRefTable::clone() const {
  std::unique_ptr<DynamicRefTable> clone =
      std::unique_ptr<DynamicRefTable>(new DynamicRefTable(mAssignedPackageId, mAppAsLib));
...
status_t DynamicRefTable::load(const ResTable_lib_header* const header) {
  ...
  if (sizeOfEntries > expectedSize) {
    ALOGE("ResTable_lib_header size %u is too small to fit %u entries (x %u).", expectedSize,
          entryCount, (uint32_t)sizeof(ResTable_lib_entry));
    return UNKNOWN_ERROR;
  }

  const ResTable_lib_entry* entry =
      (const ResTable_lib_entry*)(((uint8_t*)header) + dtohl(header->header.headerSize));
  for (uint32_t entryIndex = 0; entryIndex < entryCount; entryIndex++) {
    ...
    if (kDebugLibNoisy) {
      ALOGV("Found lib entry %s with id %d\n", String8(tmpName).string(), dtohl(entry->packageId));
    }
    if (packageId >= 256) {
...
status_t ResTable::createIdmap(const ResTable& overlay, uint32_t targetCrc, uint32_t overlayCrc,
  ...
  *outSize += 2 * sizeof(uint16_t);
  ...
  for (size_t typeIndex = 0; typeIndex < pg->types.size(); ++typeIndex) {
    ...
    for (size_t entryIndex = 0; entryIndex < typeConfigs->entryCount; ++entryIndex) {
      ...
      uint32_t overlayResID = overlay.identifierForName(
          overlayName.string(), overlayName.size(), overlayType.string(), overlayType.size(),
          overlayPackage.string(), overlayPackage.size(), &typeSpecFlags);
      if (overlayResID == 0) {
      ...
      // it can't.
      if ((dtohl(typeConfigs->typeSpecFlags[entryIndex]) & ResTable_typeSpec::SPEC_OVERLAYABLE) ==
          0) {
        forcedOverlayCount++;
      }
      ...
      if (Res_GETTYPE(overlayResID) + 1 != static_cast<size_t>(typeMap.overlayTypeId)) {
        ALOGE(
            "idmap: can't mix type ids in entry map. Resource 0x%08x maps to 0x%08x"
...
bool ResTable::getIdmapInfo(const void* idmap, size_t sizeBytes, uint32_t* pVersion,
                            uint32_t* pTargetCrc, uint32_t* pOverlayCrc, String8* pTargetPath,
...
static void print_complex(uint32_t complex, bool isFraction) {
  ...
  const float RADIX_MULTS[] = {1.0f * MANTISSA_MULT, 1.0f / (1 << 7) * MANTISSA_MULT,
                               1.0f / (1 << 15) * MANTISSA_MULT, 1.0f / (1 << 23) * MANTISSA_MULT};

  float value =
      (complex & (Res_value::COMPLEX_MANTISSA_MASK << Res_value::COMPLEX_MANTISSA_SHIFT)) *
      RADIX_MULTS[(complex >> Res_value::COMPLEX_RADIX_SHIFT) & Res_value::COMPLEX_RADIX_MASK];
  printf("%f", value);
...
void ResTable::print(bool inclValues) const {
  ...
  for (size_t pgIndex = 0; pgIndex < pgCount; pgIndex++) {
    ...
    printf("Package Group %d id=0x%02x packageCount=%d name=%s\n", (int)pgIndex, pg->id,
           (int)pg->packages.size(), String8(pg->name).string());
    ...
    for (size_t splitIndex = 0; splitIndex < splitCount; splitIndex++) {
      ...
      for (size_t typeIndex = 0; typeIndex < pg->types.size(); typeIndex++) {
        ...
        printf("    type %d configCount=%d entryCount=%d\n", (int)typeIndex, (int)NTC,
               (int)typeConfigs->entryCount);
        if (typeConfigs->typeSpecFlags != NULL) {
          for (size_t entryIndex = 0; entryIndex < typeConfigs->entryCount; entryIndex++) {
            uint32_t resID = (0xff000000 & ((packageId) << 24)) |
                             (0x00ff0000 & ((typeIndex + 1) << 16)) | (0x0000ffff & (entryIndex));
            ...
            if (this->getResourceName(resID, true, &resName)) {
              ...
              printf("      spec resource 0x%08x %s:%s/%s: flags=0x%08x\n", resID,
                     CHAR16_TO_CSTR(resName.package, resName.packageLen), type8.string(),
                     name8.string(), dtohl(typeConfigs->typeSpecFlags[entryIndex]));
            } else {
        ...
        for (size_t configIndex = 0; configIndex < NTC; configIndex++) {
          ...
          printf("      config %s", configStr.size() > 0 ? configStr.string() : "(default)");
          if (type->flags != 0u) {
          ...
          for (size_t entryIndex = 0; entryIndex < entryCount; entryIndex++) {
            ...
            if (type->flags & ResTable_type::FLAG_SPARSE) {
              const ResTable_sparseTypeEntry* entry =
                  reinterpret_cast<const ResTable_sparseTypeEntry*>(eindex + entryIndex);
            ...
            uint32_t resID = (0xff000000 & ((packageId) << 24)) |
                             (0x00ff0000 & ((typeIndex + 1) << 16)) | (0x0000ffff & (entryId));
            if (packageId == 0) {
              pg->dynamicRefTable.lookupResourceId(&resID);
            }
            resource_name resName;
            ...
            const ResTable_entry* ent =
                (const ResTable_entry*)(((const uint8_t*)type) + entriesStart + thisOffset);
            ...
            if ((thisOffset + esize) > typeSize) {
              printf("ResTable_entry OUT OF BOUNDS: 0x%x+0x%x+%p (size is 0x%x)\n", entriesStart,
                     thisOffset, (void*)esize, typeSize);
              continue;
            }
            ...
            if ((dtohs(ent->flags) & ResTable_entry::FLAG_COMPLEX) != 0) {
              ...
              printf("t=0x%02x d=0x%08x (s=0x%04x r=0x%02x)", (int)value.dataType, (int)value.data,
            ...
            if (inclValues) {
              if (valuePtr != NULL) {
                ...
                for (int i = 0; i < N && mapOffset < (typeSize - sizeof(ResTable_map)); i++) {
                  printf("          #%i (Key=0x%08x): ", i, dtohl(mapPtr->name.ident));
