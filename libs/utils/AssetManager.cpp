    ...
    if (strncmp(path.string(), "/system/framework/", 18) == 0) {
        ...
        if (S_ISREG(st.st_mode)) {
            ...
            if ((dir = opendir(overlayPath.string())) == NULL) {
                LOGW("Failed to open overlay directory %s: %s\n",
            ...
            LOGW("Skipping overlay path %s: not a directory nor a regular file\n",
                    overlayPath.string());
        }
    }
no_overlay:
        ...
        uint32_t** outData, uint32_t* outSize)
{
    ...
    return tables[0].createIdmap(tables[1], origCrc, skinCrc,
            origApkPath, skinApkPath, (void**)outData, outSize) == NO_ERROR;
}
...
{
    if (TEMP_FAILURE_RETRY(access(path.string(), R_OK)) == 0) {
        asset_path oap;
        oap.path = path;
...
#if 0
            LOGD("%s:%d: applying overlay: path=%s oap.path=%s oap.idmap=%s\n",
...
bool AssetManager::addOverlayFromIdmapLocked(const String8& idmapPath)
{
    ...
    if (!ResTable::getIdmapInfo(idmap->getBuffer(false), idmap->getLength(),
                NULL, NULL, &origPath, &skinPath)) {
        delete idmap;
        return false;
    }
    delete idmap;

    if (TEMP_FAILURE_RETRY(access(origPath.string(), R_OK)) != 0) {
        return false;
    }
    if (TEMP_FAILURE_RETRY(access(idmapPath.string(), R_OK)) != 0) {
        return false;
    }
    if (TEMP_FAILURE_RETRY(access(skinPath.string(), R_OK)) != 0) {
        return false;
    }

    asset_path oap;
    oap.path = skinPath;
    oap.type = ::getFileType(skinPath.string());
    oap.idmap = idmapPath;
#if 0
    LOGD("Overlay added: origPath=%s skinPath=%s idmapPath=%s\n",
            origPath.string(), skinPath.string(), idmapPath.string());
#endif
    mAssetPaths.add(oap);
...
void AssetManager::addOverlaysFromIdmapsInDirLocked(const String8& dirPath)
{
    ...
    for (size_t i = 0; i < N; ++i) {
        if (!addOverlayFromIdmapLocked(symlinks[i])) {
            LOGW("Failed to add overlay for idmap file %s\n", symlinks[i].string());
        }
    }
}
