...
public class WifiAwareManagerTest {
    ...
    public void testPublishFlow() throws Exception {
        ...
        byte[][] parsedMatchFilter = new TlvBufferUtils.TlvIterable(0, 1, matchFilter).toArray();
        collector.checkThat("match-filter-size", parsedMatchFilter.length,
                equalTo(matchFilterCaptor.getValue().length));
        collector.checkThat("match-filter-entry0", parsedMatchFilter[0],
                equalTo(matchFilterCaptor.getValue()[0]));
        collector.checkThat("match-filter-entry1", parsedMatchFilter[1],
