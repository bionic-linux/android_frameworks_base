...
public final class PPSMOParser {
        ...
        for (PPSNode child : node.getChildren()) {
            switch(child.getName()) {
                ...
                case NODE_URI:
                    ...
                    break;
                case NODE_USERNAME_PASSWORD:
                    parseUpdateUserCredential(child, updateParam);
                    break;
                case NODE_TRUST_ROOT:
                    parseUpdateTrustRoot(child, updateParam);
                    break;
                case NODE_OTHER:
     ...
     * @param updateParam Instance of UpdateParameter
     * @throws ParsingException
     */
    private static void parseUpdateUserCredential(PPSNode node, UpdateParameter updateParam)
            throws ParsingException {
        if (node.isLeaf()) {
            throw new ParsingException("Leaf node not expected for UsernamePassword");
        }
        for (PPSNode child : node.getChildren()) {
            switch (child.getName()) {
                case NODE_USERNAME:
                    updateParam.username = getPpsNodeValue(child);
                    break;
                case NODE_PASSWORD:
                    updateParam.base64EncodedPassword = getPpsNodeValue(child);
                    break;
                default:
     ...
     * @throws ParsingException
     */
    private static void parseUpdateTrustRoot(PPSNode node, UpdateParameter updateParam)
            throws ParsingException {
        if (node.isLeaf()) {
        ...
        for (PPSNode child : node.getChildren()) {
            switch (child.getName()) {
                case NODE_CERT_URL:
                    updateParam.trustRootCertUrl = getPpsNodeValue(child);
                    break;
                case NODE_CERT_SHA256_FINGERPRINT:
                    updateParam.trustRootCertSha256Fingerprint =
                            parseHexString(getPpsNodeValue(child));
                    break;
                default:
