...
public final class PPSMOParser {
        ...
        for (PPSNode child : node.getChildren()) {
            switch (child.getName()) {
                ...
                case NODE_POLICY_UPDATE:
                    parsePolicyUpdate(child, policy);
                    break;
        ...
        for (PPSNode child : node.getChildren()) {
            switch (child.getName()) {
                case NODE_FQDN_MATCH:
                    // FQDN_Match field is in the format of "[FQDN],[MatchInfo]", where [MatchInfo] is either
                    // "exactMatch" for exact match of FQDN or "includeSubdomains" for matching all FQDNs with
                    // the same sub-domain.
    ...
    /**
     * Parse configurations under PerProviderSubscription/Policy/PolicyUpdate subtree.
     *
     * @param node PPSNode representing the root of the PerProviderSubscription/Policy/PolicyUpdate
     *             subtree
     * @throws ParsingException
     */
    private static void parsePolicyUpdate(PPSNode node, Policy policy) throws ParsingException {
        if (node.isLeaf()) {
            throw new ParsingException("Leaf node not expected for PolicyUpdate");
        }
        ...
        for (PPSNode child : node.getChildren()) {
            switch(child.getName()) {
                case NODE_UPDATE_INTERVAL:
                    try {
                        policy.updateIntervalInMinutes = Long.parseLong(getPpsNodeValue(child));
                    } catch (NumberFormatException e) {
                ...
                case NODE_UPDATE_METHOD:
                    policy.updateMethod = getPpsNodeValue(child);
                    break;
                case NODE_RESTRICTION:
                    policy.restriction = getPpsNodeValue(child);
                    break;
                case NODE_URI:
                    policy.policyServerUri = getPpsNodeValue(child);
                    break;
                case NODE_USERNAME_PASSWORD:
                    parsePolicyUserCredential(child, policy);
                    break;
                case NODE_TRUST_ROOT:
                    parseTrustRoot(child, policy);
                    break;
                case NODE_OTHER:
                    Log.d(TAG, "Ignore unsupported paramter: " + child.getName());
                    break;
                default:
                    throw new ParsingException("Unknown node under PolicyUpdate: "
                            + child.getName());
            }
        }
    }
    ...
    /**
     * Parse configurations under PerProviderSubscription/Policy/PolicyUpdate/UsernamePassword
     * subtree.
     *
     * @param node PPSNode representing the root of the
     *             PerProviderSubscription/Policy/PolicyUpdate/UsernamePassword subtree
     * @throws ParsingException
     */
    private static void parsePolicyUserCredential(PPSNode node, Policy policy)
            throws ParsingException {
        if (node.isLeaf()) {
            throw new ParsingException("Leaf node not expected for UsernamePassword");
        }
        for (PPSNode child : node.getChildren()) {
            switch (child.getName()) {
                case NODE_USERNAME:
                    policy.username = getPpsNodeValue(child);
                    break;
                case NODE_PASSWORD:
                    policy.base64EncodedPassword = getPpsNodeValue(child);
                    break;
                default:
    ...
    /**
     * Parse configurations under PerProviderSubscription/Policy/PolicyUpdate/TrustRoot subtree.
     *
     * @param node PPSNode representing the root of the
     *             PerProviderSubscription/Policy/PolicyUpdate/TrustRoot subtree
     * @throws ParsingException
     */
    private static void parseTrustRoot(PPSNode node, Policy policy)
            throws ParsingException {
        if (node.isLeaf()) {
        ...
        for (PPSNode child : node.getChildren()) {
            switch (child.getName()) {
                case NODE_CERT_URL:
                    policy.trustRootCertUrl = getPpsNodeValue(child);
                    break;
                case NODE_CERT_SHA256_FINGERPRINT:
                    policy.trustRootCertSha256Fingerprint = parseHexString(getPpsNodeValue(child));
                    break;
                default:
                    throw new ParsingException("Unknown node under UsernamePassword: "
                            + child.getName());
