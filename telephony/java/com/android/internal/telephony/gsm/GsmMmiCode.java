...
public final class GsmMmiCode extends Handler implements MmiCode {
    ...
    // Special Called line presentation
    static final String SC_SPECIAL_CLIP = "230";
    static final String SC_SPECIAL_CLIR = "23";
...
*/
    ...
    /**
     * Special requirement for CLIP and CLIR
     *
     * @return true if dialing number is a special number for CLIP or CLIR
     */
    boolean isSpecialTemporaryModeCLIRCLIP() {
        if (!context.getResources().getBoolean(
                com.android.internal.R.bool.config_special_temporary_clip_clir)) {
            return false;
        }
        if (null == sc || null == dialingNumber || phone.getServiceState().getRoaming()) {
            return false;
        }
        if ((sc.equals(SC_SPECIAL_CLIP) || sc.equals(SC_SPECIAL_CLIR)) && isActivate()) {
            Log.d(LOG_TAG, "special temporary CLIP or CLIR");
            return true;
