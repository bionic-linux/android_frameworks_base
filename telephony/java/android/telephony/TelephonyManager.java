...
public class TelephonyManager {
    ...
    private Map<SignalStrengthUpdateRequest, IBinder> mSignalStrengthUpdateRequestMap =
            new HashMap<>();
...
     * available and valid at the time of call and bootstrapping is not requested,
    ...
    /**
     * Add a {@link SignalStrengthUpdateRequest} to receive notification when signal strengths
     * breach the specified thresholds.
     *
     * To be notified, add the signal strength update request and then register
     ...
     * {@link PhoneStateListener#onSignalStrengthChanged}.
     *
     ...
     * should not expect to be notified with the exactly the same thresholds.
     *
     * @see #removeSignalStrengthUpdateRequest(SignalStrengthUpdateRequest)
     *
    ...
    public void addSignalStrengthUpdateRequest(@NonNull SignalStrengthUpdateRequest request) {
        if (mSignalStrengthUpdateRequestMap.get(request) != null) {
            Log.w(TAG, "SignalStrengthUpdateRequest already added");
            return;
        }
        ...
        try {
            ...
            if (service != null) {
                ...
                mSignalStrengthUpdateRequestMap.put(request, token);
                service.addSignalStrengthUpdateRequest(token, getSubId(), request);
            }
        } catch (RemoteException e) {
    ...
    /**
     * Remove a {@link SignalStrengthUpdateRequest} from the system.
     *
     * <p>Requires Permission:
     * {@link android.Manifest.permission#MODIFY_PHONE_STATE MODIFY_PHONE_STATE}
     * or that the calling app has carrier privileges (see
     ...
     *
     * @see #addSignalStrengthUpdateRequest(SignalStrengthUpdateRequest)
     *
    ...
    public void removeSignalStrengthUpdateRequest(@NonNull SignalStrengthUpdateRequest request) {
        if (mSignalStrengthUpdateRequestMap.get(request) == null) {
            Log.w(TAG, "SignalStrengthUpdateRequest not added before");
            return;
        }
        ...
        try {
            ITelephony service = getITelephony();
            if (service != null) {
                service.removeSignalStrengthUpdateRequest(
                        mSignalStrengthUpdateRequestMap.get(request), getSubId());
                mSignalStrengthUpdateRequestMap.remove(request);
            }
        } catch (RemoteException e) {
