...
     * available and valid at the time of call and bootstrapping is not requested,
     ...
     *
     * Note that the thresholds in the request may be adjusted when passing to system to optimize
     * power consumption and reduce contention with thresholds from other sources. The caller should
     * not expect to be notified with the exactly the same thresholds.
     *
    ...
    public void addSignalStrengthUpdateRequest(@NonNull SignalStrengthUpdateRequest request) {
        try {
            ITelephony service = getITelephony();
            if (service != null) {
                service.addSignalStrengthUpdateRequest(getSubId(), request);
            }
        } catch (RemoteException e) {
    ...
    public void removeSignalStrengthUpdateRequest(@NonNull SignalStrengthUpdateRequest request) {
        try {
            ITelephony service = getITelephony();
            if (service != null) {
                service.removeSignalStrengthUpdateRequest(getSubId(), request);
            }
        } catch (RemoteException e) {
