   ...
   @Override
    ...
    public int getNetworkType(int subscription) {
        try {
            ...
            if (iTelephony != null) {
                return iTelephony.getNetworkTypeOnSubscription(subscription);
            } else {
                // This can happen when the ITelephony interface is not up yet.
    ...
    public int getDataNetworkType(int subscription) {
        try{
            ITelephony telephony = getITelephony();
            if (telephony != null) {
                return telephony.getDataNetworkTypeOnSubscription(subscription);
            } else {
                // This can happen when the ITelephony interface is not up yet.
                return TelephonyManager.NETWORK_TYPE_UNKNOWN;
            }
    ...
    public int getVoiceNetworkType(int subscription) {
        try{
            ITelephony telephony = getITelephony();
            if (telephony != null) {
                return telephony.getVoiceNetworkTypeOnSubscription(subscription);
            } else {
                // This can happen when the ITelephony interface is not up yet.
                return TelephonyManager.NETWORK_TYPE_UNKNOWN;
            }
...
*/
   ...
   @Override
    ...
    public int getLteOnCdmaMode(int subscription) {
        try {
            return getITelephony().getLteOnCdmaModeOnSubscription(subscription);
        } catch (RemoteException ex) {
            // Assume no ICC card if remote exception which shouldn't happen
    ...
    public String getMsisdn(int subscription) {
        try {
            return getSubscriberInfo().getMsisdnOnSubscription(subscription);
        } catch (RemoteException ex) {
            return null;
        } catch (NullPointerException ex) {
            // This could happen before phone restarts due to crashing
   ...
   @Override
    public void listen(PhoneStateListener listener, int events) {
        ...
        try {
            ...
            sRegistry.listenOnSubscription(pkgForDebug, listener.callback, events, notifyNow,
