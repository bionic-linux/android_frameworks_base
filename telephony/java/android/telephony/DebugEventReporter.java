...
public final class DebugEventReporter {
    ...
    public static void sendEvent(@NonNull UUID eventId, String description) {
        // If there's no debug package, there's no Context and nothing to do. Since we can't
        // tell the difference between the case of initialized and no debug package vs the case
        // of uninitialized, just silently drop all events in this case.
        // TODO: add some records of all triggere events so that even if they aren't sent, we can
        // see them in a bugreport.
    ...
    public static synchronized boolean initialize(@NonNull Context context) {
        ...
        List<ResolveInfo> packages = pm.queryBroadcastReceivers(
                new Intent(TelephonyManager.ACTION_DEBUG_EVENT), PackageManager.MATCH_SYSTEM_ONLY);
