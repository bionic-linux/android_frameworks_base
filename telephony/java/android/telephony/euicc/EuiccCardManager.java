...
import com.android.internal.telephony.euicc.IGetRatCallback;
...
public class EuiccCardManager {
     ...
     */
    public void getRat(ResultCallback<EuiccRat> callback) {
        try {
            getIEuiccCardController().getRat(mContext.getOpPackageName(),
                    new IGetRatCallback.Stub() {
                        @Override
                        public void onComplete(int resultCode, EuiccRat rat) {
                            callback.onComplete(resultCode, rat);
                        }
                    });
        } catch (RemoteException e) {
            Log.e(TAG, "Error calling getRat", e);
            throw e.rethrowFromSystemServer();
        }
    }
     ...
     * @param serverSigned1 ASN.1 data in byte array returned by SM-DP+ server.
     * @param serverSignature1 ASN.1 data in byte array returned by SM-DP+ server.
     * @param euiccCiPkIdToBeUsed ASN.1 data in byte array returned by SM-DP+ server.
     * @param serverCertificate ASN.1 data in byte array returned by SM-DP+ server.
     ...
     *     required.
     * @param smdpSigned2 ASN.1 data in byte array returned by SM-DP+ server.
     * @param smdpSignature2 ASN.1 data in byte array returned by SM-DP+ server.
     * @param smdpCertificate ASN.1 data in byte array returned by SM-DP+ server.
     * @param callback the callback to get the result code and a byte array which represents a
