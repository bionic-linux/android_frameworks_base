    ...
    private void sendTextMessageInternal(
        ...
        if (persistMessage) {
            resolveSubscriptionForOperation(new SubscriptionResolverResult() {
                ...
                public void onSuccess(int subId) {
                    try {
                        ISms iSms = getISmsServiceOrThrow();
                        if (iSms != null) {
                            iSms.sendTextForSubscriberWithCdmaOptions(subId,
                                    ActivityThread.currentPackageName(), destinationAddress,
                                    scAddress,
                                    text, sentIntent, deliveryIntent, persistMessage, finalPriority,
                                    callbackNumber);
            ...
            try {
                ISms iSms = getISmsServiceOrThrow();
                if (iSms != null) {
                    iSms.sendTextForSubscriberWithCdmaOptions(getSubscriptionId(),
                            ActivityThread.currentPackageName(), destinationAddress,
                            scAddress,
                            text, sentIntent, deliveryIntent, persistMessage, finalPriority,
                            callbackNumber);
        ...
        if (parts.size() > 1) {
            ...
            if (persistMessage) {
                resolveSubscriptionForOperation(new SubscriptionResolverResult() {
                    ...
                    public void onSuccess(int subId) {
                        try {
                            ISms iSms = getISmsServiceOrThrow();
                            if (iSms != null) {
                                iSms.sendMultipartTextForSubscriberWithCdmaOptions(subId,
                                        ActivityThread.currentPackageName(), destinationAddress,
                                        scAddress, parts, sentIntents, deliveryIntents,
                                        persistMessage, finalPriority, callbackNumber);
                            }
                        } catch (RemoteException e) {
                            Log.e(TAG, "sendMultipartTextMessageInternal: Couldn't send SMS - "
                                    + e.getMessage());
                ...
                try {
                    ISms iSms = getISmsServiceOrThrow();
                    if (iSms != null) {
                        iSms.sendMultipartTextForSubscriberWithCdmaOptions(getSubscriptionId(),
                                ActivityThread.currentPackageName(), destinationAddress,
                                scAddress, parts, sentIntents, deliveryIntents,
                                persistMessage, finalPriority, callbackNumber);
                    }
            ...
            sendTextMessageInternal(destinationAddress, scAddress, parts.get(0),
                    sentIntent, deliveryIntent, persistMessage, priority, callbackNumber);
        }
    }
