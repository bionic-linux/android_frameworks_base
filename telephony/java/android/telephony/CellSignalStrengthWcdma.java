...
public final class CellSignalStrengthWcdma extends CellSignalStrength implements Parcelable {
    ...
    private static final int WCDMA_MAX_NUM_LEVELS = 5; // (values 0-4)
    ...
    public void updateLevel(PersistableBundle cc, ServiceState ss) {
        ...
        if (cc == null) {
            ...
            if (rscpThresholds == null) rscpThresholds = sRscpThresholds;
        }

        int level;
        switch (calcMethod) {
            case LEVEL_CALCULATION_METHOD_RSCP:
                level = Math.min(rscpThresholds.length, WCDMA_MAX_NUM_LEVELS);
                ...
                return;
            default:
                log("Invalid Level Calculation Method for CellSignalStrengthWcdma = " + calcMethod);
                /** fall through */
            case LEVEL_CALCULATION_METHOD_RSSI:
                level = Math.min(sRssiThresholds.length, WCDMA_MAX_NUM_LEVELS);
