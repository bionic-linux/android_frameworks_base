...
public class RcsGroupThread extends RcsThread {
    ...
    public void setOwner(@Nullable RcsParticipant participant) {
        try {
            IRcs iRcs = IRcs.Stub.asInterface(ServiceManager.getService("ircs"));
            if (iRcs != null) {
                iRcs.setGroupThreadOwner(mThreadId, participant.getId());
    ...
    public void addParticipant(RcsParticipant participant) {
        try {
            IRcs iRcs = IRcs.Stub.asInterface(ServiceManager.getService("ircs"));
            if (iRcs != null) {
                iRcs.addParticipantToGroupThread(mThreadId, participant.getId());
    ...
    public void removeParticipant(RcsParticipant participant) {
        ...
        try {
            IRcs iRcs = IRcs.Stub.asInterface(ServiceManager.getService("ircs"));
            if (iRcs != null) {
                iRcs.removeParticipantFromGroupThread(mThreadId, participant.getId());
     ...
     *
     * @return the unmodifyable set of {@link RcsParticipant} in this group thread.
     */
