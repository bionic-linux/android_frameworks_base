...
/**
 * RcsGroupThread represents a single RCS conversation thread with three or more
 * {@link RcsParticipant}s. It is possible that an {@link RcsParticipant} to leave an
 * {@link RcsGroupThread} with three participants, however the thread does not get converted into
 * an {@link Rcs1To1Thread} if that happens.
 *
...
public class RcsGroupThread extends RcsThread {
    ...
    public void setGroupName(String groupName) {
        if (TextUtils.equals(mGroupName, groupName)) {
            return;
        }
    ...
    public void setGroupIcon(Uri groupIcon) {
        if ((mGroupIcon != null && mGroupIcon.equals(groupIcon)) || (mGroupIcon == null
                && groupIcon == null)) {
            return;
        }
    ...
    public void setOwner(@Nullable RcsParticipant participant) {
        if (mOwnerParticipant != null && mOwnerParticipant.equals(participant)) {
            return;
        }
    ...
    public void addParticipant(RcsParticipant participant) {
        if (participant == null || mParticipants.contains(participant)) {
            return;
        }
    ...
    public void setConferenceUri(String conferenceUri) {
        if (TextUtils.equals(mConferenceUri, conferenceUri)) {
            return;
        }
        mConferenceUri = conferenceUri;
