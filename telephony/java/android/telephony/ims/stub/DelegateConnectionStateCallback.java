...
public interface DelegateConnectionStateCallback {
     ...
     * modifying the IMS registration to remove this feature tag and requires application to perform
     * an action before the IMS registration can change. The specific action required for the
     * SipDelegate to continue modifying the IMS registration can be found in the definition of each
     * {@link DelegateRegistrationState.DeregisteringReason}.
     * <p>
     ...
     * @param deniedFeatureTags A List of Pairs, where the first parameter contains the feature
     *     tags associated with this {@link SipDelegateConnection} that have no access to
     *     send/receive SIP messages and the second parameter contains a reason for why the feature
     *     tag is denied. For more information on the reason why the feature tag was denied access,
     *     see the {@link SipDelegateManager.DeniedReason} reasons.
     */
