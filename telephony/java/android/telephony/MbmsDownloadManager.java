...
import android.telephony.mbms.IDownloadProgressCallback;
...
public class MbmsDownloadManager {
     ...
     * @param progressListener Optional callback that will provide progress updates if the app
     *                         is running.
     */
    public void download(DownloadRequest request, IDownloadProgressCallback progressListener)
            throws MbmsException {
        IMbmsDownloadService downloadService = mService.get();
        if (downloadService == null) {
            throw new MbmsException(MbmsException.ERROR_MIDDLEWARE_NOT_BOUND);
