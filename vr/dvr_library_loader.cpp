...
JNIEXPORT jlong JNICALL Java_com_google_vr_platform_Dvr_nativeLoadLibrary(JNIEnv* env, jclass,
                                                                          jstring java_library) {
    if (!java_library) return 0;
    ...
    return reinterpret_cast<jlong>(dlopen(library.c_str(), RTLD_NOW | RTLD_LOCAL));
}
