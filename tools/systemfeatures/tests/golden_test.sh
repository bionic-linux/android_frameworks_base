...
# We're running via command-line, ensure we have fresh srcs and use absolute paths.
if [[ $(basename $0) == "golden_test.sh" ]]; then
  if [ -z $ANDROID_BUILD_TOP ]; then
    echo "You need to source and lunch before you can use this script directly."
    exit 1
  fi
  GEN_DIR="$ANDROID_BUILD_TOP/out/soong/.intermediates/frameworks/base/tools/systemfeatures/systemfeatures-gen-tests-srcs/gen/$GEN_DIR"
  GOLDEN_DIR="$ANDROID_BUILD_TOP/frameworks/base/tools/systemfeatures/$GOLDEN_DIR"
  rm -rf "$GEN_DIR"
  "$ANDROID_BUILD_TOP"/build/soong/soong_ui.bash --make-mode systemfeatures-gen-tests-srcs
fi
