...
/*
  virtual void TearDown() {
    if (!DeleteFile(tmpDir_, true)) {
      LOG(ERROR) << "Failed to remove temp directory: " << tmpDir_.value();
    }
  }
  */
  ...
  void CreateStubAidlFile(const std::string& package_class,
    ...
    std::unique_ptr<char[]> contents(new char[buf_len]);
  ...
  void CheckFileContents(const FilePath& rel_path,
    ...
    if (actual_contents.find(expected_content) == string::npos) {
      ...
      std::unique_ptr<char[]> diff_cmd(new char[buf_len]);
