 ...
 */
static string make_constant_name(const string& str) {
    string result;
...
static const char* cpp_type_name(java_type_t type) {
    switch (type) {
...
static const char* java_type_name(java_type_t type) {
    switch (type) {
        case JAVA_TYPE_BOOLEAN:
            return "boolean";
        case JAVA_TYPE_INT:
...
static int write_stats_log_cpp(FILE* out, const Atoms& atoms, const AtomDecl& attributionDecl) {
    ...
    for (set<vector<java_type_t>>::const_iterator signature = atoms.signatures.begin();
        ...
        fprintf(out,
                "          if ((android::elapsedRealtimeNano() - lastRetryTimestampNs) <= "
...
static void write_cpp_method_header(FILE* out, const string& method_name,
                                    const set<vector<java_type_t>>& signatures,
...
static int write_stats_log_header(FILE* out, const Atoms& atoms, const AtomDecl& attributionDecl) {
    // Print prelude
    fprintf(out, "// This file is autogenerated\n");
    fprintf(out, "\n");
...
static void write_java_method(FILE* out, const string& method_name,
                              const set<vector<java_type_t>>& signatures,
                              const AtomDecl& attributionDecl) {
    for (set<vector<java_type_t>>::const_iterator signature = signatures.begin();
         signature != signatures.end(); signature++) {
...
static int write_stats_log_java(FILE* out, const Atoms& atoms, const AtomDecl& attributionDecl) {
    // Print prelude
    fprintf(out, "// This file is autogenerated\n");
    fprintf(out, "\n");
...
static const char* jni_type_name(java_type_t type) {
    switch (type) {
        case JAVA_TYPE_BOOLEAN:
            return "jboolean";
        case JAVA_TYPE_INT:
...
static const char* jni_array_type_name(java_type_t type) {
    switch (type) {
        case JAVA_TYPE_INT:
            return "jintArray";
        case JAVA_TYPE_STRING:
...
static string jni_function_name(const string& method_name, const vector<java_type_t>& signature) {
...
static const char* java_type_signature(java_type_t type) {
    switch (type) {
        case JAVA_TYPE_BOOLEAN:
            return "Z";
        case JAVA_TYPE_INT:
...
static string jni_function_signature(const vector<java_type_t>& signature,
                                     const AtomDecl& attributionDecl) {
    string result("(I");
    for (vector<java_type_t>::const_iterator arg = signature.begin(); arg != signature.end();
         arg++) {
...
static int write_stats_log_jni(FILE* out, const string& java_method_name,
                               const string& cpp_method_name,
                               const set<vector<java_type_t>>& signatures,
                               const AtomDecl& attributionDecl) {
    // Print write methods
    for (set<vector<java_type_t>>::const_iterator signature = signatures.begin();
        ...
        for (vector<java_type_t>::const_iterator arg = signature->begin(); arg != signature->end();
            ...
            if (*arg == JAVA_TYPE_STRING) {
                ...
                fprintf(out,
                        ...
                        argIndex, argIndex);
                fprintf(out, "        str%d_length = env->GetArrayLength(arg%d);\n", argIndex,
                        argIndex);
                fprintf(out,
                        "        str%d = "
...
static int write_stats_log_jni(FILE* out, const Atoms& atoms, const AtomDecl& attributionDecl) {
    // Print prelude
    fprintf(out, "// This file is autogenerated\n");
    fprintf(out, "\n");
...
static int run(int argc, char const* const* argv) {
    ...
    if (cppFilename.size() != 0) {
        ...
        errorCount = android::stats_log_api_gen::write_stats_log_cpp(out, atoms, attributionDecl);
        fclose(out);
    }
    ...
    if (headerFilename.size() != 0) {
        ...
        errorCount =
                android::stats_log_api_gen::write_stats_log_header(out, atoms, attributionDecl);
        fclose(out);
    }
    ...
    if (javaFilename.size() != 0) {
        ...
        errorCount = android::stats_log_api_gen::write_stats_log_java(out, atoms, attributionDecl);
        fclose(out);
    }
    ...
    if (jniFilename.size() != 0) {
        ...
        errorCount = android::stats_log_api_gen::write_stats_log_jni(out, atoms, attributionDecl);
        fclose(out);
    }
