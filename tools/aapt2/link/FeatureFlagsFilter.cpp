...
class FlagsVisitor : public xml::Visitor {
 public:
  explicit FlagsVisitor(
      android::IDiagnostics* diagnostics,
      const std::unordered_map<std::string, std::optional<bool>>& feature_flag_values,
 ...
 private:
  bool ShouldRemove(std::unique_ptr<xml::Node>& node) {
    if (const auto* el = NodeCast<Element>(node.get())) {
      ...
      bool negated = false;
      StringPiece flag_name = util::TrimWhitespace(attr->value);
  ...
  const std::unordered_map<std::string, std::optional<bool>>& feature_flag_values_;
