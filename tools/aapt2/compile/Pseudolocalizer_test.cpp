...
static ::testing::AssertionResult CompoundHelper(const char* in1, const char* in2, const char* in3,
  ...
  Pseudolocalizer pseudo(method);
  std::string result =
      pseudo.Start() + pseudo.Text(in1) + pseudo.Text(in2) + pseudo.Text(in3) + pseudo.End();
  if (result != expected) {
    return ::testing::AssertionFailure() << expected << " != " << result;
  }
  return ::testing::AssertionSuccess();
...
TEST(PseudolocalizerTest, NoPseudolocalization) {
  ...
  EXPECT_TRUE(
      CompoundHelper("Hello,", " world", "", "Hello, world", Pseudolocalizer::Method::kNone));
}

TEST(PseudolocalizerTest, PlaintextAccent) {
  EXPECT_TRUE(SimpleHelper("", "[]", Pseudolocalizer::Method::kAccent));
  EXPECT_TRUE(
      SimpleHelper("Hello, world", "[Ĥéļļö, ŵöŕļð one two]", Pseudolocalizer::Method::kAccent));

  EXPECT_TRUE(
      SimpleHelper("Hello, %1d", "[Ĥéļļö, »%1d« one two]", Pseudolocalizer::Method::kAccent));

  EXPECT_TRUE(
      SimpleHelper("Battery %1d%%", "[βåţţéŕý »%1d«%% one two]", Pseudolocalizer::Method::kAccent));
...
TEST(PseudolocalizerTest, PlaintextBidi) {
  ...
  EXPECT_TRUE(SimpleHelper("word", "\xe2\x80\x8f\xE2\x80\xaeword\xE2\x80\xac\xe2\x80\x8f",
...
TEST(PseudolocalizerTest, SimpleICU) {
  ...
  EXPECT_TRUE(SimpleHelper("{placeholder}", "[»{placeholder}«]", Pseudolocalizer::Method::kAccent));
  ...
  EXPECT_TRUE(SimpleHelper("Copy from {path1} to {path2}",
                           ...
                           Pseudolocalizer::Method::kAccent));
  EXPECT_TRUE(SimpleHelper("Today is {1,date} {1,time}", "[Ţöðåý îš »{1,date}« »{1,time}« one two]",
                           Pseudolocalizer::Method::kAccent));

  // Multi-fragment messages
  EXPECT_TRUE(CompoundHelper("{USER}", " ", "is offline", "[»{USER}« îš öƒƒļîñé one two]",
                             Pseudolocalizer::Method::kAccent));
...
TEST(PseudolocalizerTest, ICUBidi) {
  ...
  EXPECT_TRUE(SimpleHelper("{placeholder}",
                           ...
                           Pseudolocalizer::Method::kBidi));
  EXPECT_TRUE(SimpleHelper("{COUNT, plural, one {one} other {other}}",
                           "{COUNT, plural, "
...
TEST(PseudolocalizerTest, Escaping) {
  // Single-fragment messages
  EXPECT_TRUE(SimpleHelper("'{USER'} is offline", "['{ÛŠÉŔ'} îš öƒƒļîñé one two three]",
                           Pseudolocalizer::Method::kAccent));

  // Multi-fragment messages
  EXPECT_TRUE(CompoundHelper("'{USER}", " ", "''is offline", "['{ÛŠÉŔ} ''îš öƒƒļîñé one two three]",
                             Pseudolocalizer::Method::kAccent));
}

TEST(PseudolocalizerTest, PluralsAndSelects) {
  EXPECT_TRUE(SimpleHelper("{COUNT, plural, one {Delete a file} other {Delete {COUNT} files}}",
                           "[{COUNT, plural, one {Ðéļéţé å ƒîļé one two} "
                           "other {Ðéļéţé »{COUNT}« ƒîļéš one two}}]",
                           Pseudolocalizer::Method::kAccent));

  EXPECT_TRUE(SimpleHelper("Distance is {COUNT, plural, one {# mile} other {# miles}}",
                           "[Ðîšţåñçé îš {COUNT, plural, one {# ḿîļé one two} "
                           "other {# ḿîļéš one two}}]",
                           Pseudolocalizer::Method::kAccent));

  EXPECT_TRUE(
      SimpleHelper("{1, select, female {{1} added you} "
