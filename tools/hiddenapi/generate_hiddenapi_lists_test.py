...
class TestHiddenapiListGeneration(unittest.TestCase):
    ...
    def test_get_valid_subset_of_unassigned_keys(self):
        ...
        self.assertEqual(flags.get_valid_subset_of_unassigned_apis(['A', 'B', 'D']), set([ 'B' ]))
    ...
    def test_assign_flag(self):
        ...
        # Test assigning an already assigned flag.
        flags.assign_flag(FLAG_WHITELIST, [ 'A' ])
        self.assertEquals(flags.generate_csv(), [ 'A,' + FLAG_WHITELIST, 'B' ])

        # Test new additions.
        flags.assign_flag(FLAG_GREYLIST, [ 'A', 'B' ])
        self.assertEquals(flags.generate_csv(),
            [ 'A,' + FLAG_GREYLIST + "," + FLAG_WHITELIST, 'B,' + FLAG_GREYLIST ])

        # Test invalid API signature.
        with self.assertRaises(AssertionError):
            flags.assign_flag(FLAG_WHITELIST, [ 'C' ])

        # Test invalid flag.
        with self.assertRaises(AssertionError):
            flags.assign_flag('foo', [ 'A' ])
