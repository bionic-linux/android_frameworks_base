...
status_t compileXmlFile(const Bundle* bundle, const sp<AaptAssets>& assets,
                        ...
                        ResourceTable* table, int options) {
...
status_t compileXmlFile(const Bundle* bundle, const sp<AaptAssets>& assets,
                        const String16& resourceName, const sp<AaptFile>& target,
                        const sp<AaptFile>& outTarget, ResourceTable* table, int options) {
    sp<XMLNode> root = XMLNode::parse(target);
    if (root == NULL) {
        return UNKNOWN_ERROR;
    }
...
status_t compileXmlFile(const Bundle* bundle, const sp<AaptAssets>& assets,
                        const String16& resourceName, const sp<XMLNode>& root,
                        const sp<AaptFile>& target, ResourceTable* table, int options) {
    if (table->versionForCompat(bundle, resourceName, target, root)) {
        ...
        sp<AaptDir> dir = resDir->getDirs().valueFor(
    ...
    status_t err = root->flatten(target, (options & XML_COMPILE_STRIP_COMMENTS) != 0,
...
static const char16_t referenceArray[] = {'r', 'e', 'f', 'e', 'r', 'e', 'n', 'c', 'e'};
static const char16_t stringArray[] = {'s', 't', 'r', 'i', 'n', 'g'};
static const char16_t integerArray[] = {'i', 'n', 't', 'e', 'g', 'e', 'r'};
static const char16_t booleanArray[] = {'b', 'o', 'o', 'l', 'e', 'a', 'n'};
static const char16_t colorArray[] = {'c', 'o', 'l', 'o', 'r'};
static const char16_t floatArray[] = {'f', 'l', 'o', 'a', 't'};
static const char16_t dimensionArray[] = {'d', 'i', 'm', 'e', 'n', 's', 'i', 'o', 'n'};
static const char16_t fractionArray[] = {'f', 'r', 'a', 'c', 't', 'i', 'o', 'n'};
static const char16_t enumArray[] = {'e', 'n', 'u', 'm'};
static const char16_t flagsArray[] = {'f', 'l', 'a', 'g', 's'};
...
static const flag_entry gFormatFlags[] = {
        ...
        {fractionArray, sizeof(fractionArray) / 2, ResTable_map::TYPE_FRACTION,
         ...
         "as \"<code>14.5%</code>\".\n"
         "The % suffix always means a percentage of the base size; the optional %p suffix provides "
         "a size relative to\n"
...
static uint32_t parse_flags(const char16_t* str, size_t len, const flag_entry* flags,
                            bool* outError = NULL) {
...
static void appendTypeInfo(ResourceTable* outTable, const String16& pkg, const String16& typeName,
                           const String16& ident, int type, const flag_entry* flags) {
    bool hadType = false;
    while (flags->name) {
...
struct PendingAttribute {
    ...
    PendingAttribute(String16 _package, const sp<AaptFile>& in, ResXMLTree& block,
                     bool _appendComment)
        : myPackage(_package),
...
static status_t compileAttribute(const sp<AaptFile>& in, ResXMLTree& block,
                                 const String16& myPackage, ResourceTable* outTable,
                                 String16* outIdent = NULL, bool inStyleable = false) {
    ...
    attr.comment = String16(block.getComment(&len) ? block.getComment(&len) : nulStr);
    ...
    if (minIdx >= 0) {
        ...
        attr.createIfNeeded(outTable);
        if (!attr.hasErrors) {
            err = outTable->addBag(attr.sourcePos, myPackage, attr16, attr.ident, String16(""),
                                   String16("^min"), String16(val), NULL, NULL);
            if (err != NO_ERROR) {
                attr.hasErrors = true;
            }
        }
    ...
    if (maxIdx >= 0) {
        ...
        if (!attr.hasErrors) {
            err = outTable->addBag(attr.sourcePos, myPackage, attr16, attr.ident, String16(""),
                                   String16("^max"), String16(val), NULL, NULL);
            attr.hasErrors = true;
        }
    }
    ...
    if (l10nIdx >= 0) {
        ...
        if (!attr.hasErrors) {
            ...
            err = outTable->addBag(attr.sourcePos, myPackage, attr16, attr.ident, String16(""),
                                   String16("^l10n"), String16(buf), NULL, NULL);
            if (err != NO_ERROR) {
                attr.hasErrors = true;
            }
        }
        ...
        if (code == ResXMLTree::START_TAG) {
            ...
            if ((attr.type & (ResTable_map::TYPE_ENUM | ResTable_map::TYPE_FLAGS)) == 0) {
                ...
                if (!attr.hasErrors) {
                    ...
                    err = outTable->addBag(
                            SourcePos(in->getPrintableSource(), block.getLineNumber()), myPackage,
                            attr16, attr.ident, String16(""), String16("^type"),
                            String16(numberStr), NULL, NULL, true);
                    if (err != NO_ERROR) {
                        attr.hasErrors = true;
                    }
                }
            } else if ((uint32_t)(attr.type & (ResTable_map::TYPE_ENUM |
                                               ResTable_map::TYPE_FLAGS)) != localType) {
            ...
            if (!attr.hasErrors) {
                if (enumOrFlagsComment.size() == 0) {
                    enumOrFlagsComment.append(mayOrMust(
                            attr.type, ResTable_map::TYPE_ENUM | ResTable_map::TYPE_FLAGS));
                    enumOrFlagsComment.append(
                            (attr.type & ResTable_map::TYPE_ENUM)
                                    ? String16(" be one of the following constant values.")
                                    : String16(" be one or more (separated by '|') of the "
                ...
                err = outTable->addBag(SourcePos(in->getPrintableSource(), block.getLineNumber()),
                                       myPackage, attr16, attr.ident, String16(""), itemIdent,
...
status_t parseAndAddBag(Bundle* bundle, const sp<AaptFile>& in, ResXMLTree* block,
                        const ResTable_config& config, const String16& myPackage,
                        const String16& curType, const String16& ident, const String16& parentIdent,
                        const String16& itemIdent, int32_t curFormat, bool isFormatted,
                        const String16& /* product */, PseudolocalizationMethod pseudolocalize,
                        const bool overwrite, ResourceTable* outTable) {
    status_t err;
    const String16 item16("item");

    String16 str;
    Vector<StringPool::entry_style_span> spans;
    err = parseStyledString(bundle, in->getPrintableSource().string(), block, item16, &str, &spans,
    ...
    if (kIsDebug) {
        printf("Adding resource bag entry l=%c%c c=%c%c orien=%d d=%d "
               " pid=%s, bag=%s, id=%s: %s\n",
               config.language[0], config.language[1], config.country[0], config.country[1],
               config.orientation, config.density, String8(parentIdent).string(),
               String8(ident).string(), String8(itemIdent).string(), String8(str).string());
    }

    err = outTable->addBag(SourcePos(in->getPrintableSource(), block->getLineNumber()), myPackage,
                           curType, ident, parentIdent, itemIdent, str, &spans, &config, overwrite,
                           false, curFormat);
    return err;
}
...
status_t parseAndAddEntry(Bundle* bundle, const sp<AaptFile>& in, ResXMLTree* block,
                          const ResTable_config& config, const String16& myPackage,
                          const String16& curType, const String16& ident, const String16& curTag,
                          bool curIsStyled, int32_t curFormat, bool isFormatted,
                          const String16& product, PseudolocalizationMethod pseudolocalize,
    ...
    String16 str;
    Vector<StringPool::entry_style_span> spans;
    err = parseStyledString(bundle, in->getPrintableSource().string(), block, curTag, &str,
                            curIsStyled ? &spans : NULL, isFormatted, pseudolocalize);
    ...
    if (product.size() != 0) {
        ...
        if (bundleProduct[0] == '\0') {
            if (strcmp16(String16("default").string(), product.string()) != 0) {
                 ...
                 */
                skippedResourceNames->replaceValueFor(type_ident_pair_t(curType, ident), true);
                return NO_ERROR;
            }
        } else {
            /*
    ...
    if (kIsDebug) {
        printf("Adding resource entry l=%c%c c=%c%c orien=%d d=%d id=%s: %s\n", config.language[0],
               config.language[1], config.country[0], config.country[1], config.orientation,
               config.density, String8(ident).string(), String8(str).string());
    }

    err = outTable->addEntry(SourcePos(in->getPrintableSource(), block->getLineNumber()), myPackage,
                             curType, ident, str, &spans, &config, false, curFormat, overwrite);
...
status_t compileResourceFile(Bundle* bundle, const sp<AaptAssets>& assets, const sp<AaptFile>& in,
    ...
    size_t len;
    if (code != ResXMLTree::START_TAG) {
        SourcePos(in->getPrintableSource(), block.getLineNumber()).error("No start tag found\n");
        return UNKNOWN_ERROR;
    }
    if (strcmp16(block.getElementName(&len), resources16.string()) != 0) {
        SourcePos(in->getPrintableSource(), block.getLineNumber())
                .error("Invalid start tag %s\n", String8(block.getElementName(&len)).string());
        return UNKNOWN_ERROR;
    }
    ...
    if ((bundle->getPseudolocalize() & PSEUDO_ACCENTED &&
        ...
        SourcePos(in->getPrintableSource(), 0)
                .warning(
        ...
        if (code == ResXMLTree::START_TAG) {
            ...
            if (strcmp16(block.getElementName(&len), skip16.string()) == 0) {
                ...
                if (identIdx >= 0) {
                    ...
                    srcPos.error(
                            "No 'id' attribute supplied <public>,"
                ...
                if (startIdx >= 0) {
                    ...
                    srcPos.error(
                            "No 'start' attribute supplied <public-padding>,"
                            " and no previous id defined in this file.\n");
                    hasErrors = localHasErrors = true;
                } else if (!localHasErrors) {
                ...
                for (uint32_t curIdent = start; curIdent <= end; curIdent++) {
                    ...
                    err = outTable->addEntry(srcPos, myPackage, type, curName, String16("padding"),
                                             NULL, &curParams, false, ResTable_map::TYPE_STRING,
                                             overwrite);
                    if (err < NO_ERROR) {
                        hasErrors = localHasErrors = true;
                        break;
                    }
                    err = outTable->addPublic(srcPos, myPackage, type, curName, curIdent);
                    if (err < NO_ERROR) {
                        hasErrors = localHasErrors = true;
                        break;
                    }
                ...
                if (pkgIdx < 0) {
                    SourcePos(in->getPrintableSource(), block.getLineNumber())
                            .error("A 'package' attribute is required for <private-symbols>\n");
                    hasErrors = localHasErrors = true;
                }
                ...
                if (!localHasErrors) {
                    ...
                    if (assets->havePrivateSymbols()) {
                        SourcePos(in->getPrintableSource(), block.getLineNumber())
                                .warning("private symbol package already specified. Ignoring...\n");
                    } else {
                ...
                while ((code = block.next()) != ResXMLTree::END_DOCUMENT &&
                    ...
                    if (code == ResXMLTree::START_TAG) {
                        if (strcmp16(block.getElementName(&len), skip16.string()) == 0) {
                            ...
                            SourcePos(in->getPrintableSource(), block.getLineNumber())
                                    .error("Tag <%s> can not appear inside <declare-styleable>, "
                                           "only <attr>\n",
                                           String8(block.getElementName(&len)).string());
                            return UNKNOWN_ERROR;
                        }
                        ...
                        SourcePos(in->getPrintableSource(), block.getLineNumber())
                                .error("Found tag </%s> where </attr> is expected\n",
                                       String8(block.getElementName(&len)).string());
                        return UNKNOWN_ERROR;
                    }
                }
                ...
                if (attri >= 0) {
                    ...
                    if (formatIdx >= 0) {
                        String16 formatStr =
                                String16(block.getAttributeStringValue(formatIdx, &len));
                        curFormat = parse_flags(formatStr.string(), formatStr.size(), gFormatFlags);
                        if (curFormat == 0) {
                            SourcePos(in->getPrintableSource(), block.getLineNumber())
                                    .error("Tag <item> 'format' attribute value \"%s\" not valid\n",
                    ...
                    SourcePos(in->getPrintableSource(), block.getLineNumber())
                            .error("A 'type' attribute is required for <item>\n");
                    hasErrors = localHasErrors = true;
                }
                curIsStyled = true;
                ...
                if (name.size() > 0) {
                    ...
                    if (translatable == false16) {
                        ...
                        if (locale.size() > 0) {
                            SourcePos(in->getPrintableSource(), block.getLineNumber())
                                    .warning(
                                            "string '%s' marked untranslatable but exists in "
                                            "locale '%s'\n",
                                            String8(name).string(), locale.string());
                            // hasErrors = localHasErrors = true;
                        } else {
                ...
                if (attri >= 0) {
                    ...
                    SourcePos(in->getPrintableSource(), block.getLineNumber())
                            .error("A 'type' attribute is required for <bag>\n");
                    hasErrors = localHasErrors = true;
                }
                ...
                ssize_t formatIdx = block.indexOfAttribute(NULL, "format");
                if (formatIdx >= 0) {
                    String16 formatStr = String16(block.getAttributeStringValue(formatIdx, &len));
                    curFormat = parse_flags(formatStr.string(), formatStr.size(), gFormatFlags);
                    if (curFormat == 0) {
                        SourcePos(in->getPrintableSource(), block.getLineNumber())
                                .error("Tag <array> 'format' attribute value \"%s\" not valid\n",
                                       String8(formatStr).string());
                        hasErrors = localHasErrors = true;
                    }
                }
                ...
                SourcePos(in->getPrintableSource(), block.getLineNumber())
                        .error("Found tag %s where item is expected\n",
                               String8(block.getElementName(&len)).string());
                return UNKNOWN_ERROR;
            }
            ...
            if (identIdx >= 0) {
                ...
                SourcePos(in->getPrintableSource(), block.getLineNumber())
                        .error("A 'name' attribute is required for <%s>\n",
            ...
            if (curIsBag) {
                ...
                if (!localHasErrors) {
                    err = outTable->startBag(
                            SourcePos(in->getPrintableSource(), block.getLineNumber()), myPackage,
                            curType, ident, parentIdent, &curParams, overwrite,
                ...
                while ((code = block.next()) != ResXMLTree::END_DOCUMENT &&
                    ...
                    if (code == ResXMLTree::START_TAG) {
                        if (strcmp16(block.getElementName(&len), item16.string()) != 0) {
                            SourcePos(in->getPrintableSource(), block.getLineNumber())
                                    .error("Tag <%s> can not appear inside <%s>, only <item>\n",
                                           String8(block.getElementName(&len)).string(),
                                           String8(*curTag).string());
                            return UNKNOWN_ERROR;
                        }
                        ...
                        if (curType == array16) {
                            ...
                            if (itemIdentIdx >= 0) {
                                ...
                                if (quantity16 == other16) {
                                    ...
                                    SourcePos(in->getPrintableSource(), block.getLineNumber())
                                            .error("Illegal 'quantity' attribute is <item> inside "
                                                   "<plurals>\n");
                                    hasErrors = localHasErrors = true;
                                }
                            } else {
                                SourcePos(in->getPrintableSource(), block.getLineNumber())
                                        .error("A 'quantity' attribute is required for <item> "
                                               "inside <plurals>\n");
                                hasErrors = localHasErrors = true;
                            }
                        } else {
                            ...
                            if (itemIdentIdx >= 0) {
                                itemIdent =
                                        String16(block.getAttributeStringValue(itemIdentIdx, &len));
                            } else {
                                SourcePos(in->getPrintableSource(), block.getLineNumber())
                                        .error("A 'name' attribute is required for <item>\n");
                                hasErrors = localHasErrors = true;
                            }
                        }
                        ...
                        err = parseAndAddBag(bundle, in, &block, curParams, myPackage, curType,
                                             ...
                                             curIsFormatted, product, NO_PSEUDOLOCALIZATION,
                                             overwrite, outTable);
                        if (err == NO_ERROR) {
                            if (curIsPseudolocalizable && localeIsDefined(curParams) &&
                                ...
                                if ((PSEUDO_ACCENTED & bundle->getPseudolocalize()) ==
                                    ...
                                    err = parseAndAddBag(bundle, in, &block, pseudoParams,
                                                         myPackage, curType, ident, parentIdent,
                                                         itemIdent, curFormat, curIsFormatted,
                                                         product, PSEUDO_ACCENTED, overwrite,
                                                         outTable);
                                }
                                if ((PSEUDO_BIDI & bundle->getPseudolocalize()) == PSEUDO_BIDI) {
                                    block.setPosition(parserPosition);
                                    err = parseAndAddBag(bundle, in, &block, pseudoBidiParams,
                                                         myPackage, curType, ident, parentIdent,
                                                         itemIdent, curFormat, curIsFormatted,
                                                         product, PSEUDO_BIDI, overwrite, outTable);
                                }
                            }
                        }
                        if (err != NO_ERROR) {
                        ...
                        if (strcmp16(block.getElementName(&len), curTag->string()) != 0) {
                            SourcePos(in->getPrintableSource(), block.getLineNumber())
                                    .error("Found tag </%s> where </%s> is expected\n",
                                           String8(block.getElementName(&len)).string(),
                                           String8(*curTag).string());
                            return UNKNOWN_ERROR;
                        }
                ...
                err = parseAndAddEntry(bundle, in, &block, curParams, myPackage, curType, ident,
                                       *curTag, curIsStyled, curFormat, curIsFormatted, product,
                                       NO_PSEUDOLOCALIZATION, overwrite, &skippedResourceNames,
                                       outTable);
                ...
                if (err < NO_ERROR) {  // Why err < NO_ERROR instead of err != NO_ERROR?
                    ...
                    if (curIsPseudolocalizable && localeIsDefined(curParams) &&
                        bundle->getPseudolocalize() > 0) {
                        // pseudolocalize here
                        if ((PSEUDO_ACCENTED & bundle->getPseudolocalize()) == PSEUDO_ACCENTED) {
                            block.setPosition(parserPosition);
                            err = parseAndAddEntry(bundle, in, &block, pseudoParams, myPackage,
                                                   curType, ident, *curTag, curIsStyled, curFormat,
                                                   curIsFormatted, product, PSEUDO_ACCENTED,
                                                   overwrite, &skippedResourceNames, outTable);
                        }
                        if ((PSEUDO_BIDI & bundle->getPseudolocalize()) == PSEUDO_BIDI) {
                            block.setPosition(parserPosition);
                            err = parseAndAddEntry(bundle, in, &block, pseudoBidiParams, myPackage,
                                                   curType, ident, *curTag, curIsStyled, curFormat,
                                                   curIsFormatted, product, PSEUDO_BIDI, overwrite,
            ...
            SourcePos(in->getPrintableSource(), block.getLineNumber())
                    .error("Found text \"%s\" where item tag is expected\n",
...
status_t ResourceTable::addIncludedResources(Bundle* bundle, const sp<AaptAssets>& assets) {
    ...
    if (!featureAfter.isEmpty()) {
        ...
        mTypeIdOffset =
                std::max(mTypeIdOffset, findLargestTypeIdForPackage(featureTable, mAssetsPackage));
    }

    return NO_ERROR;
}

status_t ResourceTable::addPublic(const SourcePos& sourcePos, const String16& package,
                                  const String16& type, const String16& name,
                                  const uint32_t ident) {
    uint32_t rid = mAssets->getIncludedResources().identifierForName(
            name.string(), name.size(), type.string(), type.size(), package.string(),
            package.size());
    if (rid != 0) {
...
status_t ResourceTable::addEntry(const SourcePos& sourcePos, const String16& package,
                                 const String16& type, const String16& name, const String16& value,
                                 const Vector<StringPool::entry_style_span>* style,
                                 const ResTable_config* params, const bool doSetIndex,
                                 const int32_t format, const bool overwrite) {
    uint32_t rid = mAssets->getIncludedResources().identifierForName(
            name.string(), name.size(), type.string(), type.size(), package.string(),
            package.size());
    if (rid != 0) {
    ...
    sp<Entry> e = getEntry(package, type, name, sourcePos, overwrite, params, doSetIndex);
    if (e == NULL) {
        return UNKNOWN_ERROR;
    }
...
status_t ResourceTable::startBag(const SourcePos& sourcePos, const String16& package,
                                 const String16& type, const String16& name,
                                 const String16& bagParent, const ResTable_config* params,
                                 bool overlay, bool replace, bool /* isId */) {
    ...
    if (overlay && !mBundle->getAutoAddOverlay() && !hasBagOrEntry(package, type, name)) {
        ...
        if (!canAdd) {
            sourcePos.error(
                    "Resource does not already exist in overlay at '%s'; use <add-resource> to "
                    "add.\n",
                    String8(name).string());
            return UNKNOWN_ERROR;
        }
...
status_t ResourceTable::addBag(const SourcePos& sourcePos, const String16& package,
                               const String16& type, const String16& name,
                               const String16& bagParent, const String16& bagKey,
                               const String16& value,
                               const Vector<StringPool::entry_style_span>* style,
                               const ResTable_config* params, bool replace, bool isId,
...
bool ResourceTable::hasBagOrEntry(const String16& package, const String16& type,
...
bool ResourceTable::hasBagOrEntry(const String16& package, const String16& type,
                                  const String16& name, const ResTable_config& config) const {
    // First look for this in the included resources...
    uint32_t rid = mAssets->getIncludedResources().identifierForName(
            name.string(), name.size(), type.string(), type.size(), package.string(),
            package.size());
...
bool ResourceTable::hasBagOrEntry(const String16& ref, const String16* defType,
                                  const String16* defPackage) {
    String16 package, type, name;
    if (!ResTable::expandResourceRef(ref.string(), ref.size(), &package, &type, &name, defType,
                                     defPackage ? defPackage : &mAssetsPackage, NULL)) {
        return false;
    }
    return hasBagOrEntry(package, type, name);
}

bool ResourceTable::appendComment(const String16& package, const String16& type,
                                  const String16& name, const String16& comment, bool onlyIfEmpty) {
    if (comment.size() <= 0) {
        return true;
    }
...
bool ResourceTable::appendTypeComment(const String16& package, const String16& type,
                                      const String16& name, const String16& comment) {
    if (comment.size() <= 0) {
        return true;
    }
...
bool ResourceTable::makeAttribute(const String16& package, const String16& name,
                                  const SourcePos& source, int32_t format, const String16& comment,
                                  bool shouldAppendComment) {
    const String16 attr16("attr");

    // First look for this in the included resources...
    uint32_t rid = mAssets->getIncludedResources().identifierForName(
            name.string(), name.size(), attr16.string(), attr16.size(), package.string(),
            package.size());
    if (rid != 0) {
    ...
    if (idx >= 0) {
        ...
        if ((format & (ResTable_map::TYPE_ENUM | ResTable_map::TYPE_FLAGS)) ||
            ...
            source.error(
                    "Attribute \"%s\" already defined with incompatible format.\n"
...
void ResourceTable::canAddEntry(const SourcePos& pos, const String16& package, const String16& type,
                                const String16& name) {
...
inline uint32_t ResourceTable::getResId(const sp<Package>& p, const sp<Type>& t, uint32_t nameId) {
    return makeResId(p->getAssignedId(), t->getIndex(), nameId);
}

uint32_t ResourceTable::getResId(const String16& package, const String16& type,
                                 const String16& name, bool onlyPublic) const {
    ...
    return ResourceIdCache::store(package, type, name, onlyPublic, getResId(p, t, ei));
}

uint32_t ResourceTable::getResId(const String16& ref, const String16* defType,
                                 const String16* defPackage, const char** outErrorMsg,
                                 bool onlyPublic) const {
    String16 package, type, name;
    bool refOnlyPublic = true;
    if (!ResTable::expandResourceRef(ref.string(), ref.size(), &package, &type, &name, defType,
                                     defPackage ? defPackage : &mAssetsPackage, outErrorMsg,
                                     &refOnlyPublic)) {
        if (kIsDebug) {
    ...
    if (kIsDebug) {
        printf("Expanded resource: p=%s, t=%s, n=%s, res=%d\n", String8(package).string(),
               String8(type).string(), String8(name).string(), res);
    }
    if (res == 0) {
        if (outErrorMsg) *outErrorMsg = "No resource found that matches the given name";
    }
    return res;
}
...
bool ResourceTable::stringToValue(Res_value* outValue, StringPool* pool, const String16& str,
                                  bool preserveSpaces, bool coerceType, uint32_t attrID,
                                  const Vector<StringPool::entry_style_span>* style,
                                  String16* outStr, void* accessorCookie, uint32_t attrType,
    ...
    if (style == NULL || style->size() == 0) {
        ...
        res = mAssets->getIncludedResources().stringToValue(
                outValue, &finalStr, str.string(), str.size(), preserveSpaces, coerceType, attrID,
                NULL, &mAssetsPackage, this, accessorCookie, attrType);
    } else {
        ...
        if (pool) {
            ...
            if (kIsDebug) {
                printf("Adding to pool string style #%zu config %s: %s\n",
                       style != NULL ? style->size() : 0U, configStr.string(),
                       String8(finalStr).string());
            }
            if (style != NULL && style->size() > 0) {
...
uint32_t ResourceTable::getCustomResource(const String16& package, const String16& type,
                                          const String16& name) const {
...
uint32_t ResourceTable::getCustomResourceWithCreation(const String16& package, const String16& type,
                                                      const String16& name,
...
bool ResourceTable::getAttributeKeys(uint32_t attrID, Vector<String16>* outKeys) {
...
bool ResourceTable::getAttributeEnum(uint32_t attrID, const char16_t* name, size_t nameLen,
                                     Res_value* outValue) {
...
bool ResourceTable::getAttributeFlags(uint32_t attrID, const char16_t* name, size_t nameLen,
                                      Res_value* outValue) {
        ...
        for (ti = 0; ti < N; ti++) {
            ...
            for (size_t ci = 0; ci < N; ci++) {
                ...
                if (Res_GETPACKAGE(rid) + 1 == p->getAssignedId()) {
                    if (skipSymbolsWithoutDefaultLocalization && t->getName() == stringType) {
...
void ResourceTable::addLocalization(const String16& name, const String8& locale,
                                    const SourcePos& src) {
    mLocalizations[name][locale] = src;
}

void ResourceTable::addDefaultLocalization(const String16& name) {
 ...
 */
status_t ResourceTable::validateLocalizations(void) {
        ...
        if (mBundle->getConfigurations().size() > 0 && mBundle->getRequireLocalization()) {
            ...
            if (!missingConfigs.empty()) {
                ...
                SourcePos().warning("string '%s' is missing %u required localizations:%s",
                                    String8(nameIter.first).string(),
                                    (unsigned int)missingConfigs.size(), configStr.string());
            }
        }
    }
...
status_t ResourceTable::flatten(Bundle* bundle, const sp<const ResourceFilter>& filter,
    ...
    for (size_t i = 0; i < basePackageCount; i++) {
        ...
        if (packageId > 0x01 && packageId != 0x7f && packageName != String16("android")) {
    ...
    for (pi = 0; pi < N; pi++) {
        ...
        for (size_t ti = 0; ti < N; ti++) {
            ...
            if (configTypeName == "drawable" || configTypeName == "layout" ||
                ...
                configTypeName == "xml" || configTypeName == "menu" || configTypeName == "mipmap" ||
                configTypeName == "raw") {
            ...
            for (size_t ci = 0; ci < N; ci++) {
                ...
                for (size_t ei = 0; ei < N; ei++) {
                    ...
                    status_t err = e->prepareFlatten(&valueStrings, this, &configTypeName, &config);
                    if (err != NO_ERROR) {
                        return err;
                    }
                    allEntries.add(e);
    ...
    for (pi = 0; pi < N; pi++) {
        ...
        for (size_t ti = 0; ti < N; ti++) {
            ...
            LOG_ALWAYS_FATAL_IF(t == NULL && typeName != String16("<empty>"),
                                "Type name %s not found", String8(typeName).string());
            if (t == NULL) {
                continue;
            }
            const bool filterable = (typeName != mipmap16);
            ...
            // each resource entry in this type.
            {
                ...
                ResTable_typeSpec* tsHeader =
                        (ResTable_typeSpec*)(((uint8_t*)data->editData(typeSpecStart +
                                                                       typeSpecSize)) +
                                             typeSpecStart);
                if (tsHeader == NULL) {
                ...
                uint32_t* typeSpecFlags = (uint32_t*)(((uint8_t*)data->editData()) + typeSpecStart +
            ...
            for (size_t ci = 0; ci < NC; ci++) {
                ...
                if (kIsDebug) {
                    printf("Writing config %zu config: imsi:%d/%d lang:%c%c cnt:%c%c "
                           ...
                           ti + 1, config.mcc, config.mnc,
                           config.language[0] ? config.language[0] : '-',
                           config.language[1] ? config.language[1] : '-',
                           config.country[0] ? config.country[0] : '-',
                           config.country[1] ? config.country[1] : '-', config.orientation,
                           config.uiMode, config.touchscreen, config.density, config.keyboard,
                           config.inputFlags, config.navigation, config.screenWidth,
                           config.screenHeight, config.smallestScreenWidthDp, config.screenWidthDp,
                           config.screenHeightDp, config.screenLayout);
                }
                ...
                ResTable_type* tHeader =
                        (ResTable_type*)(((uint8_t*)data->editData(typeStart + typeSize)) +
                                         typeStart);
                if (tHeader == NULL) {
                ...
                if (kIsDebug) {
                    printf("Writing type %zu config: imsi:%d/%d lang:%c%c cnt:%c%c "
                           "orien:%d ui:%d touch:%d density:%d key:%d inp:%d nav:%d sz:%dx%d "
                           "sw%ddp w%ddp h%ddp layout:%d\n",
                           ti + 1, tHeader->config.mcc, tHeader->config.mnc,
                           ...
                           tHeader->config.orientation, tHeader->config.uiMode,
                           tHeader->config.touchscreen, tHeader->config.density,
                           tHeader->config.keyboard, tHeader->config.inputFlags,
                           tHeader->config.navigation, tHeader->config.screenWidth,
                           tHeader->config.screenHeight, tHeader->config.smallestScreenWidthDp,
                           tHeader->config.screenWidthDp, tHeader->config.screenHeightDp,
                ...
                for (size_t ei = 0; ei < N; ei++) {
                    ...
                    uint32_t* index = (uint32_t*)(((uint8_t*)data->editData()) + typeStart +
                ...
                tHeader = (ResTable_type*)(((uint8_t*)data->editData()) + typeStart);
            ...
            if (bundle->getSplitConfigurations().isEmpty()) {
                ...
                const char* log_prefix =
                        bundle->getErrorOnMissingConfigEntry() ? "error" : "warning";
                for (size_t i = 0; i < N; ++i) {
                    if (!validResources[i]) {
                        sp<ConfigList> c = t->getOrderedConfigs().itemAt(i);
                        if (c != NULL) {
                            fprintf(stderr, "%s: no entries written for %s/%s (0x%08zx)\n",
                                    log_prefix, String8(typeName).string(),
                                    String8(c->getName()).string(),
    ...
    for (pi = 0; pi < flatPackages.size(); pi++) {
        err = dest->writeData(flatPackages[pi]->getData(), flatPackages[pi]->getSize());
        if (err != NO_ERROR) {
    ...
    ResTable_header* header = (ResTable_header*)(((uint8_t*)dest->getData()) + dataStart);
    header->header.size = htodl(dest->getSize() - dataStart);

    if (kPrintStringMetrics) {
        fprintf(stderr, "**** total resource table size: %zu / %zu%% strings\n", dest->getSize(),
                (size_t)(strAmt * 100) / dest->getSize());
    }

    return NO_ERROR;
}

status_t ResourceTable::flattenLibraryTable(const sp<AaptFile>& dest,
                                            const Vector<sp<Package>>& libs) {
    ...
    if (libs.size() > 0) {
        ...
        ResTable_lib_header* libHeader =
                (ResTable_lib_header*)dest->editDataInRange(libStart, sizeof(ResTable_lib_header));
        ...
        for (size_t i = 0; i < NT; i++) {
            ...
            for (size_t j = 0; j < NC; j++) {
                ...
                if (!didHeader) {
                    if (pub) {
                        fprintf(fp,
                                "  <!-- PUBLIC SECTION.  These resources have been declared "
                                "public.\n");
                        fprintf(fp,
                                "       Changes to these definitions will break binary "
                                "compatibility. -->\n\n");
                    } else {
                        fprintf(fp,
                                "  <!-- PRIVATE SECTION.  These resources have not been declared "
                                "public.\n");
                        fprintf(fp,
                                "       You can make them public my moving these lines into a file "
                                "in res/values. -->\n\n");
                    }
                    didHeader = true;
                }
                ...
                fprintf(fp, "  <public type=\"%s\" name=\"%s\" id=\"0x%08x\" />\n",
                        String8(t->getName()).string(), String8(c->getName()).string(),
...
ResourceTable::Item::Item(const SourcePos& _sourcePos, bool _isId, const String16& _value,
                          const Vector<StringPool::entry_style_span>* _style, int32_t _format)
...
status_t ResourceTable::Entry::makeItABag(const SourcePos& sourcePos) {
    ...
    sourcePos.error(
            ...
            String8(mName).string(), mItem.sourcePos.file.string(), mItem.sourcePos.line);
    return UNKNOWN_ERROR;
}

status_t ResourceTable::Entry::setItem(const SourcePos& sourcePos, const String16& value,
                                       const Vector<StringPool::entry_style_span>* style,
                                       int32_t format, const bool overwrite) {
    ...
    if (mType == TYPE_BAG) {
        if (mBag.size() == 0) {
            ...
            sourcePos.error(
                    ...
                    "%s:%d: Originally defined here.\n",
                    String8(mName).string(), item.sourcePos.file.string(), item.sourcePos.line);
        }
        return UNKNOWN_ERROR;
    }
...
status_t ResourceTable::Entry::addToBag(const SourcePos& sourcePos, const String16& key,
                                        const String16& value,
                                        const Vector<StringPool::entry_style_span>* style,
    ...
    if (origKey >= 0) {
        if (!replace) {
            ...
            sourcePos.error(
                    ...
                    "%s:%d: Originally defined here.\n",
                    String8(mName).string(), String8(key).string(), item.sourcePos.file.string(),
                    item.sourcePos.line);
            return UNKNOWN_ERROR;
        }
...
status_t ResourceTable::Entry::generateAttributes(ResourceTable* table, const String16& package) {
    const String16 attr16("attr");
    const String16 id16("id");
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        if (it.isId) {
            if (!table->hasBagOrEntry(key, &id16, &package)) {
                ...
                status_t err = table->addEntry(SourcePos(String8("<generated>"), 0), package, id16,
                                               key, value);
                if (err != NO_ERROR) {
                    return err;
                }
...
//             return UNKNOWN_ERROR;
            ...
            status_t err =
                    table->addBag(SourcePos("<generated>", 0), package, attr16, key, String16(""),
                                  String16("^type"), String16(numberStr), NULL, NULL);
            if (err != NO_ERROR) {
                return err;
            }
#endif
...
status_t ResourceTable::Entry::assignResourceIds(ResourceTable* table,
    ...
    if (mType == TYPE_BAG) {
        ...
        for (size_t i = 0; i < N; i++) {
            ...
            it.bagKeyId = table->getResId(key, it.isId ? &id16 : &attr16, NULL, &errorMsg);
...
status_t ResourceTable::Entry::prepareFlatten(StringPool* strings, ResourceTable* table,
    ...
    if (mType == TYPE_ITEM) {
        ...
        if (!table->stringToValue(&it.parsedValue, strings, it.value, false, true, 0, &it.style,
                                  NULL, &ac, mItemFormat, configTypeName, config)) {
            return UNKNOWN_ERROR;
        }
        ...
        for (size_t i = 0; i < N; i++) {
            ...
            if (!table->stringToValue(&it.parsedValue, strings, it.value, false, true, it.bagKeyId,
                                      &it.style, NULL, &ac, it.format, configTypeName, config)) {
                return UNKNOWN_ERROR;
            }
        }
    } else {
...
ssize_t ResourceTable::Entry::flatten(Bundle* /* bundle */, const sp<AaptFile>& data,
                                      bool isPublic) {
    size_t amt = 0;
...
void ResourceTable::ConfigList::appendComment(const String16& comment, bool onlyIfEmpty) {
    if (comment.size() <= 0) {
        return;
    }
...
status_t ResourceTable::Type::addPublic(const SourcePos& sourcePos, const String16& name,
                                        const uint32_t ident) {
#if 0
    ...
    if (typeIdx >= 0) {
        ...
        if (mPublicIndex > 0 && mPublicIndex != typeIdx) {
            sourcePos.error(
                    "Public resource %s/%s has conflicting type codes for its"
                    " public identifiers (0x%x vs 0x%x).\n",
                    String8(mName).string(), String8(name).string(), mPublicIndex, typeIdx);
            return UNKNOWN_ERROR;
        }
    ...
    if (mPublic.indexOfKey(name) < 0) {
        ...
        if (p.ident != ident) {
            sourcePos.error(
                    "Public resource %s/%s has conflicting public identifiers"
                    " (0x%08x vs 0x%08x).\n"
                    "%s:%d: Originally defined here.\n",
...
sp<ResourceTable::Entry> ResourceTable::Type::getEntry(const String16& entry,
    ...
    if (c == NULL) {
        if (overlay && !autoAddOverlay && mCanAddEntries.indexOf(entry) < 0) {
            sourcePos.error(
                    "Resource at %s appears in overlay but not"
        ...
        if (kIsDebug) {
            if (config != NULL) {
                printf("New entry at %s:%d: imsi:%d/%d lang:%c%c cnt:%c%c "
                       ...
                       "sw%ddp w%ddp h%ddp layout:%d\n",
                       sourcePos.file.string(), sourcePos.line, config->mcc, config->mnc,
                       config->language[0] ? config->language[0] : '-',
                       config->language[1] ? config->language[1] : '-',
                       config->country[0] ? config->country[0] : '-',
                       config->country[1] ? config->country[1] : '-', config->orientation,
                       config->touchscreen, config->density, config->keyboard, config->inputFlags,
                       config->navigation, config->screenWidth, config->screenHeight,
                       config->smallestScreenWidthDp, config->screenWidthDp, config->screenHeightDp,
                       config->screenLayout);
            } else {
                printf("New entry at %s:%d: NULL config\n", sourcePos.file.string(),
                       sourcePos.line);
            }
        }
        ...
        if (doSetIndex) {
            if (pos < 0) {
                ...
                if (pos >= (int)mOrderedConfigs.size()) {
                    sourcePos.error("Internal error: config not found in mOrderedConfigs when adding
        entry"); return NULL;
                }
            }
            e->setEntryIndex(pos);
...
sp<ResourceTable::ConfigList> ResourceTable::Type::removeEntry(const String16& entry) {
    ...
    Vector<sp<ConfigList>>::iterator iter =
            std::find(mOrderedConfigs.begin(), mOrderedConfigs.end(), removed);
        ...
        for (i = 0; i < N; i++) {
            ...
            if (e->getName() == name) {
                ...
                if (mOrderedConfigs.itemAt(idx) == NULL) {
                    ...
                    p.sourcePos.error(
                            "Multiple entry names declared for public entry"
                            " identifier 0x%x in type %s (%s vs %s).\n"
                            "%s:%d: Originally defined here.",
                            idx + 1, String8(mName).string(), String8(oe->getName()).string(),
                            String8(name).string(), oe->getPublicSourcePos().file.string(),
...
status_t ResourceTable::Package::setStrings(const sp<AaptFile>& data, ResStringPool* strings,
    ...
    for (i = 0; i < N; i++) {
        ...
        if (idx > 0) {
            ...
            if (mOrderedTypes.itemAt(idx) != NULL) {
                ...
                t->getFirstPublicSourcePos().error(
                        "Multiple type names declared for public type"
                        " identifier 0x%x (%s vs %s).\n"
                        "%s:%d: Originally defined here.",
                        idx, String8(ot->getName()).string(), String8(t->getName()).string(),
...
void ResourceTable::Package::movePrivateAttrs() {
    ...
    for (size_t i = 0; i < privateAttrCount; i++) {
        ...
        for (size_t j = 0; j < entryCount; j++) {
            ...
            sp<Entry> entry =
                    privateAttrType->getEntry(cl->getName(), oldEntry->getPos(), &entries.keyAt(j));
            *entry = *oldEntry;
        }
...
sp<ResourceTable::Type> ResourceTable::getType(const String16& package, const String16& type,
                                               const SourcePos& sourcePos, bool doSetIndex) {
...
sp<ResourceTable::Entry> ResourceTable::getEntry(const String16& package, const String16& type,
                                                 const String16& name, const SourcePos& sourcePos,
                                                 bool overlay, const ResTable_config* config,
                                                 bool doSetIndex) {
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        if (it.bagKeyId == 0) {
            fprintf(stderr, "warning: ID not yet assigned to '%s' in bag '%s'\n",
                    String8(e->getName()).string(), String8(e->getBag().keyAt(i)).string());
        }
...
bool ResourceTable::getItemValue(uint32_t resID, uint32_t attrID, Res_value* outValue) {
        ...
        if (kIsDebug) {
            if (res) {
                printf("getItemValue of #%08x[#%08x] (%s): type=#%08x, data=#%08x\n", resID, attrID,
                       String8(getEntry(resID)->getName()).string(), outValue->dataType,
                       outValue->data);
            } else {
...
bool ResourceTable::shouldGenerateVersionedResource(const sp<ResourceTable::ConfigList>& configList,
    ...
    for (size_t pi = 0; pi < packageCount; pi++) {
        ...
        for (size_t ti = 0; ti < typeCount; ti++) {
            ...
            for (size_t ci = 0; ci < configCount; ci++) {
                ...
                const DefaultKeyedVector<ConfigDescription, sp<Entry>>& entries = c->getEntries();
                const size_t entryCount = entries.size();
                for (size_t ei = 0; ei < entryCount; ei++) {
                    ...
                    for (size_t i = 0; i < sdkCount; i++) {
                        ...
                        entriesToAdd.add(key_value_pair_t<ConfigDescription, sp<Entry>>(newConfig,
                                                                                        newEntry));
                    }
                ...
                for (size_t i = 0; i < entriesToAddCount; i++) {
                    ...
                    if (bundle->getVerbose()) {
                        entriesToAdd[i].value->getPos().printf(
                                "using v%d attributes; synthesizing resource %s:%s/%s for "
                                "configuration %s.",
                                entriesToAdd[i].key.sdkVersion, String8(p->getName()).string(),
                                String8(t->getName()).string(),
                                String8(entriesToAdd[i].value->getName()).string(),
                                entriesToAdd[i].key.toString().string());
                    }

                    sp<Entry> newEntry = t->getEntry(c->getName(), entriesToAdd[i].value->getPos(),
                                                     &entriesToAdd[i].key);
...
static bool IsTransitionElement(const String16& name) {
    for (int i = 0, size = sizeof(kTransitionElements) / sizeof(kTransitionElements[0]); i < size;
         ++i) {
...
bool ResourceTable::versionForCompat(const Bundle* bundle, const String16& resourceName,
    ...
    sp<AaptFile> newFile = new AaptFile(target->getSourceFile(), AaptGroupEntry(newConfig),
                                        target->getResourceType());
    String8 resPath = String8::format(
            "res/%s/%s.xml", newFile->getGroupEntry().toDirName(target->getResourceType()).string(),
            String8(resourceName).string());
    resPath.convertToResPath();

    // Add a resource table entry.
    addEntry(SourcePos(), String16(mAssets->getPackage()), String16(target->getResourceType()),
             resourceName, String16(resPath), NULL, &newConfig);
...
status_t ResourceTable::modifyForCompat(const Bundle* bundle, const String16& resourceName,
                                        const sp<AaptFile>& target, const sp<XMLNode>& root) {
    ...
    while (!nodesToVisit.isEmpty()) {
        ...
        if (bundle->getNoVersionVectors() &&
            (node->getElementName() == vector16 || node->getElementName() == animatedVector16 ||
             node->getElementName() == objectAnimator16 ||
             node->getElementName() == pathInterpolator16 || node->getElementName() == gradient16 ||
        ...
        for (size_t i = 0; i < attrs.size(); i++) {
            ...
            if (sdkLevel > 1 && sdkLevel > config.sdkVersion && sdkLevel > minSdk) {
                ...
                if (bundle->getVerbose()) {
                    SourcePos(node->getFilename(), node->getStartLineNumber())
                            .printf("removing attribute %s%s%s from <%s>",
                                    String8(attr.ns).string(), (attr.ns.size() == 0 ? "" : ":"),
    ...
    if (shouldGenerateVersionedResource(cl, config, sdkVersionToGenerate)) {
        ...
        // Add a resource table entry.
        if (bundle->getVerbose()) {
            SourcePos(target->getSourceFile(), -1)
                    .printf("using v%d attributes; synthesizing resource %s:%s/%s for "
                            "configuration %s.",
                            newConfig.sdkVersion, mAssets->getPackage().string(),
            ...
            for (size_t c = 0; c < configCount; c++) {
                ...
                for (size_t ce = 0; ce < configEntryCount; ce++) {
                    ...
                    if (AaptConfig::isDensityOnly(config)) {
                        ...
                        const Symbol symbol(mOrderedPackages[p]->getName(), type->getName(),
                                            configList->getName(),
 ...
 * </vector>
 */
status_t ResourceTable::processBundleFormat(const Bundle* bundle, const String16& resourceName,
                                            const sp<AaptFile>& target, const sp<XMLNode>& root) {
...
status_t ResourceTable::processBundleFormatImpl(const Bundle* bundle, const String16& resourceName,
    ...
    for (size_t i = 0; i < children.size(); i++) {
        ...
        if (child->getElementNamespace() != kAaptNamespaceUri16 ||
            ...
            status_t result =
                    processBundleFormatImpl(bundle, resourceName, target, child, namespaces);
            if (result != NO_ERROR) {
                return result;
            }
        ...
        bool result = ResTable::expandResourceRef(attr->string.string(), attr->string.size(),
                                                  &attrPackage, &attrType, &attrName, &kAttr16,
                                                  &kAssetPackage16, &errorMsg, NULL);
        if (!result) {
        ...
        while (true) {
            ...
            nestedResourceName = String8::format("%s_%d", String8(resourceName).string(), suffix++);
            nestedResourcePath = String8::format(
                    "res/%s/%s.xml",
                    target->getGroupEntry().toDirName(target->getResourceType()).string(),
                    nestedResourceName.string());

            // Lookup or create the entry for this name.
            sp<Entry> entry = getEntry(kAssetPackage16, String16(target->getResourceType()),
                                       String16(nestedResourceName), source, false,
                                       &target->getGroupEntry().toParams(), true);
            if (entry == NULL) {
                return UNKNOWN_ERROR;
            }
        ...
        if (bundle->getVerbose()) {
            source.printf("generating nested resource %s:%s/%s", mAssets->getPackage().string(),
                          target->getResourceType().string(), nestedResourceName.string());
        }

        // Build the attribute reference and assign it to the parent.
        String16 nestedResourceRef = String16(
                String8::format("@%s:%s/%s", mAssets->getPackage().string(),
                                target->getResourceType().string(), nestedResourceName.string()));
