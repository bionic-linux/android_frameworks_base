...
static const String16 k_expansion_string = String16(
        "one two three "
...
Pseudolocalizer::Pseudolocalizer(PseudolocalizationMethod m) : mImpl(nullptr), mLastDepth(0) {
    setMethod(m);
}
...
static const char* pseudolocalize_char(const char16_t c) {
    switch (c) {
        case 'a':
            return "\xc3\xa5";
        case 'b':
...
String16 PseudoMethodAccent::end() {
    ...
    if (mLength) {
        ...
        result.append(pseudo_generate_expansion(mWordCount > 3 ? mLength : mLength / 2));
    }
    mWordCount = mLength = 0;
    mDepth--;
    if (mDepth == 0) {
