...
void MethodBuilder::BuildReturn() { instructions_.push_back(Instruction{Op::kReturn}); }

void MethodBuilder::BuildReturn(Value src) {
  instructions_.push_back(Instruction{Op::kReturn, /*destination=*/{}, src});
}

void MethodBuilder::BuildConst4(Value target, int value) {
  DCHECK_LT(value, 16);
  instructions_.push_back(Instruction{Op::kMove, target, Value::Immediate(value)});
}
...
const MethodDeclData& DexBuilder::GetOrDeclareMethod(TypeDescriptor type, const std::string& name,
  ...
  if (entry.decl == nullptr) {
    ...
    decl->prototype = prototype.Encode(this);
