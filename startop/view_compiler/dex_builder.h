...
class Instruction {
  public:
    ...
    static inline Instruction InvokeVirtual(size_t method_id, std::optional<const Value> dest,
        ...
        return Instruction{Op::kInvokeVirtual, method_id, /*result_is_object=*/false, dest,
                           this_arg,           args...};
    }
    // Returns an object
    template <typename... T>
    static inline Instruction InvokeVirtualObject(size_t method_id, std::optional<const Value> dest,
                                                  Value this_arg, T... args) {
        return Instruction{Op::kInvokeVirtual, method_id, /*result_is_object=*/true, dest,
                           this_arg,           args...};
    }
    ...
    static inline Instruction InvokeDirect(size_t method_id, std::optional<const Value> dest,
                                           Value this_arg, T... args) {
        return Instruction{Op::kInvokeDirect, method_id, /*result_is_object=*/false, dest,
                           this_arg,          args...};
    }
    // Returns an object
    template <typename... T>
    static inline Instruction InvokeDirectObject(size_t method_id, std::optional<const Value> dest,
                                                 Value this_arg, T... args) {
        return Instruction{Op::kInvokeDirect, method_id, /*result_is_object=*/true, dest,
                           this_arg,          args...};
    }
    // For static calls.
    template <typename... T>
    ...
    static inline Instruction InvokeInterface(size_t method_id, std::optional<const Value> dest,
                                              T... args) {
        return Instruction{Op::kInvokeInterface, method_id, /*result_is_object=*/false, dest,
                           args...};
    }
...
class MethodBuilder {
  public:
    ...
    // Create a new register to be used to storing values. Note that these are not SSA registers,
    // like might be expected in similar code generators. This does no liveness tracking or
    // anything, so it's up to the caller to reuse registers as appropriate.
    Value MakeRegister();
  ...
  private:
    ...
    // bound, a reference will be saved and it will automatically be patched when the label is
    // bound.
    ...
    // Stores information needed to back-patch a label once it is bound. We need to know the start
    // of the instruction that refers to the label, and the offset to where the actual label value
    // should go.
    ...
    // During encoding, keep track of the largest number of arguments needed, so we can use it for
    // our outs count
    size_t max_args_{0};
};
...
class DexBuilder {
  ...
  private:
    ...
    // We'll need to allocate buffers for all of the encoded strings we create. This is where we
    // store all of them.
    ...
    // When we go to actually write the DEX file, slicer will re-assign these after correctly
    // sorting the methods list.
...
void MethodBuilder::BuildNew(Value target, TypeDescriptor type, Prototype constructor, T... args) {
    ...
    AddInstruction(Instruction::OpWithArgs(Instruction::Op::kNew, target,
