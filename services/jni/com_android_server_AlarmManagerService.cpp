...
{
public:
    AlarmImplTimerFd(int fds[N_ANDROID_TIMERFDS]) :
        AlarmImpl(fds, N_ANDROID_TIMERFDS) { }
...
{
    pollfd pollfds[N_ANDROID_TIMERFDS];
    for (size_t i = 0; i < N_ANDROID_TIMERFDS; i++) {
        pollfds[i].fd = fds[i];
        pollfds[i].events = POLLIN;
    }

    int err = poll(pollfds, N_ANDROID_TIMERFDS, -1);
    if (err < 0)
        return err;
    ...
    for (size_t i = 0; i < N_ANDROID_TIMERFDS; i++) {
        if (pollfds[i].revents & POLLIN) {
            uint64_t unused;
            ssize_t err = read(fds[i], &unused, sizeof(unused));
            if (err < 0) {
                if (i == ANDROID_ALARM_TYPE_COUNT && errno == ECANCELED)
...
static jlong init_timerfd()
{
    ...
    AlarmImpl *ret = new AlarmImplTimerFd(fds);
