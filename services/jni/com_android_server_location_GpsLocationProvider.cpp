...
static jbyteArray convert_v4_ip(uint32_t ip, bool net_order)
{
...
static jbyteArray convert_v6_ip(const jbyte *ip)
{
    jbyteArray byteArray = NULL;
    ...
    if (ip) {
        ...
        if (byteArray) {
            env->SetByteArrayRegion(byteArray, 0, 16, ip);
        } else {
...
static void agps_status_callback(AGpsStatus* agps_status)
{
    ...
    switch (agps_status->size)
    {
    case sizeof(AGpsStatus_v1):
        // do nothing
        break;
    case sizeof(AGpsStatus_v2):
    {
        byteArray = convert_v4_ip(agps_status->ipaddr, false);
    }
        break;
    case sizeof(AGpsStatus_v3):
    {
        byteArray = convert_v4_ip(agps_status->ipaddr, false);
        if (NULL == byteArray) {
            AGpsStatus_v3 *agpsStatus_v3 = (AGpsStatus_v3*)agps_status;
            byteArray = convert_v6_ip((const jbyte*)&(agpsStatus_v3->ipv6_addr));
        }
    }
        break;
    case sizeof(AGpsStatus_v4):
        switch (agps_status->addr.ss_family)
        {
        case AF_INET:
        {
            struct sockaddr_in *s = (struct sockaddr_in*)&(agps_status->addr);
            uint32_t *pAddr = (uint32_t*)&(s->sin_addr);
            byteArray = convert_v4_ip(*pAddr, true);
        }
            break;
        case AF_INET6:
        {
            struct sockaddr_in6 *s = (struct sockaddr_in6*)&(agps_status->addr);
            byteArray = convert_v6_ip((const jbyte*)&(s->sin6_addr));
        }
            break;
        default:
            isSupported = false;
        ...
        jstring apn, jint apnIpType)
{
    ...
    if (sizeof(AGpsInterface_v1) == sAGpsInterface->size) {
        ...
        sAGpsInterface->data_conn_open_with_apniptype(apnStr, apnIpType);
    }
