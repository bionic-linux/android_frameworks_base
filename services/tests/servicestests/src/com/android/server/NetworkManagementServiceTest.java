...
public class NetworkManagementServiceTest extends AndroidTestCase {
    ...
    public void testNetworkObservers() throws Exception {
        ...
        expectSoon(observer).interfaceClassDataActivityChanged(eq("rmnet0"), eq(true), anyLong());

        sendMessage("613 IfaceClass active rmnet0 1234");
        expectSoon(observer).interfaceClassDataActivityChanged(eq("rmnet0"), eq(true), eq(1234l));

        sendMessage("613 IfaceClass idle eth0");
        expectSoon(observer).interfaceClassDataActivityChanged(eq("eth0"), eq(false), anyLong());

        sendMessage("613 IfaceClass idle eth0 1234");
        expectSoon(observer).interfaceClassDataActivityChanged(eq("eth0"), eq(false), eq(1234l));

        sendMessage("613 IfaceClass reallyactive rmnet0 1234");
        expectSoon(observer).interfaceClassDataActivityChanged(eq("rmnet0"), eq(false), eq(1234l));
