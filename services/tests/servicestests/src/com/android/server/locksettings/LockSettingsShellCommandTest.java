...
public class LockSettingsShellCommandTest {
    ...
    @Test
    public void testSetStrongAuth_STRONG_AUTH_REQUIRED_AFTER_USER_LOCKDOWN() throws Exception {
        when(mLockPatternUtils.isSecure(mUserId)).thenReturn(true);

        assertEquals(0, mCommand.exec(new Binder(), in, out, err,
                new String[] { "set-strong-auth", "STRONG_AUTH_REQUIRED_AFTER_USER_LOCKDOWN"},
                mShellCallback, mResultReceiver));
