...
static struct { jclass clazz; } gInputDeviceClassInfo;

static struct { jclass clazz; } gKeyEventClassInfo;

static struct { jclass clazz; } gMotionEventClassInfo;
...
static jobject getInputApplicationHandleObjLocalRef(
        JNIEnv* env, const sp<InputApplicationHandle>& inputApplicationHandle) {
...
static void loadSystemIconAsSpriteWithPointerIcon(JNIEnv* env, jobject contextObj, int32_t style,
    ...
    status_t status =
            android_view_PointerIcon_loadSystemIcon(env, contextObj, style, outPointerIcon);
    if (!status) {
...
class NativeInputManager : public virtual RefBase,
  ...
  public:
    ...
    virtual bool checkInjectEventsPermissionNonReentrant(int32_t injectorPid, int32_t injectorUid);
        ...
        if (v != NULL && *v != viewport) {
            ...
            *v = viewport;
            ...
            if (viewportType == ViewportType::VIEWPORT_INTERNAL) {
                ...
                if (controller != NULL) {
                    controller->setDisplayViewport(viewport.logicalRight - viewport.logicalLeft,
...
void NativeInputManager::getReaderConfiguration(InputReaderConfiguration* outConfig) {
    ...
    jint virtualKeyQuietTime =
            env->CallIntMethod(mServiceObj, gServiceClassInfo.getVirtualKeyQuietTimeMillis);
    if (!checkAndClearExceptionFromCallback(env, "getVirtualKeyQuietTimeMillis")) {
        outConfig->virtualKeyQuietTime = milliseconds_to_nanoseconds(virtualKeyQuietTime);
    }

    outConfig->excludedDeviceNames.clear();
    jobjectArray excludedDeviceNames = jobjectArray(
            env->CallObjectMethod(mServiceObj, gServiceClassInfo.getExcludedDeviceNames));
    ...
    jint hoverTapTimeout = env->CallIntMethod(mServiceObj, gServiceClassInfo.getHoverTapTimeout);
    if (!checkAndClearExceptionFromCallback(env, "getHoverTapTimeout")) {
        jint doubleTapTimeout =
                env->CallIntMethod(mServiceObj, gServiceClassInfo.getDoubleTapTimeout);
        if (!checkAndClearExceptionFromCallback(env, "getDoubleTapTimeout")) {
            jint longPressTimeout =
                    env->CallIntMethod(mServiceObj, gServiceClassInfo.getLongPressTimeout);
            if (!checkAndClearExceptionFromCallback(env, "getLongPressTimeout")) {
                ...
                jint tapDragInterval =
                        max(min(longPressTimeout - 100, doubleTapTimeout), hoverTapTimeout);
    ...
    jint hoverTapSlop = env->CallIntMethod(mServiceObj, gServiceClassInfo.getHoverTapSlop);
...
sp<PointerControllerInterface> NativeInputManager::obtainPointerController(int32_t /* deviceId */) {
    ...
    if (controller == NULL) {
        ...
        controller->setDisplayViewport(v.logicalRight - v.logicalLeft,
                                       v.logicalBottom - v.logicalTop, v.orientation);
...
void NativeInputManager::notifyInputDevicesChanged(const Vector<InputDeviceInfo>& inputDevices) {
    ...
    jobjectArray inputDevicesObjArray =
            env->NewObjectArray(count, gInputDeviceClassInfo.clazz, NULL);
...
sp<KeyCharacterMap> NativeInputManager::getKeyboardLayoutOverlay(
    ...
    ScopedLocalRef<jobject> identifierObj(
            env,
            env->NewObject(gInputDeviceIdentifierInfo.clazz, gInputDeviceIdentifierInfo.constructor,
                           descriptor.get(), identifier.vendor, identifier.product));
    ScopedLocalRef<jobjectArray> arrayObj(
            env,
            jobjectArray(env->CallObjectMethod(
                    mServiceObj, gServiceClassInfo.getKeyboardLayoutOverlay, identifierObj.get())));
    if (arrayObj.get()) {
...
String8 NativeInputManager::getDeviceAlias(const InputDeviceIdentifier& identifier) {
    ...
    ScopedLocalRef<jstring> aliasObj(
            env, jstring(env->CallObjectMethod(mServiceObj, gServiceClassInfo.getDeviceAlias,
                                               uniqueIdObj.get())));
    String8 result;
    if (aliasObj.get()) {
...
void NativeInputManager::notifySwitch(nsecs_t when, uint32_t switchValues, uint32_t switchMask,
    ...
    env->CallVoidMethod(mServiceObj, gServiceClassInfo.notifySwitch, when, switchValues,
                        switchMask);
...
nsecs_t NativeInputManager::notifyANR(const sp<InputApplicationHandle>& inputApplicationHandle,
    ...
    jlong newTimeout =
            env->CallLongMethod(mServiceObj, gServiceClassInfo.notifyANR, inputApplicationHandleObj,
                                inputWindowHandleObj, reasonObj);
...
void NativeInputManager::setInputWindows(JNIEnv* env, jobjectArray windowHandleObjArray) {
    ...
    for (size_t i = 0; i < numWindows; i++) {
        ...
        if (windowInfo && windowInfo->hasFocus &&
            (windowInfo->inputFeatures &
...
void NativeInputManager::updateInactivityTimeoutLocked(const sp<PointerController>& controller) {
    ...
    controller->setInactivityTimeout(lightsOut ? PointerController::INACTIVITY_TIMEOUT_SHORT
...
TouchAffineTransformation NativeInputManager::getTouchAffineTransformation(JNIEnv* env,
                                                                           jfloatArray matrixArr) {
    ATRACE_CALL();
...
TouchAffineTransformation NativeInputManager::getTouchAffineTransformation(
    ...
    jobject cal =
            env->CallObjectMethod(mServiceObj, gServiceClassInfo.getTouchCalibrationForInputDevice,
                                  descriptorObj.get(), surfaceRotation);

    jfloatArray matrixArr =
            jfloatArray(env->CallObjectMethod(cal, gTouchCalibrationClassInfo.getAffineTransform));
...
bool NativeInputManager::filterInputEvent(const InputEvent* inputEvent, uint32_t policyFlags) {
    ...
    switch (inputEvent->getType()) {
        ...
        case AINPUT_EVENT_TYPE_MOTION:
            inputEventObj = android_view_MotionEvent_obtainAsCopy(
                    env, static_cast<const MotionEvent*>(inputEvent));
            break;
        default:
...
void NativeInputManager::interceptKeyBeforeQueueing(const KeyEvent* keyEvent,
    ...
    if ((policyFlags & POLICY_FLAG_TRUSTED)) {
        ...
        if (keyEventObj) {
            wmActions =
                    env->CallIntMethod(mServiceObj, gServiceClassInfo.interceptKeyBeforeQueueing,
    ...
    if ((policyFlags & POLICY_FLAG_TRUSTED) && !(policyFlags & POLICY_FLAG_INJECTED)) {
        if (policyFlags & POLICY_FLAG_INTERACTIVE) {
            ...
            jint wmActions = env->CallIntMethod(
                    mServiceObj, gServiceClassInfo.interceptMotionBeforeQueueingNonInteractive,
                    when, policyFlags);
...
bool NativeInputManager::dispatchUnhandledKey(const sp<InputWindowHandle>& inputWindowHandle,
    ...
    if (policyFlags & POLICY_FLAG_TRUSTED) {
        ...
        jobject keyEventObj = android_view_KeyEvent_fromNative(env, keyEvent);
        if (keyEventObj) {
            jobject fallbackKeyEventObj =
                    env->CallObjectMethod(mServiceObj, gServiceClassInfo.dispatchUnhandledKey,
                                          inputWindowHandleObj, keyEventObj, policyFlags);
...
bool NativeInputManager::checkInjectEventsPermissionNonReentrant(int32_t injectorPid,
    ...
    jboolean result = env->CallBooleanMethod(
            mServiceObj, gServiceClassInfo.checkInjectEventsPermission, injectorPid, injectorUid);
...
void NativeInputManager::loadPointerIcon(SpriteIcon* icon) {
    ...
    ScopedLocalRef<jobject> pointerIconObj(
            env, env->CallObjectMethod(mServiceObj, gServiceClassInfo.getPointerIcon));
    ...
    PointerIcon pointerIcon;
    status_t status =
            android_view_PointerIcon_load(env, pointerIconObj.get(), mContextObj, &pointerIcon);
...
void NativeInputManager::loadAdditionalMouseResources(
    ...
    for (int iconId = POINTER_ICON_STYLE_CONTEXT_MENU; iconId <= POINTER_ICON_STYLE_GRABBING;
         ++iconId) {
        PointerIcon pointerIcon;
        loadSystemIconAsSpriteWithPointerIcon(env, mContextObj, iconId, &pointerIcon,
                                              &((*outResources)[iconId]));
        if (!pointerIcon.bitmapFrames.empty()) {
            ...
            animationData.animationFrames.push_back(
                    SpriteIcon(pointerIcon.bitmap, pointerIcon.hotSpotX, pointerIcon.hotSpotY));
...
static void nativeSetDisplayViewport(JNIEnv* env, jclass /* clazz */, jlong ptr, jint viewportType,
                                     jint displayId, jint orientation, jint logicalLeft,
                                     jint logicalTop, jint logicalRight, jint logicalBottom,
                                     jint physicalLeft, jint physicalTop, jint physicalRight,
                                     jint physicalBottom, jint deviceWidth, jint deviceHeight,
                                     jstring uniqueId) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static jint nativeGetScanCodeState(JNIEnv* /* env */, jclass /* clazz */, jlong ptr, jint deviceId,
...
static jint nativeGetKeyCodeState(JNIEnv* /* env */, jclass /* clazz */, jlong ptr, jint deviceId,
...
static jint nativeGetSwitchState(JNIEnv* /* env */, jclass /* clazz */, jlong ptr, jint deviceId,
                                 jint sourceMask, jint sw) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static jboolean nativeHasKeys(JNIEnv* env, jclass /* clazz */, jlong ptr, jint deviceId,
                              jint sourceMask, jintArray keyCodes, jbooleanArray outFlags) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static void throwInputChannelNotInitialized(JNIEnv* env) {
    jniThrowException(env, "java/lang/IllegalStateException", "inputChannel is not initialized");
}
...
static void nativeRegisterInputChannel(JNIEnv* env, jclass /* clazz */, jlong ptr,
                                       jobject inputChannelObj, jobject inputWindowHandleObj,
                                       jboolean monitor) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static void nativeSetInputFilterEnabled(JNIEnv* /* env */, jclass /* clazz */, jlong ptr,
                                        jboolean enabled) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static jint nativeInjectInputEvent(JNIEnv* env, jclass /* clazz */, jlong ptr,
                                   jobject inputEventObj, jint displayId, jint injectorPid,
                                   jint injectorUid, jint syncMode, jint timeoutMillis,
                                   jint policyFlags) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static void nativeToggleCapsLock(JNIEnv* env, jclass /* clazz */, jlong ptr, jint deviceId) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);

    im->getInputManager()->getReader()->toggleCapsLockState(deviceId);
}

static void nativeSetInputWindows(JNIEnv* env, jclass /* clazz */, jlong ptr,
...
static void nativeSetFocusedApplication(JNIEnv* env, jclass /* clazz */, jlong ptr,
...
static void nativeSetPointerCapture(JNIEnv* env, jclass /* clazz */, jlong ptr, jboolean enabled) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);

    im->setPointerCapture(enabled);
}

static void nativeSetInputDispatchMode(JNIEnv* /* env */, jclass /* clazz */, jlong ptr,
                                       jboolean enabled, jboolean frozen) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);

    im->setInputDispatchMode(enabled, frozen);
}

static void nativeSetSystemUiVisibility(JNIEnv* /* env */, jclass /* clazz */, jlong ptr,
                                        jint visibility) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static jboolean nativeTransferTouchFocus(JNIEnv* env, jclass /* clazz */, jlong ptr,
                                         jobject fromChannelObj, jobject toChannelObj) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);

    sp<InputChannel> fromChannel = android_view_InputChannel_getInputChannel(env, fromChannelObj);
    sp<InputChannel> toChannel = android_view_InputChannel_getInputChannel(env, toChannelObj);

    if (fromChannel == NULL || toChannel == NULL) {
        return JNI_FALSE;
    }

    if (im->getInputManager()->getDispatcher()->transferTouchFocus(fromChannel, toChannel)) {
        return JNI_TRUE;
    } else {
        return JNI_FALSE;
    }
...
static void nativeSetShowTouches(JNIEnv* /* env */, jclass /* clazz */, jlong ptr,
                                 jboolean enabled) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static void nativeSetInteractive(JNIEnv* env, jclass clazz, jlong ptr, jboolean interactive) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static void nativeVibrate(JNIEnv* env, jclass /* clazz */, jlong ptr, jint deviceId,
                          jlongArray patternObj, jint repeat, jint token) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
    ...
    jlong* patternMillis = static_cast<jlong*>(env->GetPrimitiveArrayCritical(patternObj, NULL));
    nsecs_t pattern[patternSize];
    for (size_t i = 0; i < patternSize; i++) {
        pattern[i] = max(jlong(0), min(patternMillis[i],
                                       (jlong)(MAX_VIBRATE_PATTERN_DELAY_NSECS / 1000000LL))) *
                     1000000LL;
    }
...
static void nativeCancelVibrate(JNIEnv* /* env */, jclass /* clazz */, jlong ptr, jint deviceId,
                                jint token) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static void nativeReloadKeyboardLayouts(JNIEnv* /* env */, jclass /* clazz */, jlong ptr) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static void nativeReloadDeviceAliases(JNIEnv* /* env */, jclass /* clazz */, jlong ptr) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static jboolean nativeIsInputDeviceEnabled(JNIEnv* env /* env */, jclass /* clazz */, jlong ptr,
                                           jint deviceId) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static void nativeEnableInputDevice(JNIEnv* /* env */, jclass /* clazz */, jlong ptr,
                                    jint deviceId) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static void nativeDisableInputDevice(JNIEnv* /* env */, jclass /* clazz */, jlong ptr,
                                     jint deviceId) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static void nativeSetPointerIconType(JNIEnv* /* env */, jclass /* clazz */, jlong ptr,
                                     jint iconId) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
static void nativeSetCustomPointerIcon(JNIEnv* env, jclass /* clazz */, jlong ptr,
                                       jobject iconObj) {
    NativeInputManager* im = reinterpret_cast<NativeInputManager*>(ptr);
...
int register_android_server_InputManager(JNIEnv* env) {
    ...
    GET_METHOD_ID(gServiceClassInfo.notifyConfigurationChanged, clazz, "notifyConfigurationChanged",
                  "(J)V");

    GET_METHOD_ID(gServiceClassInfo.notifyInputDevicesChanged, clazz, "notifyInputDevicesChanged",
                  "([Landroid/view/InputDevice;)V");

    GET_METHOD_ID(gServiceClassInfo.notifySwitch, clazz, "notifySwitch", "(JII)V");

    GET_METHOD_ID(gServiceClassInfo.notifyInputChannelBroken, clazz, "notifyInputChannelBroken",
                  "(Lcom/android/server/input/InputWindowHandle;)V");

    GET_METHOD_ID(gServiceClassInfo.notifyANR, clazz, "notifyANR",
                  "(Lcom/android/server/input/InputApplicationHandle;Lcom/android/server/input/"
                  "InputWindowHandle;Ljava/lang/String;)J");

    GET_METHOD_ID(gServiceClassInfo.filterInputEvent, clazz, "filterInputEvent",
                  "(Landroid/view/InputEvent;I)Z");

    GET_METHOD_ID(gServiceClassInfo.interceptKeyBeforeQueueing, clazz, "interceptKeyBeforeQueueing",
                  "(Landroid/view/KeyEvent;I)I");
    ...
    GET_METHOD_ID(gServiceClassInfo.dispatchUnhandledKey, clazz, "dispatchUnhandledKey",
                  "(Lcom/android/server/input/InputWindowHandle;Landroid/view/KeyEvent;I)Landroid/"
                  "view/KeyEvent;");
    ...
    GET_METHOD_ID(gServiceClassInfo.getExcludedDeviceNames, clazz, "getExcludedDeviceNames",
                  "()[Ljava/lang/String;");

    GET_METHOD_ID(gServiceClassInfo.getKeyRepeatTimeout, clazz, "getKeyRepeatTimeout", "()I");

    GET_METHOD_ID(gServiceClassInfo.getKeyRepeatDelay, clazz, "getKeyRepeatDelay", "()I");

    GET_METHOD_ID(gServiceClassInfo.getHoverTapTimeout, clazz, "getHoverTapTimeout", "()I");

    GET_METHOD_ID(gServiceClassInfo.getHoverTapSlop, clazz, "getHoverTapSlop", "()I");

    GET_METHOD_ID(gServiceClassInfo.getDoubleTapTimeout, clazz, "getDoubleTapTimeout", "()I");

    GET_METHOD_ID(gServiceClassInfo.getLongPressTimeout, clazz, "getLongPressTimeout", "()I");

    GET_METHOD_ID(gServiceClassInfo.getPointerLayer, clazz, "getPointerLayer", "()I");

    GET_METHOD_ID(gServiceClassInfo.getPointerIcon, clazz, "getPointerIcon",
    ...
    GET_METHOD_ID(gServiceClassInfo.getDeviceAlias, clazz, "getDeviceAlias",
