...
static int usb_device_added(const char* devAddress, void* clientData) {
    ...
    if (numBytes > 0) {
        ...
        env->CallBooleanMethod(thiz, method_usbDeviceAdded, deviceAddress, classID, subClassID,
                               descriptorsArray);
...
int register_android_server_UsbHostManager(JNIEnv* env) {
    ...
    method_usbDeviceAdded = env->GetMethodID(clazz, "usbDeviceAdded", "(Ljava/lang/String;II[B)Z");
    ...
    return jniRegisterNativeMethods(env, "com/android/server/usb/UsbHostManager", method_table,
                                    NELEM(method_table));
}
