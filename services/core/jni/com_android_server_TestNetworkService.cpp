...
static int create_tun_interface(const char* iface) {
    ...
    inet4CtrlSock = socket(AF_INET, SOCK_DGRAM, 0);
    ifr.ifr_flags = IFF_UP;

    if (ioctl(inet4CtrlSock, SIOCSIFFLAGS, &ifr)) {
...
static int reset_tun_interface(const char* iface) {
    ...
    int inet4CtrlSock = socket(AF_INET, SOCK_DGRAM, 0);
    if (ioctl(inet4CtrlSock, SIOCSIFFLAGS, &ifr) && errno != ENODEV) {
