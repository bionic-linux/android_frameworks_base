...
static void android_server_UsbAlsaJackDetector_jackDetect(JNIEnv* env, jobject thiz, jint card) {
...
int register_android_server_UsbAlsaJackDetector(JNIEnv* env) {
    ...
    if (!jniRegisterNativeMethods(env, "com/android/server/usb/UsbAlsaJackDetector", method_table,
                                  NELEM(method_table))) {
