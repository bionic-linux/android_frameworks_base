 ...
 */
class UserLevelHibernationProto implements ProtoReadWriter<List<UserLevelState>> {
    ...
    public void writeToProto(@NonNull ProtoOutputStream stream,
        ...
        for (int i = 0, size = data.size(); i < size; i++) {
            ...
            stream.write(UserLevelHibernationStateProto.PACKAGE, state.packageName);
        ...
        while (stream.nextField() != ProtoInputStream.NO_MORE_FIELDS) {
            ...
            while (stream.nextField() != ProtoInputStream.NO_MORE_FIELDS) {
                switch (stream.getFieldNumber()) {
                    case (int) UserLevelHibernationStateProto.PACKAGE:
                        state.packageName =
                                stream.readString(UserLevelHibernationStateProto.PACKAGE);
                        break;
