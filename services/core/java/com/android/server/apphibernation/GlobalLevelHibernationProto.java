 ...
 */
class GlobalLevelHibernationProto implements ProtoReadWriter<List<GlobalLevelState>> {
    ...
    public void writeToProto(@NonNull ProtoOutputStream stream,
        ...
        for (int i = 0, size = data.size(); i < size; i++) {
            ...
            stream.write(GlobalLevelHibernationStateProto.PACKAGE, state.packageName);
        ...
        while (stream.nextField() != ProtoInputStream.NO_MORE_FIELDS) {
            ...
            while (stream.nextField() != ProtoInputStream.NO_MORE_FIELDS) {
                switch (stream.getFieldNumber()) {
                    case (int) GlobalLevelHibernationStateProto.PACKAGE:
                        state.packageName =
                                stream.readString(GlobalLevelHibernationStateProto.PACKAGE);
                        break;
