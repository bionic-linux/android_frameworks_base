...
public abstract class ExtconUEventObserver extends UEventObserver {
    ...
    public static final class ExtconCable {
        ...
        public String getStatePath() {
            return TextUtils.formatSimple(
                    "/sys/class/extcon/%s/cable.%d/state", mExtconInfo.mName, mIndex);
        }
    }
    ...
    public static final class ExtconInfo {
        ...
        public static List<ExtconInfo> getExtconInfoForTypes(
            ...
            synchronized (mLock) { initExtconInfos(); }
            return Arrays.stream(sExtconInfos)
                    .filter(extconInfo
                            -> Arrays.stream(extconTypes)
                                       .anyMatch(type -> extconInfo.hasCableType(type)))
        ...
        private ExtconCable[] initCables() {
            File dir = new File("/sys/class/extcon", mName);
            if (!dir.exists()) {
                if (LOG)
                    Slog.w(TAG, "/sys/class/extcon/" + mName + " doesn't exist");
                return new ExtconCable[0];
            }
            File[] cableDirs =
                    FileUtils.listFilesOrEmpty(dir, (dir1, name) -> name.startsWith("cable."));
            if (cableDirs.length == 0) {
                Slog.d(TAG, "Unable to list cable files. " + SELINUX_POLICIES_NEED_TO_BE_CHANGED);
            }

            ArrayList<ExtconCable> cables = new ArrayList<>();
            for (int i = 0; i < cableDirs.length; i++) {
                String cableCanonicalPath = null;
                try {
                    cableCanonicalPath = cableDirs[i].getCanonicalPath();
                    String name = FileUtils.readTextFile(new File(cableDirs[i], "name"), 0, null);
                    name = name.replace("\n", "").replace("\r", "");
                    if (LOG)
                        Slog.v(TAG, "Add extcon cable " + cableCanonicalPath);
                    cables.add(new ExtconCable(this, i, name));
                } catch (IOException e) {
                    Slog.w(TAG,
                            "Unable to initialize cable " + cableCanonicalPath
                                    + "/name because IO exception. "
                                    + SELINUX_POLICIES_NEED_TO_BE_CHANGED);
                }
            }

            return cables.toArray(new ExtconCable[0]);
        }
