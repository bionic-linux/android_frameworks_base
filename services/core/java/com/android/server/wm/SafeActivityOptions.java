...
public class SafeActivityOptions {
    ...
    private void checkPermissions(@Nullable Intent intent, @Nullable ActivityInfo aInfo,
        ...
        if (taskDisplayArea == null) {
            int launchTaskDisplayFeatureId = options.getLaunchTaskDisplayAreaFeature();
            if (launchTaskDisplayFeatureId != FEATURE_UNDEFINED) {
                final int optionLaunchId = options.getLaunchDisplayId() == INVALID_DISPLAY
                ...
                if (dc != null) {
                    taskDisplayArea = dc.getItemFromTaskDisplayAreas(tda ->
                            tda.mFeatureId == launchTaskDisplayFeatureId ? tda : null);
                }
            }
        }
