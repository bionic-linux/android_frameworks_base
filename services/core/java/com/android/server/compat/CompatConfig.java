...
final class CompatConfig {
    ...
    boolean addOverride(long changeId, String packageName, boolean enabled) {
        boolean alreadyKnown = addOverrideUnsafe(changeId, packageName,
                new PackageOverride.Builder().addForAllVersions(enabled).build());
        saveOverrides();
        invalidateCache();
        return alreadyKnown;
    }
    ...
    private boolean removeOverrideUnsafe(long changeId, String packageName) {
        ...
        synchronized (mChanges) {
            ...
            if (c != null) {
                OverrideAllowedState allowedState =
                        mOverrideValidator.getOverrideAllowedState(changeId, packageName);
                overrideExists = c.removePackageOverride(packageName, allowedState, mContext);
                if (overrideExists) {
                    allowedState.enforce(changeId, packageName);
                    invalidateCache();
                }
    ...
    int enableTargetSdkChangesForPackage(String packageName, int targetSdkVersion) {
        ...
        for (long changeId : changes) {
            addOverrideUnsafe(changeId, packageName,
                    new PackageOverride.Builder().addForAllVersions(true).build());
        }
        saveOverrides();
        invalidateCache();
    ...
    int disableTargetSdkChangesForPackage(String packageName, int targetSdkVersion) {
        ...
        for (long changeId : changes) {
            addOverrideUnsafe(changeId, packageName,
                    new PackageOverride.Builder().addForAllVersions(false).build());
        }
        saveOverrides();
        invalidateCache();
        return changes.length;
