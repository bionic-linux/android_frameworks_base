...
public class VcnManagementService extends IVcnManagementService.Stub {
    ...
    private class TrackingNetworkCallback extends ConnectivityManager.NetworkCallback {
        ...
        private boolean hasSameTransportAndCapabilities(
                NetworkCapabilities caps, NetworkCapabilities capsOther) {
            // Although Wifi should not have other capabilities except INTERNET, only check the
            // existence of TRANSPORT_WIFI to preserve the previous behavior.
            if (caps.hasTransport(TRANSPORT_WIFI)) {
                return capsOther.hasTransport(TRANSPORT_WIFI);
            }
            ...
            synchronized (mCaps) {
                for (NetworkCapabilities existing : mCaps.values()) {
                    if (caps.getSubscriptionIds().equals(existing.getSubscriptionIds())
                            && hasSameTransportAndCapabilities(caps, existing)) {
