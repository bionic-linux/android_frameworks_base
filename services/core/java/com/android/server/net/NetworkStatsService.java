...
public class NetworkStatsService extends INetworkStatsService.Stub {
    ...
    private static final int MAX_STATS_PROVIDER_COUNT = 100;
    private final RemoteCallbackList<NetworkStatsProviderCallbackImpl> mStatsProviderCbList =
            new RemoteCallbackList<>();
    /** Semaphore that used to wait for stats provider respond to request stats update. */
    private final Semaphore mStatsProviderSem = new Semaphore(MAX_STATS_PROVIDER_COUNT, true);
    ...
    private void performPollLocked(int flags) {
        ...
        try {
            mStatsProviderSem.tryAcquire(mStatsProviderCbList.getRegisteredCallbackCount(), 100,
                    TimeUnit.MILLISECONDS);
        } catch (InterruptedException e) {
            Log.d(TAG, "requestStatsUpdate timeout: " + e);
        }
        Trace.traceEnd(TRACE_TAG_NETWORK);
