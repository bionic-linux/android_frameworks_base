...
public class NetworkStatsService extends INetworkStatsService.Stub {
    ...
    private void registerGlobalAlert() {
        try {
            ...
            for (NetworkStatsProviderCallbackImpl cb: mStatsProviderCbList) {
    ...
    private void performPollLocked(int flags) {
        ...
        for (NetworkStatsProviderCallbackImpl cb: mStatsProviderCbList) {
            try {
                cb.mProvider.requestStatsUpdate(STATS_PER_IFACE);
                cb.mProvider.requestStatsUpdate(STATS_PER_UID);
            } catch (RemoteException e) {
     ...
     * @param provider the binder interface of {@link android.net.NetworkStatsProviderBase} that
     *                 needs to be registered to the system.
     *
     * @return a binder interface of {@link NetworkStatsProviderCallback}, which can be used to
     *         report events to the system.
     */
    public @NonNull INetworkStatsProviderCallback registerNetworkStatsProvider(
            @NonNull String tag, @NonNull INetworkStatsProvider provider) {
        mContext.enforceCallingOrSelfPermission(PACKAGE_USAGE_STATS, TAG);
        checkNotNull(provider, "provider is null");
        try {
            NetworkStatsProviderCallbackImpl callback = new NetworkStatsProviderCallbackImpl(tag,
                    provider);
    ...
    private @NonNull NetworkStats getNetworkStatsFromProviders(int how) {
        ...
        for (NetworkStatsProviderCallbackImpl cb: mStatsProviderCbList) {
            ret.combineAllValues(cb.getCachedStats(how));
        }
        return ret;
    }
    ...
    private class NetworkStatsProviderCallbackImpl extends INetworkStatsProviderCallback.Stub
        ...
        @NonNull final Object mProviderStatsLock = new Object();
        @NonNull final INetworkStatsProvider mProvider;

        @GuardedBy("mProviderStatsLock")
        // STATS_PER_IFACE and STATS_PER_UID
        private final NetworkStats[] mStats = {new NetworkStats(0L, 0), new NetworkStats(0L, 0)};
        ...
        public NetworkStats getCachedStats(int how) {
            synchronized (mProviderStatsLock) {
                ...
                return mStats[how].clone();
            }
        }

        @Override
        public void onStatsUpdated(int how, @NonNull NetworkStats stats) {
            if (stats == null) throw new NullPointerException("NonNull stats is expected");
            synchronized (mProviderStatsLock) {
                mStats[how] = stats;
            }
        }
