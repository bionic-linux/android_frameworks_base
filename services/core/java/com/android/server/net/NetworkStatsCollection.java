...
public class NetworkStatsCollection implements FileRotator.Reader {
    ...
    public NetworkStatsHistory getHistory(NetworkTemplate template, SubscriptionPlan augmentPlan,
        ...
        if (augmentStart != SubscriptionPlan.TIME_UNKNOWN) {
            ...
            final long targetRxBytes = NetworkStats.multiplySafe(targetBytes, rawRxBytes, rawBytes);
            final long targetTxBytes = NetworkStats.multiplySafe(targetBytes, rawTxBytes, rawBytes);
            ...
            for (int i = 0; i < combined.size(); i++) {
                ...
                if (entry.bucketStart >= augmentStart
                    ...
                    entry.rxBytes = NetworkStats.multiplySafe(targetRxBytes, entry.rxBytes, rawRxBytes);
                    entry.txBytes = NetworkStats.multiplySafe(targetTxBytes, entry.txBytes, rawTxBytes);
