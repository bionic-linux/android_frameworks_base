...
final class NewDeviceAction extends HdmiCecFeatureAction {
    ...
    private HdmiDeviceInfo oldDeviceInfo;
    ...
    public boolean start() {
        oldDeviceInfo = tv().getCecDeviceInfo(mDeviceLogicalAddress);
        ...
        if (oldDeviceInfo != null &&
                oldDeviceInfo.getPhysicalAddress() == mDevicePhysicalAddress ) {
            Slog.d(TAG, "Start NewDeviceAction with old deviceInfo:["
                    + oldDeviceInfo.toString() + "]");
        } else {
            ...
            if (oldDeviceInfo != null) {
                ...
                tv().removeCecDevice(mDeviceLogicalAddress);
            }
    ...
    private void addDeviceInfo() {
        ...
        // If so, don't add newDevice info, prevernting ARC or HDMI source re-connection
        HdmiDeviceInfo deviceInfo = new HdmiDeviceInfo(
                mDeviceLogicalAddress, mDevicePhysicalAddress,
                tv().getPortId(mDevicePhysicalAddress),
                mDeviceType, mVendorId, mDisplayName);
        if (oldDeviceInfo != null
                && oldDeviceInfo.getLogicalAddress() == mDeviceLogicalAddress
                && oldDeviceInfo.getPhysicalAddress() == mDevicePhysicalAddress
                && oldDeviceInfo.getDeviceType() == mDeviceType
                && oldDeviceInfo.getVendorId() == mVendorId
                && oldDeviceInfo.getDisplayName().equals(mDisplayName)) {
            tv().processDelayedMessages(mDeviceLogicalAddress);
            Slog.d(TAG, "Ignore NewDevice, deviceInfo is same as current device");
            Slog.d(TAG, "Old:[" + oldDeviceInfo.toString()
