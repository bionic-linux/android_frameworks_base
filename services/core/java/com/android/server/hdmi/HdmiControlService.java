...
public class HdmiControlService extends SystemService {
    ...
    void onHotplug(int portId, boolean connected) {
        ...
        for (HdmiCecLocalDevice device : mCecController.getLocalDeviceList()) {
            if (!connected) {
                // Update the HdmiDeviceInfo on hotplug out.
                HdmiDeviceInfo deviceInfo =
                        createDeviceInfo(
                                device.mDeviceInfo.getLogicalAddress(),
                                device.mDeviceInfo.getDeviceType(),
                                HdmiControlManager.POWER_STATUS_ON);
                ...
                public void run() {
                    ...
                    sendCecCommand(
                            HdmiCecMessageBuilder.buildSetSystemAudioMode(
                                    audioSystem().mDeviceInfo.getLogicalAddress(),
    ...
    protected void setAndBroadcastActiveSource(
        ...
        if (deviceType == HdmiDeviceInfo.DEVICE_PLAYBACK) {
            ...
            setActiveSource(playback.mDeviceInfo.getLogicalAddress(), physicalAddress);
        }
        ...
        if (deviceType == HdmiDeviceInfo.DEVICE_AUDIO_SYSTEM) {
            ...
            if (playback() != null) {
                ...
                setActiveSource(audioSystem.mDeviceInfo.getLogicalAddress(), physicalAddress);
            }
        }
    }
