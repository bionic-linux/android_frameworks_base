...
public abstract class AuthenticationClient extends ClientMonitor {
    ...
    public AuthenticationClient(Context context, long halDeviceId, IBinder token,
            IIrisServiceReceiver receiver, int targetUserId, int groupId, long opId,
    ...
    public boolean onAuthenticated(int irisId, int groupId) {
                ...
                if (!authenticated) {
                    ...
                    if (DEBUG) {
                        Slog.v(TAG, "onAuthenticated(owner=" + getOwnerString()
                                + ", id=" + irisId + ", gp=" + groupId + ")");
                    }
                    Iris iris = !getIsRestricted()
                            ? new Iris("" /* TODO */, groupId, irisId, getHalDeviceId())
                            : null;
                    receiver.onAuthenticationSucceeded(getHalDeviceId(), iris, getTargetUserId());
                }
        ...
        if (!authenticated) {
            ...
            if (lockoutMode != LOCKOUT_NONE) {
                try {
                    Slog.w(TAG, "Forcing lockout (iris driver code should do this!), mode(" +
                            lockoutMode + ")");
                    stop(false);
            ...
            result |= true; // we have a valid iris, done
            resetFailedAttempts();
        }
        return result;
    }
    ...
    public int start() {
        ...
        if (daemon == null) {
            Slog.w(TAG, "start authentication: no iris HAL!");
            return ERROR_ESRCH;
        }
        try {
    ...
    public int stop(boolean initiatedByClient) {
        ...
        IBiometricsIris daemon = getIrisDaemon();
        if (daemon == null) {
            Slog.w(TAG, "stopAuthentication: no iris HAL!");
            return ERROR_ESRCH;
        }
        try {
