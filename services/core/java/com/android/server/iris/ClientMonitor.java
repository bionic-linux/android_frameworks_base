...
/**
 * Abstract base class for keeping track and dispatching events from iris HAL to the
 * the current client.  Subclasses are responsible for coordinating the interaction with
 * iris HAL for the specific action (e.g. authenticate, enroll, enumerate, etc.).
 */
public abstract class ClientMonitor implements IBinder.DeathRecipient {
    protected static final String TAG = IrisService.TAG; // TODO: get specific name
    protected static final int ERROR_ESRCH = 3; // Likely iris HAL is dead. See errno.h.
    protected static final boolean DEBUG = IrisService.DEBUG;
    ...
    // True if client does not have MANAGE_IRIS permission
    private final boolean mIsRestricted;
    private final String mOwner;
    private final VibrationEffect mSuccessVibrationEffect;
    private final VibrationEffect mErrorVibrationEffect;
    private IBinder mToken;
    private IIrisServiceReceiver mReceiver;
    protected boolean mAlreadyCancelled;

    /**
     * @param context context of IrisService
     * @param halDeviceId the HAL device ID of the associated iris hardware
     * @param token a unique token for the client
     * @param receiver recipient of related events (e.g. authentication)
     * @param userId target user id for operation
     * @param groupId groupId for the iris set
     * @param restricted whether or not client has the {@link Manifest#MANAGE_IRIS}
     * permission
     * @param owner name of the client that owns this
     */
    public ClientMonitor(Context context, long halDeviceId, IBinder token,
            IIrisServiceReceiver receiver, int userId, int groupId,boolean restricted,
            String owner) {
        mContext = context;
    ...
    /**
     * Contacts iris HAL to start the client.
     * @return 0 on success, errno from driver on failure
     */
    public abstract int start();

    /**
     * Contacts iris HAL to stop the client.
    ...
    /**
     * Gets the iris daemon from the cached state in the container class.
     */
    public abstract IBiometricsIris getIrisDaemon();
    ...
    // to the next client (e.g. authentication accepts or rejects a iris).
    public abstract boolean onEnrollResult(int irisId, int groupId, int rem);
    public abstract boolean onAuthenticated(int irisId, int groupId);
    public abstract boolean onRemoved(int irisId, int groupId, int remaining);
    public abstract boolean onEnumerationResult(int irisId, int groupId, int remaining);

    /**
     * Called when we get notification from iris HAL that an image has been acquired.
    ...
    public boolean onAcquired(int acquiredInfo, int vendorCode) {
        ...
        try {
            ...
            if (acquiredInfo == IrisManager.IRIS_ACQUIRED_GOOD) {
                notifyUserActivity();
            }
        }
    }
    ...
    /**
     * Called when we get notification from iris HAL that an error has occurred with the
    ...
    public final IIrisServiceReceiver getReceiver() {
        return mReceiver;
    }
