...
public final class BatteryService extends SystemService {
    ...
    private boolean isPoweredLocked(int plugTypeSet) {
        ...
        if ((plugTypeSet & BatteryManager.BATTERY_PLUGGED_AC) != 0
                && mHealthInfo.legacy.chargerAcOnline) {
            return true;
        }
        if ((plugTypeSet & BatteryManager.BATTERY_PLUGGED_USB) != 0
                && mHealthInfo.legacy.chargerUsbOnline) {
            return true;
        }
        if ((plugTypeSet & BatteryManager.BATTERY_PLUGGED_WIRELESS) != 0
                && mHealthInfo.legacy.chargerWirelessOnline) {
            return true;
        }
        return false;
    }
    ...
    private void shutdownIfNoPowerLocked() {
        ...
        if (mHealthInfo.legacy.batteryLevel == 0
    ...
    private static void copy(HealthInfo dst, HealthInfo src) {
        ...
        // Ignore diskStats and storageInfos because they are not used.
    }
    ...
    private void processValuesLocked(boolean force) {
        ...
        mBatteryLevelCritical =
            mHealthInfo.legacy.batteryStatus != BatteryManager.BATTERY_STATUS_UNKNOWN
            && mHealthInfo.legacy.batteryLevel <= mCriticalBatteryLevel;
        ...
        try {
            mBatteryStats.setBatteryState(mHealthInfo.legacy.batteryStatus,
                    mHealthInfo.legacy.batteryHealth,
                    mPlugType, mHealthInfo.legacy.batteryLevel,
                    mHealthInfo.legacy.batteryTemperature,
        ...
        if (force || (mHealthInfo.legacy.batteryStatus != mLastBatteryStatus
            ...
            if (mPlugType != mLastPlugType) {
                if (mLastPlugType == BATTERY_PLUGGED_NONE) {
                    ...
                    mChargeStartLevel = mHealthInfo.legacy.batteryLevel;
                    ...
                    builder.addTaggedData(MetricsEvent.FIELD_BATTERY_LEVEL_START,
                            mHealthInfo.legacy.batteryLevel);
            ...
            if (mHealthInfo.legacy.batteryStatus != mLastBatteryStatus
                ...
                EventLog.writeEvent(EventLogTags.BATTERY_STATUS,
                        mHealthInfo.legacy.batteryStatus, mHealthInfo.legacy.batteryHealth,
                        mHealthInfo.legacy.batteryPresent ? 1 : 0,
            ...
            if (mHealthInfo.legacy.batteryLevel != mLastBatteryLevel) {
                ...
                EventLog.writeEvent(EventLogTags.BATTERY_LEVEL,
                        mHealthInfo.legacy.batteryLevel, mHealthInfo.legacy.batteryVoltage,
                        mHealthInfo.legacy.batteryTemperature);
            }
            ...
            if (mLastBatteryLevel != mHealthInfo.legacy.batteryLevel
    ...
    private void sendBatteryChangedIntentLocked() {
        ...
        intent.putExtra(BatteryManager.EXTRA_MAX_CHARGING_CURRENT,
                mHealthInfo.legacy.maxChargingCurrent);
        intent.putExtra(BatteryManager.EXTRA_MAX_CHARGING_VOLTAGE,
                mHealthInfo.legacy.maxChargingVoltage);
        intent.putExtra(BatteryManager.EXTRA_CHARGE_COUNTER,
                mHealthInfo.legacy.batteryChargeCounter);
        if (DEBUG) {
            Slog.d(TAG, "Sending ACTION_BATTERY_CHANGED. scale:" + BATTERY_SCALE
                    + ", info:" + mHealthInfo.legacy.toString());
        }
    ...
    private void sendBatteryLevelChangedIntentLocked() {
        ...
        event.putInt(BatteryManager.EXTRA_STATUS, mHealthInfo.legacy.batteryStatus);
        event.putInt(BatteryManager.EXTRA_HEALTH, mHealthInfo.legacy.batteryHealth);
        event.putBoolean(BatteryManager.EXTRA_PRESENT, mHealthInfo.legacy.batteryPresent);
        event.putInt(BatteryManager.EXTRA_LEVEL, mHealthInfo.legacy.batteryLevel);
        ...
        event.putInt(BatteryManager.EXTRA_VOLTAGE, mHealthInfo.legacy.batteryVoltage);
                ...
                try {
                    ...
                    switch (key) {
                        ...
                        case "level":
                            ...
                            break;
                        case "counter":
                            mHealthInfo.legacy.batteryChargeCounter = Integer.parseInt(value);
                            break;
                        case "temp":
    ...
    private void dumpProto(FileDescriptor fd) {
        ...
        synchronized (mLock) {
            ...
            proto.write(BatteryServiceDumpProto.MAX_CHARGING_CURRENT,
                    mHealthInfo.legacy.maxChargingCurrent);
            proto.write(BatteryServiceDumpProto.MAX_CHARGING_VOLTAGE,
                    mHealthInfo.legacy.maxChargingVoltage);
            proto.write(BatteryServiceDumpProto.CHARGE_COUNTER,
                    mHealthInfo.legacy.batteryChargeCounter);
    ...
    private final class LocalService extends BatteryManagerInternal {
        ...
        public int getBatteryChargeCounter() {
            synchronized (mLock) {
                return mHealthInfo.legacy.batteryChargeCounter;
            }
        }
        ...
        public int getBatteryFullCharge() {
            synchronized (mLock) {
                return mHealthInfo.legacy.batteryFullCharge;
            }
        }
