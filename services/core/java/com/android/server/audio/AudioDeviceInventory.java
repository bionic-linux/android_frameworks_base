...
public class AudioDeviceInventory {
     ...
     /*package*/ void disconnectLeAudio() {
        synchronized (mDevicesLock) {
            ...
            if (toRemove.size() > 0) {
                ...
                toRemove.stream().forEach(deviceAddress ->
                        // TODO delay not used?
                        makeLeAudioDeviceUnavailable(deviceAddress, AudioSystem.DEVICE_OUT_BLE_HEADSET)
                );
            }
        }
    }
	...
	final int leAudioVolIndex = mDeviceBroker.getVssVolumeForDevice(streamType,
                                    AudioSystem.DEVICE_OUT_BLE_HEADSET);
        mDeviceBroker.postSetLeAudioVolumeIndex(leAudioVolIndex, streamType);
    ...
    static {
        ...
        BECOMING_NOISY_INTENT_DEVICES_SET.add(AudioSystem.DEVICE_OUT_BLE_SPEAKER);
