...
public class BtHelper {
    ...
    /*package*/ synchronized void setLeAudioVolume(int index, int streamType) {
        if (mLeAudio == null) {
            if (AudioService.DEBUG_VOL) {
         ...
         * it seems that index is in that range, therefore just send it out to
         * the leAudio device.
         *
         * TODO: Confirm with audio team. */
        int volume = index;
        if (volume >= BT_LE_AUDIO_MAX_VOL) {
            volume = BT_LE_AUDIO_MAX_VOL;
        } else if (volume < BT_LE_AUDIO_MIN_VOL) {
            volume = BT_LE_AUDIO_MIN_VOL;
        }
        ...
        AudioService.sVolumeLogger.log(new AudioServiceEvents.VolumeEvent(
                AudioServiceEvents.VolumeEvent.VOL_SET_LE_AUDIO_VOL, index, volume));
