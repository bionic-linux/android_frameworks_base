...
class RebootEscrowData {
    ...
    static class AesEncryptedBlob {
        ...
        private final byte[] mOriginalData;
        private final byte[] mEncryptedBlob;
        private final byte[] mIv;
        private final SecretKey mKey;

        AesEncryptedBlob(byte[] originalData,  byte[] encryptedBlob, byte[] iv, SecretKey key) {
        ...
        static AesEncryptedBlob fromOriginalData(SecretKey key, byte[] originalData)
                throws IOException {
            Objects.requireNonNull(key);
            ...
            try {
                ...
                cipherText = cipher.doFinal(originalData);
                iv = cipher.getIV();
            ...
            return new AesEncryptedBlob(originalData, bos.toByteArray(), iv, key);
        }
    }
    ...
    static RebootEscrowData fromEncryptedData(RebootEscrowKey ks, byte[] blob,
        ...
        AesEncryptedBlob unencryptedBlob = AesEncryptedBlob.fromEncryptedBlob(ks.getKey(),
                ksEncryptedBlob.getOriginalData());

        final byte[] syntheticPassword = unencryptedBlob.getOriginalData();
    ...
    static RebootEscrowData fromSyntheticPassword(RebootEscrowKey ks, byte spVersion,
        ...
        AesEncryptedBlob encryptedBlob = AesEncryptedBlob.fromOriginalData(ks.getKey(),
                syntheticPassword);
        AesEncryptedBlob wrappedBlob = AesEncryptedBlob.fromOriginalData(kk,
