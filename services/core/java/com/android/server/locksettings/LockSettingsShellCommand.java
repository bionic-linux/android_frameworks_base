...
class LockSettingsShellCommand extends ShellCommand {
    ...
    private static final String COMMAND_SET_STRONG_AUTH = "set-strong-auth";
    ...
    public void onHelp() {
        try (final PrintWriter pw = getOutPrintWriter();) {
            ...
            pw.println("    Sets the package name for server based resume on reboot service "
                    + "provider.");
            pw.println("");
            pw.println("  set-strong-auth [--user USER_ID] <reason>");
            pw.println("    Sets the strong authentication.");
            pw.println("");
        }
    }
    ...
    private boolean runSetStrongAuth() {
