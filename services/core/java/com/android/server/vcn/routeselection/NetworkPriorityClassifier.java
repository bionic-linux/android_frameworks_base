...
public class NetworkPriorityClassifier {
    ...
    public static class PriorityCalculationConfig {
        public ParcelUuid subscriptionGroup;
        public TelephonySubscriptionSnapshot snapshot;
        public UnderlyingNetworkRecord currentlySelected;
        public PersistableBundle carrierConfig;

        public PriorityCalculationConfig(
     ...
     * </ol>
     */
    public int calculatePriorityClass(
            UnderlyingNetworkRecord networkRecord, PriorityCalculationConfig calculationConfig) {
        ...
        if (caps.hasTransport(TRANSPORT_CELLULAR)
                && isOpportunistic(calculationConfig.snapshot, caps.getSubscriptionIds())) {
            ...
            if (!calculationConfig
                            .snapshot
                            .getAllSubIdsInGroup(calculationConfig.subscriptionGroup)
        ...
        if (caps.hasTransport(TRANSPORT_WIFI)) {
            if (caps.getSignalStrength()
                            >= getWifiExitRssiThreshold(calculationConfig.carrierConfig)
                    && calculationConfig.currentlySelected != null
                    && networkRecord.network.equals(calculationConfig.currentlySelected.network)) {
                return PRIORITY_WIFI_IN_USE;
            }

            if (caps.getSignalStrength()
                    >= getWifiEntryRssiThreshold(calculationConfig.carrierConfig)) {
