...
class NetworkPriorityClassifier {
    ...
    public static boolean checkMatchesPriorityRule(
        ...
        final int unmeteredMatch = networkPriority.getNotMeteredMatch();
        final boolean hasNotMetered = caps.hasCapability(NET_CAPABILITY_NOT_METERED);
        if (unmeteredMatch == MATCH_REQUIRED && !hasNotMetered
                || unmeteredMatch == MATCH_FORBIDDEN && hasNotMetered) {
            return false;
        }
    ...
    public static boolean checkMatchesCellPriorityRule(
        ...
        final int notRoamingMatch = networkPriority.getNotRoamingMatch();
        final boolean hasNotNotRoaming = caps.hasCapability(NET_CAPABILITY_NOT_ROAMING);
        if (notRoamingMatch == MATCH_REQUIRED && !hasNotNotRoaming
                || notRoamingMatch == MATCH_FORBIDDEN && hasNotNotRoaming) {
            return false;
        }

        final int opportunisticMatch = networkPriority.getOpportunisticMatch();
