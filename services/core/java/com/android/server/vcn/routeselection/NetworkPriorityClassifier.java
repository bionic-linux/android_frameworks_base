...
class NetworkPriorityClassifier {
    ...
    private static boolean checkMatchesPriorityRule(
            VcnContext vcnContext,
    ...
    public static boolean checkMatchesCellPriorityRule(
        ...
        final int subId =
                ((TelephonyNetworkSpecifier) caps.getNetworkSpecifier()).getSubscriptionId();
        ...
        if (!networkPriority.getAllowedSpecificCarrierIds().isEmpty()) {
            final int carrierId =
                    defaultTelephonyMgr.createForSubscriptionId(subId).getSimSpecificCarrierId();
