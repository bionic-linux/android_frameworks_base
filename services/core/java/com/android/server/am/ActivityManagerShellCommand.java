...
//                    options != null ? options.toBundle() : null, mUserId);
    ...
    private int runCompat(PrintWriter pw) {
        ...
        long changeId = Long.parseLong(getNextArgRequired());
        String packageName = getNextArgRequired();
        switch(toggleValue) {
            case "enable":
                if (!config.addOverride(changeId, packageName, true)) {
                    pw.println("Warning! Change " + changeId + " is not known yet. This could "
    ...
    static void dumpHelp(PrintWriter pw, boolean dumping) {
        if (dumping) {
            ...
            pw.println("  compat enable|disable|set-default <PACKAGE_NAME> <CHANGE_ID>");
            pw.println("      Toggles <CHANGE_ID> for <PACKAGE_NAME>.");
            pw.println();
