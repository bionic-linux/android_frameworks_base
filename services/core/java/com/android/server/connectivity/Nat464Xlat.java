...
public class Nat464Xlat extends BaseNetworkObserver {
    ...
    private void leaveStartedState() {
        ...
        if (requiresClat(mNetwork) && isPrefixDiscoveryNeeded()) {
            if (!mPrefixDiscoveryRunning) startPrefixDiscovery();
    ...
    public void update() {
        ...
        switch (mState) {
            case IDLE:
                if (!requiresClat(mNetwork)) return;  // Stay in IDLE.
    ...
    private LinkProperties makeLinkProperties(LinkAddress clatAddress) {
        ...
        // clat IPv4 address itself (fonur those apps, it doesn't matter what
