    ...
    public void updateUpstreamIPv6LinkProperties(LinkProperties v6only) {
        ...
        if (v6only != null) {
            ...
            for (LinkAddress linkAddr : v6only.getLinkAddresses()) {
                if (linkAddr.getPrefixLength() != NetworkConstants.RFC7421_IP_PREFIX_LENGTH) {
                    continue;
                }
    ...
    private void configureLocalDns(
        ...
        if (!deprecatedDnses.isEmpty()) {
            for (Inet6Address dns : deprecatedDnses) {
                ...
                try {
                    netd.interfaceDelAddress(mIfName, dnsString,
                            NetworkConstants.RFC7421_IP_PREFIX_LENGTH);
        ...
        if (newDnses != null && !newDnses.isEmpty()) {
            ...
            for (Inet6Address dns : addedDnses) {
                final String dnsString = dns.getHostAddress();
                try {
                    netd.interfaceAddAddress(mIfName, dnsString,
                            NetworkConstants.RFC7421_IP_PREFIX_LENGTH);
                } catch (ServiceSpecificException | RemoteException e) {
