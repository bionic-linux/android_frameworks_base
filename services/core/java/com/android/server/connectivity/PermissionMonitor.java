...
public class PermissionMonitor {
    ...
    public synchronized void onPackageRemoved(int uid) {
        ...
        if (permission == mApps.get(UserHandle.getAppId(uid))) {
            // The permissions of this UID have not changed. Nothing to do.
            return;
        } else if (permission != null) {
            mApps.put(UserHandle.getAppId(uid), permission);
            apps.put(UserHandle.getAppId(uid), permission);
            update(mUsers, apps, true);
        } else {
            mApps.remove(UserHandle.getAppId(uid));
            apps.put(UserHandle.getAppId(uid), NETWORK);  // doesn't matter which permission we pick here
