 ...
 *
 * InternalScore is very similar to NetworkScore, but it contains the bits that are managed
...
public class InternalScore {
    ...
    @VisibleForTesting
    static final int MIN_AGENT_MANAGED_POLICY = 0;
    @VisibleForTesting
    static final int MAX_AGENT_MANAGED_POLICY = -1;
    @VisibleForTesting
    ...
    InternalScore(final int legacyInt, final long policy) {
     ...
     * @return an InternalScore that is appropriate to use for ranking.
     */
    public static InternalScore fromNetworkScore(@NonNull final NetworkScore score,
     ...
     * @return an InternalScore appropriate for comparing to actual network's scores.
     */
    public static InternalScore prospectiveScore(@NonNull final NetworkScore score,
        ...
        final boolean everSelected = false;
        // Don't assume the user will accept unvalidated connectivity.
        final boolean acceptUnvalidated = false;
        return withPolicies(score.getLegacyInt(), mayValidate, vpn, everSelected,
                acceptUnvalidated);
    }
     ...
     */
    public InternalScore mixInScore(@NonNull final NetworkCapabilities caps,
    ...
    private static InternalScore withPolicies(@NonNull final int legacyInt,
            final boolean isValidated,
            final boolean isVpn,
            final boolean onceChosenByUser,
            final boolean acceptUnvalidated) {
        return new InternalScore(legacyInt,
                (isValidated         ? 1L << POLICY_IS_VALIDATED : 0)
                | (isVpn             ? 1L << POLICY_IS_VPN : 0)
                | (onceChosenByUser  ? 1L << POLICY_EVER_USER_SELECTED : 0)
