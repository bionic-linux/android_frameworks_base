 ...
 * to leave maintain mode first. If a remote packet arrives, maintain
 * mode would be turned off and offload would be stopped. Application
 * would receive a callback to know it can start to read data.
 * @hide
 */
public class TcpKeepaliveController {
    ...
    /** Keeps track of packet lisnters. */
    ...
    private static boolean checkIfSocketIdle(FileDescriptor fd)
            throws ErrnoException {
        return receiveQueueIsEmpty(fd) && sendQueueIsEmpty(fd);
    }

    private static boolean receiveQueueIsEmpty(FileDescriptor fd)
            throws ErrnoException {
