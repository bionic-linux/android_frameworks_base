 ...
 *
 * The input socket would be changed to repair mode, application
 * would not have permission to read/write data. If application want
 * to write data, it must stop tcp keepalive offload first. If remote
 * packet arrives, the socket would leave repair mode and offload would
 * be stopped, application would receive a callback to know when it can
 * start to read data.
 * @hide
 */
public class TcpKeepaliveController {
    ...
    static class TcpSocketInfo {
        ...
        TcpSocketInfo(InetAddress sAddr, int sPort, InetAddress dAddr,
    ...
    public static TcpSocketInfo switchToMaintainMode(
        ...
        Log.e(TAG, "switchToMaintainMode to start tcp keepalive....");
