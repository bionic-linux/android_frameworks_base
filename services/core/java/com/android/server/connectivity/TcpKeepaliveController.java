...
public class TcpKeepaliveController {
     ...
     */
    private final SparseArray<FileDescriptor> mListeners;
    ...
    public void startSocketMonitor(@NonNull final FileDescriptor fd,
            @NonNull final Messenger messenger, int slot) {
        synchronized (mListeners) {
            if (null != mListeners.get(slot)) return;
            ...
            mFdHandlerQueue.addOnFileDescriptorEventListener(fd, FD_EVENTS, (readyFd, events) -> {
                ...
                try {
                    ...
                    if (DBG) Log.e(TAG, "Moving socket out of repair mode for event : " + readyFd);
    ...
    public void stopSocketMonitor(final int slot) {
        ...
        try {
            if (DBG) Log.e(TAG, "Moving socket out of repair mode for stop : " + fd);
