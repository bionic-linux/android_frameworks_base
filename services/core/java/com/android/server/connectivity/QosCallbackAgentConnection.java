...
class QosCallbackAgentConnection implements IBinder.DeathRecipient {
    ...
    /**
     * Gets the network tied to the callback of this cfonnection
     * @return network
     */
    @NonNull
    Network getNetwork() {
    ...
    boolean sendCmdRegisterCallback() {
        ...
        if (exceptionType != EX_TYPE_FILTER_NONE) {
            try {
                ...
                this.mCallback.onError(exceptionType);
        ...
        if (!(mFilter instanceof QosSocketFilter)) {
            // Programming error within the connectivity stack
            throw new IllegalArgumentException("Unrecognized filter: " + mFilter);
        }

        final Message msg = Message.obtain();
        msg.what = NetworkAgent.CMD_REGISTER_QOS_CALLBACK;
        msg.arg1 = mAgentCallbackId;
        final QosSocketFilter qosSocketFilter = (QosSocketFilter) mFilter;
