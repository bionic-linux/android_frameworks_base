...
public class NsdService extends INsdManager.Stub {
            ...
            public boolean processMessage(Message msg) {
                ...
                switch (msg.what) {
                    ...
                    case NsdManager.UNREGISTER_CLIENT:
                        ...
                        cInfo = mClients.get(connector);
                        if (cInfo != null) {
                            cInfo.expungeAllRequests();
                            mClients.remove(connector);
                            cInfo.mCb.asBinder().unlinkToDeath(connector, 0);
                        }
                        // Last client
                        if (mClients.size() == 0) {
                            scheduleStop();
                    ...
                    case NsdManager.DISCOVER_SERVICES:
                        ...
                        if (cInfo != null) {
                            cInfo.notifyDiscoverServicesFailed(
                    ...
                    case NsdManager.STOP_DISCOVERY:
                        cInfo = getClientInfoForReply(msg);
                        if (cInfo != null) {
                            cInfo.notifyStopDiscoveryFailed(
                                    clientId, NsdManager.FAILURE_INTERNAL_ERROR);
                        }
                        break;
                    case NsdManager.REGISTER_SERVICE:
                        cInfo = getClientInfoForReply(msg);
                        if (cInfo != null) {
                            cInfo.notifyRegisterServiceFailed(
                                    clientId, NsdManager.FAILURE_INTERNAL_ERROR);
                        }
                        break;
                    case NsdManager.UNREGISTER_SERVICE:
                        cInfo = getClientInfoForReply(msg);
                        if (cInfo != null) {
                            cInfo.notifyUnregisterServiceFailed(
                                    clientId, NsdManager.FAILURE_INTERNAL_ERROR);
                        }
                        break;
                    case NsdManager.RESOLVE_SERVICE:
                        cInfo = getClientInfoForReply(msg);
                        if (cInfo != null) {
                            cInfo.notifyResolveServiceFailed(
                                    clientId, NsdManager.FAILURE_INTERNAL_ERROR);
                        }
                        break;
                ...
                switch (msg.what) {
                    ...
                    case NsdManager.DISCOVER_SERVICES:
                        ...
                        if (requestLimitReached(clientInfo)) {
                            clientInfo.notifyDiscoverServicesFailed(
                        ...
                        if (discoverServices(id, args.serviceInfo.getServiceType())) {
                            ...
                            clientInfo.notifyDiscoverServicesStarted(clientId, args.serviceInfo);
                        } else {
                            stopServiceDiscovery(id);
                            clientInfo.notifyDiscoverServicesFailed(clientId,
                    ...
                    case NsdManager.STOP_DISCOVERY:
                        ...
                        try {
                            ...
                            clientInfo.notifyStopDiscoveryFailed(
                                    clientId, NsdManager.FAILURE_INTERNAL_ERROR);
                            break;
                        }
                        removeRequestMap(clientId, id, clientInfo);
                        if (stopServiceDiscovery(id)) {
                            clientInfo.notifyStopDiscoverySucceeded(clientId);
                        } else {
                            clientInfo.notifyStopDiscoveryFailed(
                                    clientId, NsdManager.FAILURE_INTERNAL_ERROR);
                        }
                    ...
                    case NsdManager.REGISTER_SERVICE:
                        ...
                        clientInfo = mClients.get(args.connector);
                        if (requestLimitReached(clientInfo)) {
                            clientInfo.notifyRegisterServiceFailed(
                                    clientId, NsdManager.FAILURE_MAX_LIMIT);
                            break;
                        }
                        ...
                        maybeStartDaemon();
                        id = getUniqueId();
                        if (registerServiceInternal(id, args.serviceInfo)) {
                            if (DBG) Slog.d(TAG, "Register " + clientId + " " + id);
                            storeRequestMap(clientId, id, clientInfo, msg.what);
                            // Return success after mDns reports success
                        } else {
                            unregisterServiceInternal(id);
                            clientInfo.notifyRegisterServiceFailed(
                                    clientId, NsdManager.FAILURE_INTERNAL_ERROR);
                        }
                        break;
                    case NsdManager.UNREGISTER_SERVICE:
                        ...
                        try {
                            ...
                            clientInfo.notifyUnregisterServiceFailed(
                                    NsdManager.UNREGISTER_SERVICE_FAILED,
                                    NsdManager.FAILURE_INTERNAL_ERROR);
                            break;
                        }
                        removeRequestMap(clientId, id, clientInfo);
                        if (unregisterServiceInternal(id)) {
                            clientInfo.notifyUnregisterServiceSucceeded(clientId);
                        } else {
                            clientInfo.notifyUnregisterServiceFailed(
                                    clientId, NsdManager.FAILURE_INTERNAL_ERROR);
                        }
                    ...
                    case NsdManager.RESOLVE_SERVICE:
                        ...
                        if (clientInfo.mResolvedService != null) {
                            clientInfo.notifyResolveServiceFailed(
            ...
            private boolean handleNativeEvent(int code, String raw, String[] cooked) {
                ...
                switch (code) {
                    case NativeResponseCode.SERVICE_FOUND:
                        ...
                        clientInfo.notifyServiceFound(clientId, servInfo);
                        break;
                    case NativeResponseCode.SERVICE_LOST:
                        /* NNN uniqueId serviceName regType domain */
                        servInfo = new NsdServiceInfo(cooked[2], cooked[3]);
                        clientInfo.notifyServiceLost(clientId, servInfo);
                        break;
                    ...
                    case NativeResponseCode.SERVICE_REGISTERED:
                        ...
                        clientInfo.notifyRegisterServiceSucceeded(clientId, servInfo);
                        break;
                    ...
                    case NativeResponseCode.SERVICE_GET_ADDR_SUCCESS:
                        ...
                        try {
                            ...
                            clientInfo.notifyResolveServiceSucceeded(
    ...
    private boolean registerServiceInternal(int regId, NsdServiceInfo service) {
        if (DBG) {
    ...
    private boolean unregisterServiceInternal(int regId) {
    ...
    private class ClientInfo {
            ...
            for (i = 0; i < mClientIds.size(); i++) {
                ...
                switch (mClientRequests.get(clientId)) {
                    ...
                    case NsdManager.RESOLVE_SERVICE:
                        ...
                        break;
                    case NsdManager.REGISTER_SERVICE:
                        unregisterServiceInternal(globalId);
                        break;
                    default:
                        break;
                }
        ...
        void notifyDiscoverServicesStarted(int listenerKey, NsdServiceInfo info) {
            try {
        ...
        void notifyDiscoverServicesFailed(int listenerKey, int error) {
            try {
        ...
        void notifyServiceFound(int listenerKey, NsdServiceInfo info) {
            try {
        ...
        void notifyServiceLost(int listenerKey, NsdServiceInfo info) {
            try {
        ...
        void notifyStopDiscoveryFailed(int listenerKey, int error) {
            try {
        ...
        void notifyStopDiscoverySucceeded(int listenerKey) {
            try {
        ...
        void notifyRegisterServiceFailed(int listenerKey, int error) {
            try {
        ...
        void notifyRegisterServiceSucceeded(int listenerKey, NsdServiceInfo info) {
            try {
        ...
        void notifyUnregisterServiceFailed(int listenerKey, int error) {
            try {
        ...
        void notifyUnregisterServiceSucceeded(int listenerKey) {
            try {
        ...
        void notifyResolveServiceFailed(int listenerKey, int error) {
            try {
        ...
        void notifyResolveServiceSucceeded(int listenerKey, NsdServiceInfo info) {
            try {
