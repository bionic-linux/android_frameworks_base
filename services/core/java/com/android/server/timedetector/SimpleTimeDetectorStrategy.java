...
public final class SimpleTimeDetectorStrategy implements TimeDetectorStrategy {
     ...
     */
    private static final long SYSTEM_CLOCK_PARANOIA_THRESHOLD = 2 * 1000;
    ...
    private static boolean validateNewNitzTime(
            TimestampedValue<Long> newNitzUtcTime, TimestampedValue<Long> lastNitzTime) {

        if (lastNitzTime == null) {
            Slog.d(TAG, "validateNewNitzTime: No previous NITZ time received."
    ...
    private void setSystemClockIfRequired(
        ...
        try {
            ...
            if (mLastSystemClockTimeSet != null) {
                ...
                if (absSystemClockDifference > SYSTEM_CLOCK_PARANOIA_THRESHOLD) {
            ...
            adjustAndSetDeviceSystemClock(
                    time, sendNetworkBroadcast, elapsedRealtimeMillis, actualTimeMillis);
        } finally {
            ...
            if (mLastSystemClockTime != null) {
                ...
                try {
                    ...
                    adjustAndSetDeviceSystemClock(mLastSystemClockTime, sendNetworkBroadcast,
                            elapsedRealtimeMillis, actualTimeMillis);
                } finally {
                    mCallback.releaseWakeLock();
                }
            }
        } else {
            // CLOCK_PARANOIA: We are losing "control" of the system clock.
    ...
    private void adjustAndSetDeviceSystemClock(
            ...
            long elapsedRealtimeMillis, long actualTimeMillis) {

        // Adjust for the time that has elapsed since the signal was received.
        long newTimeMillis = TimeDetectorStrategy.getTimeAt(newTime, elapsedRealtimeMillis);

        // Check if the new signal would make sufficient difference to the system clock. If it's
        // below the threshold then ignore it.
        long absTimeDifference = Math.abs(newTimeMillis - actualTimeMillis);
        long systemClockUpdateThreshold = mCallback.systemClockUpdateThresholdMillis();
        if (absTimeDifference < systemClockUpdateThreshold) {
            Slog.d(TAG, "adjustAndSetDeviceSystemClock: absTimeDifference=" + absTimeDifference);
            return;
        }
        ...
        mCallback.setSystemClock(newTimeMillis);
        ...
        if (sendNetworkBroadcast) {
            ...
            intent.putExtra("time", newTimeMillis);
