...
public class MediaSessionService extends SystemService implements Monitor {
    ...
    private void destroySessionLocked(MediaSessionRecord session) {
         ...
         * So this requests PAUSE then change Playback state to PAUSE
         * if the closed session is active.
         */
        if (session.isPlaybackActive()) {
            PlaybackState.Builder stateBuilder =
                    new PlaybackState.Builder().setActions(PlaybackState.ACTION_PAUSE);
            try {
                session.getControllerBinder().pause(getContext().getPackageName(), null);
