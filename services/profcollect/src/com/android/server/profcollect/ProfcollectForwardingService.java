...
public final class ProfcollectForwardingService extends SystemService {
    ...
    private void traceOnDex2oatStart() {
        ...
        final int traceFrequency = DeviceConfig.getInt(DeviceConfig.NAMESPACE_PROFCOLLECT_NATIVE_BOOT,
                "dex2oat_trace_freq", 2);
        int randomNum = ThreadLocalRandom.current().nextInt(100);
        if (randomNum < traceFrequency) {
            if (DEBUG) {
                Log.d(LOG_TAG, "Tracing on app launch event: " + packageName);
            ...
            BackgroundThread.get().getThreadHandler().post(() -> {
                // Dex2oat could take a while before it starts. Add a delay before start tracing.
                Thread.sleep(500);
                try {
