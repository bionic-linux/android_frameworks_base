 ...
 *                     goto 3
 *
 *     [3] stop() calls onStop() if not already stopped
 *
 ...
 * @hide
 */
public abstract class FdEventsReader<T> {
    ...
    protected FdEventsReader(@NonNull Handler h, @NonNull T buffer) {
        mHandler = h;
    ...
    protected abstract int recvBufSize(@NonNull T buffer);
     ...
     */
    protected abstract int readPacket(@NonNull FileDescriptor fd, @NonNull T buffer)
            throws Exception;
     ...
     */
    protected void handlePacket(@NonNull T recvbuf, int length) {}
