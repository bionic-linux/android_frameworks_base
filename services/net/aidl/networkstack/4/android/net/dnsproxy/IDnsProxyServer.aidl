...
interface IDnsProxyServer {
  oneway void start(in android.net.Network network, in android.net.INetworkStackStatusCallback cb);
  oneway void stop(in android.net.INetworkStackStatusCallback cb);
  oneway void updateUpstream(in android.net.Network network, in android.net.INetworkStackStatusCallback cb);
  const int STATUS_SUCCESS = 0;
  const int STATUS_UNKNOWN_ERROR = 1;
}
