...
static std::span<const uint8_t> toSpan(const ::std::optional<::std::vector<uint8_t>>& content) {
    ...
    // TODO: Replace with {content->data(), content->size()} after libc++
    // upgrade. The type of the second std::span ctor param changed from
    // ptrdiff_t to size_t between the old libc++ and the finalized C++20.
