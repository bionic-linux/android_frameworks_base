...
static uint32_t get_length (void *id) {
    ...
    // Now we use fstat to determine information about the file

    if(fstat (fileno (file), &statbuf) < 0) {
       return 0;
    }
    
    // Now check that it is a regular file
...
static int can_seek (void *id) {
    ...
    // We get the information about the file using fstat. If id is a regular file then it is seekable
...
status_t WavPackPlayer::setDataSource(
