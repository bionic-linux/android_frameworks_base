...
jobjectArray getJavaPerformancePointArrayFromNative(JNIEnv *env,
    ...
    for (int i = 0; i < performancePoints.size(); i++) {
        ...
        jobject jPerformancePoint = env->NewObject(performancePointClazz, performancePointConstructID,
...
static VideoCapabilities::PerformancePoint GetNativePerfromancePointFromJava(JNIEnv *env, jobject pp) {
    jclass clazz = env->FindClass("android/media/MediaCodecInfo$VideoCapabilities$PerformancePoint");
    CHECK(clazz != NULL);
    if (!env->IsInstanceOf(pp, clazz)) {
        return NULL;
    }
    ...
    jint bockHeight = env->CallIntMethod(pp, getBlockHeightID);

    VideoCapabilities::PerformancePoint res = VideoCapabilities::PerformancePoint(width, height, maxFrameRate,
            maxMacroBlockRate, VideoSize(blockWidth, blockHeight));

    return res;
}

static jboolean android_media_VideoCapabilities_PerformancePoint_covers(JNIEnv *env, jobject thiz, jobject other) {
    VideoCapabilities::PerformancePoint& pp0 = getPerformancePoints(env, thiz);
    VideoCapabilities::PerformancePoint pp1 = GetNativePerfromancePointFromJava(env, other);
...
static jboolean android_media_VideoCapabilities_PerformancePoint_equals(JNIEnv *env, jobject thiz, jobject other) {
    VideoCapabilities::PerformancePoint& pp0 = getPerformancePoints(env, thiz);
    VideoCapabilities::PerformancePoint pp1 = GetNativePerfromancePointFromJava(env, other);
...
static jstring android_media_VideoCapabilities_PerformancePoint_toString(JNIEnv *env, jobject thiz) {
...
int register_android_media_CodecCapabilities(JNIEnv *env) {
    ...
    result = AndroidRuntime::registerNativeMethods(env, "android/media/MediaCodecInfo$VideoCapabilities$PerformancePoint",
