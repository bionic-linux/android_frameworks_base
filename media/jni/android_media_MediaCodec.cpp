...
static std::string GetExceptionMessage(const sp<JMediaCodec> &codec, const char *msg) {
    if (codec == NULL) {
        ...
        return "codec is released already";
    }
    ...
    NativeCryptoInfo(JNIEnv *env, jobject cryptoInfoObj)
        ...
        if (jmode == gCryptoModes.Unencrypted) {
            ...
            throwExceptionAsNecessary(
                    ...
                    AStringPrintf("unrecognized crypto mode: %d", jmode).c_str());
            return;
        }
    ...
    if (codec->hasCryptoOrDescrambler()) {
        if (!memory) {
            ...
            throwExceptionAsNecessary(
                    ...
                    "input buffer not compatible with secure codec");
            return;
        }
        auto cryptoInfo =
        ...
        if (!buffer) {
            ...
            throwExceptionAsNecessary(
                    env, BAD_VALUE, ACTION_CODE_FATAL,
                    "input buffer not compatible with codec");
            return;
        }
