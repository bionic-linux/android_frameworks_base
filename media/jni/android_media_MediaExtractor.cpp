...
JMediaExtractor::JMediaExtractor(JNIEnv* env, jobject thiz) : mClass(NULL), mObject(NULL) {
...
status_t JMediaExtractor::setDataSource(const sp<IMediaHTTPService>& httpService, const char* path,
...
status_t JMediaExtractor::seekTo(int64_t timeUs, MediaSource::ReadOptions::SeekMode mode) {
...
status_t JMediaExtractor::readSampleData(jobject byteBuf, size_t offset, size_t* sampleSize) {
    JNIEnv* env = AndroidRuntime::getJNIEnv();
    ...
    *sampleSize = buffer->size();
    ...
    jmethodID positionID = env->GetMethodID(byteBufClass.get(), "position", "(I)Ljava/nio/Buffer;");

    CHECK(positionID != NULL);

    jmethodID limitID = env->GetMethodID(byteBufClass.get(), "limit", "(I)Ljava/nio/Buffer;");

    CHECK(limitID != NULL);

    jobject me = env->CallObjectMethod(byteBuf, limitID, offset + *sampleSize);
    env->DeleteLocalRef(me);
    me = env->CallObjectMethod(byteBuf, positionID, offset);
    env->DeleteLocalRef(me);
    me = NULL;
...
static sp<JMediaExtractor> setMediaExtractor(JNIEnv* env, jobject thiz,
                                             const sp<JMediaExtractor>& extractor) {
    sp<JMediaExtractor> old = (JMediaExtractor*)env->GetLongField(thiz, gFields.context);
...
static jint android_media_MediaExtractor_getTrackCount(JNIEnv* env, jobject thiz) {
...
static jobject android_media_MediaExtractor_getTrackFormatNative(JNIEnv* env, jobject thiz,
                                                                 jint index) {
    sp<JMediaExtractor> extractor = getMediaExtractor(env, thiz);
...
static jobject android_media_MediaExtractor_getFileFormatNative(JNIEnv* env, jobject thiz) {
    sp<JMediaExtractor> extractor = getMediaExtractor(env, thiz);
...
static void android_media_MediaExtractor_selectTrack(JNIEnv* env, jobject thiz, jint index) {
    sp<JMediaExtractor> extractor = getMediaExtractor(env, thiz);
...
static void android_media_MediaExtractor_unselectTrack(JNIEnv* env, jobject thiz, jint index) {
    sp<JMediaExtractor> extractor = getMediaExtractor(env, thiz);
...
static void android_media_MediaExtractor_seekTo(JNIEnv* env, jobject thiz, jlong timeUs,
                                                jint mode) {
    sp<JMediaExtractor> extractor = getMediaExtractor(env, thiz);
...
static jboolean android_media_MediaExtractor_advance(JNIEnv* env, jobject thiz) {
    sp<JMediaExtractor> extractor = getMediaExtractor(env, thiz);
...
static jint android_media_MediaExtractor_readSampleData(JNIEnv* env, jobject thiz, jobject byteBuf,
                                                        jint offset) {
    sp<JMediaExtractor> extractor = getMediaExtractor(env, thiz);
...
static jint android_media_MediaExtractor_getSampleTrackIndex(JNIEnv* env, jobject thiz) {
    sp<JMediaExtractor> extractor = getMediaExtractor(env, thiz);
...
static jlong android_media_MediaExtractor_getSampleTime(JNIEnv* env, jobject thiz) {
    sp<JMediaExtractor> extractor = getMediaExtractor(env, thiz);
...
static jlong android_media_MediaExtractor_getSampleSize(JNIEnv* env, jobject thiz) {
    sp<JMediaExtractor> extractor = getMediaExtractor(env, thiz);
...
static jint android_media_MediaExtractor_getSampleFlags(JNIEnv* env, jobject thiz) {
    sp<JMediaExtractor> extractor = getMediaExtractor(env, thiz);
...
static jboolean android_media_MediaExtractor_getSampleCryptoInfo(JNIEnv* env, jobject thiz,
    ...
    env->CallVoidMethod(cryptoInfoObj, gFields.cryptoInfoSetID, (jint)numSubSamples,
                        numBytesOfPlainDataObj, numBytesOfEncryptedDataObj, keyObj, ivObj, mode);
    ...
    env->CallVoidMethod(cryptoInfoObj, gFields.cryptoInfoSetPatternID, encryptedByteBlock,
                        skipByteBlock);
...
static void android_media_MediaExtractor_native_init(JNIEnv* env) {
    ...
    gFields.cryptoInfoSetPatternID = env->GetMethodID(clazz, "setPattern", "(II)V");
}

static void android_media_MediaExtractor_native_setup(JNIEnv* env, jobject thiz) {
    sp<JMediaExtractor> extractor = new JMediaExtractor(env, thiz);
    setMediaExtractor(env, thiz, extractor);
}

static void android_media_MediaExtractor_setDataSource(JNIEnv* env, jobject thiz,
                                                       jobject httpServiceBinderObj,
                                                       jstring pathObj, jobjectArray keysArray,
    ...
    if (!ConvertKeyValueArraysToKeyedVector(env, keysArray, valuesArray, &headers)) {
        return;
    }
    ...
    if (err != OK) {
        jniThrowException(env, "java/io/IOException", "Failed to instantiate extractor.");
        return;
    }
}

static void android_media_MediaExtractor_setDataSourceFd(JNIEnv* env, jobject thiz,
...
static void android_media_MediaExtractor_setDataSourceCallback(JNIEnv* env, jobject thiz,
                                                               jobject callbackObj) {
    sp<JMediaExtractor> extractor = getMediaExtractor(env, thiz);
    ...
    sp<DataSource> bridge = CreateDataSourceFromIDataSource(new JMediaDataSource(env, callbackObj));
...
static void android_media_MediaExtractor_setMediaCas(JNIEnv* env, jobject thiz,
                                                     jobject casBinderObj) {
    sp<JMediaExtractor> extractor = getMediaExtractor(env, thiz);
...
static jlong android_media_MediaExtractor_getCachedDurationUs(JNIEnv* env, jobject thiz) {
    sp<JMediaExtractor> extractor = getMediaExtractor(env, thiz);
...
static jboolean android_media_MediaExtractor_hasCacheReachedEOS(JNIEnv* env, jobject thiz) {
    sp<JMediaExtractor> extractor = getMediaExtractor(env, thiz);
...
static void android_media_MediaExtractor_native_finalize(JNIEnv* env, jobject thiz) {
    android_media_MediaExtractor_release(env, thiz);
}

static jobject android_media_MediaExtractor_native_getMetrics(JNIEnv* env, jobject thiz) {
...
static const JNINativeMethod gMethods[] = {
        ...
        {"getSampleSize", "()J", (void*)android_media_MediaExtractor_getSampleSize},
        ...
        {"native_setup", "()V", (void*)android_media_MediaExtractor_native_setup},

        {"native_finalize", "()V", (void*)android_media_MediaExtractor_native_finalize},
        ...
        {"getCachedDuration", "()J", (void*)android_media_MediaExtractor_getCachedDurationUs},
...
int register_android_media_MediaExtractor(JNIEnv* env) {
    return AndroidRuntime::registerNativeMethods(env, "android/media/MediaExtractor", gMethods,
                                                 NELEM(gMethods));
}
