...
SoundPool::~SoundPool() {
    ...
    mChannels.clear();
    if (mChannelPool) delete[] mChannelPool;
    // clean up samples
    ALOGV("clear samples");
    mSamples.clear();

    if (mDecodeThread) delete mDecodeThread;
}
...
bool SoundPool::startThreads() {
    ...
    if (mDecodeThread == NULL) mDecodeThread = new SoundPoolThread(this);
...
int SoundPool::load(int fd, int64_t offset, int64_t length, int priority __unused) {
    ALOGV("load: fd=%d, offset=%" PRId64 ", length=%" PRId64 ", priority=%d", fd, offset, length,
          priority);
    int sampleID;
    {
        Mutex::Autolock lock(&mLock);
...
int SoundPool::play(int sampleID, float leftVolume, float rightVolume, int priority, int loop,
                    float rate) {
...
void SoundPool::mute(bool muting) {
    ...
    mMuted = muting;
    if (!mChannels.empty()) {
        for (List<SoundChannel*>::iterator iter = mChannels.begin(); iter != mChannels.end();
             ++iter) {
            (*iter)->mute(muting);
        }
    }
...
void SoundPool::stop(int channelID) {
    ...
    if (channel) {
        ...
        if (channel) channel->clearNextEvent();
    }
}
...
Sample::Sample(int sampleID, int fd, int64_t offset, int64_t length) {
    ...
    mOffset = offset;
    mLength = length;
    ALOGV("create sampleID=%d, fd=%d, offset=%" PRId64 " length=%" PRId64, mSampleID, mFd, mLength,
          mOffset);
}
...
static status_t decode(int fd, int64_t offset, int64_t length, uint32_t* rate, int* numChannels,
                       audio_format_t* audioFormat, sp<MemoryHeapBase> heap, size_t* memsize) {
    ...
    *audioFormat = AUDIO_FORMAT_PCM_16_BIT;
    ...
    for (size_t i = 0; i < numTracks; i++) {
        ...
        if (strncmp(mime, "audio/", 6) == 0) {
            ...
            if (codec == NULL ||
                AMediaCodec_configure(codec, format, NULL /* window */, NULL /* drm */,
                                      0 /* flags */) != AMEDIA_OK ||
            ...
            while (!sawOutputEOS) {
                if (!sawInputEOS) {
                    ...
                    if (bufidx >= 0) {
                        ...
                        media_status_t mstatus = AMediaCodec_queueInputBuffer(
                                codec, bufidx, 0 /* offset */, sampleSize, presentationTimeUs,
...
status_t Sample::doLoad() {
    ...
    ALOGV("Start decode");
    status = decode(mFd, mOffset, mLength, &sampleRate, &numChannels, &format, mHeap, &mSize);
    ALOGV("close(%d)", mFd);
    ::close(mFd);
    mFd = -1;
    if (status != NO_ERROR) {
    ...
    ALOGV("pointer = %p, size = %zu, sampleRate = %u, numChannels = %d", mHeap->getBase(), mSize,
...
void SoundChannel::play(const sp<Sample>& sample, int nextChannelID, float leftVolume,
    ...
    {  // scope for the lock
        ...
        ALOGV("SoundChannel::play %p: sampleID=%d, channelID=%d, leftVolume=%f, rightVolume=%f,"
              ...
              this, sample->sampleID(), nextChannelID, leftVolume, rightVolume, priority, loop,
              rate);
        ...
        if (loop) {
            ...
            const size_t frameSize =
                    audio_is_linear_pcm(format) ? numChannels * audio_bytes_per_sample(format) : 1;
        ...
        if (newTrack == 0) {
            ...
            newTrack = new AudioTrack(
                    streamType, sampleRate, sample->format(), channelMask, sample->getIMemory(),
                    AUDIO_OUTPUT_FLAG_FAST, callback, userData, 0 /*default notification frames*/,
                    AUDIO_SESSION_ALLOCATE, AudioTrack::TRANSFER_DEFAULT, NULL /*offloadInfo*/,
                    -1 /*uid*/, -1 /*pid*/, mSoundPool->attributes());
#else
            uint32_t bufferFrames = (totalFrames + (kDefaultBufferCount - 1)) / kDefaultBufferCount;
            newTrack = new AudioTrack(streamType, sampleRate, sample->format(), channelMask,
                                      frameCount, AUDIO_OUTPUT_FLAG_FAST, callback, userData,
                                      bufferFrames, AUDIO_SESSION_ALLOCATE,
                                      AudioTrack::TRANSFER_DEFAULT, NULL /*offloadInfo*/,
                                      -1 /*uid*/, -1 /*pid*/, mSoundPool->attributes());
#endif
            ...
            // From now on, AudioTrack callbacks received with previous toggle value will be
            // ignored.
            mToggle = toggle;
    ...
    if (event == AudioTrack::EVENT_MORE_DATA) {
        ...
        if (sample != 0) {
            ...
            if (mPos < (int)sample->size()) {
                ...
                memcpy(q, p, count);
                //              ALOGV("fill: q=%p, p=%p, mPos=%u, b->size=%u, count=%d", q, p, mPos,
                //              b->size,
                //                      count);
                ...
                memset(q, 0, count);
                //              ALOGV("fill extra: q=%p, mPos=%u, b->size=%u, count=%d", q, mPos,
                //              b->size, count);
            }
        ...
        ALOGV("process %p channel %d event %s", this, mChannelID,
              (event == AudioTrack::EVENT_UNDERRUN) ? "UNDERRUN" : "BUFFER_END");
...
void SoundChannel::setVolume_l(float leftVolume, float rightVolume) {
    mLeftVolume = leftVolume;
    mRightVolume = rightVolume;
    if (mAudioTrack != NULL && !mMuted) mAudioTrack->setVolume(leftVolume, rightVolume);
}
...
void SoundChannel::setLoop(int loop) {
    ...
    if (mAudioTrack != NULL && mSample != 0) {
        uint32_t loopEnd = mSample->size() / mNumChannels /
                           ((mSample->format() == AUDIO_FORMAT_PCM_16_BIT) ? sizeof(int16_t)
                                                                           : sizeof(uint8_t));
...
void SoundChannel::dump() {
    ALOGV("mState = %d mChannelID=%d, mNumChannels=%d, mPos = %d, mPriority=%d, mLoop=%d", mState,
          mChannelID, mNumChannels, mPos, mPriority, mLoop);
}

void SoundEvent::set(const sp<Sample>& sample, int channelID, float leftVolume, float rightVolume,
