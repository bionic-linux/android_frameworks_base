...
static void android_mtp_MtpServer_setup(JNIEnv* env, jobject thiz, jobject javaDatabase,
                                        jobject jControlFd, jboolean usePtp,
                                        jstring deviceInfoManufacturer, jstring deviceInfoModel,
    ...
    MtpServer* server =
            new MtpServer(getMtpDatabase(env, javaDatabase), controlFd, usePtp,
...
static void android_mtp_MtpServer_cleanup(JNIEnv* env, jobject thiz) {
...
static void android_mtp_MtpServer_send_object_removed(JNIEnv* env, jobject thiz, jint handle) {
    Mutex::Autolock autoLock(sMutex);
...
static void android_mtp_MtpServer_send_device_property_changed(JNIEnv* env, jobject thiz,
                                                               jint property) {
    Mutex::Autolock autoLock(sMutex);
    ...
    if (server) {
        ...
        if (pathStr != NULL) {
            ...
            if (descriptionStr != NULL) {
                MtpStorage* storage =
                        new MtpStorage(storageID, pathStr, descriptionStr, removable, maxFileSize);
...
static void android_mtp_MtpServer_remove_storage(JNIEnv* env, jobject thiz, jint storageId) {
    Mutex::Autolock autoLock(sMutex);
...
static const JNINativeMethod gMethods[] = {
        {"native_setup",
         "(Landroid/mtp/MtpDatabase;Ljava/io/FileDescriptor;ZLjava/lang/String;Ljava/lang/"
...
int register_android_mtp_MtpServer(JNIEnv* env) {
    ...
    return AndroidRuntime::registerNativeMethods(env, "android/mtp/MtpServer", gMethods,
                                                 NELEM(gMethods));
}
