...
public class MiniThumbFile {
    ...
    * @param create when you want to create a new thumbnail, set to true; when generally query thumbnail saved index, set to false.
    */
    private long getIndex(long id, boolean create){
        ...
        if (r != null) {
            ...
            //so if create set, then begin from 0, others begin from real index (id+1)*BYTES_PER_MINTHUMB_INDEX;
            if (!create) {
                ...
                if (read == BYTES_PER_MINTHUMB_INDEX) {
                    ...
                    if (create) {
                        ...
                        if(BYTES_PER_MINTHUMB_INDEX == write) {
                            ...
                            if(debug) Log.d(TAG, "getIndex with create. index: " + now + "corresponding id: " + id + ", index is: " + pos);
                        }
                        return now;
                    } else {
                        long p = buf.getLong();
                    ...
                    if(0 == f.length()){
                        ...
                        if(debug) Log.d(TAG, "getIndex create index file");
                        if(MINI_THUMB_DATA_FILE_VERSION == write) return 0;
                    }
                }
            } catch (IOException ex) {
