...
public class AudioManagerTest extends AudioVolumesTestBase {
    ...
    public void testStreamAttributesSetVolumeCompatibility() throws Exception {
        List<AudioVolumeGroup> audioVolumeGroups = mAudioManager.getAudioVolumeGroups();
        assertTrue(audioVolumeGroups.size() > 0);
        ...
        try {
            ...
            for (final AudioVolumeGroup avg : audioVolumeGroups) {
                ...
                assertEquals(indexMin, mAudioManager.getVolumeIndexForAttributes(aa));
                assertEquals(indexMin, mAudioManager.getLastAudibleAttributesVolume(aa));
                for (final int stream : avgStreamTypes) {
                    if (stream != AudioSystem.STREAM_DEFAULT && mPublicStreams.contains(stream)) {
                    ...
                    if (canUseStreamApi(avg, aa, stream)) {
                       ...
                       assertEquals(indexMin, mAudioManager.getLastAudibleStreamVolume(stream));
                    }
                }

                mAudioManager.setVolumeIndexForAttributes(aa, indexMin + 1, 0/*flags*/);

                assertTrue(mVgCbReceiver.waitForExpectedVolumeGroupChanged(
                        AudioVolumeGroupCallbackHelper.ASYNC_TIMEOUT_MS));

                assertFalse(mAudioManager.isAttributesMuted(aa));
                assertEquals(indexMin + 1, mAudioManager.getVolumeIndexForAttributes(aa));
                assertEquals(indexMin + 1, mAudioManager.getLastAudibleAttributesVolume(aa));
                for (final int stream : avgStreamTypes) {
                    if (stream != AudioSystem.STREAM_DEFAULT && mPublicStreams.contains(stream)) {
                        assertEquals(indexMin + 1, mAudioManager.getStreamVolume(stream));
                    }
                    if (canUseStreamApi(avg, aa, stream)) {
                       assertFalse(mAudioManager.isStreamMute(stream));
                       assertEquals(indexMin + 1, mAudioManager.getLastAudibleStreamVolume(stream));
                    }
                }
                ...
                for (final int stream : avgStreamTypes) {
                    ...
                    if (stream != AudioSystem.STREAM_DEFAULT && mPublicStreams.contains(stream)) {
                        mAudioManager.setStreamVolume(stream, indexMin, 0/*flags*/);
                ...
                for (final int stream : avgStreamTypes) {
                    ...
                    if (stream != AudioSystem.STREAM_DEFAULT && mPublicStreams.contains(stream)) {
                        mAudioManager.setStreamVolume(stream, indexMin + 1, 0/*flags*/);
                        ...
                        if (canUseStreamApi(avg, aa, stream)) {
                            assertFalse(mAudioManager.isStreamMute(stream));
                            assertEquals(indexMin + 1,
                                    mAudioManager.getLastAudibleStreamVolume(stream));
                        }
                        assertEquals(indexMin + 1, mAudioManager.getVolumeIndexForAttributes(aa));
                        assertFalse(mAudioManager.isAttributesMuted(aa));
                        assertEquals(indexMin + 1,
                                mAudioManager.getLastAudibleAttributesVolume(aa));
                        break;
                    }
                }
