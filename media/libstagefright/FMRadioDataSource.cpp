...
FMRadioDataSource::FMRadioDataSource()
{
    ...
    mFlags = 0;
    mOverwrittenBytes = 0;
    mInputClientId = AudioSystem::AUDIO_INPUT_CLIENT_PLAYBACK;

    mStream = (AudioStreamIn*) AudioSystem::getInput(AUDIO_SOURCE_FM_RADIO_RX,
                                   mSampleRate, mFormat, mChannels, (AudioSystem::audio_in_acoustics) mFlags,
                                   &mInputClientId);
    if (mStream != NULL) {
...
ssize_t FMRadioDataSource::readAt(off_t offset, void *data, size_t size) {
    if (mStream != NULL) {
        const sp<IAudioFlinger>& af = AudioSystem::get_audio_flinger();
        return af->readInput((uint32_t*)mStream, mInputClientId, data, size);
    }
    return 0;
}

status_t FMRadioDataSource::getSize(off_t *size) {
    *size = 0;
    return OK;
}
...
{
    return AudioSystem::popCount(mChannels);
}
...
uint32_t FMRadioDataSource::getFormat()
{
    ...
    else if(mFormat == AudioSystem::PCM_8_BIT) {
        return 8;
    }
    return 0;
}
