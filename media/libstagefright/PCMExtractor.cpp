...
status_t PCMExtractor::init() {
    if((mNumChannels = mDataSource->getNumChannels()) == 0)
    {
        mNumChannels = DEFAULT_NUM_CHANNELS;
    }

    if((mSampleRate = mDataSource->getSampleRate()) == 0)
    {
        mSampleRate = DEFAULT_SAMPLE_RATE;
    }

    if((mBitsPerSample = mDataSource->getFormat()) == 0)
    {
        mBitsPerSample = DEFAULT_FORMAT;
    }
...
status_t PCMSource::start(MetaData *params) {
    ...
    if((mBufferSize = mDataSource->getBufferSize()) == 0){
...
status_t PCMSource::read(
    ...
    *out = NULL;
    ...
    if (err != OK) {
        LOGE("Error when acquiring buffer %d", err);
        return err;
    }
