...
ShaderProgram::ShaderProgram(GLEnv* gl_env, const std::string& vertex_shader,
...
ShaderProgram::~ShaderProgram() {
    ...
    for (iter = attrib_values_.begin(); iter != attrib_values_.end(); ++iter) {
        ...
        if (attrib.owned_data) delete[] attrib.owned_data;
    }
}

void ShaderProgram::SetDefaultCoords() {
    if (!source_coords_) source_coords_ = new float[8];
    if (!target_coords_) target_coords_ = new float[8];
...
void ShaderProgram::SetSourceRect(float x, float y, float width, float height) {
    Quad quad(Point(x, y), Point(x + width, y), Point(x, y + height), Point(x + width, y + height));
...
bool ShaderProgram::BindInputTextures(const std::vector<GLuint>& textures,
    ...
    for (unsigned i = 0; i < textures.size(); ++i) {
        ...
        if (GLEnv::CheckGLError("Activating Texture Unit")) return false;

        // Bind our texture
        glBindTexture(targets[i], textures[i]);
        LOG_FRAME("Binding texture %d", textures[i]);
        if (GLEnv::CheckGLError("Binding Texture")) return false;
        ...
        if (tex_var >= 0) {
            ...
            ALOGE("ShaderProgram: Shader does not seem to support %zd number of "
                  "inputs! Missing uniform 'tex_sampler_%d'!",
                  textures.size(), i);
            return false;
        }

        if (GLEnv::CheckGLError("Texture Variable Binding")) return false;
    }
...
bool ShaderProgram::RenderFrame(const std::vector<GLuint>& textures,
    ...
    const bool success =
            (!requestTile || !manage_coordinates_ || vertex_count_ != 4) ? Draw() : DrawTiled();
...
bool ShaderProgram::CheckValueCount(const std::string& var_type, const std::string& var_name,
    ...
    if (expected_count != (value_size / components)) {
        ALOGE("Shader Program: %s Value Error (%s): Expected value length %d "
              ...
              var_type.c_str(), var_name.c_str(), expected_count, components * expected_count,
...
bool ShaderProgram::CheckValueMult(const std::string& var_type, const std::string& var_name,
                                   int components, int value_size) {
    if (value_size % components != 0) {
        ALOGE("Shader Program: %s Value Error (%s): Value must be multiple of %d, "
              "but %d elements were passed!",
              var_type.c_str(), var_name.c_str(), components, value_size);
        return false;
    }
    return true;
}
...
bool ShaderProgram::SetUniformValue(ProgramVar var, int value) {
    if (!CheckVarValid(var)) return false;
...
bool ShaderProgram::SetUniformValue(ProgramVar var, const int* values, int count) {
    if (!CheckUniformValid(var)) return false;
...
bool ShaderProgram::SetUniformValue(ProgramVar var, const float* values, int count) {
    if (!CheckUniformValid(var)) return false;
...
bool ShaderProgram::SetUniformValue(ProgramVar var, const std::vector<float>& values) {
    return SetUniformValue(var, &values[0], values.size());
}
...
ProgramVar ShaderProgram::GetAttribute(const std::string& name) const {
    if (!IsExecutable()) {
        ...
        ALOGW("ShaderProgram: Attempting to overwrite internal vertex attribute '%s'!",
              name.c_str());
    }
    return glGetAttribLocation(program_, name.c_str());
}

bool ShaderProgram::SetAttributeValues(ProgramVar var, const VertexFrame* vbo, GLenum type,
                                       int components, int stride, int offset, bool normalize) {
    if (!CheckVarValid(var)) return false;
...
bool ShaderProgram::SetAttributeValues(ProgramVar var, const uint8_t* data, GLenum type,
                                       int components, int stride, int offset, bool normalize) {
    if (!CheckVarValid(var)) return false;
...
bool ShaderProgram::SetAttributeValues(ProgramVar var, const std::vector<float>& data,
                                       int components) {
    return SetAttributeValues(var, &data[0], data.size(), components);
}

bool ShaderProgram::SetAttributeValues(ProgramVar var, const float* data, int total,
                                       int components) {
    if (!CheckVarValid(var)) return false;
    ...
    if (StoreAttribute(attrib)) return true;
...
bool ShaderProgram::PushAttributes() {
    for (VertexAttribMap::const_iterator iter = attrib_values_.begin();
         iter != attrib_values_.end(); ++iter) {
        const VertexAttrib& attrib = iter->second;

        if (attrib.is_const) {
            // Set constant attribute values (must be specified as host values)
            if (!attrib.values) return false;

            const float* values = reinterpret_cast<const float*>(attrib.values);
            switch (attrib.components) {
                case 1:
                    glVertexAttrib1fv(attrib.index, values);
                    break;
                case 2:
                    glVertexAttrib2fv(attrib.index, values);
                    break;
                case 3:
                    glVertexAttrib3fv(attrib.index, values);
                    break;
                case 4:
                    glVertexAttrib4fv(attrib.index, values);
                    break;
                default:
                    return false;
            }
            ...
            if (attrib.values) {
                ...
                glVertexAttribPointer(attrib.index, attrib.components, attrib.type,
                                      attrib.normalized, attrib.stride, attrib.values);
            } else if (attrib.vbo) {
        ...
        if (GLEnv::CheckGLError("Pushing Vertex Attributes")) return false;
    }
