...
VertexFrame::VertexFrame(int size) : vbo_(0), size_(size) {}
...
bool VertexFrame::WriteData(const uint8_t* data, int size) {
    ...
    if (GLEnv::CheckGLError("VBO Bind Buffer")) return false;
    ...
    if (first_upload && size == size_)
        ...
        ALOGE("VertexFrame: Attempting to upload more data (%d bytes) than fits "
              "inside the vertex frame (%d bytes)!",
              size, size_);
        return false;
    }

    // Make sure it worked
    if (GLEnv::CheckGLError("VBO Data Upload")) return false;
