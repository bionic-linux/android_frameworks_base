...
GLEnv::~GLEnv() {
    // Destroy surfaces
    for (std::map<int, SurfaceWindowPair>::iterator it = surfaces_.begin(); it != surfaces_.end();
         ++it) {
    ...
    // Destroy contexts
    for (std::map<int, EGLContext>::iterator it = contexts_.begin(); it != contexts_.end(); ++it) {
        if (it->first != 0 || created_context_) eglDestroyContext(display(), it->second);
    }
    ...
    // Destroy display
    if (initialized_) eglTerminate(display());

    // Log error if this did not work
    if (CheckEGLError("TearDown!")) ALOGE("GLEnv: Error tearing down GL Environment!");
}

bool GLEnv::IsInitialized() const {
    return (contexts_.size() > 0 && surfaces_.size() > 0 && display_ != EGL_NO_DISPLAY);
}
...
bool GLEnv::Activate() {
    ALOGV("Activate()");
    if (display() != eglGetCurrentDisplay() || context() != eglGetCurrentContext() ||
...
bool GLEnv::InitWithNewContext() {
    ...
    surfaceTexture_ = new GLConsumer(consumer, 0, GLConsumer::TEXTURE_EXTERNAL, true, false);
    window_ = new Surface(producer);

    surfaces_[0] =
            SurfaceWindowPair(eglCreateWindowSurface(display(), config, window_.get(), NULL), NULL);
    if (CheckEGLError("eglCreateWindowSurface")) return false;

    // Create context
    EGLint context_attribs[] = {EGL_CONTEXT_CLIENT_VERSION, 2, EGL_NONE};
    contexts_[0] = eglCreateContext(display(), config, EGL_NO_CONTEXT, context_attribs);
...
bool GLEnv::IsActive() const {
    ALOGV("IsActive()");
    return context() == eglGetCurrentContext() && display() == eglGetCurrentDisplay() &&
           surface() == eglGetCurrentSurface(EGL_DRAW);
}
...
bool GLEnv::ReleaseSurfaceId(int surface_id) {
    if (surface_id > 0) {
        ...
        if (surface_window_pair) {
            if (surface_id_ == surface_id) SwitchToSurfaceId(0);
...
bool GLEnv::SetSurfaceTimestamp(int64_t timestamp) {
    if (surface_id_ > 0) {
        const SurfaceWindowPair* surface_window_pair = FindOrNull(surfaces_, surface_id_);
        if (surface_window_pair) {
            ANativeWindow* window =
                    static_cast<ANativeWindow*>(surface_window_pair->second->InternalHandle());
...
void GLEnv::ReleaseContextId(int context_id) {
    if (context_id > 0) {
        ...
        if (context) {
            ...
            if (context_id_ == context_id && IsActive()) SwitchToContextId(0);
...
void GLEnv::AttachShader(int key, ShaderProgram* shader) {
    ...
    if (existingShader) delete existingShader;
...
void GLEnv::AttachVertexFrame(int key, VertexFrame* frame) {
    ...
    if (existingFrame) delete existingFrame;
