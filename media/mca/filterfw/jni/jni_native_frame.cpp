...
jboolean Java_android_filterfw_core_NativeFrame_nativeAllocate(JNIEnv* env, jobject thiz,
                                                               jint size) {
...
jboolean Java_android_filterfw_core_NativeFrame_setNativeData(JNIEnv* env, jobject thiz,
    ...
    if (frame && data) {
        ...
        if (bytes) {
            const bool success =
                    frame->WriteData(reinterpret_cast<const uint8_t*>(bytes + offset), 0, length);
...
jbyteArray Java_android_filterfw_core_NativeFrame_getNativeData(JNIEnv* env, jobject thiz,
                                                                jint size) {
    NativeFrame* frame = ConvertFromJava<NativeFrame>(env, thiz);
    if (frame) {
        const uint8_t* data = frame->Data();
        if (!data || size > frame->Size()) return NULL;
...
jboolean Java_android_filterfw_core_NativeFrame_getNativeBuffer(JNIEnv* env, jobject thiz,
                                                                jobject buffer) {
    NativeFrame* frame = ConvertFromJava<NativeFrame>(env, thiz);
    if (frame) {
...
jboolean Java_android_filterfw_core_NativeFrame_setNativeInts(JNIEnv* env, jobject thiz,
                                                              jintArray ints) {
    NativeFrame* frame = ConvertFromJava<NativeFrame>(env, thiz);
    if (frame && ints) {
        ...
        if (int_ptr) {
            const bool success = frame->WriteData(reinterpret_cast<const uint8_t*>(int_ptr), 0,
                                                  length * sizeof(jint));
...
jintArray Java_android_filterfw_core_NativeFrame_getNativeInts(JNIEnv* env, jobject thiz,
                                                               jint size) {
    NativeFrame* frame = ConvertFromJava<NativeFrame>(env, thiz);
    if (frame) {
        const uint8_t* data = frame->Data();
        if (!data || size > frame->Size() || (size % sizeof(jint)) != 0) return NULL;
...
jboolean Java_android_filterfw_core_NativeFrame_setNativeFloats(JNIEnv* env, jobject thiz,
                                                                jfloatArray floats) {
    NativeFrame* frame = ConvertFromJava<NativeFrame>(env, thiz);
    if (frame && floats) {
        ...
        if (float_ptr) {
            const bool success = frame->WriteData(reinterpret_cast<const uint8_t*>(float_ptr), 0,
...
jfloatArray Java_android_filterfw_core_NativeFrame_getNativeFloats(JNIEnv* env, jobject thiz,
                                                                   jint size) {
    NativeFrame* frame = ConvertFromJava<NativeFrame>(env, thiz);
    if (frame) {
        const uint8_t* data = frame->Data();
        if (!data || size > frame->Size() || (size % sizeof(jfloat)) != 0) return NULL;
...
jboolean Java_android_filterfw_core_NativeFrame_setNativeBitmap(JNIEnv* env, jobject thiz,
                                                                jobject bitmap, jint size,
    ...
    if (frame && bitmap) {
        ...
        Pixel* src_ptr;
        const int result =
                AndroidBitmap_lockPixels(env, bitmap, reinterpret_cast<void**>(&src_ptr));
...
jboolean Java_android_filterfw_core_NativeFrame_getNativeBitmap(JNIEnv* env, jobject thiz,
                                                                jobject bitmap, jint size,
                                                                jint bytes_per_sample) {
    NativeFrame* frame = ConvertFromJava<NativeFrame>(env, thiz);
    if (frame && bitmap) {
        Pixel* dst_ptr;
        const int result =
                AndroidBitmap_lockPixels(env, bitmap, reinterpret_cast<void**>(&dst_ptr));
        if (result == ANDROID_BITMAP_RESULT_SUCCESS) {
            // Make sure frame size matches bitmap size
            if ((size / 4) != (frame->Size() / bytes_per_sample)) {
...
jboolean Java_android_filterfw_core_NativeFrame_nativeCopyFromNative(JNIEnv* env, jobject thiz,
                                                                     jobject frame) {
...
jboolean Java_android_filterfw_core_NativeFrame_nativeCopyFromGL(JNIEnv* env, jobject thiz,
                                                                 jobject frame) {
    NativeFrame* this_frame = ConvertFromJava<NativeFrame>(env, thiz);
