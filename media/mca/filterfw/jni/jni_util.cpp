...
std::string ToCppString(JNIEnv* env, jstring value) {
    ...
    if (isCopy == JNI_TRUE) env->ReleaseStringUTFChars(value, c_value);
    return result;
}
...
template <typename T>
jobject CreateJObject(JNIEnv* env, const std::string& class_name, const std::string& signature,
                      T value) {
...
jobject ToJObject(JNIEnv* env, const Value& value) {
    ...
    if (ValueIsInt(value)) {
        ...
        env->SetIntArrayRegion(static_cast<jintArray>(result), 0, GetValueCount(value),
        ...
        env->SetFloatArrayRegion(static_cast<jfloatArray>(result), 0, GetValueCount(value),
