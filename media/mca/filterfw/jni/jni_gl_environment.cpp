...
class NativeWindowHandle : public WindowHandle {
  public:
    ...
    virtual const void* InternalHandle() const { return window_; }

    virtual void* InternalHandle() { return window_; }
...
jint Java_android_filterfw_core_GLEnvironment_nativeAddSurface(JNIEnv* env, jobject thiz,
                                                               jobject surface) {
    GLEnv* gl_env = ConvertFromJava<GLEnv>(env, thiz);
    if (!surface) {
        ...
        if (result == -1) {
            ...
            EGLSurface egl_surface =
                    eglCreateWindowSurface(gl_env->display(), config, window, NULL);
...
jint Java_android_filterfw_core_GLEnvironment_nativeAddSurfaceWidthHeight(JNIEnv* env, jobject thiz,
                                                                          jobject surface,
...
jint Java_android_filterfw_core_GLEnvironment_nativeAddSurfaceFromMediaRecorder(
        JNIEnv* env, jobject thiz, jobject jmediarecorder) {
    ...
    // Create the EGL surface
    EGLSurface egl_surface = eglCreateWindowSurface(gl_env->display(), config, window.get(), NULL);
...
jboolean Java_android_filterfw_core_GLEnvironment_nativeActivateSurfaceId(JNIEnv* env, jobject thiz,
                                                                          jint surfaceId) {
    GLEnv* gl_env = ConvertFromJava<GLEnv>(env, thiz);
    return gl_env ? ToJBool(gl_env->SwitchToSurfaceId(surfaceId) && gl_env->Activate()) : JNI_FALSE;
}

jboolean Java_android_filterfw_core_GLEnvironment_nativeRemoveSurfaceId(JNIEnv* env, jobject thiz,
                                                                        jint surfaceId) {
    GLEnv* gl_env = ConvertFromJava<GLEnv>(env, thiz);
