...
jboolean Java_android_filterfw_core_ShaderProgram_allocate(JNIEnv* env, jobject thiz,
                                                           jobject gl_env, jstring vertex_shader,
                                                           jstring fragment_shader) {
    // Get the GLEnv pointer
    GLEnv* gl_env_ptr = ConvertFromJava<GLEnv>(env, gl_env);

    // Create the shader
    if (!fragment_shader || !gl_env_ptr) return false;
...
jboolean Java_android_filterfw_core_ShaderProgram_setUniformValue(JNIEnv* env, jobject thiz,
    ...
    if (c_value.value) {
        ...
        ALOGE("ShaderProgram: Could not convert java object value passed for key '%s'!",
              c_key.c_str());
        return JNI_FALSE;
    }
}

jobject Java_android_filterfw_core_ShaderProgram_getUniformValue(JNIEnv* env, jobject thiz,
                                                                 jstring key) {
    ShaderProgram* program = ConvertFromJava<ShaderProgram>(env, thiz);
    const std::string c_key = ToCppString(env, key);
...
jboolean Java_android_filterfw_core_ShaderProgram_shaderProcess(JNIEnv* env, jobject thiz,
                                                                jobjectArray inputs,
                                                                jobject output) {
    ShaderProgram* program = ConvertFromJava<ShaderProgram>(env, thiz);
...
jboolean Java_android_filterfw_core_ShaderProgram_setSourceRegion(JNIEnv* env, jobject thiz,
                                                                  jfloat x0, jfloat y0, jfloat x1,
...
jboolean Java_android_filterfw_core_ShaderProgram_setTargetRegion(JNIEnv* env, jobject thiz,
                                                                  jfloat x0, jfloat y0, jfloat x1,
                                                                  jfloat y1, jfloat x2, jfloat y2,
                                                                  jfloat x3, jfloat y3) {
    ShaderProgram* program = ConvertFromJava<ShaderProgram>(env, thiz);
...
jboolean Java_android_filterfw_core_ShaderProgram_setShaderClearsOutput(JNIEnv* env, jobject thiz,
                                                                        jboolean clears) {
    ShaderProgram* program = ConvertFromJava<ShaderProgram>(env, thiz);
    if (program) {
...
jboolean Java_android_filterfw_core_ShaderProgram_setShaderBlendEnabled(JNIEnv* env, jobject thiz,
                                                                        jboolean enable) {
    ShaderProgram* program = ConvertFromJava<ShaderProgram>(env, thiz);
    if (program) {
...
jboolean Java_android_filterfw_core_ShaderProgram_setShaderBlendFunc(JNIEnv* env, jobject thiz,
...
jboolean Java_android_filterfw_core_ShaderProgram_setShaderClearColor(JNIEnv* env, jobject thiz,
                                                                      jfloat r, jfloat g,
                                                                      jfloat b) {
    ShaderProgram* program = ConvertFromJava<ShaderProgram>(env, thiz);
    if (program) {
...
jboolean Java_android_filterfw_core_ShaderProgram_setShaderDrawMode(JNIEnv* env, jobject thiz,
                                                                    jint draw_mode) {
    ShaderProgram* program = ConvertFromJava<ShaderProgram>(env, thiz);
    if (program) {
...
jboolean Java_android_filterfw_core_ShaderProgram_setShaderTileCounts(JNIEnv* env, jobject thiz,
                                                                      jint x_count, jint y_count) {
    ShaderProgram* program = ConvertFromJava<ShaderProgram>(env, thiz);
    if (program) {
...
jboolean Java_android_filterfw_core_ShaderProgram_setShaderVertexCount(JNIEnv* env, jobject thiz,
                                                                       jint vertex_count) {
    ShaderProgram* program = ConvertFromJava<ShaderProgram>(env, thiz);
    if (program) {
...
jboolean Java_android_filterfw_core_ShaderProgram_setShaderAttributeValues(
    ...
    if (program) {
        ...
        if (float_ptr && ShaderProgram::IsVarValid(program_var)) {
            const bool success = program->SetAttributeValues(
                    program_var, reinterpret_cast<float*>(float_ptr), length, component_count);
...
jboolean Java_android_filterfw_core_ShaderProgram_setShaderAttributeVertexFrame(
    ...
    if (program) {
        ...
        if (v_frame && ShaderProgram::IsVarValid(program_var)) {
            const bool success =
                    program->SetAttributeValues(program_var, v_frame, type, component_count, stride,
                                                offset, ToCppBool(normalize));
            return ToJBool(success);
        }
    }
    return JNI_FALSE;
