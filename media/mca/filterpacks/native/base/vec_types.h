...
class VecBase {
  public:
    ...
    T Length() {
        double sum = 0;
        for (int i = 0; i < dim; ++i) sum += static_cast<double>(data[i] * data[i]);
...
Vec<T, dim> operator+(const Vec<T, dim>& x, const Vec<T, dim>& y) {
    Vec<T, dim> out;
    for (int i = 0; i < dim; i++) out.data[i] = x.data[i] + y.data[i];
    return out;
}
...
T dot(const Vec<T, dim>& x, const Vec<T, dim>& y) {
    T out = 0;
    for (int i = 0; i < dim; i++) out += x.data[i] * y.data[i];
    return out;
}
...
Vec<T, dim> operator*(const Vec<T, dim>& x, T scale) {
    Vec<T, dim> out;
    for (int i = 0; i < dim; i++) out.data[i] = x.data[i] * scale;
    return out;
}
...
Vec<T, dim> operator+(const Vec<T, dim>& x, T val) {
    Vec<T, dim> out;
    for (int i = 0; i < dim; i++) out.data[i] = x.data[i] + val;
    return out;
}
