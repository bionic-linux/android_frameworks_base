...
/* this table defines valid transitions. (turn off indent, we want this easy readable) */
   ...
   /* FMRADIO_EVENT_GET_SIGNAL_STRENGTH */{false,false,false,false,false,false,false},
   /* FMRADIO_EVENT_SCAN */          {false,false,false,false,false,false,false},
   /* FMRADIO_EVENT_FULL_SCAN */     {false,false,false,false,false,false,false},
   /* Tx Only */
   /* FMRADIO_EVENT_BLOCK_SCAN */    {false,false,true, true, false,false,false},
};
/* *INDENT-ON* */

static void androidFmRadioTxCallbackOnStateChanged(int newState,
                                                   int oldState);
                                                   ...
                                                   int oldState)
{
    ...
    LOGI("androidFmRadioTxCallbackOnStateChanged: New state %d, old state %d\n", newState, oldState);
...
static void androidFmRadioTxCallbackOnStarted(void)
{
    ...
    err =
        AudioSystem::
        setDeviceConnectionState(AudioSystem::DEVICE_OUT_FM_RADIO_TX,
                      ...
                      int highFreq, int defaultFreq, int grid)
{
    ...
    if (fmTransmitterSession.jobj_p == NULL) {
        fmTransmitterSession.jobj_p =
            (jobject *) malloc(sizeof(*fmTransmitterSession.jobj_p));
    }
    *fmTransmitterSession.jobj_p = obj;
...
static void androidFmRadioTxPause(JNIEnv * env, jobject obj)
{
    ...
    androidFmRadioPause(&fmTransmitterSession);
...
static void androidFmRadioTxResume(JNIEnv * env, jobject obj)
{
    ...
    androidFmRadioResume(&fmTransmitterSession);
...
static jint androidFmRadioTxReset(JNIEnv * env, jobject obj)
{
    ...
    if (retval >= 0) {
        (void) AudioSystem::setDeviceConnectionState(AudioSystem::
...
androidFmRadioTxSetRDSData(JNIEnv * env, jobject obj, jobject bundle)
{
    ...
    if (bundle == NULL) {
        /* just shut down RDS transmission and return */
    ...
    while (env->CallBooleanMethod(iter2, hasNext)) {
        ...
        if (!str) {             // Out of memory
            ...
            jniThrowException(env, "java/lang/RuntimeException",
                              "Out of memory");
            /* note, jniThrowException does return and continue here! */
            ...
            if (value == NULL) {
                LOGI("android_setRdsData:No key found for %s\n", str);
                rv = -1;
            } else {
        ...
        if (rv == -1) {
            char throwMesg[128];
            sprintf(throwMesg, "type '%s' - Not accepted by vendor", str);
            jniThrowException(env,
                              "java/lang/IllegalArgumentException",
                              throwMesg);
            /* note - will continue and free memory below */
        }
        ...
        /* if we got an error we have sent exception and there is no use to continue */
        if (rv < 0) {
            break;
        }
    }
...
static jint androidFmRadioTxForcedPause(JNIEnv * env, jobject obj)
{
    int retval;

    LOGI("FMRadioTxForcedPaused\n");
    retval = androidFmRadioForcedPause(&fmTransmitterSession);
...
static void *execute_androidFmRadioTxBlockScan(void *args)
{
    int *iargs = (int *) args;
    int startFreq = iargs[0];
    int endFreq = iargs[1];
    int retval;
...
static JNINativeMethod gMethods[] = {
    {"_fm_transmitter_getState", "()I",
     (void *) androidFmRadioTxGetState},
    {"_fm_transmitter_start", "(IIII)V",
     (void *) androidFmRadioTxStart},
    {"_fm_transmitter_startAsync", "(IIII)V",
     (void *) androidFmRadioTxStartAsync},
    {"_fm_transmitter_pause", "()V", (void *) androidFmRadioTxPause},
    {"_fm_transmitter_resume", "()V", (void *) androidFmRadioTxResume},
    {"_fm_transmitter_reset", "()I", (void *) androidFmRadioTxReset},
    {"_fm_transmitter_setFrequency", "(I)V",
     (void *) androidFmRadioTxSetFrequency},
    {"_fm_transmitter_getFrequency", "()I",
     (void *) androidFmRadioTxGetFrequency},
    {"_fm_transmitter_forcedPause", "()I",
     (void *) androidFmRadioTxForcedPause},
    {"_fm_transmitter_isBlockScanSupported", "()Z",
     (void *) androidFmRadioTxIsBlockScanSupported},
    {"_fm_transmitter_startBlockScan", "(II)V",
     (void *) androidFmRadioTxStartBlockScan},
    {"_fm_transmitter_stopScan", "()V",
     (void *) androidFmRadioTxStopScan},
    {"_fm_transmitter_setRdsData", "(Landroid/os/Bundle;)V",
     (void *) androidFmRadioTxSetRDSData},
    {"_fm_transmitter_sendExtraCommand",
...
int registerAndroidFmRadioTransmitter(JavaVM * vm, JNIEnv * env)
{
     ...
     */
    if (!fmTransmitterSession.isRegistered) {
        fmTransmitterSession.vendorMethods_p = (fmradio_vendor_methods_t *)
            malloc(sizeof(*fmTransmitterSession.vendorMethods_p));
        if (fmTransmitterSession.vendorMethods_p == NULL) {
            LOGE("malloc failed\n");
        } else if (androidFmRadioLoadFmLibrary
                   (&fmTransmitterSession, FMRADIO_TX)) {
            fmTransmitterSession.isRegistered = true;
        } else {
            LOGE("vendor registration failed\n");
            free(fmTransmitterSession.vendorMethods_p);
        }
    }
    ...
    return jniRegisterNativeMethods(env,
                                    "android/fm/FmTransmitterService",
