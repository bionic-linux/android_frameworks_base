...
void androidFmRadioPauseIfTempResumed(struct FmSession_t *session_p)
{
    if (session_p->state == FMRADIO_STATE_PAUSED) {
        if (session_p->vendorMethods_p->pause(&session_p->vendorData_p)
             ...
             * a force reset to put the device in a known idle state
             */
            FMRADIO_SET_STATE(session_p, FMRADIO_STATE_IDLE);
            if (session_p->vendorMethods_p->
                reset(&session_p->vendorData_p) != FMRADIO_OK) {
                                   ...
                                   enum FmRadioCommand_t event)
{
    ...
    if (!session_p->isRegistered && (event != FMRADIO_EVENT_START)
        && (event != FMRADIO_EVENT_START_ASYNC)) {
        LOGE("ERROR - library not loaded, only start and start async is valid events\n");
        retval = false;
    } else if (!(*session_p->validEventsForStates_p)[event][session_p->state]) {
        LOGE("ERROR - Invalid event %u for state %u\n", event,
             session_p->state);
        retval = false;
    } else {
        retval = true;
...
/*
 *  Functions for loading the vendor dynamic library (libxxxxxx.so)
 */
                            ...
                            enum RadioMode_t mode)
{
    ...
    // load the library
    fmLibrary = dlopen(fmLibName, RTLD_LAZY);
    if (fmLibrary == NULL) {
    ...
    fmRegFunc = (fmradio_reg_func_t) dlsym(fmLibrary, FMRADIO_REGISTER_FUNC);
    ...
    if ((mode == FMRADIO_RX && session_p->vendorMethods_p->rx_start == NULL) ||
        (mode == FMRADIO_TX && session_p->vendorMethods_p->tx_start == NULL) ||
        (session_p->vendorMethods_p->pause == NULL) ||
        (session_p->vendorMethods_p->resume == NULL) ||
        (session_p->vendorMethods_p->reset == NULL)) {
        LOGE("Loaded function '%s' returned successful but failed to implement "
             "mandatory methods.\n", FMRADIO_REGISTER_FUNC);
        goto closelib;
    }
...
{
    int (*startFunc) (void **, struct RuntimeCallbacks_t *, int, int, int,
                      int) = ((int (**)
                               (void **, struct RuntimeCallbacks_t *, int,
                                int, int, int)) args)[0];
    struct FmSession_t *session_p = ((struct FmSession_t **) args)[1];
    struct RuntimeCallbacks_t *callbacks =
        ((struct RuntimeCallbacks_t **) args)[2];
    int lowFreq = ((int *) args)[3];
    int highFreq = ((int *) args)[4];
    int defaultFreq = ((int *) args)[5];
    int grid = ((int *) args)[6];
    int retval = 0;
    ...
    /*
     *if other session is starting and waiting to stop we should wait for them
     * to finish
     */

    if (session_p->partnerSession_p->state == FMRADIO_STATE_STARTING &&
        session_p->partnerSession_p->pendingReset) {
        int maxTime = 10000;    /* in ms */
        do {
        ...
        while (session_p->partnerSession_p->pendingReset && maxTime > 0);
        // now the other session has restarted and is either IDLE
        // or (very unlikely) starting waiting for us. send Forced reset
        LOGI("other session is finished closedown, send force reset\n");
    ...
    retval =
        startFunc(&session_p->vendorData_p, callbacks, lowFreq, highFreq,
                  defaultFreq, grid);
    ...
    if (session_p->pendingReset) {
        /* need to drop our lock while resetting since it might trigger callbacks */
        pthread_mutex_unlock(session_p->dataMutex_p);
        if ((session_p->vendorMethods_p->reset(&session_p->vendorData_p) <
             0)) {
            LOGE("Warning, can't execute pending reset\n");
        }
        pthread_mutex_lock(session_p->dataMutex_p);
        retval = -1; /* triggers state IDLE below */
    }

    session_p->pendingReset = false;
                    ...
                    int grid)
{
    ...
    session_p->state = FMRADIO_STATE_STARTING;

    // if the other session has a state different than idle it should get a
    // forced reset

    if (session_p->partnerSession_p != NULL &&
        session_p->partnerSession_p->state != FMRADIO_STATE_IDLE) {
        // if partner session is in state starting we need to await it being
        // finished, otherwise strange things could happen
        if (session_p->partnerSession_p->state == FMRADIO_STATE_STARTING) {
            LOGI("other session is starting, set pending reset\n");

            session_p->partnerSession_p->pendingReset = true;

            // for a syncronous call we should now await the other session
            // to close down. For asyncronous we do it in the new thread to
            ...
            if (!async) {
                ...
                while (session_p->partnerSession_p->pendingReset
                       && maxTime > 0);
                // now the other session has restarted and is either IDLE
                // or (very unlikely) starting waiting for us. send Forced reset
                FMRADIO_SET_STATE(session_p->partnerSession_p, FMRADIO_STATE_IDLE);
            ...
            // send partner vendor reset command(s)

            /* temporary drop lock since reset might trigger callbacks */
            pthread_mutex_unlock(session_p->dataMutex_p);
            (void) session_p->partnerSession_p->vendorMethods_p->
                reset(&session_p->partnerSession_p->vendorData_p);
            pthread_mutex_lock(session_p->dataMutex_p);
            if (session_p->state != FMRADIO_STATE_STARTING) {
                LOGE("warning, state not starting\n");
            }
            // call the OnForcedReset to send back to end user
            (void) session_p->partnerSession_p->callbacks_p->
                onForcedReset(FMRADIO_RESET_OTHER_IN_USE);
            FMRADIO_SET_STATE(session_p->partnerSession_p, FMRADIO_STATE_IDLE);
        }
    }
    ...
    if (async) {
        ...
        int *args = (int *) malloc(sizeof(int) * 7);    /* freed in created thread */
        ...
        args[0] = (int) startFunc;
        args[1] = (int) session_p;
        args[2] = (int) callbacks;
        args[3] = lowFreq;
        args[4] = highFreq;
        args[5] = defaultFreq;
        args[6] = grid;
        ...
        pthread_mutex_unlock(session_p->dataMutex_p);
        retval =
            startFunc(&session_p->vendorData_p, callbacks, lowFreq,
        ...
        if (session_p->state != FMRADIO_STATE_STARTING) {
            ...
            retval = FMRADIO_IO_ERROR;
            /* request a reset below to shutdown and go to init */
            session_p->pendingReset = true;
        }

        /* if we have a pending reset we should reset now (even if error) */
        if (session_p->pendingReset) {
            /* need to drop our lock while resetting since it might trigger callbacks */
            pthread_mutex_unlock(session_p->dataMutex_p);
            if (session_p->vendorMethods_p->
                 reset(&session_p->vendorData_p) < 0) {
                LOGE("Warning, can't execute pending reset\n");
            }
            pthread_mutex_lock(session_p->dataMutex_p);
            retval = FMRADIO_FORCED_RESET;
        }
    ...
    if (retval == 0) {
        ...
        LOGE("androidFmRadioStart failed (%d)..\n", retval);
        if (!async) {
            FMRADIO_SET_STATE(session_p, FMRADIO_STATE_IDLE);
        }
    }
  ...
  drop_lock:
    ...
    if (retval < 0 && session_p->state == FMRADIO_STATE_STARTING) {
        session_p->state = FMRADIO_STATE_IDLE;
    }
...
void androidFmRadioPause(struct FmSession_t *session_p)
{
    int retval;
...
void androidFmRadioResume(struct FmSession_t *session_p)
{
    int retval = 0;
...
int androidFmRadioReset(struct FmSession_t *session_p)
{
    ...
    if (oldState == FMRADIO_STATE_IDLE) {
        /* idle, we shouldn't even test isValidEvent, just return state idle */
        retval = 0;
        goto drop_lock;
    }
    ...
    /*
     * if we are in state starting we have not yet device to reset. Flag it should
     *  be resetted when finished. Not needed for scanning state, those function will
     *  notice the state has changed.
     */
    ...
    if (oldState == FMRADIO_STATE_STARTING) {
        session_p->pendingReset = true;
        /*
         * skip setting idle state, we do not want new commands before
         * start/scanning isfinished. But do set the retval before returning
         */
        retval = oldState;
        goto drop_lock;
        /* if we are idling we should just return, else reset the chip */
    } else if (oldState != FMRADIO_STATE_IDLE) {
        session_p->state = FMRADIO_STATE_RESETTING;
        pthread_mutex_unlock(session_p->dataMutex_p);
        retval =
            session_p->vendorMethods_p->reset(&session_p->
                                              vendorData_p);
        pthread_mutex_lock(session_p->dataMutex_p);
        /* sanity check, we don't allow state changes during a reset */
        if (oldState != FMRADIO_STATE_RESETTING) {
            LOGE("Warning, state changed during pending reset\n");
        }
    }
    // if successful update state
    if (retval >= 0) {
...
int androidFmRadioGetFrequency(struct FmSession_t *session_p)
{
    ...
    if (!androidFmRadioIsValidEventForState
        (session_p, FMRADIO_EVENT_GET_PARAMETER)) {
        retval = FMRADIO_INVALID_STATE;
        goto drop_lock;
    }
...
int androidFmRadioForcedPause(struct FmSession_t *session_p)
{
    int retval = 0;
    FmRadioState_t oldState;
    ...
    if (session_p->state == FMRADIO_STATE_IDLE) {
        /* idle, we shouldn't even test isRegistered, just return state idle */
        retval = FMRADIO_STATE_IDLE;
        goto drop_lock;
    }

    if (!session_p->isRegistered) {
        LOGE("FMRadio not registered\n");
        retval = FMRADIO_UNSUPPORTED_OPERATION;
        goto drop_lock;
    }
    switch (oldState) {
    case FMRADIO_STATE_STARTED:
        /* pause */
        retval = session_p->vendorMethods_p->pause(&session_p->
                                                  vendorData_p);
            FMRADIO_SET_STATE(session_p, FMRADIO_STATE_PAUSED);

        break;
    case FMRADIO_STATE_SCANNING:
        /* stop scan (ignore if any errors) and set pending pause */
        if (session_p->vendorMethods_p->stop_scan) {
            (void) session_p->vendorMethods_p->stop_scan(&session_p->
                                                        vendorData_p);
        }
        /* no break */
    case FMRADIO_STATE_EXTRA_COMMAND:
        session_p->pendingPause = true;
        break;
    case FMRADIO_STATE_STARTING:
        session_p->pendingReset = true;
        break;
    default:
        /* Nothing to do on IDLE & PAUSED */
        break;
    }
  drop_lock:
    if (retval == FMRADIO_INVALID_STATE) {
    ...
    // if we are successful we should return the state before forced pause
    if (retval >= 0) {
        retval = oldState;
    }
    return retval;
...
static void *execute_androidFmRadioSendExtraCommand(void *args)
{
    struct FmSession_t *session_p = ((struct FmSession_t **) args)[0];
    char *command = ((char **) args)[1];
    char **parameter = ((char ***) args)[2];
    ...
    if (retval >= 0) {
        session_p->callbacks_p->onSendExtraCommand(command, returnParam);
    } else {
        session_p->callbacks_p->onError();
    }
                               ...
                               jobjectArray parameter)
{
    ...
    args = (int *) malloc(sizeof(int) * 3);     /* freed in created thread */
    if (args == NULL) {
        LOGE("malloc failed\n");
        retval = FMRADIO_IO_ERROR;
    }
    ...
    if (retval == FMRADIO_OK) {
        ...
        args[0] = (int) session_p;
        args[1] = (int) strdup(c_command);
        args[2] = (int) cparams;
