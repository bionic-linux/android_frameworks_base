                                   ...
                                   enum FmRadioCommand_t event)
{
    ...
    if (!session_p->isRegistered && (event != FMRADIO_EVENT_START)
        ...
        LOGE("ERROR - library not loaded, only start and start async are valid events\n");
        retval = false;
                    ...
                    int grid)
{
    ...
    if (async) {
        ...
        struct FmRadioStartAsyncParameters *args_p = (struct FmRadioStartAsyncParameters *) malloc(sizeof(struct FmRadioStartAsyncParameters));    /* freed in created thread */
  ...
  early_exit:
    ...
    if (retval < 0 && retval != FMRADIO_INVALID_STATE && session_p->isRegistered) {
        androidFmRadioUnLoadFmLibrary(session_p);
        session_p->isRegistered = false;
    }

    if(retval < 0 && !session_p->pendingReset) {
...
int androidFmRadioReset(struct FmSession_t *session_p)
{
    ...
    // if successful update state and unload vendor driver
    if (retval >= 0) {
        retval = oldState;
        if (oldState != FMRADIO_STATE_IDLE) {
            FMRADIO_SET_STATE(session_p, FMRADIO_STATE_IDLE);
