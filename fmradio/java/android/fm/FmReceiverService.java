...
public class FmReceiverService extends IFmReceiver.Stub {
    ...
    private final class OnScanReceiver implements IBinder.DeathRecipient {
        ...
        public boolean callOnScan(int tunedFrequency, int signalLevel, boolean aborted) {
            try {
                synchronized (this) {
                    mListener.onScan(tunedFrequency, signalLevel, aborted);
                }
            } catch (RemoteException ex) {
    ...
    private PhoneStateListener mPhoneStateListener = new PhoneStateListener() {
        ...
        public void onCallStateChanged(int state, String incomingNumber) {
            if (state == TelephonyManager.CALL_STATE_RINGING) {
                ...
                // force the hardware to pause
                int result = _fm_receiver_forcedPause();
                if (result == FmReceiver.STATE_STARTING) {
                    notifyOnForcedReset(FmReceiver.RESET_NON_CRITICAL);
                } else if (result == FmReceiver.STATE_STARTED
                        || result == FmReceiver.STATE_SCANNING) {
                    notifyOnForcedPause();
                }
    ...
    public boolean isTunerLooping() {
        ...
        return _fm_receiver_isTunerLooping();
    }
    ...
    private void notifyOnScan(int frequency, int signalLevel, boolean aborted) {
        synchronized (mOnScanReceivers) {
            Collection c = mOnScanReceivers.values();
            Iterator iterator = c.iterator();
            while (iterator.hasNext()) {
                OnScanReceiver m = (OnScanReceiver) iterator.next();
                m.callOnScan(frequency, signalLevel, aborted);
            }
        }
    }
    ...
    private native int _fm_receiver_forcedPause();
    ...
    private native boolean _fm_receiver_isTunerLooping();
