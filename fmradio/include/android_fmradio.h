...
struct FmRadioCallbacks_t {
    ...
    void (*onError) (void);
    void (*onStarted) (void);
    void (*onScan) (int, int, bool);    /* RX only */
...
struct FmSession_t {
    ...
    bool pendingReset;               /* used for start + startAsync */
    pthread_mutex_t *dataMutex_p;    /* data access to this struct */
    struct ThreadCtrl_t *signalStrengthThreadCtrl_p;    /* RX Only */
};

#define FMRADIO_SET_STATE(session,newstate) {printf("FMRADIO_SET_STATE called from %s, %d (%s)\n", __FILE__, __LINE__, __FUNCTION__); int oldstate = (session)->state; (session)->state = newstate;(session)->callbacks_p->onStateChanged(newstate, oldstate);}

/* exceptions */

#define THROW_ILLEGAL_ARGUMENT(session_p) \
      androidFmRadioThrowException(session_p,\
				   "java/lang/IllegalArgumentException",\
				   "Illegal argument", __FILE__, __LINE__,\
				   __FUNCTION__)
#define THROW_UNSUPPORTED_OPERATION(session_p) \
      androidFmRadioThrowException(session_p,\
				   "java/lang/UnsupportedOperationException",\
				   "Unsupported operation", __FILE__, __LINE__,\
				   __FUNCTION__)
#define THROW_INVALID_STATE(session_p)	\
      androidFmRadioThrowException(session_p,\
				   "java/lang/IllegalStateException",\
				   "State is invalid", __FILE__, __LINE__,\
				   __FUNCTION__)
#define THROW_IO_ERROR(session_p) \
      androidFmRadioThrowException(session_p,\
				   "java/io/IOException",\
