...
struct FmSession_t {
    ...
    bool ongoingReset;               /* used in reset for temp lock drop */
    pthread_mutex_t *dataMutex_p;    /* data access to this struct */
    pthread_cond_t  sync_cond;
    struct ThreadCtrl_t *signalStrengthThreadCtrl_p;    /* RX Only */
};

#define FMRADIO_SET_STATE(session,newstate) {int oldstate = (session)->state; (session)->state = newstate;(session)->callbacks_p->onStateChanged(newstate, oldstate);}
