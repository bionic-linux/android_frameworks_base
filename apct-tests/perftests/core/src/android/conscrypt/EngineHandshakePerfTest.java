...
public final class EngineHandshakePerfTest {
    ...
    public Collection getParams() {
        ...
        for (EngineFactory engineFactory : EngineFactory.values()) {
            for (BufferType bufferType : BufferType.values()) {
                for (PerfTestProtocol protocol : PerfTestProtocol.values()) {
                    params.add(new Object[] {new Config(bufferType, engineFactory, "TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256", protocol, 100)});
                }
            }
        }
        return params;
