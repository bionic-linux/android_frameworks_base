...
public class StringEqualsPerfTest {
    ...
    private final String mLong1 =
            "Ahead-of-time compilation is possible as the compiler may justconvert an instruction"
                ...
                + " a bool to state whether a Class were good or bad. In ART the failcase becomes"
                + " either a soft or hard failure. Classes have new states to represent that asoft"
                + " failure occurred at compile time and should be re-verified at run-time.";

    private final String mVeryLong =
            "Garbage collection has two phases. The first distinguisheslive objects from garbage"
                + " objects.  The second is reclaiming the rage of garbage objectIn the mark-sweep"
                + " algorithm used by Dalvik, the first phase is achievd by computing theclosure of"
                + " all reachable objects in a process known as tracing from theoots.  After"
