...
def _create_enable_multiple_overlays_task():
    tasks = [
        ...
        PushTask('/data/app/com.android.overlaytest.multipleoverlays.apk', '/vendor/overlay/framework_a.apk'),
        PushTask('/data/app/com.android.overlaytest.first_app_overlay.apk', '/vendor/overlay/app_a.apk'),
...
if __name__ == '__main__':
    ...
    if opts.do_build:
        ...
        tasks.append(CompilationTask('OverlayTestMultipleOverlays/Android.mk'))
    ...
    if opts.test_idmap:
        ...
        tasks.append(GrepIdmapTest(idmaps + '/a.idmap', 'array/rro_test_fibonacci', 1))
        tasks.append(GrepIdmapTest(idmaps + '/a.idmap', 'no_type/no_name', 0))

        # idmap --scan
        idmap_a = idmaps + '/vendor@overlay@framework_a.apk@idmap'
        idmap_b = idmaps + '/vendor@overlay@framework_b.apk@idmap'
        tasks.append(StopTask())
        ...
        tasks.append(FileExistsTest(idmap_a))
        tasks.append(FileExistsTest(idmap_b))
        tasks.append(GrepIdmapTest(idmap_a, 'string/rro_test_fruit', 1))
        tasks.append(GrepIdmapTest(idmap_b, 'bool/rro_test_usually_false', 1))
...
/vendor/overlay/framework_a.apk /data/resource-cache/vendor@overlay@framework_a.apk@idmap
'''
