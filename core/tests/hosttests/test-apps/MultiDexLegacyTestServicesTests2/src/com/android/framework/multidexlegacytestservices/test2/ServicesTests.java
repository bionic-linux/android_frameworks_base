...
public class ServicesTests {
    ...
    @Test
    public void testStressConcurentLaunchWithOdexDeletion() throws Exception {
        Thread startServices =
                new Thread() {
            @Override
            public void run() {
    ...
    private void deleteAllOdex() throws IOException {
        Log.i(TAG, "Deleting odex files.");
        for (File odex : getSecondaryFolder().listFiles(new ExtensionFilter(".dex"))) {
            Assert.assertTrue("Failed to delete '" + odex.getAbsolutePath() + "'", odex.delete());
        }
    }
