...
public class ServicesTests {
    ...
    private final class ExtensionFilter implements FileFilter {
    ...
    private String getServicesCompletionStatus() {
        for (int i = MIN_SERVICE; i <= MAX_SERVICE; i++) {
            ...
            if (!resultFile.isFile()) {
                return "Service" + i + " never completed.";
            }
            if (resultFile.length() != 8) {
                return "Service" + i + " was restarted.";
            }
        }
    ...
    private String getServiceCompletionStatus(int serviceId) {
        ...
        if (!resultFile.isFile()) {
            return "Service" + serviceId + " never completed.";
        }
        if (resultFile.length() != 8) {
            return "Service" + serviceId + " was restarted.";
        }
        return COMPLETION_SUCCESS;
    }
    ...
    private void tamperAllExctractedZips() throws IOException {
        ...
        for (File zip : getSecondaryFolder().listFiles(new ExtensionFilter(".zip"))) {
    ...
    private void tamperAllOdex() throws IOException {
        Log.i(
                TAG,
                "Tamper extracted zip files by overwriting all content but the central directory"
                        + " by '\\0's.");
        byte[] zeros = new byte[4 * 1024];
