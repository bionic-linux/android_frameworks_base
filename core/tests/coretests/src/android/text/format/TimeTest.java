...
public class TimeTest extends TestCase {
    ...
    public void testNormalize0() throws Exception {
        ...
        Fields.set(expected, 2006, 3, 32, 1, 2, 3, -1 /* isDst */, 0, 0, 0);
        Fields.assertTimeEquals(Fields.ALL, expected, t);

        t.normalize(false /* use isDst */);
        Time expected2 = new Time(tz);
        Fields.set(expected2, 2006, 4, 2, 1, 2, 3, 0 /* isDst */, 0, 121, 2);
        Fields.assertTimeEquals(Fields.ALL, expected2, t);
    }
    ...
    public void testCompare_null() throws Exception {
        try {
            Time.compare(null, new Time(Time.TIMEZONE_UTC));
            fail();
        } catch (NullPointerException expected) {
        }
        try {
            Time.compare(new Time(Time.TIMEZONE_UTC), null);
            fail();
        } catch (NullPointerException expected) {
        }
        try {
    ...
    public void testFormat() throws Exception {
        Time t = new Time(Time.TIMEZONE_UTC);
        assertFormatEquals(t, "%Y%m%dT%H%M%S", "19700101T000000");
    }
    ...
    public void testGetJulianDay() throws Exception {
        ...
        for (int monthDay = 1; monthDay <= 366; monthDay++) {
            for (int zoneIndex = 0; zoneIndex < mTimeZones.length; zoneIndex++) {
                ...
                // This is the Julian day for 12pm for this day of the year in the chosen timezone.
    ...
    public void testUnknownTz() {
        // Historically the code used UTC if the timezone was unrecognized.
