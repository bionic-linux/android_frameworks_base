    ...
    switch (event) {
        case AudioRecord::EVENT_MARKER: {
            ...
            if (user != NULL && env != NULL) {
                env->CallStaticVoidMethod(callbackInfo->audioRecord_class,
...
static sp<JNIDeviceCallback> getJniDeviceCallback(JNIEnv* env, jobject thiz) {
    ...
    JNIDeviceCallback* const cb =
            (JNIDeviceCallback*)env->GetLongField(thiz, javaAudioRecordFields.nativeDeviceCallback);
    return sp<JNIDeviceCallback>(cb);
}

static sp<JNIDeviceCallback> setJniDeviceCallback(JNIEnv* env, jobject thiz,
...
static jint android_media_AudioRecord_setup(JNIEnv* env, jobject thiz, jobject weak_this,
                                            ...
                                            jstring opPackageName, jlong nativeRecordInJavaObj) {
    ...
    if (nativeRecordInJavaObj == 0) {
        ...
        if (channelIndexMask) {
            ...
            localChanMask = audio_channel_mask_from_representation_and_bits(
                    AUDIO_CHANNEL_REPRESENTATION_INDEX, channelIndexMask);
        }
        ...
        ALOGV("AudioRecord_setup for source=%d tags=%s flags=%08x", paa->source, paa->tags,
              paa->flags);
        ...
        const status_t status = lpRecorder->set(paa->source, sampleRateInHertz,
        ...
        if (status != NO_ERROR) {
            ALOGE("Error creating AudioRecord instance: initialization check failed with status "
                  "%d.",
                  status);
    ...
    // read the audio session ID back from AudioRecord in case a new session was created during
    // set()
...
// ----------------------------------------------------------------------------
static jint android_media_AudioRecord_start(JNIEnv* env, jobject thiz, jint event,
                                            jint triggerSession) {
...
static inline jint interpretReadSizeError(ssize_t readSize) {
...
// ----------------------------------------------------------------------------
static jint android_media_AudioRecord_readInDirectBuffer(JNIEnv* env, jobject thiz, jobject jBuffer,
                                                         jint sizeInBytes,
                                                         jboolean isReadBlocking) {
    // get the audio recorder from which we'll read new audio samples
    sp<AudioRecord> lpRecorder = getAudioRecord(env, thiz);
    if (lpRecorder == NULL) return (jint)AUDIO_JAVA_INVALID_OPERATION;
    ...
    ssize_t readSize =
            lpRecorder->read(nativeFromJavaBuf, capacity < sizeInBytes ? capacity : sizeInBytes,
                             isReadBlocking == JNI_TRUE /* blocking */);
    if (readSize < 0) {
        return interpretReadSizeError(readSize);
    }
...
// ----------------------------------------------------------------------------
static jint android_media_AudioRecord_set_marker_pos(JNIEnv* env, jobject thiz, jint markerPos) {
    sp<AudioRecord> lpRecorder = getAudioRecord(env, thiz);
    if (lpRecorder == NULL) {
        jniThrowException(env, "java/lang/IllegalStateException",
...
static jint android_media_AudioRecord_get_min_buff_size(JNIEnv* env, jobject thiz,
                                                        ...
                                                        jint audioFormat) {
    ALOGV(">> android_media_AudioRecord_get_min_buff_size(%d, %d, %d)", sampleRateInHertz,
          channelCount, audioFormat);

    size_t frameCount = 0;
    audio_format_t format = audioFormatToNative(audioFormat);
    status_t result = AudioRecord::getMinFrameCount(&frameCount, sampleRateInHertz, format,
...
static jboolean android_media_AudioRecord_setInputDevice(JNIEnv* env, jobject thiz,
                                                         jint device_id) {
    sp<AudioRecord> lpRecorder = getAudioRecord(env, thiz);
    if (lpRecorder == 0) {
        return false;
...
static jint android_media_AudioRecord_getRoutedDeviceId(JNIEnv* env, jobject thiz) {
    sp<AudioRecord> lpRecorder = getAudioRecord(env, thiz);
    if (lpRecorder == 0) {
        return 0;
    }
...
static void android_media_AudioRecord_enableDeviceCallback(JNIEnv* env, jobject thiz) {
    sp<AudioRecord> lpRecorder = getAudioRecord(env, thiz);
    if (lpRecorder == 0) {
        return;
    }
    ...
    audiorecord_callback_cookie* cookie = (audiorecord_callback_cookie*)env->GetLongField(
            thiz, javaAudioRecordFields.nativeCallbackCookie);
    if (cookie == NULL) {
        return;
    }
...
static void android_media_AudioRecord_disableDeviceCallback(JNIEnv* env, jobject thiz) {
    sp<AudioRecord> lpRecorder = getAudioRecord(env, thiz);
    if (lpRecorder == 0) {
        return;
    }
...
// ----------------------------------------------------------------------------
static jint android_media_AudioRecord_get_timestamp(JNIEnv* env, jobject thiz, jobject timestamp,
                                                    jint timebase) {
    sp<AudioRecord> lpRecorder = getAudioRecord(env, thiz);
    ...
    if (status == AUDIO_JAVA_SUCCESS) {
        ...
        if (status == AUDIO_JAVA_SUCCESS) {
            env->SetLongField(timestamp, javaAudioTimestampFields.fieldFramePosition, position);
            env->SetLongField(timestamp, javaAudioTimestampFields.fieldNanoTime, time);
        }
    }
    return status;
}
...
// ----------------------------------------------------------------------------
static jobject android_media_AudioRecord_native_getMetrics(JNIEnv* env, jobject thiz) {
...
// ----------------------------------------------------------------------------
static jint android_media_AudioRecord_get_active_microphones(JNIEnv* env, jobject thiz,
...
int register_android_media_AudioRecord(JNIEnv* env) {
    ...
    javaAudioRecordFields.postNativeEventInJava =
            GetStaticMethodIDOrDie(env, audioRecordClass, JAVA_POSTEVENT_CALLBACK_NAME,
                                   "(Ljava/lang/Object;IIILjava/lang/Object;)V");

    // Get the variables
    //    mNativeRecorderInJavaObj
    javaAudioRecordFields.nativeRecorderInJavaObj =
            GetFieldIDOrDie(env, audioRecordClass, JAVA_NATIVERECORDERINJAVAOBJ_FIELD_NAME, "J");
    //     mNativeCallbackCookie
    javaAudioRecordFields.nativeCallbackCookie =
            GetFieldIDOrDie(env, audioRecordClass, JAVA_NATIVECALLBACKINFO_FIELD_NAME, "J");

    javaAudioRecordFields.nativeDeviceCallback =
            GetFieldIDOrDie(env, audioRecordClass, JAVA_NATIVEDEVICECALLBACK_FIELD_NAME, "J");
