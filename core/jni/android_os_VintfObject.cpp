...
static void tryAddSchema(const std::shared_ptr<const T>& object, const XmlConverter<T>& converter,
                         const std::string& description, std::vector<std::string>* cStrings) {
    if (object == nullptr) {
...
static jobjectArray android_os_VintfObject_report(JNIEnv* env, jclass) {
    ...
    tryAddSchema(VintfObject::GetDeviceHalManifest(), gHalManifestConverter, "device manifest",
                 &cStrings);
...
static jint android_os_VintfObject_verify(JNIEnv* env, jclass, jobjectArray packageInfo) {
    ...
    if (status) LOG(WARNING) << "VintfObject.verify() returns " << status << ": " << error;
    return status;
}
...
static jobjectArray android_os_VintfObject_getHalNamesAndVersions(JNIEnv* env, jclass) {
    ...
    tryAddHalNamesAndVersions(VintfObject::GetDeviceHalManifest(), "device manifest", &halNames);
    tryAddHalNamesAndVersions(VintfObject::GetFrameworkHalManifest(), "framework manifest",
                              &halNames);
...
static jobject android_os_VintfObject_getVndkSnapshots(JNIEnv* env, jclass) {
    ...
    for (const auto& vndk : manifest->vendorNdks()) {
        ...
        env->CallObjectMethod(jMap, gHashMapPut, env->NewStringUTF(key.c_str()),
                              toJavaStringArray(env, vndk.libraries()));
    }
    return jMap;
}

static jobject android_os_VintfObject_getTargetFrameworkCompatibilityMatrixVersion(JNIEnv* env,
                                                                                   jclass) {
    std::shared_ptr<const HalManifest> manifest = VintfObject::GetDeviceHalManifest();
    if (manifest == nullptr || manifest->level() == Level::UNSPECIFIED) {
        return nullptr;
    }
    return env->CallStaticObjectMethod(gLongClazz, gLongValueOf,
