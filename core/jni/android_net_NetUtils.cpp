...
static void throwErrnoException(JNIEnv* env, const char* functionName, int error) {
    ...
    static jmethodID errnoExceptionCtor =
            GetMethodIDOrDie(env, JniConstants::errnoExceptionClass,
            "<init>", "(Ljava/lang/String;I)V");

    jobject exception = env->NewObject(JniConstants::errnoExceptionClass,
                                       errnoExceptionCtor,
                                       detailMessage.get(),
                                       error);
...
static jobject android_net_utils_resNetworkQuery(JNIEnv *env, jobject thiz, jint netId,
    ...
    const jsize dnameLength = env->GetStringUTFLength(dname);
    std::vector<char> queryname(dnameLength + 1, 0);

    env->GetStringUTFRegion(dname, 0, dnameLength, queryname.data());
...
static jbyteArray android_net_utils_resNetworkResult(JNIEnv *env, jobject thiz, jobject javaFd) {
    ...
    int res = resNetworkResult(fd, &rcode, buf.data(), MAXPACKET);
    if (res < 0) {
    ...
    if (answer == nullptr) {
        ALOGE("Couldn't allocate byte array for answer");
        return nullptr;
    } else {
