...
static jobject android_net_utils_getTcpRepairWindow(JNIEnv *env, jobject thiz, jobject javaFd) {
    ...
    if (rc == -1) {
      throwErrnoException(env, "getsockopt : TCP_REPAIR_WINDOW", rc);
      return NULL;
    }

    struct tcp_info tcpinfo;
    socklen_t tcpinfo_size = sizeof(tcp_info);
    memset(&tcpinfo, 0, tcpinfo_size);
    ...
    if (rc == -1) {
      throwErrnoException(env, "getsockopt : TCP_INFO", rc);
      return NULL;
    }

    trw.rcv_wnd <<= tcpinfo.tcpi_rcv_wscale;

    jclass class_TcpRepairWindow = env->FindClass("android/net/TcpRepairWindow");
    jmethodID ctor = env->GetMethodID(class_TcpRepairWindow, "<init>", "(IIIII)V");

    return env->NewObject(class_TcpRepairWindow, ctor, trw.snd_wl1, trw.snd_wnd, trw.max_window, trw.rcv_wnd, trw.rcv_wup);
}
