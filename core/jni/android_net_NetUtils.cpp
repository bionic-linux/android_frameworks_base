...
static void android_net_utils_attachDhcpFilter(JNIEnv* env, jobject clazz, jobject javaFd) {
    struct sock_filter filter_code[] = {
            ...
            // Accept or reject.
            BPF_STMT(BPF_RET | BPF_K, 0xffff), BPF_STMT(BPF_RET | BPF_K, 0)};
    ...
    if (setsockopt(fd, SOL_SOCKET, SO_ATTACH_FILTER, &filter, sizeof(filter)) != 0) {
        jniThrowExceptionFmt(env, "java/net/SocketException", "setsockopt(SO_ATTACH_FILTER): %s",
                             strerror(errno));
    }
}
...
static void android_net_utils_attachRaFilter(JNIEnv* env, jobject clazz, jobject javaFd,
    ...
    struct sock_filter filter_code[] = {// Check IPv6 Next Header is ICMPv6.
...
static void android_net_utils_attachControlPacketFilter(JNIEnv* env, jobject clazz, jobject javaFd,
                                                        jint hardwareAddressType) {
    if (hardwareAddressType != ARPHRD_ETHER) {
        jniThrowExceptionFmt(env, "java/net/SocketException",
...
static void android_net_utils_setupRaSocket(JNIEnv* env, jobject clazz, jobject javaFd,
    ...
    if (setsockopt(fd, IPPROTO_ICMPV6, ICMP6_FILTER, &rs_only, len) != 0) {
        jniThrowExceptionFmt(env, "java/net/SocketException", "setsockopt(ICMP6_FILTER): %s",
                             strerror(errno));
        return;
    }
    ...
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &hops, len) != 0) {
        jniThrowExceptionFmt(env, "java/net/SocketException", "setsockopt(IPV6_MULTICAST_HOPS): %s",
                             strerror(errno));
        return;
    }
    ...
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_UNICAST_HOPS, &hops, len) != 0) {
        jniThrowExceptionFmt(env, "java/net/SocketException", "setsockopt(IPV6_UNICAST_HOPS): %s",
                             strerror(errno));
        return;
    }
    ...
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_MULTICAST_LOOP, &off, len) != 0) {
        jniThrowExceptionFmt(env, "java/net/SocketException", "setsockopt(IPV6_MULTICAST_LOOP): %s",
                             strerror(errno));
        return;
    }
    ...
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_MULTICAST_IF, &ifIndex, len) != 0) {
        jniThrowExceptionFmt(env, "java/net/SocketException", "setsockopt(IPV6_MULTICAST_IF): %s",
                             strerror(errno));
        return;
    }
    ...
    len = sizeof(sin6);
    if (bind(fd, sa, len) != 0) {
        jniThrowExceptionFmt(env, "java/net/SocketException", "bind(IN6ADDR_ANY): %s",
                             strerror(errno));
        return;
    }
    ...
    if (setsockopt(fd, IPPROTO_IPV6, IPV6_JOIN_GROUP, &all_rtrs, len) != 0) {
        jniThrowExceptionFmt(env, "java/net/SocketException", "setsockopt(IPV6_JOIN_GROUP): %s",
                             strerror(errno));
        return;
    }
}
...
static jobject android_net_utils_resNetworkQuery(JNIEnv* env, jobject thiz, jint netId,
                                                 jstring dname, jint ns_class, jint ns_type,
                                                 jint flags) {
...
static jbyteArray android_net_utils_resNetworkResult(JNIEnv* env, jobject thiz, jobject javaFd) {
    ...
    if (answer == nullptr) {
        ...
        env->SetByteArrayRegion(answer, 0, res, reinterpret_cast<jbyte*>(buf.data()));
    }
...
static const JNINativeMethod gNetworkUtilMethods[] = {
        ...
        {"addArpEntry", "([B[BLjava/lang/String;Ljava/io/FileDescriptor;)V",
         (void*)android_net_utils_addArpEntry},
        {"attachDhcpFilter", "(Ljava/io/FileDescriptor;)V",
         (void*)android_net_utils_attachDhcpFilter},
        {"attachRaFilter", "(Ljava/io/FileDescriptor;I)V", (void*)android_net_utils_attachRaFilter},
        {"attachControlPacketFilter", "(Ljava/io/FileDescriptor;I)V",
         (void*)android_net_utils_attachControlPacketFilter},
        ...
        {"resNetworkResult", "(Ljava/io/FileDescriptor;)[B",
