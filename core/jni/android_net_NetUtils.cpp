...
static jobject android_net_utils_resNetworkQuery(JNIEnv *env, jobject thiz, jint netId,
    ...
    const jsize lengthUTF16 = env->GetStringLength(dname);
    const jsize lengthUTF8 = env->GetStringUTFLength(dname);

    if (lengthUTF8 != lengthUTF16) {
        throwErrnoException(env, "resNetworkQuery", EINVAL);
        return nullptr;
    }
    std::vector<char> queryname(lengthUTF8 + 1, 0);

    env->GetStringUTFRegion(dname, 0, lengthUTF8, queryname.data());
