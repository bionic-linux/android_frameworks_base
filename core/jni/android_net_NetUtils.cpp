...
static jobject android_net_utils_resNetworkResult(JNIEnv *env, jobject thiz, jobject javaFd) {
    ...
    jclass class_TcpRepairWindow = env->FindClass("android/net/DnsResolver$DnsResponse");
    jmethodID ctor = env->GetMethodID(class_TcpRepairWindow, "<init>", "([BI)V");

    return env->NewObject(class_TcpRepairWindow, ctor, answer, rcode);
}
