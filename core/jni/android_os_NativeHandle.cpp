...
static jint JNativeHandle_dup(JNIEnv* /*env*/, jclass /*clazz*/, jint fd) {
    int result = dup(fd);
    ALOGE("asdfasdf duping: %d %d", fd, result);
    return result;
}

static void JNativeHandle_close(JNIEnv* env, jclass /*clazz*/, jint fd) {
    if (close(fd) < 0) {
...
static JNINativeMethod gMethods[] = {
    { "dup", "(I)I", (void *)JNativeHandle_dup },
    { "close", "(I)V", (void *)JNativeHandle_close },
};
