...
jobject JNativeHandle::MakeJavaNativeHandleObj(
    ...
    return env->NewObject(gNativeHandleFields.clazz,
                          gNativeHandleFields.constructID, fds, ints);
}
...
jobjectArray JNativeHandle::AllocJavaNativeHandleObjArray(
    ...
    return env->NewObjectArray(length, gNativeHandleFields.clazz, NULL);
}
...
int register_android_os_NativeHandle(JNIEnv *env) {
    ...
    gNativeHandleFields.constructID = GetMethodIDOrDie(
            env, clazz, "<init>", "([I[I)V");

    gNativeHandleFields.getFdsID = GetMethodIDOrDie(
            env, clazz, "getFdsAsIntArray", "()[I");

    gNativeHandleFields.getIntsID = GetMethodIDOrDie(
            env, clazz, "getInts", "()[I");
