...
static struct { jfieldID native_ptr; } gApkAssetsFields;
    ...
    switch (pid_t pid = fork()) {
        ...
        case 0: {
            ...
            if (!overlay_theme_path.empty()) {
                overlay_theme_path =
                        std::string(AssetManager::OVERLAY_DIR) + "/" + overlay_theme_path;
...
static jlong NativeCreate(JNIEnv* /*env*/, jclass /*clazz*/) {
    ...
    // and AssetManager2 in a contiguous block (GuardedAssetManager).
...
static jobjectArray NativeList(JNIEnv* env, jclass /*clazz*/, jlong ptr, jstring path) {
    ...
    ScopedLock<AssetManager2> assetmanager(AssetManagerFromLong(ptr));
    std::unique_ptr<AssetDir> asset_dir = assetmanager->OpenDir(path_utf8.c_str());
...
static jobject NativeOpenAssetFd(JNIEnv* env, jclass /*clazz*/, jlong ptr, jstring asset_path,
    ...
    ATRACE_NAME(
            base::StringPrintf("AssetManager::OpenAssetFd(%s)", asset_path_utf8.c_str()).c_str());

    ScopedLock<AssetManager2> assetmanager(AssetManagerFromLong(ptr));
    std::unique_ptr<Asset> asset =
            assetmanager->Open(asset_path_utf8.c_str(), Asset::ACCESS_RANDOM);
    if (!asset) {
        jniThrowException(env, "java/io/FileNotFoundException", asset_path_utf8.c_str());
        return nullptr;
    }
...
static jlong NativeOpenNonAsset(JNIEnv* env, jclass /*clazz*/, jlong ptr, jint jcookie,
    ...
    ATRACE_NAME(
            base::StringPrintf("AssetManager::OpenNonAsset(%s)", asset_path_utf8.c_str()).c_str());
...
static jobject NativeOpenNonAssetFd(JNIEnv* env, jclass /*clazz*/, jlong ptr, jint jcookie,
    ...
    ATRACE_NAME(base::StringPrintf("AssetManager::OpenNonAssetFd(%s)", asset_path_utf8.c_str())
                        .c_str());
...
static jlong NativeOpenXmlAsset(JNIEnv* env, jobject /*clazz*/, jlong ptr, jint jcookie,
    ...
    ATRACE_NAME(
            base::StringPrintf("AssetManager::OpenXmlAsset(%s)", asset_path_utf8.c_str()).c_str());
...
static jint NativeGetResourceValue(JNIEnv* env, jclass /*clazz*/, jlong ptr, jint resid,
    ...
    ApkAssetsCookie cookie = assetmanager->GetResource(
            static_cast<uint32_t>(resid), false /*may_be_bag*/, static_cast<uint16_t>(density),
...
static jint NativeGetResourceBagValue(JNIEnv* env, jclass /*clazz*/, jlong ptr, jint resid,
    ...
    uint32_t ref = static_cast<uint32_t>(resid);
    ResTable_config selected_config;
    cookie = assetmanager->ResolveReference(cookie, &value, &selected_config, &type_spec_flags,
                                            &ref);
    if (cookie == kInvalidCookie) {
        return ApkAssetsCookieToJavaCookie(kInvalidCookie);
    }
...
static jobjectArray NativeGetResourceStringArray(JNIEnv* env, jclass /*clazz*/, jlong ptr,
    ...
    for (uint32_t i = 0; i < bag->entry_count; i++) {
        ...
        uint32_t flags;
        uint32_t ref;
        ApkAssetsCookie cookie = assetmanager->ResolveReference(entry.cookie, &value,
                                                                &selected_config, &flags, &ref);
        if (cookie == kInvalidCookie) {
            return nullptr;
        }
...
static jobjectArray NativeGetSizeConfigurations(JNIEnv* env, jclass /*clazz*/, jlong ptr) {
    ...
    std::set<ResTable_config> configurations = assetmanager->GetResourceConfigurations(
            true /*exclude_system*/, false /*exclude_mipmap*/);
...
static void NativeApplyStyle(JNIEnv* env, jclass /*clazz*/, jlong ptr, jlong theme_ptr,
    ...
    ApplyStyle(theme, xml_parser, static_cast<uint32_t>(def_style_attr),
               static_cast<uint32_t>(def_style_resid), reinterpret_cast<uint32_t*>(attrs),
...
static jboolean NativeResolveAttrs(JNIEnv* env, jclass /*clazz*/, jlong ptr, jlong theme_ptr,
    ...
    if (out_java_indices != nullptr) {
        ...
        if (out_indices_len > attrs_len) {
            out_indices = reinterpret_cast<jint*>(
                    env->GetPrimitiveArrayCritical(out_java_indices, nullptr));
...
static jboolean NativeRetrieveAttributes(JNIEnv* env, jclass /*clazz*/, jlong ptr,
    ...
    bool result = RetrieveAttributes(
            assetmanager.get(), xml_parser, reinterpret_cast<uint32_t*>(attrs), attrs_len,
            reinterpret_cast<uint32_t*>(out_values), reinterpret_cast<uint32_t*>(out_indices));
...
static jint NativeThemeGetAttributeValue(JNIEnv* env, jclass /*clazz*/, jlong ptr, jlong theme_ptr,
    ...
    if (resolve_references) {
        ResTable_config selected_config;
        cookie = theme->GetAssetManager()->ResolveReference(cookie, &value, &selected_config,
                                                            &flags, &ref);
        if (cookie == kInvalidCookie) {
            return ApkAssetsCookieToJavaCookie(kInvalidCookie);
        }
...
static jlong NativeAssetSeek(JNIEnv* env, jclass /*clazz*/, jlong asset_ptr, jlong offset,
    ...
    return static_cast<jlong>(
            asset->seek(static_cast<off64_t>(offset),
                        (whence > 0 ? SEEK_END : (whence < 0 ? SEEK_SET : SEEK_CUR))));
}
