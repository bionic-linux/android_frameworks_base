...
class NativeInputEventSender : public LooperCallback {
  public:
    NativeInputEventSender(JNIEnv* env, jobject senderWeak, const sp<InputChannel>& inputChannel,
  ...
  private:
    ...
    const std::string getInputChannelName() { return mInputPublisher.getChannel()->getName(); }
...
status_t NativeInputEventSender::sendKeyEvent(uint32_t seq, const KeyEvent* event) {
    ...
    status_t status = mInputPublisher.publishKeyEvent(
            publishedSeq, event->getDeviceId(), event->getSource(), event->getAction(),
    ...
    if (status) {
        ALOGW("Failed to send key event on channel '%s'.  status=%d", getInputChannelName().c_str(),
              status);
        return status;
    }
...
status_t NativeInputEventSender::sendMotionEvent(uint32_t seq, const MotionEvent* event) {
    ...
    for (size_t i = 0; i <= event->getHistorySize(); i++) {
        ...
        status_t status = mInputPublisher.publishMotionEvent(
                ...
                event->getXOffset(), event->getYOffset(), event->getXPrecision(),
                event->getYPrecision(), event->getDownTime(), event->getHistoricalEventTime(i),
...
static jlong nativeInit(JNIEnv* env, jclass clazz, jobject senderWeak, jobject inputChannelObj,
    ...
    sp<NativeInputEventSender> sender =
            new NativeInputEventSender(env, senderWeak, inputChannel, messageQueue);
...
static jboolean nativeSendKeyEvent(JNIEnv* env, jclass clazz, jlong senderPtr, jint seq,
                                   jobject eventObj) {
    sp<NativeInputEventSender> sender = reinterpret_cast<NativeInputEventSender*>(senderPtr);
    KeyEvent event;
...
static jboolean nativeSendMotionEvent(JNIEnv* env, jclass clazz, jlong senderPtr, jint seq,
                                      jobject eventObj) {
    sp<NativeInputEventSender> sender = reinterpret_cast<NativeInputEventSender*>(senderPtr);
