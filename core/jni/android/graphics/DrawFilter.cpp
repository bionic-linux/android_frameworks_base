...
class CompatFlagsDrawFilter : public SkPaintFlagsDrawFilter {
  public:
    CompatFlagsDrawFilter(uint32_t clearFlags, uint32_t setFlags, SkFilterQuality desiredQuality)
        : SkPaintFlagsDrawFilter(clearFlags, setFlags), fDesiredQuality(desiredQuality) {}
...
class SkDrawFilterGlue {
  public:
    ...
    static jlong CreatePaintFlagsDF(JNIEnv* env, jobject clazz, jint clearFlags, jint setFlags) {
        if (clearFlags | setFlags) {
            ...
            if (turnFilteringOn) {
                ...
                filter = new CompatFlagsDrawFilter(clearFlags, setFlags, kLow_SkFilterQuality);
            } else if (turnFilteringOff) {
                filter = new CompatFlagsDrawFilter(clearFlags, setFlags, kNone_SkFilterQuality);
            } else {
