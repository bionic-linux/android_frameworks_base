...
namespace PaintGlue {
enum MoveOpt { AFTER, AT_OR_AFTER, BEFORE, AT_OR_BEFORE, AT };
...
static int breakText(JNIEnv* env, const Paint& paint, const Typeface* typeface, const jchar text[],
...
static jint breakTextC(JNIEnv* env, jobject clazz, jlong paintHandle, jcharArray jtext, jint index,
                       jint count, jfloat maxWidth, jint bidiFlags, jfloatArray jmeasuredWidth) {
...
static jint breakTextS(JNIEnv* env, jobject clazz, jlong paintHandle, jstring jtext,
                       jboolean forwards, jfloat maxWidth, jint bidiFlags,
...
static jfloat doTextAdvances(JNIEnv* env, Paint* paint, const Typeface* typeface, const jchar* text,
                             jint start, jint count, jint contextCount, jint bidiFlags,
...
static jfloat getTextAdvances___CIIIII_FI(JNIEnv* env, jobject clazz, jlong paintHandle,
                                          ...
                                          jint contextIndex, jint contextCount, jint bidiFlags,
                                          jfloatArray advances, jint advancesIndex) {
    Paint* paint = reinterpret_cast<Paint*>(paintHandle);
    const Typeface* typeface = paint->getAndroidTypeface();
...
static jfloat getTextAdvances__StringIIIII_FI(JNIEnv* env, jobject clazz, jlong paintHandle,
                                              jstring text, jint start, jint end, jint contextStart,
                                              jint contextEnd, jint bidiFlags, jfloatArray advances,
                                              jint advancesIndex) {
    Paint* paint = reinterpret_cast<Paint*>(paintHandle);
    const Typeface* typeface = paint->getAndroidTypeface();
...
static jint doTextRunCursor(JNIEnv* env, Paint* paint, const Typeface* typeface, const jchar* text,
    ...
    MinikinUtils::measureText(paint, bidiFlags, typeface, text, start, count, start + count,
                              advancesArray.get());
    size_t result = minikin::GraphemeBreak::getTextRunCursor(advancesArray.get(), text, start,
                                                             count, offset, moveOpt);
    return static_cast<jint>(result);
}

static jint getTextRunCursor___C(JNIEnv* env, jobject clazz, jlong paintHandle, jcharArray text,
                                 jint contextStart, jint contextCount, jint dir, jint offset,
                                 jint cursorOpt) {
    Paint* paint = reinterpret_cast<Paint*>(paintHandle);
    const Typeface* typeface = paint->getAndroidTypeface();
    jchar* textArray = env->GetCharArrayElements(text, nullptr);
    jint result = doTextRunCursor(env, paint, typeface, textArray, contextStart, contextCount, dir,
                                  offset, cursorOpt);
    env->ReleaseCharArrayElements(text, textArray, JNI_ABORT);
    return result;
}
...
static jint getTextRunCursor__String(JNIEnv* env, jobject clazz, jlong paintHandle, jstring text,
                                     jint contextStart, jint contextEnd, jint dir, jint offset,
                                     jint cursorOpt) {
    Paint* paint = reinterpret_cast<Paint*>(paintHandle);
    const Typeface* typeface = paint->getAndroidTypeface();
    const jchar* textArray = env->GetStringChars(text, nullptr);
    jint result = doTextRunCursor(env, paint, typeface, textArray, contextStart,
...
class GetTextFunctor {
  public:
    GetTextFunctor(const minikin::Layout& layout, SkPath* path, jfloat x, jfloat y, Paint* paint,
...
static void getTextPath(JNIEnv* env, Paint* paint, const Typeface* typeface, const jchar* text,
    ...
    minikin::Layout layout = MinikinUtils::doLayout(paint, static_cast<minikin::Bidi>(bidiFlags),
                                                    typeface, text, 0, count, count, nullptr);
...
static void getTextPath___C(JNIEnv* env, jobject clazz, jlong paintHandle, jint bidiFlags,
                            jcharArray text, jint index, jint count, jfloat x, jfloat y,
                            jlong pathHandle) {
    Paint* paint = reinterpret_cast<Paint*>(paintHandle);
    const Typeface* typeface = paint->getAndroidTypeface();
...
static void getTextPath__String(JNIEnv* env, jobject clazz, jlong paintHandle, jint bidiFlags,
                                jstring text, jint start, jint end, jfloat x, jfloat y,
                                jlong pathHandle) {
    Paint* paint = reinterpret_cast<Paint*>(paintHandle);
    const Typeface* typeface = paint->getAndroidTypeface();
    SkPath* path = reinterpret_cast<SkPath*>(pathHandle);
...
static void getCharArrayBounds(JNIEnv* env, jobject, jlong paintHandle, jcharArray text, jint index,
...
static jboolean hasGlyph(JNIEnv* env, jclass, jlong paintHandle, jint bidiFlags, jstring string) {
    const Paint* paint = reinterpret_cast<Paint*>(paintHandle);
    const Typeface* typeface = paint->getAndroidTypeface();
    ...
    minikin::Layout layout =
            MinikinUtils::doLayout(paint, static_cast<minikin::Bidi>(bidiFlags), typeface,
                                   str.get(), 0, str.size(), str.size(), nullptr);
...
static jfloat doRunAdvance(const Paint* paint, const Typeface* typeface, const jchar buf[],
    ...
    if (offset == start + count) {
        return MinikinUtils::measureText(paint, bidiFlags, typeface, buf, start, count, bufSize,
                                         nullptr);
    }
    std::unique_ptr<float[]> advancesArray(new float[count]);
...
static jint doOffsetForAdvance(const Paint* paint, const Typeface* typeface, const jchar buf[],
                               jint start, jint count, jint bufSize, jboolean isRtl,
                               jfloat advance) {
    minikin::Bidi bidiFlags = isRtl ? minikin::Bidi::FORCE_RTL : minikin::Bidi::FORCE_LTR;
    std::unique_ptr<float[]> advancesArray(new float[count]);
    MinikinUtils::measureText(paint, bidiFlags, typeface, buf, start, count, bufSize,
...
static jint getOffsetForAdvance___CIIIIZF_I(JNIEnv* env, jclass, jlong paintHandle, jcharArray text,
...
static jfloat getFontMetrics(JNIEnv* env, jobject, jlong paintHandle, jobject metricsObj) {
    ...
    if (metricsObj) {
        ...
        env->SetFloatField(metricsObj, gFontMetrics_fieldID.ascent,
                           SkScalarToFloat(metrics.fAscent));
        env->SetFloatField(metricsObj, gFontMetrics_fieldID.descent,
                           SkScalarToFloat(metrics.fDescent));
        env->SetFloatField(metricsObj, gFontMetrics_fieldID.bottom,
                           SkScalarToFloat(metrics.fBottom));
        env->SetFloatField(metricsObj, gFontMetrics_fieldID.leading,
                           SkScalarToFloat(metrics.fLeading));
    }
...
static jint getFontMetricsInt(JNIEnv* env, jobject, jlong paintHandle, jobject metricsObj) {
    ...
    if (metricsObj) {
        ...
        env->SetIntField(metricsObj, gFontMetricsInt_fieldID.bottom,
                         SkScalarCeilToInt(metrics.fBottom));
...
static void setFlags(jlong paintHandle, jint flags) {
    ...
    nativePaint->setFilterQuality(flags & sFilterBitmapFlag ? kLow_SkFilterQuality
...
static jint getHinting(jlong paintHandle) {
    return reinterpret_cast<Paint*>(paintHandle)->getHinting() == Paint::kNo_Hinting ? 0 : 1;
}

static void setHinting(jlong paintHandle, jint mode) {
    reinterpret_cast<Paint*>(paintHandle)
            ->setHinting(mode == 0 ? Paint::kNo_Hinting : Paint::kNormal_Hinting);
}
...
static void setFilterBitmap(jlong paintHandle, jboolean filterBitmap) {
    reinterpret_cast<Paint*>(paintHandle)
            ->setFilterQuality(filterBitmap ? kLow_SkFilterQuality : kNone_SkFilterQuality);
}
...
static void setTextLocalesByMinikinLocaleListId(jlong objHandle, jint minikinLocaleListId) {
    Paint* obj = reinterpret_cast<Paint*>(objHandle);
    obj->setMinikinLocaleListId(minikinLocaleListId);
}
...
static void setElegantTextHeight(jlong paintHandle, jboolean aa) {
    Paint* obj = reinterpret_cast<Paint*>(paintHandle);
    obj->setFamilyVariant(aa ? minikin::FontFamily::Variant::ELEGANT
                             : minikin::FontFamily::Variant::DEFAULT);
}
...
static void setShadowLayer(jlong paintHandle, jfloat radius, jfloat dx, jfloat dy, jint color) {
    Paint* paint = reinterpret_cast<Paint*>(paintHandle);
    if (radius <= 0) {
