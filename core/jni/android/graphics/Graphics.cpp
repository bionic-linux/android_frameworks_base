...
AutoJavaFloatArray::AutoJavaFloatArray(JNIEnv* env, jfloatArray array, int minLength,
                                       JNIAccess access)
...
AutoJavaIntArray::AutoJavaIntArray(JNIEnv* env, jintArray array, int minLength)
    : fEnv(env), fArray(array), fPtr(NULL), fLen(0) {
    ALOG_ASSERT(env);
    if (array) {
        fLen = env->GetArrayLength(array);
...
AutoJavaShortArray::AutoJavaShortArray(JNIEnv* env, jshortArray array, int minLength,
                                       JNIAccess access)
    : fEnv(env), fArray(array), fPtr(NULL), fLen(0) {
    ALOG_ASSERT(env);
    if (array) {
...
AutoJavaByteArray::AutoJavaByteArray(JNIEnv* env, jbyteArray array, int minLength)
    : fEnv(env), fArray(array), fPtr(NULL), fLen(0) {
    ALOG_ASSERT(env);
    if (array) {
        fLen = env->GetArrayLength(array);
...
SkIRect* GraphicsJNI::jrect_to_irect(JNIEnv* env, jobject obj, SkIRect* ir) {
    ...
    ir->set(env->GetIntField(obj, gRect_leftFieldID), env->GetIntField(obj, gRect_topFieldID),
            env->GetIntField(obj, gRect_rightFieldID), env->GetIntField(obj, gRect_bottomFieldID));
    return ir;
}
...
SkRect* GraphicsJNI::jrectf_to_rect(JNIEnv* env, jobject obj, SkRect* r) {
    ...
    r->set(env->GetFloatField(obj, gRectF_leftFieldID), env->GetFloatField(obj, gRectF_topFieldID),
...
SkIPoint* GraphicsJNI::jpoint_to_ipoint(JNIEnv* env, jobject obj, SkIPoint* point) {
    ...
    point->set(env->GetIntField(obj, gPoint_xFieldID), env->GetIntField(obj, gPoint_yFieldID));
    return point;
}
...
SkPoint* GraphicsJNI::jpointf_to_point(JNIEnv* env, jobject obj, SkPoint* point) {
    ...
    point->set(env->GetIntField(obj, gPointF_xFieldID), env->GetIntField(obj, gPointF_yFieldID));
    return point;
}
...
SkColorType GraphicsJNI::legacyBitmapConfigToColorType(jint legacyConfig) {
    const uint8_t gConfig2ColorType[] = {kUnknown_SkColorType, kAlpha_8_SkColorType,
...
jobject GraphicsJNI::createBitmapRegionDecoder(JNIEnv* env, SkBitmapRegionDecoder* bitmap) {
    ...
    jobject obj =
            env->NewObject(gBitmapRegionDecoder_class, gBitmapRegionDecoder_constructorMethodID,
...
android::Bitmap* GraphicsJNI::mapAshmemBitmap(JNIEnv* env, SkBitmap* bitmap, int fd, void* addr,
...
SkColorSpaceTransferFn GraphicsJNI::getNativeTransferParameters(JNIEnv* env,
...
sk_sp<SkColorSpace> GraphicsJNI::getNativeColorSpace(JNIEnv* env, jobject colorSpace) {
    ...
    jobject transferParams =
            env->CallObjectMethod(colorSpace, gColorSpaceRGB_getTransferParametersMethodID);
    if (transferParams == nullptr) {
        doThrowIAE(env, "The color space must use an ICC parametric transfer function");
    }

    jfloatArray illuminantD50 = (jfloatArray)env->GetStaticObjectField(
            gColorSpace_class, gColorSpace_IlluminantD50FieldID);
    jobject colorSpaceD50 = env->CallStaticObjectMethod(
            gColorSpace_class, gColorSpace_adaptMethodID, colorSpace, illuminantD50);

    jfloatArray xyzD50 =
            (jfloatArray)env->CallObjectMethod(colorSpaceD50, gColorSpaceRGB_getTransformMethodID);
...
jobject GraphicsJNI::getColorSpace(JNIEnv* env, sk_sp<SkColorSpace>& decodeColorSpace,
    ...
    if (decodeColorType == kRGBA_F16_SkColorType) {
        ...
        if (decodeColorSpace->isSRGB()) {
            jobject sRGB = env->GetStaticObjectField(gColorSpace_Named_class,
                                                     gColorSpace_Named_sRGBFieldID);
            colorSpace =
                    env->CallStaticObjectMethod(gColorSpace_class, gColorSpace_getMethodID, sRGB);
            ...
            jobject params = env->NewObject(
                    gTransferParameters_class, gTransferParameters_constructorMethodID,
                    transferParams.fA, transferParams.fB, transferParams.fC, transferParams.fD,
                    transferParams.fE, transferParams.fF, transferParams.fG);

            jfloatArray xyzArray = env->NewFloatArray(9);
            jfloat xyz[9] = {
                    xyzMatrix.getFloat(0, 0), xyzMatrix.getFloat(1, 0), xyzMatrix.getFloat(2, 0),
                    xyzMatrix.getFloat(0, 1), xyzMatrix.getFloat(1, 1), xyzMatrix.getFloat(2, 1),
                    xyzMatrix.getFloat(0, 2), xyzMatrix.getFloat(1, 2), xyzMatrix.getFloat(2, 2)};
            env->SetFloatArrayRegion(xyzArray, 0, 9, xyz);

            colorSpace = env->CallStaticObjectMethod(gColorSpace_class, gColorSpace_matchMethodID,
                                                     xyzArray, params);
            ...
            if (colorSpace == nullptr) {
                ...
                colorSpace =
                        env->NewObject(gColorSpaceRGB_class, gColorSpaceRGB_constructorMethodID,
...
bool RecyclingClippingPixelAllocator::allocPixelRef(SkBitmap* bitmap) {
    ...
    if (bytesNeeded <= mRecycledBytes) {
        ...
        mRecycledBitmap->reconfigure(
                mRecycledBitmap->info().makeColorSpace(bitmap->refColorSpace()), rowBytes);
...
void RecyclingClippingPixelAllocator::copyIfNecessary() {
    if (mNeedsCopy) {
        ...
        const size_t bytesToCopy =
                std::min(mRecycledBitmap->info().minRowBytes(), mSkiaBitmap->info().minRowBytes());
...
AshmemPixelAllocator::AshmemPixelAllocator(JNIEnv* env) {
    LOG_ALWAYS_FATAL_IF(env->GetJavaVM(&mJavaVM) != JNI_OK, "env->GetJavaVM failed");
}
...
int register_android_graphics_Graphics(JNIEnv* env) {
    ...
    gBitmapRegionDecoder_class =
            MakeGlobalRefOrDie(env, FindClassOrDie(env, "android/graphics/BitmapRegionDecoder"));
    gBitmapRegionDecoder_constructorMethodID =
            GetMethodIDOrDie(env, gBitmapRegionDecoder_class, "<init>", "(J)V");

    gBitmapConfig_class =
            MakeGlobalRefOrDie(env, FindClassOrDie(env, "android/graphics/Bitmap$Config"));
    ...
    gVMRuntime_addressOf =
            GetMethodIDOrDie(env, gVMRuntime_class, "addressOf", "(Ljava/lang/Object;)J");

    jclass transfer_params_class =
            FindClassOrDie(env, "android/graphics/ColorSpace$Rgb$TransferParameters");
    ...
    gColorSpace_IlluminantD50FieldID =
            GetStaticFieldIDOrDie(env, gColorSpace_class, "ILLUMINANT_D50", "[F");
    gColorSpace_adaptMethodID = GetStaticMethodIDOrDie(
            env, gColorSpace_class, "adapt",
    ...
    gColorSpaceRGB_getTransferParametersMethodID =
            GetMethodIDOrDie(env, gColorSpaceRGB_class, "getTransferParameters",
                             "()Landroid/graphics/ColorSpace$Rgb$TransferParameters;");
    gColorSpaceRGB_getTransformMethodID =
            GetMethodIDOrDie(env, gColorSpaceRGB_class, "getTransform", "()[F");
