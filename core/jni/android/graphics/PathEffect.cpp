...
class SkPathEffectGlue {
  public:
    ...
    static jlong Compose_constructor(JNIEnv* env, jobject, jlong outerHandle, jlong innerHandle) {
        SkPathEffect* outer = reinterpret_cast<SkPathEffect*>(outerHandle);
        SkPathEffect* inner = reinterpret_cast<SkPathEffect*>(innerHandle);
        SkPathEffect* effect =
                SkPathEffect::MakeCompose(sk_ref_sp(outer), sk_ref_sp(inner)).release();
        return reinterpret_cast<jlong>(effect);
    }

    static jlong Sum_constructor(JNIEnv* env, jobject, jlong firstHandle, jlong secondHandle) {
        ...
        SkPathEffect* effect = SkPathEffect::MakeSum(sk_ref_sp(first), sk_ref_sp(second)).release();
        return reinterpret_cast<jlong>(effect);
    }

    static jlong Dash_constructor(JNIEnv* env, jobject, jfloatArray intervalArray, jfloat phase) {
...
#error Need to convert float array to SkScalar array before calling the following function.
    ...
    static jlong OneD_constructor(JNIEnv* env, jobject, jlong shapeHandle, jfloat advance,
                                  jfloat phase, jint style) {
        const SkPath* shape = reinterpret_cast<SkPath*>(shapeHandle);
        SkASSERT(shape != NULL);
        SkPathEffect* effect =
                SkPath1DPathEffect::Make(*shape, advance, phase, (SkPath1DPathEffect::Style)style)
                        .release();
        return reinterpret_cast<jlong>(effect);
    }
    ...
    static jlong Discrete_constructor(JNIEnv* env, jobject, jfloat length, jfloat deviation) {
