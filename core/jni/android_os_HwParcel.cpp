...
template<typename T>
static void WriteHidlVector(JNIEnv *env, jobject thiz,
    ...
    size_t parentHandle;
    status_t err = parcel->writeBuffer(vec, sizeof(hidl_vec<T>),
                                       &parentHandle);
    if (err == OK) {
        size_t childHandle;
        err = ::android::hardware::writeEmbeddedToParcel(
...
static void JHwParcel_native_writeStringVector(
    ...
    void *vecPtr = storage->allocTemporaryStorage(
            sizeof(hidl_vec<hidl_string>));
    ...
    for (jsize i = 0; i < len; ++i) {
        ScopedLocalRef<jstring> stringObj(
                env, (jstring) env->GetObjectArrayElement(arrayObj, i));

        const hidl_string *s = storage->allocTemporaryString(
                env, stringObj.get());
...
static void JHwParcel_native_writeNativeHandleVector(
    ...
    for (jsize i = 0; i < len; i++) {
        ScopedLocalRef<jobject> jHandle(
                env, env->GetObjectArrayElement(jHandleArray, i));

        native_handle_t* handle = JNativeHandle::MakeCppNativeHandle(
                env, jHandle.get(), storage);
...
static jobject ReadNativeHandle(JNIEnv *env, jobject thiz, jboolean embedded,
    ...
    if (embedded) {
        err = parcel->readNullableEmbeddedNativeHandle(parentHandle, offset,
                                                       &handle);
    } else {
...
static jobjectArray JHwParcel_native_readNativeHandleVector(
    ...
    for (jsize i = 0; i < length; i++) {
        jobject jHandle = JNativeHandle::MakeJavaNativeHandleObj(
                env, (*vec)[i].getNativeHandle());
