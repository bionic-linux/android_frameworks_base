...
// static
sp<JHwParcel> JHwParcel::SetNativeContext(JNIEnv* env, jobject thiz, const sp<JHwParcel>& context) {
...
JHwParcel::JHwParcel(JNIEnv* env, jobject thiz)
    : mParcel(NULL), mOwnsParcel(false), mTransactCallback(nullptr), mWasSent(false) {}
...
jobject JHwParcel::NewObject(JNIEnv* env) {
    ...
    jmethodID constructID = GetMethodIDOrDie(env, clazz.get(), "<init>", "(Z)V");

    return env->NewObject(clazz.get(), constructID, false /* allocate */);
}

void JHwParcel::setTransactCallback(::android::hardware::IBinder::TransactCallback cb) {
...
static void JHwParcel_native_setup(JNIEnv* env, jobject thiz, jboolean allocate) {
...
static void JHwParcel_native_writeInterfaceToken(JNIEnv* env, jobject thiz,
                                                 jstring interfaceNameObj) {
    ...
    if (interfaceName) {
        String8 nameCopy = String8(String16(reinterpret_cast<const char16_t*>(interfaceName),
...
static void JHwParcel_native_enforceInterface(JNIEnv* env, jobject thiz, jstring interfaceNameObj) {
    ...
    const jchar* interfaceName = env->GetStringCritical(interfaceNameObj, NULL);
    if (interfaceName) {
        String8 interfaceNameCopy =
                String8(String16(reinterpret_cast<const char16_t*>(interfaceName),
                                 env->GetStringLength(interfaceNameObj)));
...
#define DEFINE_PARCEL_WRITER(Suffix, Type)                                              \
    static void JHwParcel_native_write##Suffix(JNIEnv* env, jobject thiz, Type val) {   \
        hardware::Parcel* parcel = JHwParcel::GetNativeContext(env, thiz)->getParcel(); \
                                                                                        \
...
#define DEFINE_PARCEL_READER(Suffix, Type)                                              \
    static Type JHwParcel_native_read##Suffix(JNIEnv* env, jobject thiz) {              \
        hardware::Parcel* parcel = JHwParcel::GetNativeContext(env, thiz)->getParcel(); \
                                                                                        \
        Type val;                                                                       \
...
static void JHwParcel_native_writeStatus(JNIEnv* env, jobject thiz, jint statusCode) {
...
static void JHwParcel_native_release(JNIEnv* env, jobject thiz) {
    JHwParcel::GetNativeContext(env, thiz)->setParcel(NULL, false /* assumeOwnership */);
}

static void JHwParcel_native_releaseTemporaryStorage(JNIEnv* env, jobject thiz) {
...
static void JHwParcel_native_writeString(JNIEnv* env, jobject thiz, jstring valObj) {
    if (valObj == NULL) {
        jniThrowException(env, "java/lang/NullPointerException", NULL);
        return;
    }
    ...
    const hidl_string* s = impl->getStorage()->allocTemporaryString(env, valObj);
    ...
    if (err == OK) {
        err = ::android::hardware::writeEmbeddedToParcel(*s, parcel, parentHandle,
                                                         0 /* parentOffset */);
    }
...
#define DEFINE_PARCEL_VECTOR_WRITER(Suffix, Type)                                                 \
    static void JHwParcel_native_write##Suffix##Vector(JNIEnv* env, jobject thiz,                 \
                                                       Type##Array valObj) {                      \
        if (valObj == NULL) {                                                                     \
            jniThrowException(env, "java/lang/NullPointerException", NULL);                       \
            return;                                                                               \
        ...
        if (err == OK) {                                                                          \
            size_t childHandle;                                                                   \
                                                                                                  \
            err = ::android::hardware::writeEmbeddedToParcel(*vec, parcel, parentHandle,          \
                                                             0 /* parentOffset */, &childHandle); \
        }                                                                                         \
                                                                                                  \
        signalExceptionForError(env, err);                                                        \
    }
...
static void JHwParcel_native_writeBoolVector(JNIEnv* env, jobject thiz, jbooleanArray valObj) {
    if (valObj == NULL) {
        jniThrowException(env, "java/lang/NullPointerException", NULL);
        return;
    }
    ...
    void* vecPtr = impl->getStorage()->allocTemporaryStorage(sizeof(hidl_vec<bool>));
    ...
    bool* dst = (bool*)impl->getStorage()->allocTemporaryStorage(len * sizeof(bool));
...
static void WriteHidlVector(JNIEnv* env, jobject thiz, const hidl_vec<T>& vec) {
    ...
    if (err == OK) {
        ...
        for (size_t i = 0; (err == OK) && (i < vec.size()); ++i) {
            err = ::android::hardware::writeEmbeddedToParcel(vec[i], parcel, childHandle,
                                                             i * sizeof(T));
        }
    }
...
static void JHwParcel_native_writeStringVector(JNIEnv* env, jobject thiz, jobjectArray arrayObj) {
    if (arrayObj == nullptr) {
...
static void JHwParcel_native_writeNativeHandleVector(JNIEnv* env, jobject thiz,
                                                     jobjectArray jHandleArray) {
    ...
    hidl_handle* handles =
            static_cast<hidl_handle*>(storage->allocTemporaryStorage(len * sizeof(hidl_handle)));
...
static void JHwParcel_native_writeStrongBinder(JNIEnv* env, jobject thiz, jobject binderObj) {
    ...
    if (binderObj != NULL) {
        ...
        if (env->IsInstanceOf(binderObj, hwBinderKlass.get())) {
            ...
            binder = JHwRemoteBinder::GetNativeContext(env, binderObj)->getBinder();
        } else {
...
static jstring MakeStringObjFromHidlString(JNIEnv* env, const hidl_string& s) {
    ...
    return env->NewString(reinterpret_cast<const jchar*>(utf16String.string()), utf16String.size());
}
...
static jstring JHwParcel_native_readString(JNIEnv* env, jobject thiz) {
    ...
    const hidl_string* s;
    status_t err =
            parcel->readBuffer(sizeof(*s), &parentHandle, reinterpret_cast<const void**>(&s));
    ...
    err = ::android::hardware::readEmbeddedFromParcel(const_cast<hidl_string&>(*s), *parcel,
                                                      parentHandle, 0 /* parentOffset */);
...
static jobject ReadNativeHandle(JNIEnv* env, jobject thiz, jboolean embedded, jlong parentHandle,
                                jlong offset) {
    hardware::Parcel* parcel = JHwParcel::GetNativeContext(env, thiz)->getParcel();
...
static jobject JHwParcel_native_readEmbeddedNativeHandle(JNIEnv* env, jobject thiz,
...
#define DEFINE_PARCEL_VECTOR_READER(Suffix, Type, NewType)                                     \
    static Type##Array JHwParcel_native_read##Suffix##Vector(JNIEnv* env, jobject thiz) {      \
        hardware::Parcel* parcel = JHwParcel::GetNativeContext(env, thiz)->getParcel();        \
        size_t parentHandle;                                                                   \
                                                                                               \
        const hidl_vec<Type>* vec;                                                             \
        ...
        size_t childHandle;                                                                    \
                                                                                               \
        err = ::android::hardware::readEmbeddedFromParcel(const_cast<hidl_vec<Type>&>(*vec),   \
                                                          *parcel, parentHandle,               \
                                                          0 /* parentOffset */, &childHandle); \
                                                                                               \
        if (err != OK) {                                                                       \
...
static jbooleanArray JHwParcel_native_readBoolVector(JNIEnv* env, jobject thiz) {
    ...
    status_t err =
            parcel->readBuffer(sizeof(*vec), &parentHandle, reinterpret_cast<const void**>(&vec));
    ...
    err = ::android::hardware::readEmbeddedFromParcel(const_cast<hidl_vec<bool>&>(*vec), *parcel,
                                                      parentHandle, 0 /* parentOffset */,
                                                      &childHandle);
...
static jobjectArray MakeStringArray(JNIEnv* env, const hidl_string* array, size_t size) {
    ScopedLocalRef<jclass> stringKlass(env, env->FindClass("java/lang/String"));
    ...
    for (size_t i = 0; i < size; ++i) {
        ...
        env->SetObjectArrayElement(arrayObj, i, stringObj);
    }
...
static const hidl_vec<T>* ReadHidlVector(JNIEnv* env, jobject thiz) {
    ...
    size_t parentHandle;
    status_t err = parcel->readBuffer(sizeof(hidl_vec<T>), &parentHandle,
                                      reinterpret_cast<const void**>(&vec));

    if (err == OK) {
        size_t childHandle;
        err = ::android::hardware::readEmbeddedFromParcel(const_cast<hidl_vec<T>&>(*vec), *parcel,
                                                          parentHandle, 0 /* parentOffset */,
                                                          &childHandle);

        for (size_t i = 0; (err == OK) && (i < vec->size()); i++) {
            err = android::hardware::readEmbeddedFromParcel(const_cast<T&>((*vec)[i]), *parcel,
                                                            childHandle,
...
static jobjectArray JHwParcel_native_readStringVector(JNIEnv* env, jobject thiz) {
    const hidl_vec<hidl_string>* vec = ReadHidlVector<hidl_string>(env, thiz);
    return MakeStringArray(env, &(*vec)[0], vec->size());
}

static jobjectArray JHwParcel_native_readNativeHandleVector(JNIEnv* env, jobject thiz) {
    const hidl_vec<hidl_handle>* vec = ReadHidlVector<hidl_handle>(env, thiz);

    jsize length = vec->size();
    jobjectArray objArray = JNativeHandle::AllocJavaNativeHandleObjArray(env, length);
...
static jobject JHwParcel_native_readBuffer(JNIEnv* env, jobject thiz, jlong expectedSize) {
    hardware::Parcel* parcel = JHwParcel::GetNativeContext(env, thiz)->getParcel();
...
static jobject JHwParcel_native_readEmbeddedBuffer(JNIEnv* env, jobject thiz, jlong expectedSize,
                                                   jlong parentHandle, jlong offset,
                                                   jboolean nullable) {
    hardware::Parcel* parcel = JHwParcel::GetNativeContext(env, thiz)->getParcel();
    ...
    const void* ptr;
    status_t status = parcel->readNullableEmbeddedBuffer(expectedSize, &childHandle, parentHandle,
                                                         offset, &ptr);
...
static void JHwParcel_native_writeBuffer(JNIEnv* env, jobject thiz, jobject blobObj) {
    if (blobObj == nullptr) {
        jniThrowException(env, "java/lang/NullPointerException", NULL);
        return;
    }
...
static JNINativeMethod gMethods[] = {
        ...
        {"writeString", "(Ljava/lang/String;)V", (void*)JHwParcel_native_writeString},
        ...
        {"enforceInterface", "(Ljava/lang/String;)V", (void*)JHwParcel_native_enforceInterface},
        ...
        {"readString", "()Ljava/lang/String;", (void*)JHwParcel_native_readString},
        ...
        {"readBoolVectorAsArray", "()[Z", (void*)JHwParcel_native_readBoolVector},

        {"readInt8VectorAsArray", "()[B", (void*)JHwParcel_native_readInt8Vector},

        {"readInt16VectorAsArray", "()[S", (void*)JHwParcel_native_readInt16Vector},

        {"readInt32VectorAsArray", "()[I", (void*)JHwParcel_native_readInt32Vector},

        {"readInt64VectorAsArray", "()[J", (void*)JHwParcel_native_readInt64Vector},

        {"readFloatVectorAsArray", "()[F", (void*)JHwParcel_native_readFloatVector},

        {"readDoubleVectorAsArray", "()[D", (void*)JHwParcel_native_readDoubleVector},
        ...
        {"releaseTemporaryStorage", "()V", (void*)JHwParcel_native_releaseTemporaryStorage},

        {"send", "()V", (void*)JHwParcel_native_send},

        {"readBuffer", "(J)L" PACKAGE_PATH "/HwBlob;", (void*)JHwParcel_native_readBuffer},

        {"readEmbeddedBuffer", "(JJJZ)L" PACKAGE_PATH "/HwBlob;",
         (void*)JHwParcel_native_readEmbeddedBuffer},

        {"writeBuffer", "(L" PACKAGE_PATH "/HwBlob;)V", (void*)JHwParcel_native_writeBuffer},

        {"release", "()V", (void*)JHwParcel_native_release},
