...
enum pss_rollup_support { PSS_ROLLUP_UNTRIED, PSS_ROLLUP_SUPPORTED, PSS_ROLLUP_UNSUPPORTED };
...
static void read_mapinfo(FILE* fp, stats_t* stats, bool* foundSwapPss) {
    ...
    *foundSwapPss = false;
    ...
    while (!done) {
        ...
        if (sscanf(line, "%" SCNx64 "-%" SCNx64 " %*s %*x %*x:%*x %*d%n", &start, &end,
                   &name_pos) != 2) {
            skip = true;
        } else {
                ...
                if (strncmp(name, "[anon:dalvik-", 13) == 0) {
                    ...
                    if (strstr(name, "[anon:dalvik-LinearAlloc") == name) {
                        ...
                        subHeap = HEAP_DALVIK_NORMAL;
                               ...
                               strstr(name, "[anon:dalvik-free list large object space") == name) {
                        whichHeap = HEAP_DALVIK;
        ...
        while (true) {
            ...
            } else if (sscanf(line, "%" SCNx64 "-%" SCNx64 " %*s %*x %*x:%*x %*d", &start, &end) ==
                       2) {
...
static void android_os_Debug_getDirtyPagesPid(JNIEnv* env, jobject clazz, jint pid,
                                              jobject object) {
    bool foundSwapPss;
...
UniqueFile OpenSmapsOrRollup(int pid) {
    enum pss_rollup_support rollup_support = g_pss_rollup_support.load(std::memory_order_relaxed);
    if (rollup_support != PSS_ROLLUP_UNSUPPORTED) {
        std::string smaps_rollup_path = base::StringPrintf("/proc/%d/smaps_rollup", pid);
        ...
        if (fp_rollup != nullptr) {
            if (rollup_support == PSS_ROLLUP_UNTRIED) {
                ...
                g_pss_rollup_support.store(PSS_ROLLUP_SUPPORTED, std::memory_order_relaxed);
            }
            return fp_rollup;
        }
        g_pss_rollup_support.store(PSS_ROLLUP_UNSUPPORTED, std::memory_order_relaxed);
    }
                ...
                switch (line[0]) {
                    ...
                    case 'S':
                        if (strncmp(line, "SwapPss:", 8) == 0) {
                            ...
                            lSwapPss = atoi(c);
                            swapPss += lSwapPss;
                            pss += lSwapPss;  // Also in swap, those pages would be accounted as Pss
                                              // without SWAP
                        }
                        break;
                }
...
static long get_allocated_vmalloc_memory() {
    ...
    static const char* const ignored_tags[] = {"ioremap", "map_lowmem", "vm_map_ram", NULL};
...
#define ZRAM_SYSFS "/sys/block/zram0/"
    ...
    if (mm_stat_file) {
        ...
        if (matched != 1) ALOGW("failed to parse " ZRAM_SYSFS "mm_stat");
    ...
    if (mem_used_total_file) {
        ...
        if (matched != 1) ALOGW("failed to parse " ZRAM_SYSFS "mem_used_total");
 ...
 * file descriptor.
 */
static void android_os_Debug_dumpNativeHeap(JNIEnv* env, jobject, jobject fileDescriptor) {
    ...
    // See
    // https://android.googlesource.com/platform/bionic/+/master/libc/malloc_debug/README.md#backtrace-heap-dump-format
 ...
 * file descriptor.
 */
static void android_os_Debug_dumpNativeMallocInfo(JNIEnv* env, jobject, jobject fileDescriptor) {
    UniqueFile fp(nullptr, safeFclose);
    if (!openFile(env, fileDescriptor, fp)) {
        return;
    }
...
static bool dumpTraces(JNIEnv* env, jint pid, jstring fileName, jint timeoutSecs,
    ...
    android::base::unique_fd fd(open(fileNameChars.c_str(),
                                     O_CREAT | O_WRONLY | O_NOFOLLOW | O_CLOEXEC | O_APPEND, 0666));
    if (fd < 0) {
...
static jstring android_os_Debug_getUnreachableMemory(JNIEnv* env, jobject clazz, jint limit,
                                                     jboolean contents) {
...
int register_android_os_Debug(JNIEnv* env) {
    ...
    if ((numOtherStats + numDvkStats) != expectedNumOtherStats) {
        jniThrowExceptionFmt(env, "java/lang/RuntimeException",
                             "android.os.Debug.Meminfo.NUM_OTHER_STATS+android.os.Debug.Meminfo."
                             "NUM_DVK_STATS=%d expected %d",
    ...
    for (int i = 0; i < _NUM_CORE_HEAP; i++) {
        stat_fields[i].pss_field = env->GetFieldID(clazz, stat_field_names[i].pss_name, "I");
        stat_fields[i].pssSwappable_field =
                env->GetFieldID(clazz, stat_field_names[i].pssSwappable_name, "I");
        stat_fields[i].rss_field = env->GetFieldID(clazz, stat_field_names[i].rss_name, "I");
