...
// String helpers. Note: using these instead of base's strings.h because we work purely on
//                       const char* and want to avoid std::string.
...
static bool ends_with(const char* source, int sourceLen, const char* other) {
    ...
    return strcmp(source + sourceLen - otherLen, other) == 0;
}
    ...
    *foundSwapPss = false;
    ...
    while (!done) {
        ...
        if (sscanf(line, "%" SCNx64 "-%" SCNx64 " %*s %*x %*x:%*x %*d%n", &start, &end, &name_pos) != 2) {
            ...
            if (!parse(name, kMapParsingData, arraysize(kMapParsingData), &whichHeap, &subHeap,
                       &is_swappable)) {
                if (nameLen > 0) {
