...
EphemeralStorage::~EphemeralStorage() {
    CHECK(mItems.empty()) << "All item storage should have been released by now.";
}
...
const hidl_string* EphemeralStorage::allocTemporaryString(JNIEnv* env, jstring stringObj) {
...
native_handle_t* EphemeralStorage::allocTemporaryNativeHandle(int numFds, int numInts) {
    Item item;
...
#define DEFINE_ALLOC_VECTOR_METHODS(Suffix, Type, NewType)                                         \
    const hidl_vec<Type>* EphemeralStorage::allocTemporary##Suffix##Vector(JNIEnv* env,            \
                                                                           Type##Array arrayObj) { \
...
#define DEFINE_RELEASE_ARRAY_CASE(Suffix, Type, NewType)                               \
    case TYPE_##Suffix##_ARRAY: {                                                      \
        env->Release##NewType##ArrayElements((Type##Array)item.mObj, (Type*)item.mPtr, \
                                             0 /* mode */);                            \
                                                                                       \
...
__attribute__((no_sanitize("unsigned-integer-overflow"))) void EphemeralStorage::release(
        JNIEnv* env) {
    for (size_t i = mItems.size(); i--;) {
        ...
        switch (item.mType) {
            ...
            case TYPE_STRING: {
                env->ReleaseStringUTFChars((jstring)item.mObj, (const char*)item.mPtr);
