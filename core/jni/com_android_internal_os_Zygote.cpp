...
static void isolateAppDataPerPackage(int userId, std::string_view package_name,
  ...
  if (!actualStorageAreaPath.empty()) {
    ...
    createAndMountAppData(package_name, package_name, mirrorStorageAreaPath, actualStorageAreaPath, fail_fn,
                          true /*call_fail_fn*/);
  }
}
...
static void isolateAppData(JNIEnv* env, const std::vector<std::string>& merged_data_info_list,
  ...
  if (!isSdkSandboxProcess) {
      ...
      PrepareDirIfNotPresent("/data/user_de/0", DEFAULT_DATA_DIR_PERMISSION, AID_ROOT, AID_ROOT,
                             fail_fn);
      PrepareDirIfNotPresent("/data/storage_area/0", DEFAULT_DATA_DIR_PERMISSION, AID_ROOT, AID_ROOT,
                             fail_fn);
      ...
      for (int i = 0; i < size; i += 3) {
          ...
          if (volUuid.compare("null") != 0) {
              ...
              PrepareDirIfNotPresent(internalStorageAreaUserPath, DEFAULT_DATA_DIR_PERMISSION, AID_ROOT,
                                        AID_ROOT, fail_fn);
