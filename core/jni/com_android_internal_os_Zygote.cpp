...
static void SetRLimits(JNIEnv* env, jobjectArray managed_rlimits, fail_fn_t fail_fn) {
    ...
    for (int i = 0; i < env->GetArrayLength(managed_rlimits); ++i) {
        ScopedLocalRef<jobject> managed_rlimit_object(
                env, env->GetObjectArrayElement(managed_rlimits, i));
...
static void MountEmulatedStorage(uid_t uid, jint mount_mode, bool force_mount_namespace,
    ...
    if (TEMP_FAILURE_RETRY(mount(storageSource.string(), "/storage", nullptr,
        ...
        fail_fn(CREATE_ERROR("Failed to mount %s to /storage: %s", storageSource.string(),
                             strerror(errno)));
    }
    ...
    if (fs_prepare_dir(userSource.string(), 0751, 0, 0) == -1) {
        fail_fn(CREATE_ERROR("fs_prepare_dir failed on %s (%s)", userSource.string(),
                             strerror(errno)));
    }

    if (TEMP_FAILURE_RETRY(
                mount(userSource.string(), "/storage/self", nullptr, MS_BIND, nullptr)) == -1) {
        fail_fn(CREATE_ERROR("Failed to mount %s to /storage/self: %s", userSource.string(),
                             strerror(errno)));
    }
}
 ...
 */
[[noreturn]] static void ZygoteFailure(JNIEnv* env, const char* process_name,
...
static pid_t ForkCommon(JNIEnv* env, bool is_system_server, const std::vector<int>& fds_to_close,
...
static void SpecializeCommon(JNIEnv* env, uid_t uid, gid_t gid, jintArray gids, jint runtime_flags,
    ...
    bool use_native_bridge = !is_system_server && instruction_set.has_value() &&
...
static jint com_android_internal_os_Zygote_nativeForkSystemServer(
    ...
    std::vector<int> fds_to_close(MakeBlastulaPipeReadFDVector()), fds_to_ignore(fds_to_close);
    ...
    pid_t pid = ForkCommon(env, true, fds_to_close, fds_to_ignore);
    if (pid == 0) {
...
static jint com_android_internal_os_Zygote_nativeForkBlastula(
    ...
    pid_t blastula_pid =
            ForkCommon(env, /* is_system_server= */ false, fds_to_close, fds_to_ignore);
...
static void com_android_internal_os_Zygote_nativeGetSocketFDs(JNIEnv* env, jclass,
    ...
    env_var_name =
            android_socket_prefix + (is_primary ? "blastula_pool" : "blastula_pool_secondary");
...
static jint com_android_internal_os_Zygote_nativeGetBlastulaPoolEventFD(JNIEnv* env, jclass) {
    if (gBlastulaPoolEventFD == -1) {
        if ((gBlastulaPoolEventFD = eventfd(0, 0)) == -1) {
            ZygoteFailure(env, "zygote", nullptr,
...
static const JNINativeMethod gMethods[] = {
        {"nativeSecurityInit", "()V", (void*)com_android_internal_os_Zygote_nativeSecurityInit},
        ...
        {"nativeForkBlastula", "(II[I)I", (void*)com_android_internal_os_Zygote_nativeForkBlastula},
...
int register_com_android_internal_os_Zygote(JNIEnv* env) {
    ...
    gCallPostForkSystemServerHooks =
            GetStaticMethodIDOrDie(env, gZygoteClass, "callPostForkSystemServerHooks", "()V");
