...
status_t CameraMetadata_getNativeMetadata(JNIEnv* env, jobject thiz,
    ...
    CameraMetadata* nativePtr =
            reinterpret_cast<CameraMetadata*>(env->GetLongField(thiz, fields.metadata_ptr));
    if (nativePtr == NULL) {
...
struct Helpers {
    ...
    static status_t updateAny(CameraMetadata* metadata, uint32_t tag, uint32_t type,
        ...
        if (dataBytes % typeSize != 0) {
            ALOGE("%s: Expected dataBytes (%zu) to be divisible by typeSize "
                  "(%zu)",
                  __FUNCTION__, dataBytes, typeSize);
            return BAD_VALUE;
        }
...
#define METADATA_UPDATE(runtime_type, compile_type)                           \
    case runtime_type: {                                                      \
        const compile_type* dataPtr = static_cast<const compile_type*>(data); \
...
static CameraMetadata* CameraMetadata_getPointerThrow(JNIEnv* env, jobject thiz,
    ...
    if (thiz == NULL) {
        ALOGV("%s: Throwing java.lang.NullPointerException for null reference", __FUNCTION__);
...
static jlong CameraMetadata_allocateCopy(JNIEnv* env, jobject thiz, jobject other) {
    ALOGV("%s", __FUNCTION__);
...
static jboolean CameraMetadata_isEmpty(JNIEnv* env, jobject thiz) {
    ...
    if (metadata == NULL) {
        ALOGW("%s: Returning early due to exception being thrown", __FUNCTION__);
        return JNI_TRUE;  // actually throws java exc.
    }

    jboolean empty = metadata->isEmpty();

    ALOGV("%s: Empty returned %d, entry count was %zu", __FUNCTION__, empty,
          metadata->entryCount());
...
static void CameraMetadata_writeValues(JNIEnv* env, jobject thiz, jint tag, jbyteArray src) {
    ...
    if (src == NULL) {
        ...
        res = Helpers::updateAny(metadata, static_cast<uint32_t>(tag), tagType, arrayReader.get(),
                                 arrayReader.size());
...
static void* CameraMetadata_writeMetadataThread(void* arg) {
    ...
    if (close(p->writeFd) < 0) {
        ALOGE("%s: Failed to close writeFd (errno = %#x, message = '%s')", __FUNCTION__, errno,
              strerror(errno));
    }
...
static void CameraMetadata_dump(JNIEnv* env, jobject thiz) {
    ...
    int writeFd, readFd;
    {
        ...
        if (socketpair(AF_LOCAL, SOCK_STREAM, /*protocol*/ 0, &sv[0]) < 0) {
            jniThrowExceptionFmt(env, "java/io/IOException",
                                 "Failed to create socketpair (errno = %#x, message = '%s')", errno,
                                 strerror(errno));
            return;
        }
        writeFd = sv[0];
    ...
    DumpMetadataParams params = {writeFd, metadata};

    {
        ...
        if (threadRet != 0) {
            ...
            jniThrowExceptionFmt(
                    ...
                    "Failed to create thread for writing (errno = %#x, message = '%s')", threadRet,
                    strerror(threadRet));
            return;
        }
    }
     ...
     */
    {
        ...
        if (res < 0) {
            jniThrowExceptionFmt(env, "java/io/IOException",
                                 "Failed to read from fd (errno = %#x, message = '%s')", errno,
                                 strerror(errno));
            // return;
    ...
    if ((res = pthread_join(writeThread, /*retval*/ NULL)) != 0) {
        ALOGE("%s: Failed to join thread (errno = %#x, message = '%s')", __FUNCTION__, res,
              strerror(res));
    }
}
...
static const JNINativeMethod gCameraMetadataMethods[] = {
        // static methods
        {"nativeGetTagFromKey", "(Ljava/lang/String;J)I", (void*)CameraMetadata_getTagFromKey},
        {"nativeGetTypeFromTag", "(IJ)I", (void*)CameraMetadata_getTypeFromTag},
        {"nativeSetupGlobalVendorTagDescriptor", "()I",
         (void*)CameraMetadata_setupGlobalVendorTagDescriptor},
        // instance methods
        {"nativeAllocate", "()J", (void*)CameraMetadata_allocate},
        {"nativeAllocateCopy", "(L" CAMERA_METADATA_CLASS_NAME ";)J",
         (void*)CameraMetadata_allocateCopy},
        {"nativeIsEmpty", "()Z", (void*)CameraMetadata_isEmpty},
        {"nativeGetEntryCount", "()I", (void*)CameraMetadata_getEntryCount},
        {"nativeClose", "()V", (void*)CameraMetadata_close},
        {"nativeSwap", "(L" CAMERA_METADATA_CLASS_NAME ";)V", (void*)CameraMetadata_swap},
        {"nativeGetTagFromKeyLocal", "(Ljava/lang/String;)I",
         (void*)CameraMetadata_getTagFromKeyLocal},
        {"nativeGetTypeFromTagLocal", "(I)I", (void*)CameraMetadata_getTypeFromTagLocal},
        {"nativeReadValues", "(I)[B", (void*)CameraMetadata_readValues},
        {"nativeWriteValues", "(I[B)V", (void*)CameraMetadata_writeValues},
        {"nativeDump", "()V", (void*)CameraMetadata_dump},
        {"nativeGetAllVendorKeys", "(Ljava/lang/Class;)Ljava/util/ArrayList;",
         (void*)CameraMetadata_getAllVendorKeys},
        // Parcelable interface
        {"nativeReadFromParcel", "(Landroid/os/Parcel;)V", (void*)CameraMetadata_readFromParcel},
        {"nativeWriteToParcel", "(Landroid/os/Parcel;)V", (void*)CameraMetadata_writeToParcel},
};
...
int register_android_hardware_camera2_CameraMetadata(JNIEnv* env) {
    ...
    gMetadataOffsets.mCharacteristicsConstr =
            GetMethodIDOrDie(env, gMetadataOffsets.mCharacteristicsKey, "<init>",
                             "(Ljava/lang/String;Ljava/lang/Class;J)V");
    gMetadataOffsets.mRequestConstr = GetMethodIDOrDie(env, gMetadataOffsets.mRequestKey, "<init>",
                                                       "(Ljava/lang/String;Ljava/lang/Class;J)V");
    gMetadataOffsets.mResultConstr = GetMethodIDOrDie(env, gMetadataOffsets.mResultKey, "<init>",
                                                      "(Ljava/lang/String;Ljava/lang/Class;J)V");
    ...
    gMetadataOffsets.mArrayListConstr =
            GetMethodIDOrDie(env, gMetadataOffsets.mArrayList, "<init>", "(I)V");
    gMetadataOffsets.mArrayListAdd =
            GetMethodIDOrDie(env, gMetadataOffsets.mArrayList, "add", "(Ljava/lang/Object;)Z");
    ...
    return RegisterMethodsOrDie(env, CAMERA_METADATA_CLASS_NAME, gCameraMetadataMethods,
