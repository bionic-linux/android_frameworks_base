...
static jlong NativeLoadFromFd(JNIEnv* env, jclass /*clazz*/, jobject file_descriptor,
    ...
    std::unique_ptr<const ApkAssets> apk_assets = ApkAssets::LoadFromFd(
            std::move(dup_fd), friendly_name_utf8.c_str(), system, force_shared_lib);
    if (apk_assets == nullptr) {
...
static jlong NativeOpenXml(JNIEnv* env, jclass /*clazz*/, jlong ptr, jstring file_name) {
    ...
    std::unique_ptr<Asset> asset =
            apk_assets->Open(path_utf8.c_str(), Asset::AccessMode::ACCESS_RANDOM);
    if (asset == nullptr) {
    ...
    std::unique_ptr<ResXMLTree> xml_tree =
