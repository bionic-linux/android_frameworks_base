...
class NativeKeyCharacterMap {
  public:
    ...
    inline const sp<KeyCharacterMap>& getMap() const { return mMap; }
...
jobject android_view_KeyCharacterMap_create(JNIEnv* env, int32_t deviceId,
    ...
    NativeKeyCharacterMap* map =
            new NativeKeyCharacterMap(deviceId, kcm.get() ? kcm : KeyCharacterMap::empty());
    if (!map) {
        return NULL;
    }
...
static jchar nativeGetCharacter(JNIEnv* env, jobject clazz, jlong ptr, jint keyCode,
                                jint metaState) {
    NativeKeyCharacterMap* map = reinterpret_cast<NativeKeyCharacterMap*>(ptr);
...
static jobjectArray nativeGetEvents(JNIEnv* env, jobject clazz, jlong ptr, jcharArray charsArray) {
    NativeKeyCharacterMap* map = reinterpret_cast<NativeKeyCharacterMap*>(ptr);
...
int register_android_view_KeyCharacterMap(JNIEnv* env) {
    ...
    gKeyCharacterMapClassInfo.ctor =
            GetMethodIDOrDie(env, gKeyCharacterMapClassInfo.clazz, "<init>", "(J)V");
