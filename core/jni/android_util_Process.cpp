...
static void signalExceptionForError(JNIEnv* env, int err, int tid) {
    switch (err) {
        case EINVAL:
            jniThrowExceptionFmt(env, "java/lang/IllegalArgumentException", "Invalid argument: %d",
                                 tid);
            break;
        case ESRCH:
...
jint android_os_Process_getUidForName(JNIEnv* env, jobject clazz, jstring name) {
    ...
    String8 name8;
    if (str16) {
        name8 = String8(reinterpret_cast<const char16_t*>(str16), env->GetStringLength(name));
...
void android_os_Process_setProcessGroup(JNIEnv* env, jobject clazz, int pid, jint grp) {
    ...
    if (!(d = opendir(proc_path))) {
        ...
        if (errno != ENOENT) signalExceptionForGroupError(env, errno, pid);
        return;
    }
...
jint android_os_Process_getThreadScheduler(JNIEnv* env, jclass clazz, jint tid) {
    int policy = 0;
...
void android_os_Process_setThreadScheduler(JNIEnv* env, jclass clazz, jint tid, jint policy,
                                           jint pri) {
...
void android_os_Process_setThreadPriority(JNIEnv* env, jobject clazz, jint pid, jint pri) {
#if GUARD_THREAD_PRIORITY
...
void android_os_Process_setCallingThreadPriority(JNIEnv* env, jobject clazz, jint pri) {
...
jboolean android_os_Process_setSwappiness(JNIEnv* env, jobject clazz, jint pid,
                                          jboolean is_increased) {
    char text[64];
...
void android_os_Process_setArgV0(JNIEnv* env, jobject clazz, jstring name) {
    ...
    String8 name8;
    if (str) {
        name8 = String8(reinterpret_cast<const char16_t*>(str), env->GetStringLength(name));
...
jintArray android_os_Process_getPids(JNIEnv* env, jobject clazz, jstring file,
                                     jintArray lastArray) {
    if (file == NULL) {
        jniThrowNullPointerException(env, NULL);
        return NULL;
...
jboolean android_os_Process_parseProcLineArray(JNIEnv* env, jobject clazz, char* buffer,
                                               jint startIndex, jint endIndex, jintArray format,
    ...
    jlong* longsData = outLongs ? env->GetLongArrayElements(outLongs, 0) : NULL;
    jfloat* floatsData = outFloats ? env->GetFloatArrayElements(outFloats, 0) : NULL;
    if (formatData == NULL || (NL > 0 && longsData == NULL) || (NR > 0 && floatsData == NULL)) {
        if (formatData != NULL) {
...
jboolean android_os_Process_parseProcLine(JNIEnv* env, jobject clazz, jbyteArray buffer,
                                          jint startIndex, jint endIndex, jintArray format,
                                          jobjectArray outStrings, jlongArray outLongs,
                                          jfloatArray outFloats) {
    jbyte* bufferArray = env->GetByteArrayElements(buffer, NULL);

    jboolean result = android_os_Process_parseProcLineArray(env, clazz, (char*)bufferArray,
                                                            startIndex, endIndex, format,
                                                            outStrings, outLongs, outFloats);
...
jboolean android_os_Process_readProcFile(JNIEnv* env, jobject clazz, jstring file, jintArray format,
                                         jobjectArray outStrings, jlongArray outLongs,
                                         jfloatArray outFloats) {
    ...
    return android_os_Process_parseProcLineArray(env, clazz, buffer, 0, len, format, outStrings,
                                                 outLongs, outFloats);
}

void android_os_Process_setApplicationObject(JNIEnv* env, jobject clazz, jobject binderObject) {
