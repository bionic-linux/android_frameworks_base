...
static void get_cpuset_cores_for_policy(SchedPolicy policy, cpu_set_t *cpu_set)
{
    ...
    switch (policy) {
        case SP_BACKGROUND:
            if (!CgroupGetAttributePath(TPA_LowCapacityCPUs.c_str(), &filename)) {
                return;
            }
            break;
        case SP_FOREGROUND:
        ...
        case SP_AUDIO_SYS:
        case SP_RT_APP:
            if (!CgroupGetAttributePath(TPA_HighCapacityCPUs.c_str(), &filename)) {
                return;
            }
            break;
        case SP_TOP_APP:
            if (!CgroupGetAttributePath(TPA_MaxCapacityCPUs.c_str(), &filename)) {
                return;
            }
            break;
        default:
