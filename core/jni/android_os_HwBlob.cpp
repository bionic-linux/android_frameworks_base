...
void JHwBlob::specializeBlobTo(JHwBlob::BlobType type) {
    CHECK(mType == BlobType::GENERIC);
    mType = type;
}
...
status_t JHwBlob::writeSubBlobsToParcel(hardware::Parcel *parcel,
    ...
    for (size_t i = 0; i < mSubBlobs.size(); ++i) {
        ...
        status_t err = info.mBlob->writeEmbeddedToParcel(parcel, parentHandle,
                                                         info.mOffset);
        if (err != OK) {
            return err;
        }
...
static void JHwBlob_native_putNativeHandle(JNIEnv *env, jobject thiz,
    ...
    native_handle_t *nativeHandle = JNativeHandle::MakeCppNativeHandle(
            env, jHandle, nullptr /* storage */);

    size_t size = (sizeof(native_handle_t) +
                   nativeHandle->numFds * sizeof(int) +
                   nativeHandle->numInts * sizeof(int));

    ScopedLocalRef<jobject> subBlobObj(env, JHwBlob::NewObject(env, size));
    sp<JHwBlob> subBlob = JHwBlob::GetNativeContext(env, subBlobObj.get());
    subBlob->specializeBlobTo(JHwBlob::BlobType::NATIVE_HANDLE);
    subBlob->write(0 /* offset */, nativeHandle, size);
    native_handle_delete(nativeHandle);

    hidl_handle cppHandle;
    cppHandle.setTo(static_cast<native_handle_t *>(subBlob->data()),
                    false /* shouldOwn */);

    sp<JHwBlob> blob = JHwBlob::GetNativeContext(env, thiz);
    blob->write(offset, &cppHandle, sizeof(cppHandle));
    blob->putBlob(offset + 0 /* offsetof(hidl_handle, mHandle) */, subBlob);
}
