...
// static
sp<JHwBlob> JHwBlob::SetNativeContext(JNIEnv* env, jobject thiz, const sp<JHwBlob>& context) {
...
JHwBlob::JHwBlob(JNIEnv* env, jobject thiz, size_t size)
    : mBuffer(nullptr), mSize(size), mType(BlobType::GENERIC), mOwnsBuffer(true), mHandle(0) {
    if (size > 0) {
        mBuffer = malloc(size);
    }
}
...
status_t JHwBlob::getString(size_t offset, const hidl_string** s) const {
    ...
    *s = reinterpret_cast<const hidl_string*>((const uint8_t*)mBuffer + offset);
...
status_t JHwBlob::writeEmbeddedToParcel(hardware::Parcel* parcel, size_t parentHandle,
    ...
    switch (mType) {
        ...
        case BlobType::NATIVE_HANDLE: {
            err = parcel->writeEmbeddedNativeHandle(static_cast<const native_handle*>(data()),
...
status_t JHwBlob::writeSubBlobsToParcel(hardware::Parcel* parcel, size_t parentHandle) const {
...
jobject JHwBlob::NewObject(JNIEnv* env, size_t size) {
    ...
    jmethodID constructID = GetMethodIDOrDie(env, clazz.get(), "<init>", "(I)V");
...
static void JHwBlob_native_setup(JNIEnv* env, jobject thiz, jint size) {
...
#define DEFINE_BLOB_GETTER(Suffix, Type)                                              \
    static Type JHwBlob_native_get##Suffix(JNIEnv* env, jobject thiz, jlong offset) { \
...
static jboolean JHwBlob_native_getBool(JNIEnv* env, jobject thiz, jlong offset) {
    sp<JHwBlob> blob = JHwBlob::GetNativeContext(env, thiz);
...
static jstring JHwBlob_native_getString(JNIEnv* env, jobject thiz, jlong offset) {
    sp<JHwBlob> blob = JHwBlob::GetNativeContext(env, thiz);
...
#define DEFINE_BLOB_ARRAY_COPIER(Suffix, Type, NewType)                                       \
    static void JHwBlob_native_copyTo##Suffix##Array(JNIEnv* env, jobject thiz, jlong offset, \
                                                     Type##Array array, jint size) {          \
        if (array == nullptr) {                                                               \
        ...
        env->Set##NewType##ArrayRegion(                                                       \
                array, 0 /* start */, size,                                                   \
                reinterpret_cast<const Type*>(static_cast<const uint8_t*>(blob->data()) +     \
                                              offset));                                       \
    }
...
static void JHwBlob_native_copyToBoolArray(JNIEnv* env, jobject thiz, jlong offset,
                                           jbooleanArray array, jint size) {
    if (array == nullptr) {
        jniThrowException(env, "java/lang/NullPointerException", nullptr);
        return;
    }
    ...
    const bool* src =
            reinterpret_cast<const bool*>(static_cast<const uint8_t*>(blob->data()) + offset);
...
#define DEFINE_BLOB_PUTTER(Suffix, Type)                                                      \
    static void JHwBlob_native_put##Suffix(JNIEnv* env, jobject thiz, jlong offset, Type x) { \
        sp<JHwBlob> blob = JHwBlob::GetNativeContext(env, thiz);                              \
                                                                                              \
...
static void JHwBlob_native_putBool(JNIEnv* env, jobject thiz, jlong offset, jboolean x) {
    sp<JHwBlob> blob = JHwBlob::GetNativeContext(env, thiz);
...
static void JHwBlob_native_putString(JNIEnv* env, jobject thiz, jlong offset, jstring stringObj) {
...
static void JHwBlob_native_putNativeHandle(JNIEnv* env, jobject thiz, jlong offset,
                                           jobject jHandle) {
...
#define DEFINE_BLOB_ARRAY_PUTTER(Suffix, Type, NewType)                                       \
    static void JHwBlob_native_put##Suffix##Array(JNIEnv* env, jobject thiz, jlong offset,    \
                                                  Type##Array array) {                        \
        ...
        sp<JHwBlob> blob = JHwBlob::GetNativeContext(env, thiz);                              \
                                                                                              \
        status_t err = blob->write(offset, autoArray.get(), autoArray.size() * sizeof(Type)); \
                                                                                              \
        if (err != OK) {                                                                      \
            signalExceptionForError(env, err);                                                \
        }                                                                                     \
...
static void JHwBlob_native_putBoolArray(JNIEnv* env, jobject thiz, jlong offset,
                                        jbooleanArray array) {
    ...
    bool* dst = reinterpret_cast<bool*>(static_cast<uint8_t*>(blob->data()) + offset);
...
static void JHwBlob_native_putBlob(JNIEnv* env, jobject thiz, jlong offset, jobject blobObj) {
    if (blobObj == nullptr) {
        jniThrowException(env, "java/lang/NullPointerException", nullptr);
        return;
    }
...
static JNINativeMethod gMethods[] = {
        ...
        {"putBlob", "(JL" PACKAGE_PATH "/HwBlob;)V", (void*)JHwBlob_native_putBlob},
