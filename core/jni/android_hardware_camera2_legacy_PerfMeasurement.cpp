...
static bool extensionEqual(const GLubyte* pExtensions, const GLubyte* pExtension) {
    while (true) {
        char a = *pExtensions++;
        char b = *pExtension++;
        bool aEnd = a == '\0' || a == ' ';
...
static const GLubyte* nextExtension(const GLubyte* pExtensions) {
    while (true) {
        char a = *pExtensions++;
        if (a == '\0') {
...
static bool checkForExtension(const GLubyte* pExtensions, const GLubyte* pExtension) {
...
bool checkGlError(JNIEnv* env) {
    ...
    if ((error = glGetError()) != GL_NO_ERROR) {
        jniThrowExceptionFmt(env, "java/lang/IllegalStateException", "GLES20 error: 0x%d", error);
        return true;
    }
    return false;
}
...
class PerfMeasurementContext {
  ...
  public:
    ...
    explicit PerfMeasurementContext(size_t maxQueryCount)
        : mTimingStartIndex(0), mTimingEndIndex(0), mTimingQueryIndex(0) {
        mTimingQueries.resize(maxQueryCount);
        mFreeQueries = maxQueryCount;
        mInitDone = false;
    }

    int getMaxQueryCount() { return mTimingQueries.size(); }
    ...
    nsecs_t getNextGlDuration() {
        ...
        GLint available;
        glGetQueryObjectivEXT(mTimingQueries[mTimingQueryIndex], GL_QUERY_RESULT_AVAILABLE_EXT,
                              &available);
        if (!available) {
            return NO_DURATION_YET;
        }
        ...
        if (!disjointOccurred) {
            glGetQueryObjecti64vEXT(mTimingQueries[mTimingQueryIndex], GL_QUERY_RESULT_EXT,
                                    &duration);
        }
...
static jlong PerfMeasurement_nativeCreateContext(JNIEnv* env, jobject thiz, jint maxQueryCount) {
    PerfMeasurementContext* context = new PerfMeasurementContext(maxQueryCount);
    return reinterpret_cast<jlong>(context);
}

static void PerfMeasurement_nativeDeleteContext(JNIEnv* env, jobject thiz, jlong contextHandle) {
...
static void PerfMeasurement_nativeStartGlTimer(JNIEnv* env, jobject thiz, jlong contextHandle) {
    PerfMeasurementContext* context = getContext(contextHandle);
...
static void PerfMeasurement_nativeStopGlTimer(JNIEnv* env, jobject thiz, jlong contextHandle) {
    PerfMeasurementContext* context = getContext(contextHandle);
...
static jlong PerfMeasurement_nativeGetNextGlDuration(JNIEnv* env, jobject thiz,
                                                     jlong contextHandle) {
    PerfMeasurementContext* context = getContext(contextHandle);
...
static const JNINativeMethod gPerfMeasurementMethods[] = {
        {"nativeCreateContext", "(I)J", (jlong*)PerfMeasurement_nativeCreateContext},
        {"nativeDeleteContext", "(J)V", (void*)PerfMeasurement_nativeDeleteContext},
        {"nativeQuerySupport", "()Z", (jboolean*)PerfMeasurement_nativeQuerySupport},
        {"nativeStartGlTimer", "(J)V", (void*)PerfMeasurement_nativeStartGlTimer},
        {"nativeStopGlTimer", "(J)V", (void*)PerfMeasurement_nativeStopGlTimer},
...
int register_android_hardware_camera2_legacy_PerfMeasurement(JNIEnv* env) {
    ...
    return RegisterMethodsOrDie(env, PERF_MEASUREMENT_CLASS_NAME, gPerfMeasurementMethods,
