                                                 ...
                                                 const struct audio_port_config *nAudioPortConfig)
{
    ...
    if (gainIndex >= 0) {
        ...
        if (audioportCreated) {
            ...
            ScopedLocalRef<jobjectArray> jGains(env, static_cast<jobjectArray>(env->GetObjectField(
                                                jAudioPort->get(), gAudioPortFields.mGains)));
            if (jGains == NULL) {
...
static jint convertAudioPortFromNative(JNIEnv *env, ScopedLocalRef<jobject>* jAudioPort,
    ...
    if (nAudioPort->type == AUDIO_PORT_TYPE_DEVICE) {
        ...
        jAudioPort->reset(env->NewObject(gAudioDevicePortClass, gAudioDevicePortCstor,
                          jHandle.get(), jDeviceName.get(), jAudioProfiles.get(), jGains.get(),
    ...
    ScopedLocalRef<jobject> jAudioPortConfigTmp(env, nullptr);

    if (int jStatus = convertAudioPortConfigFromNative(env, jAudioPort, &jAudioPortConfigTmp,
                                                       &nAudioPort->active_config);
        jStatus != AUDIO_JAVA_SUCCESS) {
        return jStatus;
    }

    ScopedLocalRef<jobject> jAudioPortConfig(env, jAudioPortConfigTmp.get());
                                           ...
                                           jobject jPatches, jintArray jGeneration)
{
    ...
    for (size_t i = 0; i < numPatches; i++) {
        ...
        for (size_t j = 0; j < nPatches[i].num_sources; j++) {
            ScopedLocalRef<jobject> jSourceTmp(env, nullptr);
            ScopedLocalRef<jobject> jAudioPort(env, nullptr);
            jStatus = convertAudioPortConfigFromNative(env, &jAudioPort, &jSourceTmp,
                                                       &nPatches[i].sources[j]);
            ScopedLocalRef<jobject> jSource(env, jSourceTmp.get());
            if (jStatus != AUDIO_JAVA_SUCCESS) {
                if (!setGeneration(env, jGeneration, generation1)) {
                    jStatus = AUDIO_JAVA_ERROR;
                }
        ...
        for (size_t j = 0; j < nPatches[i].num_sinks; j++) {
            ScopedLocalRef<jobject> jSinkTmp(env, nullptr);
            ScopedLocalRef<jobject> jAudioPort(env, nullptr);
            jStatus = convertAudioPortConfigFromNative(env, &jAudioPort, &jSinkTmp,
                                                       &nPatches[i].sinks[j]);
            ScopedLocalRef<jobject> jSink(env, jSinkTmp.get());
