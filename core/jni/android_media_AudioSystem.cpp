...
static jint convertAudioPortConfigFromNative(JNIEnv *env,
                                                 ScopedLocalRef<jobject>* jAudioPort,
                                                 ...
                                                 const struct audio_port_config *nAudioPortConfig)
{
    ...
    if (*jAudioPort == nullptr) {
        ScopedLocalRef<jobject> jHandle(env, env->NewObject(gAudioHandleClass, gAudioHandleCstor,
                                                            nAudioPortConfig->id));
    ...
    if (gainIndex >= 0) {
        ...
        if (audioportCreated) {
            ...
            ScopedLocalRef<jobjectArray> jGains(env, static_cast<jobjectArray>(env->GetObjectField(jAudioPort->get(),
...
static jint convertAudioPortFromNative(JNIEnv *env, ScopedLocalRef<jobject>* jAudioPort,
    ...
    if (nAudioPort->type == AUDIO_PORT_TYPE_DEVICE) {
        ...
        jAudioPort->reset(env->NewObject(gAudioDevicePortClass, gAudioDevicePortCstor, jHandle.get(),
                                           ...
                                           jobject jPatches, jintArray jGeneration)
{
    ...
    for (size_t i = 0; i < numPatches; i++) {
        ...
        for (size_t j = 0; j < nPatches[i].num_sources; j++) {
            ...
            ScopedLocalRef<jobject> jAudioPort(env, nullptr);
            jStatus = convertAudioPortConfigFromNative(env, &jAudioPort, &jSourceTmp,
        ...
        for (size_t j = 0; j < nPatches[i].num_sinks; j++) {
            ...
            ScopedLocalRef<jobject> jAudioPort(env, nullptr);
            jStatus = convertAudioPortConfigFromNative(env, &jAudioPort, &jSinkTmp, &nPatches[i].sinks[j]);
