...
public final class SimPhonebookContract {
    ...
    public static final class SimContacts {
         ...
         *
         * <p>An IllegalArgumentException will be thrown by insert and update if this exceeds the
    ...
    public static final class SimPhonebookException extends AndroidRuntimeException implements
         ...
         *
         * @see #getValidationDetails(String)
         */
        ...
        private final ArrayMap<String, ValidationDetails> mDataValidationErrors = new ArrayMap<>();
        ...
        public SimPhonebookException(
                int errorCode, @NonNull Map<String, ValidationDetails> validationErrors) {
            this.errorCode = errorCode;
            this.mDataValidationErrors.putAll(validationErrors);
        }
        ...
        @Nullable
        public ValidationDetails getValidationDetails(@NonNull String columnName) {
            return mDataValidationErrors.get(columnName);
        }
        ...
        public void writeToParcel(@NonNull Parcel dest, int flags) {
            ...
            dest.writeTypedArrayMap(mDataValidationErrors, 0);
        }
    ...
    /**
     * Holds additional information about why an insert or update operation failed when the error
     * is {@link SimPhonebookException#ERROR_INVALID_DATA}.
     */
    public static final class ValidationDetails implements Parcelable {

        @NonNull
        public static final Creator<ValidationDetails> CREATOR =
                new Creator<ValidationDetails>() {
                    @Override
                    public ValidationDetails createFromParcel(@NonNull Parcel in) {
                        return new ValidationDetails(in);
                    }
        ...
        private final int mActualCount;
        private final boolean mIsRequired;
        private final SparseBooleanArray mInvalidIndices;

        public ValidationDetails(
                int maxCount, int actualCount, boolean isRequired) {
            this(maxCount, actualCount, isRequired, new SparseBooleanArray());
        }

        public ValidationDetails(int maxCount, int actualCount, boolean isRequired,
                @NonNull SparseBooleanArray invalidIndices) {
            mMaxCount = maxCount;
            mActualCount = actualCount;
            mIsRequired = isRequired;
            mInvalidIndices = invalidIndices;
        }

        private ValidationDetails(@NonNull Parcel in) {
            mMaxCount = in.readInt();
            mActualCount = in.readInt();
            mIsRequired = in.readBoolean();
            mInvalidIndices = in.readSparseBooleanArray();
        }
        ...
        @NonNull
        public SparseBooleanArray getInvalidIndices() {
            return mInvalidIndices;
        }

        public boolean isValid() {
            return mActualCount <= mMaxCount && (!mIsRequired || mActualCount > 0)
                    && mInvalidIndices.size() == 0;
        }
        ...
        public void writeToParcel(@NonNull Parcel dest, int flags) {
            ...
            dest.writeInt(mActualCount);
            dest.writeBoolean(mIsRequired);
            dest.writeSparseBooleanArray(mInvalidIndices);
        }
    }
}
