...
public class AdapterStateListener extends IUwbAdapterStateCallbacks.Stub {
    ...
    Map<AdapterStateCallback, Executor> mCallbackMap;

    @StateChangedReason
    int mAdapterStateChangeReason = AdapterStateCallback.STATE_CHANGED_REASON_ERROR_UNKNOWN;
    boolean mAdapterEnabledState = false;
    ...
    public void unregister(@NonNull AdapterStateCallback callback) {
        synchronized (this) {
            ...
            if (mCallbackMap.size() == 0 && mIsRegistered) {
                try {
    ...
    private void sendCurrentState(AdapterStateCallback callback) {
        if (callback == null) {
            return;
        }
        ...
        if (executor == null) {
            Log.w(TAG, "No executor for given callback");
            return;
        }
