...
public final class BluetoothLeAudio implements BluetoothProfile, AutoCloseable {
    ...
    /**
     * Indicating that group is in Idle state.
     * @hide
     */
    public static final int GROUP_STATUS_IDLE = IBluetoothLeAudio.GROUP_STATUS_IDLE;

    /**
     * Indicating that group is in Streaming state.
     * @hide
     */
    public static final int GROUP_STATUS_STREAMING = IBluetoothLeAudio.GROUP_STATUS_STREAMING;

    /**
     * Indicating that group is in Suspended state.
     * @hide
     */
    public static final int GROUP_STATUS_SUSPENDED = IBluetoothLeAudio.GROUP_STATUS_SUSPENDED;

    /**
     * Indicating that group has been destroyed.
     * @hide
     */
    public static final int GROUP_STATUS_DESTROYED = IBluetoothLeAudio.GROUP_STATUS_DESTROYED;
    ...
    public boolean groupRemoveNode(int group_id, @NonNull BluetoothDevice device) {
        ...
        try {
            if (service != null && mAdapter.isEnabled()) {
                return service.groupRemoveNode(group_id, device);
            }
            if (service == null) Log.w(TAG, "Proxy not attached to service");
            return false;
        } catch (RemoteException e) {
    ...
    public boolean groupDestroy(int group_id) {
        if (VDBG) log("groupDestroy()");
        final IBluetoothLeAudio service = getService();
        try {
            if (service != null && mAdapter.isEnabled()) {
                service.groupDestroy(group_id);
                return true;
            }
            if (service == null) Log.w(TAG, "Proxy not attached to service");
            return false;
