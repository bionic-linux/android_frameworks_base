...
public abstract class NetworkAgent {
    ...
    public final void sendQosSessionAvailable(int qosCallbackId,
        ...
        if (session.getSessionType() != QosSession.TYPE_EPS_BEARER) {
            throw new IllegalArgumentException("Session type " + session.getSessionType()
                    + " is not supported");
        }

        final Bundle bundle = new Bundle();
        bundle.putParcelable(QOS_SESSION_AVAILABLE_SESSION_KEY, session);

        if (session.getSessionType() == QosSession.TYPE_EPS_BEARER) {
            if (!(attributes instanceof EpsBearerQosSessionAttributes)) {
                throw new IllegalArgumentException("Session type of eps bearer should always have "
                        + "attributes of type EpsBearerQosSessionAttributes");
            } else {
                bundle.putParcelable(QOS_SESSION_AVAILABLE_ATTRIBUTES_KEY,
