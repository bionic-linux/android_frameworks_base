...
public class NetworkStatsProviderCallback {
    private final INetworkStatsProviderCallback mBinder;
    ...
    /**
     * Notifies that there is an updated cumulative network statistics since boot that needs to
     * be combined into system. Expected to be triggered after the
     * {@link NetworkStatsProviderBase#requestStatsUpdate(int)} has been called, or whenever the
     * provider thinks there is some statistics needs to be reported.
     ...
     * @param stats the {@link NetworkStats} to be reported. The provider should not
     *             include any traffic that is already counted by kernel interface counters.
     */
    public void onStatsUpdated(int how, @NonNull NetworkStats stats) {
        try {
            mBinder.onStatsUpdated(how, stats);
