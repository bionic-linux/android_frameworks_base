...
public final class QosSocketInfo implements Parcelable {
    ...
    public QosSocketInfo(@NonNull final Network network, @NonNull final Socket socket)
            throws IOException {
        this(network, ParcelFileDescriptor.dup(
                Objects.requireNonNull(
                        socket.getFileDescriptor$(), "socket cannot be null")),
                new InetSocketAddress(socket.getLocalAddress(), socket.getLocalPort()));
    }

    /**
     * Removes the need of mocking a socket by exposing the parameters that are taken from it
     * instead.
     */
    @VisibleForTesting
    public QosSocketInfo(@NonNull final Network network,
            @NonNull final ParcelFileDescriptor parcelFileDescriptor,
        ...
        mParcelFileDescriptor = Objects.requireNonNull(parcelFileDescriptor,
                "parcelFileDescriptor cannot be null");
        mLocalSocketAddress =
    ...
    private QosSocketInfo(final Parcel in) {
        ...
        mLocalSocketAddress = readSocketAddress(in, in.readInt());
    }
