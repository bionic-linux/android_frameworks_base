 ...
 * 1. {@link android.net.QosCallbackException.NetworkReleasedException}
 * 2. {@link android.net.QosSocketInfo.SocketNotBoundException}
 * 3. {@link UnsupportedOperationException}
 * 4. {@link android.net.QosSocketInfo.SocketLocalAddressChangedException}
 *
 * @hide
 */
@SystemApi
public final class QosCallbackException extends Exception {
    ...
    static QosCallbackException createException(@ExceptionType final int type) {
        ...
        if (type == EX_TYPE_FILTER_SOCKET_NOT_BOUND) {
            return new QosCallbackException(new QosSocketInfo.SocketNotBoundException());
        }

        if (type == EX_TYPE_FILTER_NOT_SUPPORTED) {
            return new QosCallbackException(new UnsupportedOperationException(
                    "This device does not support the specified filter"));
        }

        if (type == EX_TYPE_FILTER_SOCKET_LOCAL_ADDRESS_CHANGED) {
            return new QosCallbackException(new QosSocketInfo.SocketLocalAddressChangedException());
        }
    ...
    public static class NetworkReleasedException extends Exception {
        ...
        public NetworkReleasedException() {
            super("The network was released an no longer available");
        }
    }
}
