...
public class NetworkTemplate implements Parcelable {
     ...
     */
    @UnsupportedAppUsage
    public static NetworkTemplate buildTemplateMobileAll(String subscriberId, int subId) {
        return new NetworkTemplate(MATCH_MOBILE, subscriberId, subId, null);
    }
    ...
    public static NetworkTemplate buildTemplateMobileWithRatType(@Nullable String subscriberId,
            int subId, @NetworkType int ratType, int metered) {
        if (TextUtils.isEmpty(subscriberId)) {
            return new NetworkTemplate(MATCH_MOBILE_WILDCARD, null, null, 0, null, null,
                    metered, ROAMING_ALL, DEFAULT_NETWORK_ALL, ratType, OEM_MANAGED_ALL,
                    SUBSCRIBER_ID_MATCH_RULE_EXACT);
        }
        return new NetworkTemplate(MATCH_MOBILE, subscriberId, new String[]{subscriberId}, 0, null,
                 null, metered, ROAMING_ALL, DEFAULT_NETWORK_ALL, ratType, OEM_MANAGED_ALL,
                SUBSCRIBER_ID_MATCH_RULE_EXACT);
    }
    ...
    public static NetworkTemplate buildTemplateMobileWildcard() {
        return new NetworkTemplate(MATCH_MOBILE_WILDCARD, null, 0, null);
    }
    ...
    public static NetworkTemplate buildTemplateWifiWildcard() {
        ...
        return new NetworkTemplate(MATCH_WIFI_WILDCARD, null, 0, null);
    }
    ...
    public static NetworkTemplate buildTemplateWifi(@NonNull String networkId) {
        ...
        return new NetworkTemplate(MATCH_WIFI, null /* subscriberId */,
                new String[] { null } /* matchSubscriberIds */, 0 /* subId */,
                null /* matchSubIds */, networkId, METERED_ALL, ROAMING_ALL,
    ...
    public static NetworkTemplate buildTemplateWifi(@Nullable String networkId,
            @Nullable String subscriberId, int subId) {
        return new NetworkTemplate(MATCH_WIFI, subscriberId, new String[] { subscriberId }, subId,
                new int[] { subId }, networkId, METERED_ALL, ROAMING_ALL,
                DEFAULT_NETWORK_ALL, NETWORK_TYPE_ALL, OEM_MANAGED_ALL,
                SUBSCRIBER_ID_MATCH_RULE_EXACT);
    }
    ...
    public static NetworkTemplate buildTemplateEthernet() {
        return new NetworkTemplate(MATCH_ETHERNET, null, 0, null);
    }
    ...
    public static NetworkTemplate buildTemplateBluetooth() {
        return new NetworkTemplate(MATCH_BLUETOOTH, null, 0, null);
    }
    ...
    public static NetworkTemplate buildTemplateProxy() {
        return new NetworkTemplate(MATCH_PROXY, null, 0, null);
    }
    ...
    public static NetworkTemplate buildTemplateCarrierMetered(@NonNull String subscriberId,
        ...
        return new NetworkTemplate(MATCH_CARRIER, subscriberId,
                ...
                null /* networkId */, METERED_YES, ROAMING_ALL, DEFAULT_NETWORK_ALL,
    ...
    private final int[] mMatchSubIds;
    ...
    @UnsupportedAppUsage
    public NetworkTemplate(int matchRule, String subscriberId, int subId, String networkId) {
        this(matchRule, subscriberId, new String[] { subscriberId }, subId, new int[] { subId },
                networkId);
    }

    public NetworkTemplate(int matchRule, String subscriberId, String[] matchSubscriberIds,
            int subId, int[] matchSubIds, String networkId) {
        // Older versions used to only match MATCH_MOBILE and MATCH_MOBILE_WILDCARD templates
        // to metered networks. It is now possible to match mobile with any meteredness, but
        // in order to preserve backward compatibility of @UnsupportedAppUsage methods, this
        //constructor passes METERED_YES for these types.
        this(matchRule, subscriberId, matchSubscriberIds, subId, matchSubIds, networkId,
    ...
    public NetworkTemplate(int matchRule, String subscriberId, String[] matchSubscriberIds,
            int subId, int[] matchSubIds, String networkId, int metered, int roaming,
            int defaultNetwork, int subType, int oemManaged) {
        this(matchRule, subscriberId, matchSubscriberIds, subId, matchSubIds, networkId, metered,
                roaming, defaultNetwork, subType, oemManaged, SUBSCRIBER_ID_MATCH_RULE_EXACT);
    }
    ...
    private NetworkTemplate(Parcel in) {
        ...
        mSubId = in.readInt();
        mMatchSubIds = in.createIntArray();
    }
    ...
    public void writeToParcel(Parcel dest, int flags) {
        ...
        dest.writeIntArray(mMatchSubIds);
    }
    ...
    public String toString() {
        ...
        if (mMatchSubIds != null) {
            builder.append(", matchSubIds=").append(Arrays.toString(mMatchSubIds));
        }
    ...
    public int hashCode() {
        return Objects.hash(mMatchRule, mSubscriberId, mNetworkId, mMetered, mRoaming,
                mDefaultNetwork, mSubType, mOemManaged, mSubscriberIdMatchRule, mSubId);
    }
    ...
    public boolean isPersistable() {
        switch (mMatchRule) {
            ...
            case MATCH_WIFI_WILDCARD:
                return false;
            case MATCH_CARRIER:
                return (mSubscriberId != null) || (mSubId > 0);
            case MATCH_WIFI:
    ...
    /**
     * Check if this template matches {@code subId}. Returns true if this
     * template was created with {@code SUBSCRIBER_ID_MATCH_RULE_ALL}, or with a
     * {@code mMatchSubIds} array that contains {@code subId}.
     */
    public boolean matchesSubId(@Nullable int subId) {
        return mSubscriberIdMatchRule == SUBSCRIBER_ID_MATCH_RULE_ALL
                || ArrayUtils.contains(mMatchSubIds, subId);
    }
    ...
    private boolean matchesMobile(NetworkIdentity ident) {
        if (ident.mType == TYPE_WIMAX) {
            ...
            return ident.mType == TYPE_MOBILE && ((!ArrayUtils.isEmpty(mMatchSubscriberIds)
                    ...
                    || (!ArrayUtils.isEmpty(mMatchSubIds)
                    && ArrayUtils.contains(mMatchSubIds, ident.mSubId)))
     ...
     */
    @UnsupportedAppUsage
    public static NetworkTemplate normalize(NetworkTemplate template, String[] mergedSubscriber,
            int[] mergedSubId) {
        return normalize(template, Arrays.<String[]>asList(mergedSubscriber),
                Arrays.<int[]>asList(mergedSubId));
    }
     ...
     * A, but also matches B.
     */
    public static NetworkTemplate normalize(NetworkTemplate template,
            List<String[]> mergedSubscriberList, List<int[]> mergedSubIdList) {
        // Now there are several types of network which uses SubscriberId to store network
        // information. For instances:
        // The TYPE_WIFI with subscriberId means that it is a merged carrier wifi network.
        // The TYPE_CARRIER means that the network associate to specific carrier network.

        if (template.mSubscriberId == null && template.mSubId == 0) return template;

        if (template.mSubId > 0 && mergedSubIdList != null) {
            for (int[] merged : mergedSubIdList) {
                if (ArrayUtils.contains(merged, template.mSubId)) {
                    // Requested template subscriber is part of the merge group; return
                    // a template that matches all merged subscribers.
                    return new NetworkTemplate(template.mMatchRule, null, null, merged[0], merged,
                            template.mNetworkId);
                }
            }
        }

        if (template.mSubscriberId != null && mergedSubscriberList != null) {
            for (String[] merged : mergedSubscriberList) {
                if (ArrayUtils.contains(merged, template.mSubscriberId)) {
                    // Requested template subscriber is part of the merge group; return
                    // a template that matches all merged subscribers.
                    return new NetworkTemplate(template.mMatchRule, merged[0], merged, 0, null,
                            template.mNetworkId);
                }
            }
        }
    ...
    public static NetworkTemplate getNetworkTemplateFromBackup(DataInputStream in)
        ...
        return new NetworkTemplate(matchRule, subscriberId, 0 /* subId */, networkId);
    }
}
