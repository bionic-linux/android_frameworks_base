...
public final class IpSecManager {
     ...
     * of graceful shutdown may result in the shutdown sequence failing to complete. As such,
     * applications requiring graceful shutdown MUST close the socket prior to deactivating the
     * applied transform. By default, socket closure is performed asynchronously (in batches), and
     * the returning of a close function does not guarantee shutdown of a socket. Setting an
     * SO_LINGER timeout results in socket closure being performed synchronously, and is sufficient
     * to ensure shutdown.
     *
     ...
     * SO_LINGER option set is closed, the delayed/batched FIN packets will be dropped.
     *
     ...
     * Specifically, if the transform is deactivated (by calling {@link IpSecTransform#close()}),
     * prior to the socket being closed, the standard [FIN - FIN/ACK - ACK], or the reset [RST]
     * packets are dropped due to the lack of a valid Transform. Similarly, if a socket without
     * the SO_LINGER option set is closed, the delayed/batched FIN packets will be dropped.
     *
     * <h4>Rekey Procedure</h4>
     *
     * <p>When applying a new tranform to a socket, the previous transform will be removed. However,
