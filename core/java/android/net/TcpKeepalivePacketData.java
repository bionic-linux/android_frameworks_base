...
public class TcpKeepalivePacketData extends KeepalivePacketData implements Parcelable {
    ...
    public static TcpKeepalivePacketData tcpKeepalivePacket(
            ...
            int sequence, int ack) throws InvalidPacketException {
        final byte[] packet;
        if ((srcAddress instanceof Inet4Address) && (dstAddress instanceof Inet4Address)) {
            packet = buildV4Packet(srcAddress, srcPort, dstAddress, dstPort, sequence, ack);
        } else if ((srcAddress instanceof Inet6Address) && (dstAddress instanceof Inet6Address)) {
            packet = buildV6Packet(srcAddress, srcPort, dstAddress, dstPort, sequence, ack);
        } else {
            packet = null;
            throw new InvalidPacketException(SocketKeepalive.ERROR_INVALID_IP_ADDRESS);
        }

        return new TcpKeepalivePacketData(srcAddress, srcPort, dstAddress,
                dstPort, packet, sequence, ack);
    }
