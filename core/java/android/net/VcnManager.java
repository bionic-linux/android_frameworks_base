 ...
 *
 * <p>The VCN can be configured to expose one or more Networks (see {@link VcnConfig}), each with
 * different capabilities, allowing for APN virtualization.
 *
 * <p>Upon failure, the VCN will attempt to retry a number of times (per {@link VcnTunnelConfig}).
 * If a tunnel fails to connect after a system-determined timeout, the VCN Safe Mode (see below)
 * will be entered.
 *
