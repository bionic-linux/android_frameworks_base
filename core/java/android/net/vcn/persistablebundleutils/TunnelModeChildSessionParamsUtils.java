...
public final class TunnelModeChildSessionParamsUtils {
    ...
    private static class ConfigTypeValuePair {
        ...
        ConfigTypeValuePair(TunnelModeChildConfigRequest config) {
        ...
        ConfigTypeValuePair(PersistableBundle in) {
    ...
    public static PersistableBundle toPersistableBundle(TunnelModeChildSessionParams params) {
        ...
        final List<ConfigTypeValuePair> reqList = new ArrayList<>();
        for (TunnelModeChildConfigRequest req : params.getConfigurationRequests()) {
            reqList.add(new ConfigTypeValuePair(req));
        }
        final PersistableBundle configReqListBundle =
                PersistableBundleUtils.fromList(reqList, ConfigTypeValuePair::toPersistableBundle);
    ...
    public static TunnelModeChildSessionParams fromPersistableBundle(
        ...
        final List<ConfigTypeValuePair> reqList =
                PersistableBundleUtils.toList(configReqListBundle, ConfigTypeValuePair::new);

        boolean hasIpv4AddressReq = false;
        boolean hasIpv4NetmaskReq = false;
        for (ConfigTypeValuePair req : reqList) {
            switch (req.type) {
                case ConfigTypeValuePair.TYPE_IPV4_ADDRESS:
                    ...
                    break;
                case ConfigTypeValuePair.TYPE_IPV6_ADDRESS:
                    if (req.address == null) {
                    ...
                    break;
                case ConfigTypeValuePair.TYPE_IPV4_NETMASK:
        ...
        if (hasIpv4AddressReq != hasIpv4NetmaskReq) {
            Log.w(
                    TAG,
                    String.format(
                            "IPv4 address request and IPv4 netmask request should be either both"
                                    + " exist or both absent. Found hasIpv4AddressReq exists? %b,"
