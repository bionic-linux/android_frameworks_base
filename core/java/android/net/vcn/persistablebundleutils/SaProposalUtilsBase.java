...
abstract class SaProposalUtilsBase {
    static final String IS_IKE_PROPOSAL_KEY = "IS_IKE_PROPOSAL_KEY";
    ...
    static class EncryptionAlgoKeyPair {
        ...
        EncryptionAlgoKeyPair(int encryptionAlgo, int keyLen) {
            this.encryptionAlgo = encryptionAlgo;
            this.keyLen = keyLen;
        }

        EncryptionAlgoKeyPair(PersistableBundle in) {
     ...
     */
    @NonNull
    static PersistableBundle toPersistableBundle(SaProposal proposal, boolean isIkeProposal) {
        final PersistableBundle result = new PersistableBundle();

        result.putBoolean(IS_IKE_PROPOSAL_KEY, isIkeProposal);

        final List<EncryptionAlgoKeyPair> encryptAlgoKeyPairs = new ArrayList<>();
        for (Pair<Integer, Integer> pair : proposal.getEncryptionAlgorithms()) {
            encryptAlgoKeyPairs.add(new EncryptionAlgoKeyPair(pair.first, pair.second));
        }
        final PersistableBundle encryptionBundle =
                PersistableBundleUtils.fromList(
                        encryptAlgoKeyPairs, EncryptionAlgoKeyPair::toPersistableBundle);
