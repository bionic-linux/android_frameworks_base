...
public class NetworkPolicyManager {
    ...
    public static class NetworkPolicyCallbackProxy extends Listener {
        ...
        public void onBlockedReasonChanged(int uid, @BlockedReason int oldBlockedReason,
            ...
            final boolean oldBlocked = isUidBlocked(oldBlockedReason, false);
            final boolean newBlocked = isUidBlocked(newBlockedReason, false);
            final boolean oldBlockedOnMetered = isUidBlocked(oldBlockedReason, true);
            final boolean newBlockedOnMetered = isUidBlocked(newBlockedReason, true);
            if (oldBlocked != newBlocked || oldBlockedOnMetered != newBlockedOnMetered) {
                dispatchOnUidBlockedStatusChanged(mExecutor, mCallback, uid,
                        newBlocked, newBlockedOnMetered, newBlockedReason);
            }
            if (oldBlockedReason != newBlockedReason) {
                dispatchOnUidBlockedReasonChanged(mExecutor, mCallback, uid,
                        newBlocked, newBlockedOnMetered, newBlockedReason);
            }
        }
    }
    ...
    private static void dispatchOnUidBlockedStatusChanged(@Nullable Executor executor,
            @NonNull NetworkPolicyCallback callback, int uid, boolean blocked,
            boolean blockedOnMetered, @BlockedReason int blockedReason) {
        if (executor == null) {
            callback.onUidBlockedStatusChanged(uid, blocked, blockedOnMetered, blockedReason);
        } else {
            executor.execute(PooledLambda.obtainRunnable(callback::onUidBlockedStatusChanged,
                    uid, blocked, blockedOnMetered, blockedReason));
        }
    }
    ...
    private static void dispatchOnUidBlockedReasonChanged(@Nullable Executor executor,
            ...
            boolean blockedOnMetered, @BlockedReason int blockedReason) {
        if (executor == null) {
            callback.onUidBlockedReasonChanged(uid, blocked, blockedOnMetered, blockedReason);
        } else {
