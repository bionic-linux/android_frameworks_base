...
public class NetworkPolicyManager {
    ...
    /**
     * Flag to indicate that an app is subject to Battery saver restrictions that could
     * result in its network access blocked.
     *
     * @hide
     */
    @SystemApi(client = SystemApi.Client.MODULE_LIBRARIES)
    ...
    /**
     * Flag to indicate that an app is subject to Doze restrictions that could
     * result in its network access blocked.
     *
     * @hide
     */
    ...
    /**
     * Flag to indicate that an app is subject to App Standby restrictions that could
     * result in its network access blocked.
     *
     * @hide
     */
    ...
    /**
     * Flag to indicate that an app is subject to Restricted mode restrictions that could
     * result in its network access blocked.
     *
     * @hide
     */
    ...
    /**
     * Flag to indicate that an app is subject to Data saver restrictions that could
     * result in its metered network access blocked.
     *
     * @hide
     */
    @SystemApi(client = SystemApi.Client.MODULE_LIBRARIES)
    ...
    /**
     * Flag to indicate that an app is subject to user restrictions that could
     * result in its metered network access blocked.
     *
     * @hide
     */
    ...
    /**
     * Flag to indicate that an app is subject to Device admin restrictions that could
     * result in its metered network access blocked.
     *
     * @hide
     */
    ...
    /** @hide */
    public static final int ALLOWED_REASON_RESTRICTED_MODE_EXEMPTED = 1 << 3;
    /** @hide */
     ...
     * corresponding to it.
     *
     * {@code blockedReasons} would be a bitwise {@code OR} combination of the following constants:
     * <ol>
     *     <li>{@link #BLOCKED_REASON_NONE}</li>
     *     <li>{@link #BLOCKED_REASON_BATTERY_SAVER}</li>
     *     <li>{@link #BLOCKED_REASON_DOZE}</li>
     *     <li>{@link #BLOCKED_REASON_APP_STANDBY}</li>
     *     <li>{@link #BLOCKED_REASON_RESTRICTED_MODE}</li>
     *     <li>{@link #BLOCKED_METERED_REASON_DATA_SAVER}</li>
     *     <li>{@link #BLOCKED_METERED_REASON_USER_RESTRICTED}</li>
     *     <li>{@link #BLOCKED_METERED_REASON_ADMIN_DISABLED}</li>
     * </ol>
     *
    ...
    public static boolean isUidBlocked(@BlockedReason int blockedReasons, boolean meteredNetwork) {
        ...
        if (meteredNetwork) {
            final int blockedOnMeteredNetworkReason =
    ...
    public interface NetworkPolicyCallback {
        /**
         * Called when the blocked status of an UID changes.
         *
         ...
         */
        @SystemApi(client = SystemApi.Client.MODULE_LIBRARIES)
        default void onUidBlockedStatusChanged(int uid, @BlockedReason int blockedReasons) {}
    ...
    public static class NetworkPolicyCallbackProxy extends Listener {
        ...
        public void onBlockedReasonChanged(int uid, @BlockedReason int oldBlockedReasons,
            ...
            final boolean oldBlocked = isUidBlocked(oldBlockedReasons, false);
            final boolean newBlocked = isUidBlocked(newBlockedReasons, false);
            final boolean oldBlockedOnMetered = isUidBlocked(oldBlockedReasons, true);
            final boolean newBlockedOnMetered = isUidBlocked(newBlockedReasons, true);
            if (oldBlocked != newBlocked || oldBlockedOnMetered != newBlockedOnMetered) {
                dispatchOnUidBlockedStatusChanged(mExecutor, mCallback, uid, newBlockedReasons);
            }
    ...
    private static void dispatchOnUidBlockedStatusChanged(@Nullable Executor executor,
            @NonNull NetworkPolicyCallback callback, int uid, @BlockedReason int blockedReasons) {
        if (executor == null) {
            callback.onUidBlockedStatusChanged(uid, blockedReasons);
        } else {
            executor.execute(PooledLambda.obtainRunnable(callback::onUidBlockedStatusChanged,
                    uid, blockedReasons));
        }
    }
    ...
    private static void dispatchOnUidBlockedReasonChanged(@Nullable Executor executor,
        ...
        if (executor == null) {
            ...
            executor.execute(PooledLambda.obtainRunnable(callback::onUidBlockedReasonChanged,
                    uid, blockedReasons));
        }
    }
