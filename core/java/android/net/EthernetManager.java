...
public class EthernetManager {
    ...
    private final ArrayList<ListenerExecutor> mListeners = new ArrayList<>();
    private final IEthernetServiceListener.Stub mServiceListener =
            new IEthernetServiceListener.Stub() {
                @Override
                public void onAvailabilityChanged(String iface, boolean isAvailable) {
                    synchronized (mListeners) {
                        for (ListenerExecutor executor : mListeners) {
                            executor.executor.execute(() ->
                                    executor.listener.onAvailabilityChanged(iface, isAvailable));
                        }
                    }
                }
            };
    ...
    private static class ListenerExecutor {
        ...
        private ListenerExecutor(@NonNull Executor executor, @NonNull Listener listener) {
    ...
    public void addListener(@NonNull Listener listener, @NonNull Executor executor) {
        if (listener == null || executor == null) {
            throw new IllegalArgumentException("listener and executor must not be null");
        }
        synchronized (mListeners) {
            mListeners.add(new ListenerExecutor(executor, listener));
    ...
    public void removeListener(@NonNull Listener listener) {
        ...
        synchronized (mListeners) {
            mListeners.removeIf(l -> Objects.equals(l.listener, listener));
