...
/**
 * Base class for network providers such as telephony or Wi-Fi. NetworkProviders connect to networks
 * and register them with the core network stack by creating {@link NetworkAgent}s. The networks can
 * then be used to provide connectivity to apps and can be interacted with via networking APIs such
 * as {@link ConnectivityManager}.
 *
 ...
 * best (highest-scoring) network for a given request is generally not used by the system, and torn
 * down.
 *
 * @hide
 */
...
public class NetworkProvider {
    /** The ID of all providers that are not currently registered. */
    ...
    /** The next provider ID to allocate. */
    private static final AtomicInteger sNextProviderId = new AtomicInteger(1);

    /**
     * Returns the next provider ID.
     * @hide only used by ConnectivityService.
     */
    public static final int nextProviderId() {
        return sNextProviderId.getAndIncrement();
    }
