...
public abstract class NetworkStatsProviderBase {
     ...
     * minute, a late response might cause the system failed to record the given
     * statistics, and results in data lost.
     *
     * @param token the identifier used by the system to associate with {@link NetworkStats}.
     *              Note that custom implementation of providers MUST report locally stored token
     *              with current {@link NetworkStats} via
     *              {@link NetworkStatsProviderCallback#onStatsUpdated}
     *              before updating the given token to local variable.
     */
     ...
     * upstream interface. If needed, the custom implementation should block all egress packets on
     * the {@code iface} that associated with the provider, when the
