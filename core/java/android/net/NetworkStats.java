        ...
        for (int i = 0; i < size; i++) {
            ...
            if (Objects.equals(iface[i], tunIface) && uid[i] != tunUid) {
                if (tunIfaceTotal.rxBytes > 0) {
                    tmpEntry.rxBytes = (long) (rxBytes[i] * ((float) pool.rxBytes / tunIfaceTotal.rxBytes));
                } else {
                    tmpEntry.rxBytes = 0;
                }
                if (tunIfaceTotal.rxPackets > 0) {
                    tmpEntry.rxPackets = (long) (rxPackets[i] * ((float) pool.rxPackets / tunIfaceTotal.rxPackets));
                } else {
                    tmpEntry.rxPackets = 0;
                }
                if (tunIfaceTotal.txBytes > 0) {
                    tmpEntry.txBytes = (long) (txBytes[i] * ((float) pool.txBytes / tunIfaceTotal.txBytes));
                } else {
                    tmpEntry.txBytes = 0;
                }
                if (tunIfaceTotal.txPackets > 0) {
                    tmpEntry.txPackets = (long) (txPackets[i] * ((float) pool.txPackets / tunIfaceTotal.txPackets));
                } else {
