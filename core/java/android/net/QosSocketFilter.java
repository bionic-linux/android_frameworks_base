...
public final class QosSocketFilter extends QosFilter implements Parcelable {
     ...
     *
     *  Note: even though the API surface doesn't support UDP, a file descriptor could still
     *  be passed in through other means to the system server that is pointing to a UDP socket.
     *  Although an edge case, there's no downside in verifying that the addresses still match.
     *
     * Sent through {@link QosCallback#onError(QosCallbackException)}
     */
