...
public final class LinkProperties implements Parcelable {
    ...
    private int findRouteIndexRegardlessOfMtu(RouteInfo route) {
        for (int i = 0; i < mRoutes.size(); i++) {
            if (mRoutes.get(i).equals(route, false /* compareMtu */)) {
                return i;
            }
        }
        return -1;
     ...
     * with the same properties but not MTU, it will be updated. If the {@link RouteInfo}
    ...
    public boolean addRoute(@NonNull RouteInfo route) {
        ...
        int i = findRouteIndexRegardlessOfMtu(route);
        if (i == -1) {
