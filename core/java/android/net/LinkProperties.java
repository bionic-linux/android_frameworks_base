...
public final class LinkProperties implements Parcelable {
    ...
    public void dumpDebug(@NonNull ProtoOutputStream proto) {
        ...
        mLinkAddresses.forEach(addr -> {
            proto.write(LinkPropertiesProto.LINK_ADDRESSES, String.valueOf(addr));
        });
        mDnses.forEach(dns -> {
            proto.write(LinkPropertiesProto.DNS_SERVERS, String.valueOf(dns));
        });
        proto.write(LinkPropertiesProto.USING_PRIVATE_DNS, mUsePrivateDns);
        ...
        mPcscfs.forEach(pcscf -> {
            proto.write(LinkPropertiesProto.PCSCF_ADDRESSES, String.valueOf(pcscf));
        });
        mValidatedPrivateDnses.forEach(dns -> {
            proto.write(LinkPropertiesProto.VALIDATED_PRIVATE_DNS_SERVERS, String.valueOf(dns));
        });
        ...
        if (mDhcpServerAddress != null) {
            proto.write(LinkPropertiesProto.DHCP_SERVER_ADDRESS, mDhcpServerAddress.toString());
        }
        if (mCaptivePortalApiUrl != null) {
            proto.write(LinkPropertiesProto.CAPTIVE_PORTAL_API_URL,
                    mCaptivePortalApiUrl.toString());
        }
        if (mCaptivePortalData != null) {
            final long cToken = proto.start(LinkPropertiesProto.CAPTIVE_PORTAL_DATA);
            mCaptivePortalData.dumpDebug(proto);
            proto.end(cToken);
        }
        ...
        mRoutes.forEach(route -> {
            long token = proto.start(LinkPropertiesProto.ROUTES);
            route.dumpDebug(proto);
            proto.end(token);
        });
        if (mHttpProxy != null) {
            long token = proto.start(LinkPropertiesProto.PROXY);
            mHttpProxy.dumpDebug(proto);
            proto.end(token);
        }
        if (mNat64Prefix != null) {
