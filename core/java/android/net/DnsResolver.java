...
public final class DnsResolver {
    ...
    private static @NonNull Network getDefaultNetwork() throws ErrnoException {
        if (sConnectivityManager == null) {
            sConnectivityManager = IConnectivityManager.Stub.asInterface(
                    ServiceManager.getService(Context.CONNECTIVITY_SERVICE));
        }

        int netId = NetworkUtils.getBoundNetworkForProcess();
        if (netId != NETID_UNSET) return new Network(netId);
        try {
            Network defaultNetwork = sConnectivityManager.getActiveNetwork();
            if (defaultNetwork != null) return defaultNetwork;
        } catch (RemoteException ignore) {
        }
        throw new ErrnoException("resNetworkQuery", ENONET);
    }
    ...
    public void query(@Nullable Network network, @NonNull String domain, @QueryFlag int flags,
        ...
        if (queryIpv6) {
            try {
                queryNetwork = (network != null) ? network : getDefaultNetwork();
        ...
        if (queryIpv4) {
            try {
                queryNetwork = (queryNetwork != null) ? queryNetwork
                        : (network != null) ? network : getDefaultNetwork();
