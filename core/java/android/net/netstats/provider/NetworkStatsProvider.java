...
public abstract class NetworkStatsProvider {
    ...
    /**
     * Store the binder that given by the service when successfully registering. Or set to null
     * to indicate the binder has been unregistered.
     *
     * @hide
     */
    public void setProviderCallbackBinder(@Nullable INetworkStatsProviderCallback binder) {
        mProviderCbBinder = binder;
    }

    /**
     * Get the binder that given by the service when successfully registering. Throws an
     * {@link IllegalStateException} if the provider was never successfully registered.
     *
     * @hide
     */
    @NonNull
    public INetworkStatsProviderCallback getProviderCallbackBinderOrThrow() {
        if (!isRegistered()) {
            throw new IllegalStateException("the provider is never successfully registered");
        }
        return mProviderCbBinder;
    }

    /**
     * Return whether the provider is registered to the service.
     */
    public boolean isRegistered() {
        return mProviderBinder != null;
    }

    /**
     * Notify the system of new network statistics.
     *
     * Send the network statistics recorded since the last call to {@link #statsUpdated}. Must be
     * called within one minute of {@link NetworkStatsProvider#onRequestStatsUpdate(int)}
     ...
     *                 per uid.
     */
    public void statsUpdated(int token, @NonNull NetworkStats ifaceStats,
            @NonNull NetworkStats uidStats) {
        try {
            getProviderCallbackBinderOrThrow().statsUpdated(token, ifaceStats, uidStats);
     ...
     */
    public void alertReached() {
        try {
            getProviderCallbackBinderOrThrow().alertReached();
        } catch (RemoteException e) {
            e.rethrowAsRuntimeException();
        }
    }
    ...
    public void limitReached() {
        try {
            getProviderCallbackBinderOrThrow().limitReached();
        } catch (RemoteException e) {
            e.rethrowAsRuntimeException();
        }
    }
     ...
     * {@link NetworkStatsProvider#statsUpdated} within one minute. Responding
     ...
     * {@link NetworkStatsProvider#limitReached()}.
     *
     ...
     * MUST call {@link NetworkStatsProvider#alertReached()} when {@code quotaBytes} bytes
