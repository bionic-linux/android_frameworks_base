...
public class DnsUtils {
    ...
    private static final Comparator<SortableAddress> sRfc6727Comparator = new Rfc6727Comparator();

    /**
     * Comparator to sort SortableAddress in Rfc6724 style.
     */
    public static class Rfc6727Comparator implements Comparator<SortableAddress> {
    ...
    public static @NonNull List<InetAddress> rfc6724Sort(@Nullable Network network,
        ...
        Collections.sort(sortableAnswerList, sRfc6727Comparator);
    ...
    private static int findLabel(@NonNull InetAddress addr) {
        if (isIpv4Address(addr)) {
            ...
            if (addr.isLoopbackAddress()) {
                return 0;
            } else if (isIpv6AddressV4Mapped(addr)) {
                return 4;
    ...
    private static boolean isIpv6AddressV4Mapped(@NonNull InetAddress addr) {
        if (!isIpv6Address(addr)) return false;
        final ByteBuffer buffer;
        try {
            buffer = ByteBuffer.wrap(addr.getAddress());
        } catch (BufferUnderflowException e) {
            return false;
        }
        return (BitUtils.uint32(buffer.getInt()) == 0) && (BitUtils.uint32(buffer.getInt()) == 0)
                && (BitUtils.uint32(buffer.getInt()) == 0xffff0000);
    }
    ...
    private static int compareIpv6PrefixMatchLen(@NonNull InetAddress srcAddr,
        ...
        for (int i = 0; i < dstByte.length; ++i) {
            ...
            return i * CHAR_BIT + (numberOfLeadingZeros(x) - 24);
        }
