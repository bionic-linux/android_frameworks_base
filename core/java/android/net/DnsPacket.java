...
public abstract class DnsPacket {
    public class DnsHeader {
        ...
        DnsHeader(ByteBuffer buf) throws IndexOutOfBoundsException {
            ...
            mFlags = buf.getShort() & 0xFFFF;
            mRcode = mFlags & 0xF;
            mSectionCount = new int[4];
            for (int i = 0; i < 4; ++i) {
                mSectionCount[i] = buf.getShort() & 0xFFFF;
            }
        }
    ...
    public class DnsSection {
        ...
        DnsSection(int sectionType, ByteBuffer buf)
            ...
            mName = parseName(buf);
            mType = buf.getShort() & 0xFFFF;
            mClass = buf.getShort() & 0xFFFF;

            if (sectionType != QDSECTION) {
                mTTL = buf.getInt() & 0xFFFFFFFF;
        ...
        private String labelToString(byte[] label) {
            ...
            for (int i = 0; i < label.length; ++i) {
                ...
                } else if (b == 0x22 || b == 0x2e || b == 0x3b || b == 0x5c
                        || b == 0x28 || b == 0x29 || b == 0x40 || b == 0x24) {
                    sb.append('\\');
                    sb.append((char) b);
                } else {
                    sb.append((char) b);
