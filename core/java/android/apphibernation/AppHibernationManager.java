...
public final class AppHibernationManager {
    ...
    /**
     * Returns true if the package is hibernating for the given user, false otherwise.
     *
     * @hide
     */
    @SystemApi
    ...
    /**
     * Set hibernation state for a package for a given user.
     *
     * @hide
     */
    @SystemApi
    public void setHibernationState(@NonNull String packageName, boolean isHibernating) {
        try {
            mIAppHibernationService.setHibernationState(packageName, isHibernating,
                    mContext.getUserId());
        } catch (RemoteException e) {
            throw e.rethrowFromSystemServer();
        }
