...
public final class ServiceManager {
     ...
     */
    @SystemApi(client = SystemApi.Client.MODULE_LIBRARIES)
    @NonNull public static IBinder waitForDeclaredService(@NonNull String name) {
        if (!isDeclared(name)) {
            throw new IllegalArgumentException("Service not declared: " + name);
        }
        IBinder binder = waitForService(name);
        if (binder == null) {
