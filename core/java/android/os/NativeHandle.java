...
public final class NativeHandle implements Closeable {
    ...
    private static native int dup(int fd);
    private static native void close(int fd) throws java.io.IOException;

    /**
     * Explicitly duplicate NativeHandle (this dups all file descritptors).
     */
    public NativeHandle dup() {
        ArrayList<FileDescriptor> descriptors = new ArrayList<>(mFds.size());
        for (FileDescriptor fd : mFds) {
            FileDescriptor descriptor = new FileDescriptor();
            descriptor.setInt$(dup(fd.getInt$()));
            descriptors.add(descriptor);
        }
        return new NativeHandle(descriptors, mInts.clone(), true);
    }
