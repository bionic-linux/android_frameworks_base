...
public final class ParcelableHolder implements Parcelable {
    ...
    private int mStability = -1;

    public ParcelableHolder(int stabilty) {
        mStability = stabilty;
    }
    ...
    private static boolean canContain(int containerStability, int itemStability) {
        // Container can contain only item which is more stable than the container.
        return containerStability <= itemStability;
    }

    private static int getStabilityFromParcelable(Parcelable p) {
        try {
            Field stabilityField = p.getClass().getDeclaredField("_aidl_stability");
            return (int) stabilityField.get(p);
        } catch (Exception e) {
            e.printStackTrace();
            // TODO(b/156309715) using public API instead of hardcoded value when it is available.
            return 0;
        }
    }
    ...
    public boolean setParcelable(@Nullable Parcelable p) {
        if (p != null && !canContain(mStability, getStabilityFromParcelable(p))) {
            return false;
        }
        mParcelable = p;
        if (mParcel != null) {
    ...
    public void readFromParcel(@NonNull Parcel parcel) {
        ...
        parcel.setDataPosition(dataStartPos + dataSize);
    }
    ...
    public void writeToParcel(@NonNull Parcel parcel, int flags) {
        ...
        parcel.setDataPosition(parcel.dataPosition() + dataSize);
    }
