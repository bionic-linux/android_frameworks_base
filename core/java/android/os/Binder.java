...
public class Binder implements IBinder {
    ...
    */
     ...
     * currently being processed.
     *
     * Logs if the current thread is not currently
     * executing an incoming transaction and the calling identity has not been
     * explicitly set with {@link #clearCallingIdentity()} (meaning
     * {@link #getCallingUidOrThrow()} would throw).
     *
     * @hide
     */
    public static final int getCallingUidOrLog(String message) {
        if (!isDirectlyHandlingTransaction() && !hasExplicitIdentity()) {
            Slog.e(TAG,
