    ...
    */
    ...
    public final @Nullable String getTransactionTraceName(int transactionCode) {
        if (mTransactionTraceNames == null) {
            final int highestId = getMaxTransactionId() == 0 ? TRANSACTION_TRACE_NAME_ID_LIMIT
        ...
        if (index >= mTransactionTraceNames.length()) {
            return null;
        }

        String transactionTraceName = null;
        if (index >= 0) {
            transactionTraceName = mTransactionTraceNames.getAcquire(index);
        } else if (transactionCode == 0) {
            // For non-auto-generated txns, they may start from 0, instead of FIRST_CALL_TRANSACTION
            transactionTraceName = mTransactionTraceNames.getAcquire(transactionCode);
        } else {
            return null;
        }
