...
interface IPlatformCompat
{
     ...
     * Reports that a compatibility change is affecting an app process now.
     *
     * <p>Same as the previous option, except it receives a package name instead of an appInfo
     * object, and finds an app info object based on the package name. Doesn't log to statsd if
     ...
     */
     void reportChangeByPackage(long changeId, in String packageName);
     ...
     * @param packageName The package name of the app in question.
     * @return {@code true} if the change is enabled for the current app.
     * @throws IllegalArgumentException if there is no installed package by that name.
     */
    boolean isChangeEnabledByPackage(long changeId, in String packageName);
}