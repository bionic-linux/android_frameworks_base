...
def merge_zip_file(out_dir, zip):
  """Merge the content of the zip file into out_dir"""
  for filename in zip.namelist():
    path = Path(out_dir, filename)
    if path.exists():
      existing_xml = ET.parse(path)
      with zip.open(filename) as other_file:
...
def main():
  ...
  for zip_filename in zip_filenames:
    with zipfile.ZipFile(zip_filename) as zip:
      merge_zip_file(out_dir, zip_filen)
