...
func createMergedSystemStubs(ctx android.LoadHookContext, modules []string) {
	...
	// Now create the all-non-updatable-modules-system-stubs. We need to split the system stubs
	// into updatable and non-updatable to allow non-updatable modules to contribute to test APIs
	// stubs.
	{
		props := libraryProps{}
		props.Name = proptools.StringPtr("all-non-updatable-modules-system-stubs")
		props.Static_libs = transformArray(non_updatable_modules, "", ".stubs.system")
		props.Sdk_version = proptools.StringPtr("module_current")
		props.Visibility = []string{"//frameworks/base"}
		ctx.CreateModule(java.LibraryFactory, &props)
	}
	// Now merge them into all-modules-system-stubs.
	{
		props := libraryProps{}
		props.Name = proptools.StringPtr("all-modules-system-stubs")
		props.Static_libs = []string{"all-updatable-modules-system-stubs", "all-non-updatable-modules-system-stubs"}
		props.Sdk_version = proptools.StringPtr("module_current")
		props.Visibility = []string{"//frameworks/base"}
		ctx.CreateModule(java.LibraryFactory, &props)
	}
