...
public class NetworkCapabilitiesTest {
        ...
        if (isAtLeastR()) {
            ...
            try {
                final NetworkCapabilities nc2 = new NetworkCapabilities.Builder()
                        .setNetworkSpecifier(specifier)
                        .build();
                fail("The exception should be thrown, the test shouldn't run into this line.");
            } catch (Exception e) {
    ...
    public void testAdministratorUidsAndOwnerUid() {
        ...
        assertNotEquals(10001, nc4.getOwnerUid());
        try {
    ...
    private void testCurrentTransportTypes(NetworkCapabilities nc, int maxTransportType,
    ...
    private void testMultipleTransportTypes() {
        final NetworkCapabilities nc = new NetworkCapabilities();
        assertNoTransport(nc);
        // Test adding multiple transport types.
        for (int i = MIN_TRANSPORT; i <= MAX_TRANSPORT; i++) {
            nc.addTransportType(i);
            testCurrentTransportTypes(nc, i, true /* positiveSequence */);
        }
        // Test removing multiple transport types.
        for (int i = MIN_TRANSPORT; i <= MAX_TRANSPORT; i++) {
            nc.removeTransportType(i);
            testCurrentTransportTypes(nc, i, false /* positiveSequence */);
        }
        assertNoTransport(nc);
        nc.addTransportType(TRANSPORT_WIFI);
