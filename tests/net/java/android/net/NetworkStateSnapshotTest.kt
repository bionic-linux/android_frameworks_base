...
class NetworkStateSnapshotTest {
    ...
    fun testConvertFromNetworkState() {
        val state = NetworkState(
                TYPE_WIFI, TEST_LINK_PROPERTIES, TEST_CAPABILITIES, Network(TEST_NETID))
    ...
    fun testParcelUnparcel() {
        val emptySnapshot = NetworkStateSnapshot(TYPE_NONE, null, null, null)
        val snapshot = NetworkStateSnapshot(
                TYPE_WIFI, TEST_LINK_PROPERTIES, TEST_CAPABILITIES, Network(TEST_NETID))
        assertParcelSane(emptySnapshot, 4)
        assertParcelSane(snapshot, 4)
    }
}
