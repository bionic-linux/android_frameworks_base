...
public class Ikev2VpnProfileBuilderTest {
    ...
    public void testBuildValidProfileWithOptions() throws Exception {
        ...
        assertEquals(profile.name, SESSION_NAME_STRING);
        assertEquals(profile.server, SERVER_ADDR_STRING);
        assertEquals(profile.ipsecIdentifier, IDENTITY_STRING);
        assertEquals(profile.proxy, mProxy);
        assertTrue(profile.isBypassable);
        assertTrue(profile.isMetered);
        assertEquals(profile.maxMtu, TEST_MTU);
        assertTrue(profile.authParamsInline);
    }
    ...
    public void testBuildDigitalSignatureProfile() throws Exception {
        ...
        assertEquals(
                ...
                profile.ipsecCaCert);
        assertEquals(
                Base64.getEncoder().encodeToString(mPrivateKey.getEncoded()), profile.ipsecSecret);
    }
    ...
    public void testBuildPresharedKeyProfile() throws Exception {
        ...
        assertEquals(profile.ipsecSecret, PSK_STRING);
    }
