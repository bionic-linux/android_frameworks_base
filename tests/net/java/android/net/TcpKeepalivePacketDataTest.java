...
public final class TcpKeepalivePacketDataTest {
    ...
    public void testV6TcpKeepalivePacket() {
        final InetAddress srcAddr = InetAddressUtils.parseNumericAddress("2001:4860:4860::8888");
        final InetAddress dstAddr = InetAddressUtils.parseNumericAddress("2001:4860:4860::4444");
        final int srcPort = 1234;
        final int dstPort = 4321;
        final int sequence = 0x11111111;
        final int ack = 0x22222222;
        ...
        // Source IP address.
        byte[] ip = new byte[16];
        buf = ByteBuffer.wrap(packet, 8, 16);
        buf.get(ip);
        assertArrayEquals(ip, srcAddr.getAddress());
        // Destinatin IP address.
        buf = ByteBuffer.wrap(packet, 24, 16);
        buf.get(ip);
        assertArrayEquals(ip, dstAddr.getAddress());

        buf = ByteBuffer.wrap(packet, 40, 12);
        // Source port.
        assertEquals(buf.getShort(), srcPort);
        // Destination port.
        assertEquals(buf.getShort(), dstPort);
