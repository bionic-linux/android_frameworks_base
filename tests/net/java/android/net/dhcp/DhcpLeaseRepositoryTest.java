...
public class DhcpLeaseRepositoryTest {
    ...
    private static final Inet4Address TEST_DEF_ROUTER = parseAddr("192.168.42.247");
    private static final Inet4Address TEST_SERVER_ADDR = parseAddr("192.168.42.241");
    private static final Inet4Address TEST_RESERVED_ADDR = parseAddr("192.168.42.243");
    ...
    private static final Inet4Address TEST_INETADDR_1 = parseAddr("192.168.42.248");
    private static final Inet4Address TEST_INETADDR_2 = parseAddr("192.168.42.249");
    ...
    public void setUp() {
        ...
        mRepo = new DhcpLeaseRepository(TEST_IP_PREFIX, TEST_EXCL_SET, TEST_LEASE_TIME_MS, mClock);
    }
    ...
    public void testAddressExhaustion() throws Exception {
        ...
        try {
            mRepo.getOffer(null, TEST_MAC_2, (Inet4Address) Inet4Address.ANY,
    ...
    public void testUpdateParams_LeaseCleanup() throws Exception {
        // Inside /28:
        final Inet4Address reqAddrIn28 = parseAddr("192.168.42.242");
        final Inet4Address declinedAddrIn28 = parseAddr("192.168.42.245");

        final DhcpLease reqAddrIn28Lease = mRepo.requestLease(
                CLIENTID_UNSPEC, TEST_MAC_1, INET4_ANY, reqAddrIn28, false, HOSTNAME_NONE);
        mRepo.markLeaseUnavailable(declinedAddrIn28);

        // Inside /22, but outside /28:
        final Inet4Address reqAddrIn22 = parseAddr("192.168.42.3");
        final Inet4Address declinedAddrIn22 = parseAddr("192.168.42.4");

        final DhcpLease reqAddrIn22Lease = mRepo.requestLease(
                CLIENTID_UNSPEC, TEST_MAC_3, INET4_ANY, reqAddrIn22, false, HOSTNAME_NONE);
        mRepo.markLeaseUnavailable(declinedAddrIn22);

        // Address reserved in new config
        final Inet4Address reservedAddr = parseAddr("192.168.42.244");
        ...
        // Update from /22 to /28 and add reserved address
    ...
    public void testGetOffer_StableAddress() throws Exception {
        final DhcpLease lease = mRepo.getOffer(CLIENTID_UNSPEC, TEST_MAC_1, INET4_ANY,
                INETADDR_UNSPEC /* relayAddr */, INETADDR_UNSPEC /* reqAddr */, HOSTNAME_NONE);

        // Same lease is offered twice
        final DhcpLease newLease = mRepo.getOffer(CLIENTID_UNSPEC, TEST_MAC_1, INET4_ANY,
                INETADDR_UNSPEC /* relayAddr */, INETADDR_UNSPEC /* reqAddr */, HOSTNAME_NONE);
    ...
    public void testUpdateParams_UsesNewPrefix() throws Exception {
        final IpPrefix newPrefix = new IpPrefix(parseAddr("192.168.123.0"), 24);
        mRepo.updateParams(newPrefix, TEST_EXCL_SET, TEST_LEASE_TIME_MS);

        DhcpLease lease = mRepo.getOffer(CLIENTID_UNSPEC, TEST_MAC_1, INET4_ANY,
    ...
    public void testGetOffer_ExistingLease() throws Exception {
        ...
        DhcpLease offer = mRepo.getOffer(CLIENTID_UNSPEC, TEST_MAC_1,
                INETADDR_UNSPEC, INETADDR_UNSPEC, INETADDR_UNSPEC, HOSTNAME_NONE);
    ...
    public void testGetOffer_DifferentClientId() throws Exception {
        ...
        assertNotEquals(TEST_HOSTNAME_1, offer.getHostname());
    }

    @Test
    public void testGetOffer_RequestedAddress() throws Exception {
        DhcpLease offer = mRepo.getOffer(CLIENTID_UNSPEC, TEST_MAC_1, INET4_ANY, INET4_ANY,
    ...
    public void testGetOffer_RequestedAddressInUse() throws Exception {
        ...
        DhcpLease offer = mRepo.getOffer(CLIENTID_UNSPEC, TEST_MAC_2, INET4_ANY, INET4_ANY,
    ...
    public void testGetOffer_RequestedAddressInvalid() throws Exception {
        final Inet4Address invalidAddr = parseAddr("192.168.42.0");
        DhcpLease offer = mRepo.getOffer(CLIENTID_UNSPEC, TEST_MAC_1, INET4_ANY, INET4_ANY,
    ...
    public void testGetOffer_RequestedAddressOutsideSubnet() throws Exception {
        final Inet4Address invalidAddr = parseAddr("192.168.254.2");
        DhcpLease offer = mRepo.getOffer(CLIENTID_UNSPEC, TEST_MAC_1, INET4_ANY, INET4_ANY,
                invalidAddr, HOSTNAME_NONE);
        assertNotEquals(invalidAddr, offer.getNetAddr());
    }
    ...
    public void testGetOffer_FromInvalidSubnet() throws Exception {
        mRepo.getOffer(CLIENTID_UNSPEC, TEST_MAC_1, parseAddr("192.168.254.2") /* reqSrcAddr */,
            INET4_ANY, INETADDR_UNSPEC, HOSTNAME_NONE);
    }

    @Test(expected = DhcpLeaseRepository.InvalidAddressException.class)
    public void testGetOffer_RelayInInvalidSubnet() throws Exception {
        mRepo.getOffer(CLIENTID_UNSPEC, TEST_MAC_1, INET4_ANY,
                parseAddr("192.168.254.2") /* relayAddr */, INETADDR_UNSPEC, HOSTNAME_NONE);
    }

    @Test
    public void testRequestLease_Selecting() throws Exception {
    ...
    public void testRequestLease_InitRebootWrongSubnet() throws Exception {
        mRepo.requestLease(CLIENTID_UNSPEC, TEST_MAC_1, INET4_ANY,
                parseAddr("192.168.254.2"), false /* sidSet */, HOSTNAME_NONE);
    }
    ...
    public void testRequestLease_RenewingInvalidAddr() throws Exception {
        mRepo.requestLease(CLIENTID_UNSPEC, TEST_MAC_1, parseAddr("192.168.254.2") /* clientAddr */,
    ...
    public void testReleaseLease() throws Exception {
        ...
        DhcpLease lease = mRepo.requestLease(CLIENTID_UNSPEC, TEST_MAC_2, INET4_ANY,
                TEST_INETADDR_1, true /* sidSet */, HOSTNAME_NONE);
    ...
    @Test
    public void testReleaseLease_UnknownLease() throws Exception {
    ...
    public void testReleaseLease_StableOffer() throws Exception {
        final DhcpLease lease = mRepo.getOffer(CLIENTID_UNSPEC, TEST_MAC_1, INET4_ANY,
                INETADDR_UNSPEC /* relayAddr */, INETADDR_UNSPEC /* reqAddr */, HOSTNAME_NONE);
        mRepo.requestLease(
                CLIENTID_UNSPEC, TEST_MAC_1, INET4_ANY, lease.getNetAddr(), true, HOSTNAME_NONE);
        mRepo.releaseLease(CLIENTID_UNSPEC, TEST_MAC_1, lease.getNetAddr());
    ...
    @Test
    public void testMarkLeaseUnavailable() throws Exception {
        final DhcpLease lease = mRepo.getOffer(CLIENTID_UNSPEC, TEST_MAC_1, INET4_ANY,
                INETADDR_UNSPEC /* relayAddr */, INETADDR_UNSPEC /* reqAddr */, HOSTNAME_NONE);

        mRepo.markLeaseUnavailable(lease.getNetAddr());
    ...
    @Test
    public void testMarkLeaseUnavailable_UsedIfOutOfAddresses() throws Exception {
        // Use a /28 to quickly run out of addresses
        mRepo.updateParams(new IpPrefix(TEST_SERVER_ADDR, 28), TEST_EXCL_SET, TEST_LEASE_TIME_MS);

        mRepo.markLeaseUnavailable(TEST_INETADDR_1);
        mRepo.markLeaseUnavailable(TEST_INETADDR_2);

        // /28 should have 16 addresses, 14 w/o the first/last, 11 w/o excluded addresses
        requestAddresses((byte)9);

        // Last 2 addresses: addresses marked unavailable should be used
