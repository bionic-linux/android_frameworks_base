...
public class IpSecServiceRefcountedResourceTest {
    ...
    @Test
    public void testConstructorWithDependencies() throws RemoteException {
    ...
    public void testMultipleCallsToCleanupAndRelease() throws RemoteException {
        ...
        // Verify calling cleanupAndRelease multiple times does not trigger any other cleanup methods
        refcountedResource.cleanupAndRelease();
        assertResourceState(refcountedResource, -1, 1, 1, 1, 1);
    ...
    public void fuzzTest() throws RemoteException {
        ...
        for(int i = 0; i < 100; i++){
            // Choose a random number of dependencies from the existing list
            int numDependencies = ThreadLocalRandom.current().nextInt(0, resources.size() + 1);

            // Build a (random) list of dependencies
            Set<RefcountedResource<IResource>> dependencies = new HashSet<>();
            for(int j = 0; j < numDependencies; j++){
                int dependencyIndex = ThreadLocalRandom.current().nextInt(0, resources.size());
                dependencies.add(resources.get(dependencyIndex));
            }

            RefcountedResource<IResource> newRefcountedResource = getTestRefcountedResource(dependencies.toArray(new RefcountedResource[dependencies.size()]));
            resources.add(newRefcountedResource);
        }

        // Cleanup all resources in a random order
        List<RefcountedResource<IResource>> clonedResources = new ArrayList<>(resources); // shallow copy
