...
public class NetworkMonitorTest {
    ...
    public void setUp() throws IOException {
        ...
        when(mDependencies.getSetting(any(), eq(Settings.Global.CAPTIVE_PORTAL_USE_HTTPS), anyInt()))
                .thenReturn(1);
        when(mDependencies.getSetting(any(), eq(Settings.Global.CAPTIVE_PORTAL_HTTP_URL), anyString()))
                .thenReturn(TEST_HTTP_URL);
        when(mDependencies.getSetting(any(), eq(Settings.Global.CAPTIVE_PORTAL_HTTPS_URL), anyString()))
    ...
    public void testIsCaptivePortal_HttpsProbeFailedHttpSuccessNotUsed() throws IOException {
        ...
        when(mRandom.nextInt()).thenReturn(0); // Use the first fallback URL
    ...
    public void testIsCaptivePortal_FallbackProbeIsPortal() throws IOException {
        ...
        when(mRandom.nextInt()).thenReturn(0); // First fallback URL
    ...
    public void testIsCaptivePortal_InvalidUrlSkipped() throws IOException {
        ...
        setStatus(mHttpConnection, 500);
        setPortal302(mOtherFallbackConnection);
        // There should be only one fallback URL left
