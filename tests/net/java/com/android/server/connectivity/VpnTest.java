...
public class VpnTest {
    ...
    static final String EGRESS_IFACE = "DUMMY0";
    ...
    public void startRacoon(final String serverAddr, final String expectedAddr)
        ...
        try {
            ...
            deps.mBarrier.await(10, TimeUnit.SECONDS);
            // The production code is blocked waiting for the next barrier checkpoint.
            ...
            deps.mBarrier.reset(); // this breaks the barrier, always unblocking the runner
    ...
    private static final class TestDeps extends Vpn.Dependencies {
        ...
        // Synchronization between the test and the production code.
        // ConditonVariable.block calls Object#wait, which clears the interrupt
        // flag and throws InterruptedException. Android's implementation
        // "helpfully" catches and ignores it inside CV#block() >.> This makes
        // ConditionVariable very difficult to use in such a way as to ensure the
        // vpn runner is interrupted, so use CyclicBarrier instead.
        public final CyclicBarrier mBarrier = new CyclicBarrier(2 /* parties */);
        ...
        public void sendArgumentsToDaemon(
                ...
                final ThrowingConsumer interruptChecker) throws Exception {
        ...
        public void checkpoint() {
            try {
                ...
                mBarrier.await(10, TimeUnit.SECONDS);
                // The test is now checking state. It will release this thread when it's
                // done by breaking the barrier.
                mBarrier.await(10, TimeUnit.SECONDS);
            } catch (BrokenBarrierException | InterruptedException | TimeoutException e) {
                // For the purposes of this test, any of these just means the thread
                // should be interrupted with an exception. The runner will catch this,
                // clean up after itself and exit.
                throw new RuntimeException(e);
            }
        }
