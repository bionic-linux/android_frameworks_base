...
public class PermissionMonitorTest {
    ...
    private boolean isParticularPackage(String partition, int targetSdkVersion, int uid) {
        final PackageInfo packageInfo =
                packageInfoWithPermissions(REQUESTED_PERMISSION_GRANTED, new String[0], partition);
        packageInfo.applicationInfo.targetSdkVersion = targetSdkVersion;
        packageInfo.applicationInfo.uid = uid;
        return mPermissionMonitor.isParticularPackage(packageInfo);
    }
    ...
    @Test
    public void testIsParticularPackage() {
        doReturn(VERSION_P).when(mDeps).getDeviceFirstSdkInt();
        assertTrue(isParticularPackage(PARTITION_SYSTEM, VERSION_P, SYSTEM_UID));
        assertTrue(isParticularPackage(PARTITION_VENDOR, VERSION_P, SYSTEM_UID));
        assertFalse(isParticularPackage(PARTITION_SYSTEM, VERSION_P, MOCK_UID1));
        assertTrue(isParticularPackage(PARTITION_VENDOR, VERSION_P, MOCK_UID1));
        assertTrue(isParticularPackage(PARTITION_SYSTEM, VERSION_Q, SYSTEM_UID));
        assertTrue(isParticularPackage(PARTITION_VENDOR, VERSION_Q, SYSTEM_UID));
        assertFalse(isParticularPackage(PARTITION_SYSTEM, VERSION_Q, MOCK_UID1));
        assertFalse(isParticularPackage(PARTITION_VENDOR, VERSION_Q, MOCK_UID1));

        doReturn(VERSION_Q).when(mDeps).getDeviceFirstSdkInt();
        assertFalse(isParticularPackage(PARTITION_SYSTEM, VERSION_P, SYSTEM_UID));
        assertTrue(isParticularPackage(PARTITION_VENDOR, VERSION_P, SYSTEM_UID));
        assertFalse(isParticularPackage(PARTITION_SYSTEM, VERSION_P, MOCK_UID1));
        assertTrue(isParticularPackage(PARTITION_VENDOR, VERSION_P, MOCK_UID1));
        assertFalse(isParticularPackage(PARTITION_SYSTEM, VERSION_Q, SYSTEM_UID));
        assertFalse(isParticularPackage(PARTITION_VENDOR, VERSION_Q, SYSTEM_UID));
        assertFalse(isParticularPackage(PARTITION_SYSTEM, VERSION_Q, MOCK_UID1));
        assertFalse(isParticularPackage(PARTITION_VENDOR, VERSION_Q, MOCK_UID1));
    }
