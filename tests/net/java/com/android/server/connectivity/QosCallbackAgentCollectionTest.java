...
public class QosCallbackAgentCollectionTest {
    ...
    public void testSimpleRemoves() {
        ...
        assertNull(mConnections.get(mNetworkAgentInfoA));
    }

    @Test
    public void testNetworkAgentInfoRemove() {
        assertEquals(2, mConnections.get(mNetworkAgentInfoA).length);
        assertEquals(1, mConnections.get(mNetworkAgentInfoB).length);

        mConnections.remove(mNetworkAgentInfoA);
        assertNull(mConnections.get(mNetworkAgentInfoA));
        assertEquals(1, mConnections.get(mNetworkAgentInfoB).length);
        assertNull(mConnections.get(mConnectionA1.getAgentCallbackId()));
        assertNull(mConnections.get(mConnectionA2.getBinder()));

        mConnections.remove(mNetworkAgentInfoB);
        assertNull(mConnections.get(mNetworkAgentInfoB));
    }
