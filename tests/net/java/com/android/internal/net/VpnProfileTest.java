...
public class VpnProfileTest {
    ...
    public void testDefaults() throws Exception {
        ...
        assertTrue(p.allowedAlgorithms != null && p.allowedAlgorithms.isEmpty());
        assertFalse(p.isBypassable);
        assertFalse(p.isMetered);
        assertEquals(1400, p.maxMtu);
        assertFalse(p.authParamsInline);
    }
    ...
    @Test
    public void testParcelInvalidAlgorithmValueDelimiter() {
        final VpnProfile profile = getSampleIkev2Profile(DUMMY_PROFILE_KEY);
        profile.allowedAlgorithms = new ArrayList<>(); // Arrays.asList produces an AbstractList
        profile.allowedAlgorithms.add("test" + VpnProfile.VALUE_DELIMITER + "test");

        try {
            profile.writeToParcel(mock(Parcel.class), 0);
            fail("Expected failure in encoding due to value separator in algorithm name");
        } catch (IllegalArgumentException expected) {
        }
    }

    @Test
    public void testParcelInvalidAlgorithmListDelimiter() {
        final VpnProfile profile = getSampleIkev2Profile(DUMMY_PROFILE_KEY);
        profile.allowedAlgorithms = new ArrayList<>(); // Arrays.asList produces an AbstractList
        profile.allowedAlgorithms.add("test" + VpnProfile.LIST_DELIMITER + "test");
    ...
    @Test
    public void testEncodeInvalidAlgorithmValueDelimiter() {
        final VpnProfile profile = getSampleIkev2Profile(DUMMY_PROFILE_KEY);
        profile.allowedAlgorithms = new ArrayList<>(); // Arrays.asList produces an AbstractList
        profile.allowedAlgorithms.add("test" + VpnProfile.VALUE_DELIMITER + "test");
    ...
    @Test
    public void testEncodeInvalidAlgorithmListDelimiter() {
        final VpnProfile profile = getSampleIkev2Profile(DUMMY_PROFILE_KEY);
        profile.allowedAlgorithms = new ArrayList<>(); // Arrays.asList produces an AbstractList
        profile.allowedAlgorithms.add("test" + VpnProfile.LIST_DELIMITER + "test");
