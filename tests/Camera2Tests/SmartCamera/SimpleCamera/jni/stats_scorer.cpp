...
void Java_androidx_media_filterpacks_numeric_StatsFilter_score(JNIEnv* env, jobject thiz,
                                                               jobject imageBuffer,
...
void Java_androidx_media_filterpacks_numeric_StatsFilter_regionscore(
        JNIEnv* env, jobject thiz, jobject imageBuffer, jint width, jint height, jfloat left,
        jfloat top, jfloat right, jfloat bottom, jfloatArray statsArray) {
    unsigned char* pImg = static_cast<unsigned char*>(env->GetDirectBufferAddress(imageBuffer));
    ...
    jfloat result[2];
    result[0] = sum / numPixels;  // mean
    result[1] = (numPixels == 1) ? 0
                                 : sqrt((sumSquares - numPixels * result[0] * result[0]) /
                                        (numPixels - 1));  // stdev.
    env->SetFloatArrayRegion(statsArray, 0, 2, result);
}
