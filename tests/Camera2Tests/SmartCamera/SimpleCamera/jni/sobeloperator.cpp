...
jboolean Java_androidx_media_filterpacks_image_SobelFilter_sobelOperator(JNIEnv* env, jclass clazz,
    ...
    unsigned char* magPtr =
            (magBuffer == 0) ? 0
                             : static_cast<unsigned char*>(env->GetDirectBufferAddress(magBuffer));
    unsigned char* dirPtr =
            (dirBuffer == 0) ? 0
                             : static_cast<unsigned char*>(env->GetDirectBufferAddress(dirBuffer));
