...
int FwdLockEngine::getConvertedStatus(FwdLockConv_Status_t status) {
    ...
    switch(status) {
        ...
        case FwdLockConv_Status_UnsupportedContentTransferEncoding:
            LOG_VERBOSE("FwdLockEngine getConvertedStatus: file conversion Error %d. "
...
android::status_t FwdLockEngine::onInitialize(int uniqueId) {
    ...
    if (FwdLockGlue_InitializeKeyEncryption()) {
        ...
        LOG_VERBOSE("FwdLockEngine::onInitialize -- FwdLockGlue_InitializeKeyEncryption failed:"
             "errno = %d", errno);
    }
...
status_t FwdLockEngine::onOpenConvertSession(int uniqueId,
    ...
    if (!convertSessionMap.isCreated(convertId)) {
        ...
        if (FwdLockConv_Status_OK ==
            ...
            LOG_VERBOSE("FwdLockEngine::onOpenConvertSession -- FwdLockConv_OpenSession failed.");
            delete newSession;
        }
    }
    return result;
...
status_t FwdLockEngine::onOpenDecryptSession(int uniqueId,
    ...
    if ((-1 < fd) &&
        ...
        LOG_VERBOSE("FwdLockEngine::onOpenDecryptSession parameter error");
        return result;
    }
...
status_t FwdLockEngine::onFinalizeDecryptUnit(int uniqueId,
                                              DecryptHandle* decryptHandle,
                                              int decryptUnitId) {
    LOG_VERBOSE("FwdLockEngine::onFinalizeDecryptUnit");
    return DRM_ERROR_UNKNOWN;
}
...
ssize_t FwdLockEngine::onPread(int uniqueId,
    ...
    if ((NULL != decryptHandle) &&
        ...
        if (((off_t)-1) != decoderSession->offset) {
            ...
            if (bytesRead < 0) {
                LOG_VERBOSE("FwdLockEngine::onPread error reading");
            }
        }
    } else {
        LOG_VERBOSE("FwdLockEngine::onPread decryptId not found");
    }
