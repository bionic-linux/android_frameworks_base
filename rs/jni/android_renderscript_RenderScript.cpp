...
{
    if (kLogApi) {
        ALOGD("nElementCreate, con(%p), type(%" PRId64 "d), kind(%i), norm(%i), size(%i)", (RsContext)con,
              type, kind, norm, size);
    }
...
{
    if (kLogApi) {
        ALOGD("nScriptSetVarObj, con(%p), s(%p), slot(%i), val(%" PRId64 "d)", (RsContext)con, (void *)script,
              slot, val);
    }
...
{
    if (kLogApi) {
        ALOGD("nScriptSetVarJ, con(%p), s(%p), slot(%i), val(%" PRId64 "d)", (RsContext)con, (void *)script,
              slot, val);
    }
