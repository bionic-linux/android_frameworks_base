...
final class ConnectionServiceAdapter implements DeathRecipient {
    ...
    void onRttInitiationSuccess(String callId) {
        Log.v(this, "onRttSessionSuccess: %s", callId);
        for (IConnectionServiceAdapter adapter : mAdapters) {
            try {
     ...
     * @param callId The unique ID of the call.
     */
    void onRttInitiationFailure(String callId) {
        Log.v(this, "onRttSessionFailure: %s", callId);
        for (IConnectionServiceAdapter adapter : mAdapters) {
            try {
                adapter.onRttInitiationFailure(callId, Log.getExternalSession());
            } catch (RemoteException ignored) {
            }
        }
    }
