...
public class MeasuredText {
    ...
    private rangeCheck(int start, int end) {
        if (((start | end) < 0) | (start > end) | ((start | end) >= mChars.length)) {
            throw new IllegalArgumentException(String.format(Locale.US, "start(%d) end(%d) length(%d) out of bounds", start, end, mChars.length));
        }
    }
    ...
    private offsetCheck(int offset) {
    ...
    private void throwOffsetError(int offset) {
        throw new IllegalArgumentException(String.format(Locale.US, "offset (%d) length(%d) out of bounds", offset, mChars.length));
    }
