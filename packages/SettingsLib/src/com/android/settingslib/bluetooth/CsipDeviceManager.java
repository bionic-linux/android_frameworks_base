...
public class CsipDeviceManager {
    ...
    void initCsipDeviceIfNeeded(CachedBluetoothDevice newDevice) {
        ...
        log("initCsipDeviceIfNeeded: " + groupId);
        if (isValidGroupId(groupId)) {
            log("[Alice] initCsipDeviceIfNeeded: " + newDevice + " (group: " + groupId + ")");
    ...
    private int getBaseGroupId(BluetoothDevice device) {
        ...
        final CsipSetCoordinatorProfile profileProxy = profileManager.getCsipSetCoordinatorProfile();
        if (profileProxy != null) {
            final Map<Integer, ParcelUuid> groupIdMap = profileProxy.getGroupUuidMapByDevice(device);
            if (groupIdMap == null) {
    ...
    void onGroupIdChanged(int groupId) {
        ...
        for (int i = mCachedDevices.size() - 1; i >= 0; i--) {
            ...
            log("mCachedDevices.remove(i)");
            mCachedDevices.remove(i);
            log("dispatchDeviceRemoved: " + cachedDevice);
    ...
    boolean onProfileConnectionStateChangedIfProcessed(CachedBluetoothDevice cachedDevice,
        ...
        switch (state) {
            case BluetoothProfile.STATE_CONNECTED:
                log("onProfileConnectionStateChangedIfProcessed: " + cachedDevice);
