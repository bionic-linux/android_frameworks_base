...
public class TileUtils {
    ...
    static void loadTilesForCustomization(Context context, UserHandle user, String action,
        ...
        List<ResolveInfo> results = new ArrayList<ResolveInfo>();
        try {
            // Query intent activities twice, first with no flag and then with GET_META_DATA flag to
            // ensure that the Binder transaction limit hasn't been reached
            results = pm.queryIntentActivitiesAsUser(intent, 0, user.getIdentifier());
            results = pm.queryIntentActivitiesAsUser(intent, PackageManager.GET_META_DATA,
                    user.getIdentifier());
        } catch (Exception e) {
            Log.e(LOG_TAG, "Exception caught when querying Intent Activities for customization",
                    e);
            return;
        }
    ...
    private static void loadActivityTiles(Context context,
        ...
        try {
            ...
            Log.e(LOG_TAG, "Exception caught when querying Intent Activities for action", e);
            return;
        }
