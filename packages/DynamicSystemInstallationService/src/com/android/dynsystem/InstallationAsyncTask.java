...
class InstallationAsyncTask extends AsyncTask<String, InstallationAsyncTask.Progress, Throwable> {
    ...
    private void verifyAndPrepare() throws Exception {
        ...
        try {
            String listUrl = mContext.getString(R.string.dynamic_system_key_revocation_list_url);
            mRevocationList.fetch(new URL(listUrl));
        } catch (Exception e) {
        ...
        if (!TextUtils.isEmpty(mPubKey) && mRevocationList.isRevoked(mPubKey)) {
            Log.d(TAG, "PubKey of DSUPackage is revoked");
            keyRevocationThrowOrWarning(
                    new KeyRevokedException("Public key is revoked: " + mPubKey));
        }
    }
