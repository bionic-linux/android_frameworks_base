...
public class NetworkController extends BroadcastReceiver implements DemoMode {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (action.equals(WifiManager.RSSI_CHANGED_ACTION)
            ...
            int subscription = intent.getIntExtra(PhoneConstants.SIM_ID_KEY, 0);
    ...
    private PhoneStateListener getPhoneStateListener(int subscription) {
        PhoneStateListener mPhoneStateListener = new PhoneStateListener((int)subscription) {
            @Override
            public void onSignalStrengthsChanged(SignalStrength signalStrength) {
                final int subscription = mSubscription;
                if (subscription >= 0) {
                    if (DEBUG) {
