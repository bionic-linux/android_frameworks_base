...
public class SIMHelper {
    ...
    private static List<SubscriptionController.SubInfoRecord> sSimInfos;
    ...
    public static SubscriptionController.SubInfoRecord getSubInfoById(Context context, long subId) {
        ...
        for (SubscriptionController.SubInfoRecord info : sSimInfos) {
    ...
    public static void updateSIMInfos(Context context) {
        sSimInfos = null;
        sSimInfos = SubscriptionController.getActivatedSubInfoList(context);
    }

    public static int getSIMColorIdBySub(Context context, long subId) {
        SubscriptionController.SubInfoRecord simInfo = getSubInfoById(context, subId);
        if (simInfo == null) {
            return -1;
        }
    ...
    public static int getSimId(long subId) {
        return SubscriptionController.getSimId(subId);
    }

    public static long getSubId(int simId) {
        long[] subIds = SubscriptionController.getSubId(simId);
        if (subIds != null) {
            return subIds[0];
        } else {
            return 0;
