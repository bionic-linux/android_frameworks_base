...
public class MobileSignalController extends SignalController<
    ...
    private static final int EVENT_GET_IMS_SERVICE = 0;
    ...
    private final MmTelFeatureConnection.IFeatureUpdate mImsConnectionStatusChangeListener
            = new ImsConnectionStateChangeListener();
            ...
            switch (msg.what) {
                case EVENT_GET_IMS_SERVICE:
                    try {
                        ...
                        Log.e(mTag, "Could not be get ims service", e);
                        retryGetImsService();
                    }
                    break;
                default:
    ...
    @VisibleForTesting
    final ImsRegistrationImplBase.Callback mImsRegistrationStateListener
            = new ImsRegistrationStateListener();
    @VisibleForTesting
    final ImsFeature.CapabilityCallback mImsFeatureCapabilityListener
            = new ImsFeatureCapabilityListener();
    ...
    private int mImsServiceRetryCount = 0;
    private ServiceState mServiceState;
    private SignalStrength mSignalStrength;
    private MobileIconGroup mDefaultIcons;
    private Config mConfig;
    private ImsManager mImsManager;
    private int mImsRegisteredRadioTech = ImsRegistrationImplBase.REGISTRATION_TECH_NONE;
    ...
    public void registerListener() {
        ...
        mHandler.sendEmptyMessage(EVENT_GET_IMS_SERVICE);
    }
    ...
    public void unregisterListener() {
        ...
        if (mImsManager != null) {
            mImsManager.removeNotifyStatusChangedCallback(mImsConnectionStatusChangeListener);
            mImsManager = null;
        }
        mHandler.removeMessages(EVENT_GET_IMS_SERVICE);
    }
    ...
    private final void updateTelephony() {
        ...
        mCurrentState.wifiCallingAvailable = isWifiCallingAvailable();
    ...
    private boolean isWifiCallingAvailable() {
        if (mImsRegisteredRadioTech == ImsRegistrationImplBase.REGISTRATION_TECH_IWLAN) {
            if (mMmTelCapabilities != null) {
                return mMmTelCapabilities.isCapable(MmTelCapabilities.CAPABILITY_TYPE_VOICE)
    ...
    @VisibleForTesting
    void setImsManager(ImsManager imsManager) {
    ...
    private void getImsService() throws ImsException {
        ...
        mImsManager.addNotifyStatusChangedCallbackIfAvailable(mImsConnectionStatusChangeListener);
        // Wait for ImsService.STATE_READY to start listening for IMS capabilities change.
        // Call the callback right away for compatibility with older devices that do not use states.
        mImsConnectionStatusChangeListener.notifyStateChanged();
    }
    ...
    private void retryGetImsService() {
        ...
        Log.e(mTag, "retryGetImsService: Retrying getting ImsService...");
        mHandler.removeMessages(EVENT_GET_IMS_SERVICE);
        mHandler.sendEmptyMessageDelayed(EVENT_GET_IMS_SERVICE, mRetryTimeout.get());
    }

    private void startListeningForImsCapabilities() throws ImsException {
        mImsManager.addRegistrationCallback(mImsRegistrationStateListener);
        mImsManager.addCapabilitiesCallback(mImsFeatureCapabilityListener);
        mImsServiceRetryCount = 0;
    }

    private void clearImsCapabilities() {
        mImsRegisteredRadioTech = ImsRegistrationImplBase.REGISTRATION_TECH_NONE;
    ...
    public void dump(PrintWriter pw) {
        ...
        pw.println("  mImsRegisteredRadioTech=" + mImsRegisteredRadioTech + ",");
    ...
    private class ImsConnectionStateChangeListener implements
            MmTelFeatureConnection.IFeatureUpdate {
        @Override
        public void notifyStateChanged() {
            try {
                int status = mImsManager.getImsServiceState();
                if (DEBUG) {
                    Log.d(mTag, "notifyStateChanged status=" + status);
                }
                switch (status) {
                    ...
                    case ImsFeature.STATE_UNAVAILABLE:
                        clearImsCapabilities();
                        break;
                    default:
                        Log.e(mTag, "Unexpected State!");
                        break;
                }
            } catch (ImsException e) {
    ...
    private class ImsRegistrationStateListener extends ImsRegistrationImplBase.Callback {
        @Override
        public void onRegistered(
            ...
            mImsRegisteredRadioTech = mImsManager.getRegistrationTech();
            updateTelephony();
        }
    ...
    private class ImsFeatureCapabilityListener extends ImsFeature.CapabilityCallback {
        @Override
        public void onCapabilitiesStatusChanged(ImsFeature.Capabilities capabilities) {
            mMmTelCapabilities = new MmTelCapabilities(capabilities);
            if (DEBUG) {
                Log.d(mTag, "onCapabilitiesStatusChanged capabilities=" + mMmTelCapabilities);
            }
            updateTelephony();
        }
    }
