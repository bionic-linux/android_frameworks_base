...
public class HeadsUpAppearanceController implements OnHeadsUpChangedListener,
    ...
    private void setShown(boolean isShown) {
        if (mShown != isShown) {
            ...
            if (isShown) {
                ...
                animateShow(mHeadsUpStatusBarView);
                animateHide(mClockView, View.INVISIBLE);
                if (mOperatorNameView != null) {
                    animateHide(mOperatorNameView, View.INVISIBLE);
                }
            } else {
                animateShow(mClockView);
                if (mOperatorNameView != null) {
                    animateShow(mOperatorNameView);
                }
                animateHide(mHeadsUpStatusBarView, View.GONE);
            }
        }
    }

    private void animateHide(View view, int state) {
        animateHide(view, state, mAnimationEnabled);
    }

    private void animateHide(View view, int state, boolean animate) {
        if (animate) {
            CrossFadeHelper.fadeOut(view, CONTENT_FADE_DURATION /* duration */,
                    0 /* delay */, () -> view.setVisibility(state));
        } else {
    ...
    private void animateShow(View view) {
        animateShow(view, mAnimationEnabled);
    }
