    ...
    public String getOptrNameBySubIdx(Context context, int subIdx) {
        if (subIdx >= 0) {
            ...
            if (subId > 0) {
                ...
                SubscriptionController.SubInfoRecord info = SubscriptionController.getSubInfoUsingSubId(context, subId);
                if (null == info) {
    ...
    public Drawable getOptrDrawableBySubIdx(Context context, int subIdx) {
        ...
        if (subId > 0) {
            ...
            if (null == info) {
                ...
                return context.getResources().getDrawable(info.mSimIconRes[0]);//SubscriptionController.RES_TYPE_BACKGROUND_DARK]);
            }
        } else {
            throw new IndexOutOfBoundsException();
        }
