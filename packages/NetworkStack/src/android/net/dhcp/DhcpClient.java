...
public class DhcpClient extends StateMachine {
    ...
    public static final int EVENT_CONFIGURATION_OBTAINED    = PUBLIC_BASE + 10;
    ...
    private OnNetworkAttributesRetrievedListener mGetLastIpAddressCallback;
    private NetworkStackIpMemoryStore mIpMemoryStore;
    private AtomicInteger mSequenceNumber = new AtomicInteger(0);
    private String mL2Key;
    private boolean mLastAssignedIpv4AddressValid;
    ...
    class StoppedState extends State {
        ...
        public boolean processMessage(Message message) {
            switch (message.what) {
                case CMD_START_DHCP:
                    final Pair<String, NetworkStackIpMemoryStore> args = (Pair<String,
                            NetworkStackIpMemoryStore>) message.obj;
                    mL2Key = args.first;
                    mIpMemoryStore = args.second;
    ...
    class WaitBeforeInitRebootState extends WaitBeforeOtherState {
        WaitBeforeInitRebootState(State otherState) {
            super();
            mOtherState = otherState;
        }
    }
    ...
    class ObtainingConfigurationState extends LoggingState {
        ...
        public void enter() {
            ...
            mGetLastIpAddressCallback = new OnNetworkAttributesRetrievedListener() {
                @Override
                public void onNetworkAttributesRetrieved(Status status, String l2Key,
                        NetworkAttributes attributes) {
                    final long currentTime = System.currentTimeMillis();
                    if (!status.isSuccess() || attributes == null) return;
                    if (attributes.assignedV4Address != null
                            && (currentTime < attributes.assignedV4AddressExpiry
                                  || attributes.assignedV4AddressExpiry == Long.MAX_VALUE)) {
                        if (DBG) {
                            Log.d(TAG, "l2key: " + l2Key
                                    + " lease address: " + attributes.assignedV4Address
                                    + " lease expiry: " + attributes.assignedV4AddressExpiry
                                    + " current time: " + currentTime);
                        }
                        mLastAssignedIpv4AddressValid = true;
                        sendMessage(obtainMessage(EVENT_CONFIGURATION_OBTAINED, sequenceNumber, 0,
                                attributes.assignedV4Address));
                    }
                }
            };
            mIpMemoryStore.retrieveNetworkAttributes(mL2Key, mGetLastIpAddressCallback);
        }

        @Override
        public boolean processMessage(Message message) {
            if (message.what != EVENT_CONFIGURATION_OBTAINED) {
                deferMessage(message);
                return NOT_HANDLED;
            }

            // stale message check
            if (message.arg1 != mSequenceNumber.get()) return NOT_HANDLED;

            // Ensure no future messages (e.g., the timeout we set above) are processed.
            mSequenceNumber.getAndIncrement();

            mLastAssignedIpv4Address = (Inet4Address) message.obj;
            transitionTo(mLastAssignedIpv4AddressValid ? mDhcpInitRebootState : mDhcpInitState);
            return HANDLED;
        }
    }
