...
public class ApfFilter {
    ...
    private abstract static class TcpKeepaliveAck {
        ...
        static final int TCP_SRC_PORT_OFFSET = 0;
        static final int TCP_DST_PORT_OFFSET = 2;
        static final int TCP_SEQ_OFFSET = 4;
        static final int TCP_ACK_OFFSET = 8;
        ...
        protected final byte[] mPortSeqAck;

        TcpKeepaliveAck(final TcpKeepaliveAckData packet, final byte[] srcDstAddr) {
            mPacket = packet;
            mSrcDstAddr = srcDstAddr;
            mPortSeqAck = generatePortSeqAckFingerprint(mPacket.srcPort, mPacket.dstPort,
                    mPacket.seq, mPacket.ack);
        }

        static byte[] generatePortSeqAckFingerprint(int srcPort, int dstPort, int seq, int ack) {
            ByteBuffer fp = ByteBuffer.wrap(new byte[12]);
            fp.putShort(TCP_SRC_PORT_OFFSET, (short) srcPort);
            fp.putShort(TCP_DST_PORT_OFFSET, (short) dstPort);
            fp.putInt(TCP_SEQ_OFFSET, seq);
            fp.putInt(TCP_ACK_OFFSET, ack);
            return fp.array();
        }
    ...
    private class TcpKeepaliveAckV4 extends TcpKeepaliveAck {
        ...
        void generateFilterLocked(ApfGenerator gen) throws IllegalInstructionException {
            ...
            gen.addLoadFromMemory(Register.R1, gen.IPV4_HEADER_SIZE_MEMORY_SLOT);
            gen.addLoadImmediate(Register.R0, ETH_HEADER_LEN + TCP_SRC_PORT_OFFSET);
            gen.addAddR1();
            gen.addJumpIfBytesNotEqual(Register.R0, mPortSeqAck, nextFilterLabel);
