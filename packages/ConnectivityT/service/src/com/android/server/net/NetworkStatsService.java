...
public class NetworkStatsService extends INetworkStatsService.Stub {
    ...
    public static class Dependencies {
        ...
        public File getStatsDir() {
            final File apexDataDir = ApexEnvironment.getApexEnvironment(CONNECTIVITY_APEX_NAME)
                    .getDeviceProtectedDataDir();
            final File apexStatsDir = new File(apexDataDir, "netstats");
            if (apexDataDir.canWrite() && apexStatsDir.mkdirs()) {
                return apexStatsDir;
            }
            final File platformDataDir = new File(Environment.getDataDirectory(), "system");
            final File platformStatsDir = new File(platformDataDir, "netstats");
            if (platformDataDir.canWrite() && platformStatsDir.mkdirs()) {
                return platformStatsDir;
            }
            throw new IllegalStateException("Cannot find a valid persist data directory");
        }

        public boolean isInPlatform(@NonNull File baseDir) {
            return baseDir.getPath().startsWith(Environment.getDataDirectory().getPath());
        }
        ...
        public void setDeviceConfigPropertyInt(@NonNull String name, int value) {
            DeviceConfig.setProperty(DeviceConfig.NAMESPACE_CONNECTIVITY,
                    name, Integer.toString(value), false /* makeDefault */);
        }
    ...
    private void maybeUpgradeLegacyStatsLocked() {
        if (mDeps.isInPlatform(mStatsDir)) {
            return;
        }
    ...
    private long maybeApplyImportCollectionLocked(@NonNull NetworkStatsRecorder recorder,
        ...
        final NetworkStatsCollection collection = mDeps.readPlatformCollection(
