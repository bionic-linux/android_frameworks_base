...
public class DadProxyTest {
    ...
    @Rule
    public final TapPacketReaderRule mUpstreamReaderRule = new TapPacketReaderRule(
            DATA_BUFFER_LEN, false /* autoStart */);
    @Rule
    public final TapPacketReaderRule mTetheredReaderRule = new TapPacketReaderRule(
            DATA_BUFFER_LEN, false /* autoStart */);
    ...
    public void tearDown() throws Exception {
        mUpstreamReaderRule.stop();
        mTetheredReaderRule.stop();
    ...
    private void setupTapInterfaces() {
        ...
        mUpstreamReaderRule.start(mHandler);
        mUpstreamParams = InterfaceParams.getByName(mUpstreamReaderRule.iface.getInterfaceName());
        assertNotNull(mUpstreamParams);
        mUpstreamPacketReader = mUpstreamReaderRule.getReader();

        // Create tethered test iface.
        mTetheredReaderRule.start(mHandler);
        mTetheredParams = InterfaceParams.getByName(
                mTetheredReaderRule.getIface().getInterfaceName());
        assertNotNull(mTetheredParams);
        mTetheredPacketReader = mTetheredReaderRule.getReader();
    }
