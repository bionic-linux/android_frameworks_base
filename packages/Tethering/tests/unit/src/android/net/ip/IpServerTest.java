...
public class IpServerTest {
    ...
    private void assertTetherOffloadRuleParcelMatches(int upstreamIfindex, InetAddress dst,
            MacAddress dstMac, TetherOffloadRuleParcel parcel) {
        assertEquals(upstreamIfindex, parcel.inputInterfaceIndex);
        assertEquals(TEST_IFACE_PARAMS.index, parcel.outputInterfaceIndex);
        assertArrayEquals(dst.getAddress(), parcel.destination);
        assertEquals(128, parcel.prefixLength);
        assertArrayEquals(TEST_IFACE_PARAMS.macAddr.toByteArray(), parcel.srcL2Address);
        assertArrayEquals(dstMac.toByteArray(), parcel.dstL2Address);
    }

    private void verifyTetheringRuleAdded(int upstreamIfindex, InetAddress dst, MacAddress dstMac,
            InOrder inOrder) throws Exception {
        ArgumentCaptor<TetherOffloadRuleParcel> captor =
                ArgumentCaptor.forClass(TetherOffloadRuleParcel.class);
        if (inOrder != null) {
            inOrder.verify(mNetd).tetherOffloadRuleAdd(captor.capture());
        } else {
            verify(mNetd).tetherOffloadRuleAdd(captor.capture());
        }
        assertTetherOffloadRuleParcelMatches(upstreamIfindex, dst, dstMac, captor.getValue());
    }

    private void verifyTetheringRuleAdded(int upstreamIfindex, InetAddress dst, MacAddress dstMac)
            throws Exception {
        verifyTetheringRuleAdded(upstreamIfindex, dst, dstMac, null);
    }

    private void verifyTetheringRuleRemoved(int upstreamIfindex, InetAddress dst, MacAddress dstMac,
            InOrder inOrder) throws Exception {
        ArgumentCaptor<TetherOffloadRuleParcel> captor =
                ArgumentCaptor.forClass(TetherOffloadRuleParcel.class);
        if (inOrder != null) {
            inOrder.verify(mNetd).tetherOffloadRuleRemove(captor.capture());
        } else {
            verify(mNetd).tetherOffloadRuleRemove(captor.capture());
        }
        assertTetherOffloadRuleParcelMatches(upstreamIfindex, dst, dstMac, captor.getValue());
    }

    private void verifyTetheringRuleRemoved(int upstreamIfindex, InetAddress dst,
            MacAddress dstMac) throws Exception {
        verifyTetheringRuleRemoved(upstreamIfindex, dst, dstMac, null);
    }
