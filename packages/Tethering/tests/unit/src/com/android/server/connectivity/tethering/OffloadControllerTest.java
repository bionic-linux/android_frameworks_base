...
public class OffloadControllerTest {
    ...
    public void testGetForwardedStats() throws Exception {
        ...
        NetworkStatsParcelableWrapper ifaceStats = provider.getTetherStats(STATS_PER_IFACE);
        NetworkStatsParcelableWrapper uidStats = provider.getTetherStats(STATS_PER_UID);
        waitForIdle();
        // There is no current upstream, so no stats are fetched.
        inOrder.verify(mHardware, never()).getForwardedStats(any());
        inOrder.verifyNoMoreInteractions();

        final NetworkStatsParcelableWrapper expectedIfaceStats =
                new NetworkStatsParcelableWrapper(2);
        expectedIfaceStats.addValues(
                new Entry(mobileIface, UID_ALL, SET_DEFAULT, TAG_NONE, METERED_NO, ROAMING_NO,
                        DEFAULT_NETWORK_NO, mobileStats.rxBytes, 0L, mobileStats.txBytes, 0L, 0L))
                .addValues(new Entry(ethernetIface, UID_ALL, SET_DEFAULT, TAG_NONE, METERED_NO,
                        ROAMING_NO, DEFAULT_NETWORK_NO, 12345 + 100000, 0L, 54321 + 100000,
                        0L, 0L));

        final NetworkStatsParcelableWrapper expectedUidStats = new NetworkStatsParcelableWrapper(2);
        expectedUidStats.addValues(
                new Entry(mobileIface, UID_TETHERING, SET_DEFAULT, TAG_NONE, METERED_NO, ROAMING_NO,
                        DEFAULT_NETWORK_NO, mobileStats.rxBytes, 0L, mobileStats.txBytes, 0L, 0L))
                .addValues(new Entry(ethernetIface, UID_TETHERING, SET_DEFAULT, TAG_NONE,
                        METERED_NO, ROAMING_NO, DEFAULT_NETWORK_NO, 12345 + 100000, 0L,
                        54321 + 100000, 0L, 0L));

        assertEquals(expectedIfaceStats, ifaceStats);
        assertEquals(expectedUidStats, uidStats);
    }
