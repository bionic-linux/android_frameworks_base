...
public class TetheringServiceTest extends ServiceTestCase<TetheringService> {
    ...
    private class TestTetherInternalCallback extends ITetherInternalCallback.Stub {
        ...
        // This function will remove the recorded callbacks, so it is suggest to be called once
        // for each callback. If this is called after multiple callback, the input is order
        // matters. onCallbackCreated counts as the first call to expectUpstreamChanged with
        ...
        // This function will remove the recorded callbacks, so it is suggest to be called once
        // for each callback. If this is called after multiple callback, the input is order
        // matters. onCallbackCreated counts as the first call to onConfigurationChanged with
        // @see onCallbackCreated.
