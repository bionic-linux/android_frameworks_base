...
import static android.net.netstats.provider.NetworkStatsParcelableWrapper.Entry;
import static android.net.netstats.provider.NetworkStatsParcelableWrapper.Entry.DEFAULT_NETWORK_NO;
import static android.net.netstats.provider.NetworkStatsParcelableWrapper.Entry.METERED_NO;
import static android.net.netstats.provider.NetworkStatsParcelableWrapper.Entry.ROAMING_NO;
import static android.net.netstats.provider.NetworkStatsParcelableWrapper.Entry.SET_DEFAULT;
import static android.net.netstats.provider.NetworkStatsParcelableWrapper.Entry.TAG_NONE;
import static android.net.netstats.provider.NetworkStatsParcelableWrapper.Entry.UID_ALL;
import static android.net.netstats.provider.NetworkStatsParcelableWrapper.Entry.UID_TETHERING;
...
public class OffloadController {
    ...
    class OffloadTetheringStatsProvider extends AbstractNetworkStatsProvider {
        @NonNull final NetworkStatsParcelableWrapper mIfaceStats =
                new NetworkStatsParcelableWrapper(0);
        @NonNull final NetworkStatsParcelableWrapper mUidStats =
                new NetworkStatsParcelableWrapper(0);

        @VisibleForTesting
        @NonNull NetworkStatsParcelableWrapper getTetherStats(@NonNull StatsType how) {
            final NetworkStatsParcelableWrapper stats = new NetworkStatsParcelableWrapper(0);
        ...
        /** Push stats to service, but does not cause a force polling. */
        public void pushTetherStats() {
            // TODO: remove the accumulated stats and report the diff from HAL directly.
            final NetworkStatsParcelableWrapper ifaceDiff =
                    getTetherStats(StatsType.STATS_PER_IFACE).subtract(mIfaceStats);
            final NetworkStatsParcelableWrapper uidDiff =
                    getTetherStats(StatsType.STATS_PER_UID).subtract(mUidStats);
            if (mStatsProviderCb != null) {
                try {
                    mStatsProviderCb.onStatsUpdated(0 /* unused */, ifaceDiff, uidDiff);
                } catch (RuntimeException e) {
                    mLog.e("Cannot report network stats: " + e);
                }
            }
            mIfaceStats.combineAllValues(ifaceDiff);
            mUidStats.combineAllValues(uidDiff);
        }
