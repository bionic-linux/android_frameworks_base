...
public class EntitlementManager {
    ...
    private boolean mLastEvaluatedResult = true;
    ...
    public void notifyUpstream(boolean isCellular) {
        if (DBG) {
            mLog.i("notifyUpstream: " + isCellular
                    + ", mLastEvaluatedResult: " + mLastEvaluatedResult
    ...
    protected Intent runSilentTetherProvisioning(final int type,
        ...
        intent.putExtra(EXTRA_RUN_PROVISION, config.provisioningAppNoUi);
        intent.putExtra(EXTRA_PROVISION_RESPONSE, config.provisioningResponse);
        intent.putExtra(EXTRA_PROVISION_CALLBACK, receiver);
        intent.putExtra(EXTRA_SUBID, config.activeDataSubId);
    ...
    private void runUiTetherProvisioning(final int type, final TetheringConfiguration config) {
     ...
     */
    @VisibleForTesting
    protected Intent runUiTetherProvisioning(final int type, final TetheringConfiguration config,
             final ResultReceiver receiver) {
        if (DBG) mLog.i("runUiTetherProvisioning: " + type);

        Intent intent = new Intent(Settings.ACTION_TETHER_PROVISIONING_UI);
        intent.putExtra(EXTRA_ADD_TETHER_TYPE, type);
        intent.putExtra(EXTRA_RUN_PROVISION, config.provisioningApp);
        intent.putExtra(EXTRA_PROVISION_CALLBACK, receiver);
        intent.putExtra(EXTRA_SUBID, config.activeDataSubId);
    ...
    private void evaluateCellularPermission(final TetheringConfiguration config) {
        final boolean oldPermitted = mLastEvaluatedResult;
        mLastEvaluatedResult = isCellularUpstreamPermitted(config);

        if (DBG) {
            mLog.i("Cellular permission change from " + oldPermitted
                    + " to " + mLastEvaluatedResult);
        }

        if (mLastEvaluatedResult != oldPermitted) {
            mLog.log("Cellular permission change: " + mLastEvaluatedResult);
            mTetherMasterSM.sendMessage(mPermissionChangeMessageCode);
        }
        // Only schedule periodic re-check when tether is provisioned
        // and the result is ok.
        if (mLastEvaluatedResult && mCellularPermitted.size() > 0) {
