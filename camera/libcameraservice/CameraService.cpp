...
status_t CameraService::Client::startRecordingMode()
{
    ...
    LOGV("startRecordingMode:recordingEnabled:%d",mHardware->recordingEnabled());

    // if recording has been enabled, nothing needs to be done
    if (mHardware->recordingEnabled()) {
        return NO_ERROR;
    }
    LOGV("startRecordingMode:recordingCallback:%x",recordingCallback);
...
status_t CameraService::Client::startPreviewMode()
{
    ...
    if (mUseOverlay) {
        ...
        if (ret == NO_ERROR) {
            ...
            mSurface->unregisterBuffers();
#if 0 //HanChao change for ISurface
